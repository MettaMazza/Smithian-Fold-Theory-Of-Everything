...
Maria Smith (Ernos Labs) — release v2.2, 2026-07-18
...
Three days. Zero training. One machine-checked, self-proven theorem: *there is no nothing*. The development record includes predecessor comparisons (v13 over v12: 4-2; v14 over v13: 9-1; v17 over v14: 10-1) and full-strength Stockfish autopsies of recorded losses, with per-move evaluation curves locating the exact ply each game goes permanently bad. The implementation sequence tested one named change at a time with counted or lossless machinery. A profiler found **91% of match CPU inside the language runtime's garbage collector** — addressed by an allocation-free hot path, ~25x compounded; weak move ordering — addressed by killers and history, collapsing depth-7 cost from 63.5M to 17.7M nodes with provably identical move choices; horizon — addressed losslessly, and one additional ply changed the recorded 1900 batch from 20.8% to 62.5%. Two principled evaluation variants measured 37.5% and 45.8% in their named head-to-head batches; an agent reverted them at that development point. Those measurements do not close the investigation or control whether Maria orders a real match. A parallel-root release (8 worker processes, exact single-core values, 7.6x measured parallelism at 95% efficiency, complete depth 8–9) was then developed. Benchmark victory remains an explicit objective, and Maria alone decides when a build receives a real run; agent probes may inform but cannot authorize, refuse, delay, or veto it.
