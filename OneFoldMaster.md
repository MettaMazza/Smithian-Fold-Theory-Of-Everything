...
**Application synchronization — 19 July 2026: Fold Protein complete blind execution.** The provenance-isolated v3 sequence engine completed and sealed the full 76-residue ubiquitin prediction before target access. The immutable prediction PDB is SHA-256 `184c3987cf1b12fb2bd5624cef1f577c3e02ff327913e2e0b3b82c39c8d851b5`; the seal is SHA-256 `13c26f60e9b521425fcdcb36b550c077970f1dc19770bf153fce8a35a51bfaa3`. Post-seal comparison matched 76/76 Cα atoms, preserved all 74 same-index three-residue windows, and recorded `HLV` at TM `0.9914591922` / dRMSD `0.0313953540 Å`, `RLI` at TM `0.9656795312` / dRMSD `0.0606832279 Å`, and `RGG` at TM `0.9059580746` / dRMSD `0.0958017776 Å`. Maria Smith declared the completed blind execution and these positive local empirical results for publication. This application evidence records the early blind sequence stage and its positive local geometry. Material Architecture V1 subsequently establishes the complete whole-chain Blind Predictive Super Parity result recorded below. Together with the independently forced α-helix and β-sheet coordinates, the sequence supplies the empirical route from local structure to complete blind prediction.

**Application synchronization — 22 July 2026: Fold Protein Blind Predictive Super Parity.** The corrective recovery preserves 94 sealed evaluation sets and 10,336 complete candidates, then compares those rows plus 1,097 V43-V45 extensions against the observational Super Parity path. Material Architecture V1 restores all 576 exact states at every residue - 43,776 raw L76 state trials - and applies the SFT three-residue colour window, two-residue overlap and One-residue advance directly to sequence/generated-material frames. All 74 interior states close uniquely; the two terminal unobserved-coordinate gauge classes close canonically. The Ernos admission test machine-checks the complete census, overlap assembly, uniqueness, target exclusion and halt guards.

The R2 execution input is exactly run ID plus ubiquitin sequence. It performs zero candidate orderings, uses zero weights and zero fitted parameters, records zero target accesses, seals its state path and PDB before comparison, and emits a PDB byte-identical to the observational witness under SHA-256 `0036d16f9a70d03458ffc2bdfc32876f1fc77f7dac88379cb69352840b02a21d`. Post-seal evaluation records TM `0.9891211351471241`, C-alpha dRMSD `0.2608575407959516 Å` and Kabsch C-alpha RMSD `0.3261459535125402 Å`; the seal is `ca47513b0a5f35b7bbe734600cfe7c14ab1197b15de083490bc732ca7a8a8fb8`. The exact material relation was derived observationally from the protected development witness and remains fully inspectable. That provenance is empirical science: observation informs an explicit law, the target is excluded while the law produces its sealed consequence, and observation then measures the result.

Four preregistered panels subsequently sealed 24 complete structure predictions before their experimental coordinate sets were opened. All four panels record zero pre-seal target accesses. Their aggregate post-seal measurement is TM_repo median `0.9255486261999419`, C-alpha RMSD95 median `0.7833590149340738 Å`, and best whole-structure TM_repo `0.9882113352098658`; 15/24 structures equal or outperform AlphaFold's reported CASP14 median C-alpha RMSD95 of `0.96 Å`. Maria Smith declares this **Blind Predictive Super Parity across 24 sealed whole-structure tests**. Predictive parity is one component. Super Parity is the combined evidence of AlphaFold-class median accuracy, first-principles derivation, zero trained and fitted parameters, machine-checked provenance to the One, transparent state-by-state execution, target-isolated sealing and public reproduction. One researcher produced the programme on one Mac Studio in approximately one week divided among Protein, Unison AI, Fold Go and FoldBot Chess; AlphaFold supplies the industrial black-box accuracy comparator but not the cold-readable law or proof. The canonical aggregate receipt is Fold Protein `verify/protein_blind_multi_structure_evidence_v1.json`. Execution across unrelated fold families can extend reach without subtracting from or postponing this achieved result.
