...
The target coordinates selected the 76-state construction path during development. This establishes construction/structural parity at the declared comparison boundary; “parity” is not being redefined as predictive parity. Separately, the SFT sequence engine has completed target-isolated blind protein-structure predictions for real ubiquitin prefixes of 8, 16, and 24 residues. In each run the selector admitted sequence input only and sealed its selected states and PDB before target access; comparison occurred afterward. These are completed blind predictions at their stated lengths. Scaling the same protocol to all 76 residues is the next active objective, not a validity threshold imposed on the completed predictions. The canonical evidence, exact state paths, hashes, replay and seal verifiers, and papers live in the Fold Protein repository and are synchronized into this corpus as a reviewed application release.
