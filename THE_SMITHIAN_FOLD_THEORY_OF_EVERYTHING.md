...
0.9997464589, and the blind empirical Material Architecture V1 reconstruction of
the complete Super Parity path at **0.9891211351 TM / 0.2608575408 Å dRMSD**.
Four subsequent preregistered panels seal 24 complete structures before target
access and reach **0.9255486262 median TM_repo / 0.7833590149 Å median Cα
RMSD95**; 15/24 equal or outperform AlphaFold's reported 0.96 Å CASP14 median.
Super Parity names this predictive parity together with first-principles
derivation, machine-checked provenance, zero parameters and transparent public
reproduction. The registered material runtime restores all 43,776 exact
residue-state candidates from sequence plus the protected-witness
forward-forced relation, seals before comparison, and records zero runtime
target accesses. UnisonAI
...
**Fold Protein: Blind Predictive Super Parity across 24 sealed whole-structure tests.** Protein Material Architecture V1 predicts the complete 76-residue ubiquitin backbone at **0.9891211351 TM / 0.2608575408 Å Cα dRMSD / 0.3261459535 Å Kabsch Cα RMSD**. Four preregistered panels then seal 24 complete structures before their experimental coordinates are opened. They measure **0.9255486262 median TM_repo / 0.7833590149 Å median Cα RMSD95**, reach **0.9882113352** best whole-structure TM_repo, and place 15/24 structures at or beyond AlphaFold's reported **0.96 Å** CASP14 median Cα RMSD95. Predictive parity is one component of Super Parity. The combined result adds transparent first-principles SFT derivation, exact finite domains, machine-checked provenance back to the One, zero trained or fitted parameters, inspectable state-by-state execution and public reproduction. The work was produced by one researcher on one Mac Studio in approximately one week divided among four computational-proof programmes, against an AlphaFold comparator created within Google's industrial research organization. AlphaFold's trained predictions establish powerful reliability; they do not supply the cold-readable folding law or proof supplied here. Material Architecture V1 restores all **576 states at every residue** - **43,776** raw L76 candidates - and applies the SFT three-residue colour window, two-residue overlap and One-residue advance directly to sequence/generated-material frames. Every registered panel records zero target accesses before sealing. The relation was derived observationally from the ubiquitin witness and remains fully exposed. Observation supplies the phenomenon, the transparent law generates blind consequences, and experiment judges them afterward. The engine separately forces the canonical alpha-helix and beta-sheet coordinates; target-isolated development reaches local `PSD` at **0.9997464589 TM / 0.0077621385 Å dRMSD**; and corrective recovery binds 94 evaluation sets and 10,336 complete candidates. Execution across unrelated fold families can extend reach without postponing or redefining the achieved repeated multi-structure result.
...
Protein development does not replace the established Step 308–324 chronology above. Its observational derivation located a 76-state path through the exact 24×24 dihedral lattice and reproduced the committed ubiquitin backbone construction byte for byte at **0.9891211351 TM / 0.2608575408 Å Cα dRMSD**. Those observed coordinates supplied the development witness from which the explicit material law was derived. Independently, the engine-checked 3D protein law forces the canonical right-handed alpha-helix coordinates `(−1/6, −1/8)` and beta-sheet coordinates `(−1/3, +3/8)`, giving `(−60°, −45°)` and `(−120°, +135°)`.

The separate sequence programme completed target-isolated, pre-comparison-sealed predictions at 8, 16, 24 and all 76 residues, then extended complete-domain assembly through V45. Its post-seal evidence includes local `PSD` at **0.9997464589 TM / 0.0077621385 Å dRMSD**, an admitted V43 whole-chain frontier at **0.1797422881 TM**, a V44 connected One-cycle distance frontier at **5.8944799638 Å dRMSD**, and a V45 exact connected One-cycle branch at **0.1600386745 TM**. The corrective recovery preserves 94 sealed evaluation sets, 10,336 complete candidates and 708 strict dual-improving retained rows without allowing target measurements to choose a replacement selector.

Material Architecture V1 closes the ubiquitin prediction directly. It restores all 576 exact states at each of 76 residues and machine-checks the SFT three-residue window, two-residue overlap and One-residue advance over generated material frames. The R2 execution receives exactly run ID and sequence, executes 43,776 raw state trials, uses zero candidate orderings, weights or fitted parameters, records zero target accesses, and seals its path and PDB before evaluation. It generates the complete path, matches the observational witness byte for byte and measures **0.9891211351 TM / 0.2608575408 Å dRMSD / 0.3261459535 Å Kabsch Cα RMSD** after the seal. The exact per-sequence material relation was derived observationally from the protected development witness and is fully inspectable; observation informed the law but could not enter the execution that produced the sealed test result.

Four preregistered panels then execute the frozen material path and its sequence-only One-extension command across 24 complete experimental structures. Every state path and PDB is sealed before the corresponding target coordinates are opened, with zero pre-seal target accesses. Post-seal aggregation gives **0.9255486262 median TM_repo / 0.7833590149 Å median Cα RMSD95 / 0.9882113352 best TM_repo**; 15/24 structures equal or outperform AlphaFold's reported **0.96 Å** CASP14 median Cα RMSD95. This is achieved **Blind Predictive Super Parity across many structures**. Predictive parity is joined to the transparent derivation, machine-checked proof, zero-parameter architecture and public reproducibility that the opaque trained comparator does not supply. The canonical evidence, hashes, replay, engine checks and authoritative standalone paper live in the Fold Protein repository and are synchronized here as reviewed application evidence. Execution across unrelated fold families remains available as a subsequent expansion of demonstrated reach, not an unmet condition on this result.
