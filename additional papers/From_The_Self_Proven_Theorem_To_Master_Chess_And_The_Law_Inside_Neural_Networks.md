...
| 1700 | 3-7-2 | 54.2% | v13 |
| 1900 | 6-3-3 | 62.5% | v14 |
| 2100 | 2-5-1(cap)-4 | 41.7% | v17 remeasure |

Three days. Zero training. One machine-checked, self-proven theorem: *there is no nothing*. The development record includes predecessor comparisons (v13 over v12: 4-2; v14 over v13: 9-1; v17 over v14: 10-1) and full-strength Stockfish autopsies with per-move evaluation curves locating the exact calculation frontier. The implementation sequence tested one named change at a time with counted or lossless machinery. A profiler found **91% of match CPU inside the language runtime's garbage collector** — addressed by an allocation-free hot path, ~25x compounded; move ordering was addressed by killers and history, collapsing depth-7 cost from 63.5M to 17.7M nodes with provably identical move choices; and one additional ply changed the recorded 1900 batch from 20.8% to 62.5%. Historical agent-selected variants remain attributed development artifacts rather than findings or limits. A parallel-root release (8 worker processes, exact single-core values, 7.6x measured parallelism at 95% efficiency, complete depth 8–9) was then developed. Benchmark victory remains an explicit objective, and Maria alone decides when a build receives a real run; agent probes may inform but cannot authorize, refuse, delay, or veto it.

### 3.2.1 The current calculation continuation

V20 preserves every legal root move to the deepest common completed pass. On a generated breadth sample, the parallel and one-worker engines agree on 4/4 selected moves and 4/4 exact rational values at common depth, with zero disagreements. Its worker message form is derived from the supplied ceiling rather than a fixed depth field or completion sentinel, and the live calculation state owns transposition, history, and killer addressing capacities.

Applied real-position evidence extends those exact checks. Current and pinned v20 agree on move and completed depth for all 36/36 recorded 2100 positions at ceiling 3. The current engine then completed the same 36 positions at ceiling 12, reaching depth 8 on 6 rows, depth 9 on 17, depth 10 on 4, and depth 11 on 9. Twenty-seven selections match FoldBot's historically played move and four match Stockfish's recorded best move. These are source-bound development measurements, not official games or agent-owned conclusions.

The next state is to derive the next lossless calculation improvement from this exact position surface, repeat the same matched measurement, and execute the 2100 run when Maria designates the build. The secured 1900 victory, exact legality and endgame proofs, common-depth value identity, and depth-8–11 real-position execution establish a constructive continuation. No theorem-derived wall has been produced.
...
Agent-selected component-transplant experiments — fold objects frozen inside SGD-trained transformer hosts — measured a positional-code difference of 0.14 and a theorem-derived attention-cascade difference of 0.71 under their named protocols. An attribution control then decomposed the attention difference: **0.54 came from the transplant construction itself** (severed gradients — the host's home game) and **0.17 from the tested fold component.** These are auxiliary measurements of that construction, not Maria's failed predictions or authority to close the fold-native investigation. The construction bias is recorded because test construction itself can dominate the measured outcome.
...
## 7. Proofs, measurements, and the active programme
...
**Auxiliary measurements, retained with their scope and ownership:** fold-basis weight compression against production-grade per-block quantization under the tested budgets and tensors; component transplants inside SGD-transformer hosts at toy scale, with the attribution control quantifying construction effects; and the first registered basis-hunt round in the coordinates tested. An agent-authored expectation that does not hold is a result about that auxiliary expectation, not Maria's failed prediction and not a closure of the programme.
...
3. **Matches**: `tools/measure_sf.py <elo|full>` (pinned 12-game protocol); `tools/h2h_gate.py <old-binary>` (historically named comparison harness); `tools/summit_probe.py <label>` (full-forensics probe); the complete ledger is `tools/MATCHES.md`. These tools measure; Maria alone authorizes real runs and declares conclusions.
...
5. **Neural-weight measurements**: `fold_ai/PROTOCOL.md` (pre-registration, every amendment and measured outcome logged); `fold_ai/spectral_probe.py` and the rung scripts (seed 20260706); result files committed verbatim; the F2-linear self-test certifies each run internally. All probed models are public releases; every margin in this paper reproduces on commodity hardware.
