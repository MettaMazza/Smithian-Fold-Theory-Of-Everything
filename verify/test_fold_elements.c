...
long long _main();
long long oscillator_shell(long long);
long long magic_closure(long long);
long long spin_orbit_threshold();
long long generators_trace_to_one();
long long reproduces_known_magic();
long long no_closure_between_82_and_126();
long long smithium_proton_number();
long long smithium_neutron_number();
long long smithium_mass_number();
long long island_is_forced();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the magic numbers and SMITHIUM (the island of stability) ===");
    ok = expect_equal((long long)"the oscillator shell H(0): the first magic 2", int_to_string(oscillator_shell(0)), (long long)"2");
    ok = expect_equal((long long)"the oscillator shell H(2)", int_to_string(oscillator_shell(2)), (long long)"20");
    ok = expect_equal((long long)"the reordered closure M(3): magic 28 (spin-orbit intruder)", int_to_string(magic_closure(3)), (long long)"28");
    ok = expect_equal((long long)"the reordered closure M(5): magic 82", int_to_string(magic_closure(5)), (long long)"82");
    ok = expect_equal((long long)"the spin-orbit full-shell threshold: l = 3", int_to_string(spin_orbit_threshold()), (long long)"3");
    ok = expect_bool((long long)"the generators trace to the One (spin base 2, colour-3 orbit)", generators_trace_to_one());
    ok = expect_bool((long long)"THE GENERATOR reproduces ALL EIGHT known magic numbers", reproduces_known_magic());
    ok = expect_bool((long long)"EXCLUSION: no closure between 82 and 126 (so NOT 114 or 120)", no_closure_between_82_and_126());
    ok = expect_equal((long long)"SMITHIUM's proton number (the next closure past 82)", int_to_string(smithium_proton_number()), (long long)"126");
    ok = expect_equal((long long)"SMITHIUM's neutron number (the next closure past 126)", int_to_string(smithium_neutron_number()), (long long)"184");
    ok = expect_equal((long long)"SMITHIUM's mass number: the doubly-magic 126 + 184", int_to_string(smithium_mass_number()), (long long)"310");
    ok = expect_bool((long long)"THE ISLAND IS FORCED: SMITHIUM (Sh, Z=126, N=184, A=310)", island_is_forced());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long oscillator_shell(long long n) {
    long long ret_val = 0;

    ret_val = ((((n + 1) * (n + 2)) * (n + 3)) / 3);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long magic_closure(long long k) {
    long long ret_val = 0;

    if (k < 3) {
    ret_val = ((((k + 1) * (k + 2)) * (k + 3)) / 3);
    goto L_cleanup;
    }
    ret_val = ((((((k - 1) + 1) * ((k - 1) + 2)) * ((k - 1) + 3)) / 3) + (2 * (k + 1)));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long spin_orbit_threshold() {
    ...
    coupling = fraction_from_ratio(2, 3);
    l = 1;
    found = 0;
    while (found == 0) {
    half_l = fraction_from_ratio(l, 2);
    shift = fraction_multiply(coupling, half_l);
    one = fraction_from_whole_number(1);
    reaches = fraction_compare(shift, one) < 0;
    if (reaches) {
    l = (l + 1);
    } else {
    found = l;
...
long long generators_trace_to_one() {
    ...
    long long spin = 0;
    long long spin_ok = 0;
    long long spin_twin = 0;
    long long two_thirds = 0;
    long long ret_val = 0;

    ep_gc_push_root(&c1);
    ep_gc_push_root(&c1_twin);
    ep_gc_push_root(&c2);
    ep_gc_push_root(&doubled);
    ep_gc_push_root(&one);
    ep_gc_push_root(&spin);
    ep_gc_push_root(&spin_twin);
    ep_gc_push_root(&two_thirds);

    ep_gc_maybe_collect();

    spin = fraction_from_ratio(1, 2);
    spin_twin = fraction_from_ratio(1, 2);
    doubled = fraction_add(spin, spin_twin);
    one = fraction_from_whole_number(1);
    spin_ok = fraction_compare(doubled, one) == 0;
    c1 = fraction_from_ratio(1, 3);
    c1_twin = fraction_from_ratio(1, 3);
    c2 = fraction_add(c1, c1_twin);
    two_thirds = fraction_from_ratio(2, 3);
    c2_ok = fraction_compare(c2, two_thirds) == 0;
    if (spin_ok) {
    ret_val = c2_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(c2);
    c2 = 0;
    free_struct_Fraction(doubled);
    doubled = 0;
    free_struct_Fraction(one);
    one = 0;
...
long long reproduces_known_magic() {
    long long known_126 = 0;
    long long known_184 = 0;
    long long known_2 = 0;
    long long known_20 = 0;
    long long known_28 = 0;
    long long known_50 = 0;
    long long known_8 = 0;
    long long known_82 = 0;
    long long ret_val = 0;

    known_2 = magic_closure(0) == 2;
    known_8 = magic_closure(1) == 8;
    known_20 = magic_closure(2) == 20;
    known_28 = magic_closure(3) == 28;
    known_50 = magic_closure(4) == 50;
    known_82 = magic_closure(5) == 82;
    known_126 = magic_closure(6) == 126;
    known_184 = magic_closure(7) == 184;
    if (known_2) {
    if (known_8) {
    if (known_20) {
    if (known_28) {
...
long long no_closure_between_82_and_126() {
    ...
    clear = 1LL;
    closure = magic_closure(0);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(1);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(2);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(3);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(4);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(5);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(6);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
    }
    closure = magic_closure(7);
    if (closure > 82) {
    if (closure < 126) {
    clear = 0LL;
    }
...
long long smithium_mass_number() {
    ...
    ret_val = (magic_closure(6) + magic_closure(7));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long island_is_forced() {
    long long anchored = 0;
    long long no_intruder = 0;
    long long reproduces = 0;
    long long smithium_ok = 0;
    long long threshold_ok = 0;
    long long ret_val = 0;

    anchored = generators_trace_to_one();
    reproduces = reproduces_known_magic();
    threshold_ok = spin_orbit_threshold() == 3;
    no_intruder = no_closure_between_82_and_126();
    smithium_ok = magic_closure(6) == 126;
    if (anchored) {
    if (reproduces) {
    if (threshold_ok) {
    if (no_intruder) {
