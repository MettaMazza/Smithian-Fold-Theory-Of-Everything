...
long long rotation_balance();
long long balance_is_self_antipodal();
long long pull_plus_motion();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== galactic dynamics (flat rotation curves need a dark halo) ===");
    ok = expect_equal((long long)"rotation-support balance = 1/2 (self-antipodal)", fraction_to_text(rotation_balance()), (long long)"1/2");
    ok = expect_bool((long long)"the balance is self-antipodal (pull matched to motion)", balance_is_self_antipodal());
    ok = expect_equal((long long)"pull + motion sum to the One", fraction_to_text(pull_plus_motion()), (long long)"1");
    ok = expect_equal((long long)"the balance folds to the One (a bound, steady orbit)", fraction_to_text(balance_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long balance_is_self_antipodal() {
    ...
    complement = fraction_subtract(one, rotation_balance());
    ret_val = fraction_compare(complement, rotation_balance()) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long pull_plus_motion() {
    ...
    b = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(b, b);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long balance_folds_to_one() {
    ...
    ret_val = fold_fraction(rotation_balance());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
