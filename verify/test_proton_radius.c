...
long long quark_inner_position();
long long proton_radius();
long long edge_folds_to_center();
long long inner_plus_radius();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the proton radius (edge = complement of the quark) ===");
    ok = expect_equal((long long)"quark inner position = 1/3", fraction_to_text(quark_inner_position()), (long long)"1/3");
    ok = expect_equal((long long)"proton radius = take(One, 1/3) = 2/3", fraction_to_text(proton_radius()), (long long)"2/3");
    ok = expect_bool((long long)"the edge folds back to the quark centre (closes)", edge_folds_to_center());
    ok = expect_equal((long long)"inner + radius = the One (centre + reach)", fraction_to_text(inner_plus_radius()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long proton_radius() {
    ...
    inner = fraction_from_ratio(1, colour_count());
    one = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, inner);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long edge_folds_to_center() {
    ...
    edge = proton_radius();
    folded = fold_fraction(edge);
    inner = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, inner) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long inner_plus_radius() {
    ...
    inner = fraction_from_ratio(1, colour_count());
    edge = proton_radius();
    ret_val = fraction_add(inner, edge);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
