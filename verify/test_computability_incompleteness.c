...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold incompleteness boundary ===");
    ok = expect_true((long long)"zero missing distinctions is complete", fold_description_is_complete(0));
    ok = expect_equal((long long)"three missing distinctions have b^3 completions", int_to_string(fold_compatible_completion_count(3)), int_to_string(8));
    ok = expect_true((long long)"every positive missing depth is incomplete", every_missing_fold_distinction_is_incomplete(7));
    ok = expect_true((long long)"held degree information completes an exact source", held_degree_completes_description(85, 7, 3));
    ok = expect_true((long long)"the complete incompleteness census holds", complete_fold_incompleteness_census());
    ok = expect_true((long long)"false uniqueness with a missing label is rejected", false_unique_incomplete_description_is_rejected());
    ok = expect_true((long long)"false self-negating completion is rejected", false_self_negating_completion_is_rejected());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
