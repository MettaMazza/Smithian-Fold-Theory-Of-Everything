...
long long _main();
long long covering_depth_base(long long, long long);
long long alpha_of_generators(long long, long long);
long long anchor_agrees();
long long mutations_move();
long long assembly_with_cov(long long);
long long misbuilds_rejected();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== alpha is forced, assembly and all: mutations and mis-builds ===");
    ok = expect_bool((long long)"THE ANCHOR: alpha(2,3) equals the forced 34259/250 (two modules, one number)", anchor_agrees());
    ok = expect_equal((long long)"the depths re-counted at (3,3): base-3 cover of 27", int_to_string(covering_depth_base(27, 3)), (long long)"3");
    ok = expect_equal((long long)"mutation (2,4): 1/alpha moves to 7345/27", fraction_to_text(alpha_of_generators(2, 4)), (long long)"7345/27");
    ok = expect_equal((long long)"mutation (3,3): 1/alpha moves to 811/9", fraction_to_text(alpha_of_generators(3, 3)), (long long)"811/9");
    ok = expect_bool((long long)"EVERY generator mutation moves 1/alpha (nothing frozen)", mutations_move());
    ok = expect_bool((long long)"EVERY covering mis-build rejected (125, 500, 375, 686)", misbuilds_rejected());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long covering_depth_base(long long volume, long long base) {
    long long d = 0;
    long long p = 0;
    long long ret_val = 0;
...
long long alpha_of_generators(long long b, long long c) {
    long long cov = 0;
    long long d_down = 0;
    long long d_up = 0;
    ...
    long long surface = 0;
    long long tower = 0;
    long long tower_fraction = 0;
    long long volume_down = 0;
    long long volume_up = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cov);
    ep_gc_push_root(&d_down);
    ep_gc_push_root(&d_up);
    ep_gc_push_root(&dilation);
    ep_gc_push_root(&surface);
    ep_gc_push_root(&tower);
    ep_gc_push_root(&tower_fraction);
    ep_gc_push_root(&volume_down);
    ep_gc_push_root(&volume_up);
    ep_gc_push_root(&b);
    ep_gc_push_root(&c);

    ep_gc_maybe_collect();

    volume_down = whole_power(c, 3);
    d_down = covering_depth_base(volume_down, b);
    volume_up = whole_power(c, 4);
    d_up = covering_depth_base(volume_up, b);
    tower = whole_power(b, d_up);
    tower_fraction = fraction_from_whole_number(tower);
    surface = whole_power(c, 2);
    cov = (b * whole_power(d_down, 3));
    dilation = fraction_from_ratio((surface * (cov + 1)), cov);
    ret_val = fraction_add(tower_fraction, dilation);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(11);
    return ret_val;
...
long long anchor_agrees() {
    ...
    ep_gc_push_root(&independent);

    ep_gc_maybe_collect();

    here = alpha_of_generators(2, 3);
    independent = inverse_fine_structure_first_order();
    ret_val = fraction_compare(here, independent) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(here);
    here = 0;
    free_struct_Fraction(independent);
    independent = 0;
    return ret_val;
}

long long mutations_move() {
    long long all_moved = 0;
    long long base_up = 0;
    long long colour_five = 0;
    long long colour_up = 0;
    long long first_same = 0;
    long long second_same = 0;
    long long target = 0;
    long long target_three = 0;
    long long target_two = 0;
    long long third_same = 0;
    long long ret_val = 0;

    ep_gc_push_root(&base_up);
    ep_gc_push_root(&colour_five);
    ep_gc_push_root(&colour_up);
    ep_gc_push_root(&target);
    ep_gc_push_root(&target_three);
    ep_gc_push_root(&target_two);

    ep_gc_maybe_collect();

    target = inverse_fine_structure_first_order();
    colour_up = alpha_of_generators(2, 4);
    first_same = fraction_compare(colour_up, target) == 0;
    base_up = alpha_of_generators(3, 3);
    target_two = inverse_fine_structure_first_order();
    second_same = fraction_compare(base_up, target_two) == 0;
    colour_five = alpha_of_generators(2, 5);
    target_three = inverse_fine_structure_first_order();
    third_same = fraction_compare(colour_five, target_three) == 0;
    all_moved = 1LL;
    if (first_same) {
    all_moved = 0LL;
    }
...
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(base_up);
    base_up = 0;
    free_struct_Fraction(colour_five);
    colour_five = 0;
    free_struct_Fraction(colour_up);
    colour_up = 0;
    free_struct_Fraction(target);
    target = 0;
    free_struct_Fraction(target_three);
    target_three = 0;
    free_struct_Fraction(target_two);
    target_two = 0;
    return ret_val;
}

long long assembly_with_cov(long long cov) {
    long long dilation = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    dilation = fraction_from_ratio((9 * (cov + 1)), cov);
    ret_val = fraction_add(tower, dilation);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long misbuilds_rejected() {
    ...
    long long base_squared = 0;
    long long colour_build = 0;
    long long first_same = 0;
    long long fourth_same = 0;
    long long no_base = 0;
    long long second_same = 0;
    long long target = 0;
    long long target_four = 0;
    long long target_three = 0;
    long long target_two = 0;
    long long third_same = 0;
    long long up_cube = 0;
    ...
    ep_gc_push_root(&base_squared);
    ep_gc_push_root(&colour_build);
    ep_gc_push_root(&no_base);
    ep_gc_push_root(&target);
    ep_gc_push_root(&target_four);
    ep_gc_push_root(&target_three);
    ep_gc_push_root(&target_two);
    ep_gc_push_root(&up_cube);

    ep_gc_maybe_collect();

    target = inverse_fine_structure_first_order();
    no_base = assembly_with_cov(125);
    first_same = fraction_compare(no_base, target) == 0;
    base_squared = assembly_with_cov(500);
    target_two = inverse_fine_structure_first_order();
    second_same = fraction_compare(base_squared, target_two) == 0;
    colour_build = assembly_with_cov(375);
    target_three = inverse_fine_structure_first_order();
    third_same = fraction_compare(colour_build, target_three) == 0;
    up_cube = assembly_with_cov(686);
    target_four = inverse_fine_structure_first_order();
    fourth_same = fraction_compare(up_cube, target_four) == 0;
    all_rejected = 1LL;
    if (first_same) {
    all_rejected = 0LL;
    }
...
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(base_squared);
    base_squared = 0;
    free_struct_Fraction(colour_build);
    colour_build = 0;
    free_struct_Fraction(no_base);
    no_base = 0;
    free_struct_Fraction(target);
    target = 0;
    free_struct_Fraction(target_four);
    target_four = 0;
    free_struct_Fraction(target_three);
    target_three = 0;
    free_struct_Fraction(target_two);
    target_two = 0;
    free_struct_Fraction(up_cube);
    up_cube = 0;
    return ret_val;
}
