...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== radioactive decay ===");
    ok = expect_equal((long long)"remaining after 0 half-lives = 1", fraction_to_text(remaining_after(0)), (long long)"1");
    ok = expect_equal((long long)"remaining after 1 half-life = 1/2", fraction_to_text(remaining_after(1)), (long long)"1/2");
    ok = expect_equal((long long)"remaining after 2 half-lives = 1/4", fraction_to_text(remaining_after(2)), (long long)"1/4");
    ok = expect_equal((long long)"remaining after 3 half-lives = 1/8", fraction_to_text(remaining_after(3)), (long long)"1/8");
    ok = expect_bool((long long)"each half-life halves the survivors", half_life_halves());
    ok = expect_bool((long long)"decay never reaches zero (No-Zero)", never_reaches_zero());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
