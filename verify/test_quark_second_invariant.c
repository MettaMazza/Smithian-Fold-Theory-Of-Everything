...
long long _main();
long long dual_invariant_direct(long long);
long long dual_invariant_by_floor(long long);
long long floors_carry_their_depths();
long long routes_agree_at_both_depths();
long long lepton_form_at(long long);
long long duality_is_a_swap();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the quark second invariant (the dual, two routes) ===");
    ok = expect_equal((long long)"ROUTE A at d=5: 1/(3*32-1) = 1/95", fraction_to_text(dual_invariant_direct(5)), (long long)"1/95");
    ok = expect_equal((long long)"ROUTE A at d=7: 1/(3*128-1) = 1/383", fraction_to_text(dual_invariant_direct(7)), (long long)"1/383");
    ok = expect_bool((long long)"the floors carry their depths: period(1/31)=5, period(1/127)=7", floors_carry_their_depths());
    ok = expect_bool((long long)"ROUTE B (orbit floors) agrees at BOTH depths", routes_agree_at_both_depths());
    ok = expect_bool((long long)"the duality is a swap: the lepton form gives 1/485", duality_is_a_swap());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long dual_invariant_direct(long long d) {
    ...
    tower = whole_power(binary_count(), d);
    denominator = ((smallest_fold_period_above(binary_count()) * tower) - 1);
...
long long dual_invariant_by_floor(long long d) {
    long long denominator = 0;
    long long floor = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    floor = (whole_power(binary_count(), d) - 1);
    tower = (floor + 1);
    denominator = ((smallest_fold_period_above(binary_count()) * tower) - 1);
    ret_val = fraction_from_ratio(1, denominator);
    goto L_cleanup;
...
long long floors_carry_their_depths() {
    long long down_floor = 0;
    long long down_ok = 0;
    long long down_period = 0;
    long long up_floor = 0;
    long long up_ok = 0;
    long long up_period = 0;
    long long ret_val = 0;

    ep_gc_push_root(&down_floor);
    ep_gc_push_root(&up_floor);

    ep_gc_maybe_collect();

    down_floor = (whole_power(binary_count(), 5) - 1);
    up_floor = (whole_power(binary_count(), 7) - 1);
    down_period = fold_period_of_unit_fraction(down_floor);
    up_period = fold_period_of_unit_fraction(up_floor);
    down_ok = down_period == 5;
    up_ok = up_period == 7;
    if (down_ok) {
    ret_val = up_ok;
    goto L_cleanup;
...
long long routes_agree_at_both_depths() {
    ...
    down_a = dual_invariant_direct(5);
    down_b = dual_invariant_by_floor(5);
    up_a = dual_invariant_direct(7);
    up_b = dual_invariant_by_floor(7);
    down_ok = fraction_compare(down_a, down_b) == 0;
    up_ok = fraction_compare(up_a, up_b) == 0;
    if (down_ok) {
    ret_val = up_ok;
    goto L_cleanup;
    }
...
long long lepton_form_at(long long d) {
    long long denominator = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    denominator = ((smallest_fold_period_above(1) * tower) - 1);
    ret_val = fraction_from_ratio(1, denominator);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long duality_is_a_swap() {
    ...
    ep_gc_push_root(&lepton);
    ep_gc_push_root(&target);

    ep_gc_maybe_collect();

    lepton = lepton_form_at(5);
    target = fraction_from_ratio(1, 485);
    ret_val = fraction_compare(lepton, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(lepton);
    lepton = 0;
