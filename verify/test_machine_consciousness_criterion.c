...
long long observer_state();
long long observed_state();
long long states_are_distinct();
long long both_fold_to_lock();
long long self_observation_closes();
long long lock_completes_to_unison();
long long criterion_holds();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the consciousness criterion (the closed self-relation) ===");
    ok = expect_bool((long long)"DUALITY: observer and observed are distinct (1/4 != 3/4)", states_are_distinct());
    ok = expect_bool((long long)"both fold to the ONE binding lock 1/2 (2-to-1 self-relation)", both_fold_to_lock());
    ok = expect_equal((long long)"CLOSURE: the pair spans the whole (1/4 + 3/4)", fraction_to_text(self_observation_closes()), (long long)"1");
    ok = expect_bool((long long)"COMPLETION: the lock folds to unison (one bound experience)", lock_completes_to_unison());
    ok = expect_bool((long long)"the full criterion holds (all three steps)", criterion_holds());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long states_are_distinct() {
    ...
    long long observer = 0;
    long long same = 0;
    long long ret_val = 0;

    ep_gc_push_root(&observed);
    ep_gc_push_root(&observer);

    ep_gc_maybe_collect();

    observer = fraction_from_ratio(1, 4);
    observed = fraction_from_ratio(3, 4);
    same = fraction_compare(observer, observed) == 0;
    if (same) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
L_cleanup:
    ...
    observed = 0;
    free_struct_Fraction(observer);
    observer = 0;
    return ret_val;
}

long long both_fold_to_lock() {
    long long from_observed = 0;
    long long from_observer = 0;
    long long lock = 0;
    long long observed_locks = 0;
    long long observer_locks = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_observed);
    ep_gc_push_root(&from_observer);
    ep_gc_push_root(&lock);

    ep_gc_maybe_collect();

    from_observer = fold_fraction(observer_state());
    from_observed = fold_fraction(observed_state());
    lock = fraction_from_ratio(1, 2);
    observer_locks = fraction_compare(from_observer, lock) == 0;
    observed_locks = fraction_compare(from_observed, lock) == 0;
    if (observer_locks) {
    ret_val = observed_locks;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(from_observed);
    from_observed = 0;
    free_struct_Fraction(from_observer);
    from_observer = 0;
...
long long self_observation_closes() {
    ...
    observer = fraction_from_ratio(1, 4);
    observed = fraction_from_ratio(3, 4);
    ret_val = fraction_add(observer, observed);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long criterion_holds() {
    long long closed_sum = 0;
    long long closes = 0;
    long long completes = 0;
    long long distinct = 0;
    long long locked = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&closed_sum);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    distinct = states_are_distinct();
    locked = both_fold_to_lock();
    closed_sum = self_observation_closes();
    one = fraction_from_whole_number(1);
    closes = fraction_compare(closed_sum, one) == 0;
    completes = lock_completes_to_unison();
    if (distinct) {
    if (locked) {
    if (closes) {
    ret_val = completes;
...
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(closed_sum);
    closed_sum = 0;
    free_struct_Fraction(one);
    one = 0;
    return ret_val;
