...
long long charged_mass_part(long long);
long long neutral_mass_part(long long);
long long mass_ratio(long long);
long long mass_ratio_is_one_over_m_minus_one(long long);
long long mass_ratio_equals_mixing(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the weak mass ratio ===");
    ok = expect_equal((long long)"charged mass-part (m=3) = 1/3", fraction_to_text(charged_mass_part(3)), (long long)"1/3");
    ok = expect_equal((long long)"neutral mass-part (m=3) = 2/3", fraction_to_text(neutral_mass_part(3)), (long long)"2/3");
    ok = expect_equal((long long)"mass ratio (m=2, electroweak) = 1", fraction_to_text(mass_ratio(2)), (long long)"1");
    ok = expect_equal((long long)"mass ratio (m=3) = 1/2", fraction_to_text(mass_ratio(3)), (long long)"1/2");
    ok = expect_equal((long long)"mass ratio (m=4) = 1/3", fraction_to_text(mass_ratio(4)), (long long)"1/3");
    ok = expect_bool((long long)"mass ratio = 1/(m-1) at m=2,3,4", mass_ratio_is_one_over_m_minus_one(4));
    ok = expect_bool((long long)"mass ratio equals the mixing ratio (m=3)", mass_ratio_equals_mixing(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long neutral_channel(long long m) {
    ...
    ret_val = fraction_from_ratio(1, m);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long charged_mass_part(long long m) {
    long long channel_split = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&channel_split);
    ep_gc_push_root(&one);
    ep_gc_push_root(&m);
    ...
    channel_split = fraction_from_ratio((m - 1), m);
    ret_val = fraction_subtract(one, channel_split);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long neutral_mass_part(long long m) {
    long long channel_split = 0;
    long long one = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    channel_split = fraction_from_ratio(1, m);
    ret_val = fraction_subtract(one, channel_split);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long mass_ratio(long long m) {
    long long charged = 0;
    long long neutral = 0;
    long long ret_val = 0;

    ep_gc_push_root(&charged);
    ep_gc_push_root(&neutral);
    ep_gc_push_root(&m);

    ep_gc_maybe_collect();

    charged = charged_mass_part(m);
    neutral = neutral_mass_part(m);
    ret_val = fraction_divide(charged, neutral);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long mass_ratio_is_one_over_m_minus_one(long long m) {
    ...
    ratio = mass_ratio(m);
    expected = fraction_from_ratio(1, (m - 1));
    ret_val = fraction_compare(ratio, expected) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long mass_ratio_equals_mixing(long long m) {
    long long ratio = 0;
    long long charged = 0;
    long long mixing = 0;
    long long neutral = 0;
    ...
    ratio = mass_ratio(m);
    neutral = fraction_from_ratio(1, m);
    charged = fraction_from_ratio((m - 1), m);
    mixing = fraction_divide(neutral, charged);
    ret_val = fraction_compare(ratio, mixing) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(ratio);
    ratio = 0;
    free_struct_Fraction(mixing);
    mixing = 0;
    return ret_val;
}
