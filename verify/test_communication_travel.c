...
long long crossing_walk();
long long composition_is_composite();
long long landed_period();
long long world_period(long long);
long long least_common(long long, long long);
long long rhythm_is_joint();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the world-crossing walk (composition carries between worlds) ===");
    ok = expect_bool((long long)"the composition step lands on 8/15 (both shadows on board)", composition_is_composite());
    ok = expect_equal((long long)"the walk lands on 1/15", fraction_to_text(crossing_walk()), (long long)"1/15");
    ok = expect_equal((long long)"the landed orbit period, walked = 4", int_to_string(landed_period()), (long long)"4");
    ok = expect_equal((long long)"the 3-world rhythm = 2, walked", int_to_string(world_period(3)), (long long)"2");
    ok = expect_equal((long long)"the 5-world rhythm = 4, walked", int_to_string(world_period(5)), (long long)"4");
    ok = expect_bool((long long)"the landed rhythm = lcm of the crossed worlds (4 = lcm(2,4))", rhythm_is_joint());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long crossing_walk() {
    long long carrier = 0;
    long long composed = 0;
    long long second = 0;
    long long start = 0;
    long long ret_val = 0;

    ep_gc_push_root(&carrier);
    ep_gc_push_root(&composed);
    ep_gc_push_root(&second);
    ep_gc_push_root(&start);

    ep_gc_maybe_collect();

    start = fraction_from_ratio(1, 6);
    second = fold_fraction(start);
    carrier = fraction_from_ratio(1, 5);
    composed = fraction_add(second, carrier);
    ret_val = fold_fraction(composed);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long composition_is_composite() {
    ...
    carrier = fraction_from_ratio(1, 5);
    composed = fraction_add(second, carrier);
    target = fraction_from_ratio(8, 15);
    ret_val = fraction_compare(composed, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(composed);
    composed = 0;
...
long long landed_period() {
    ...
    period = (period + 1);
    returned = fraction_compare(state, landed) == 0;
    if (returned) {
    walking = 0LL;
    }
    }
...
long long world_period(long long p) {
    long long period = 0;
    long long s = 0;
    long long walking = 0;
    long long ret_val = 0;
...
long long least_common(long long x, long long y) {
    long long a = 0;
    long long b = 0;
    long long r = 0;
    long long ret_val = 0;
...
long long rhythm_is_joint() {
    long long five_rhythm = 0;
    long long joint = 0;
    long long landed = 0;
    long long three_rhythm = 0;
    long long ret_val = 0;

    ep_gc_push_root(&five_rhythm);
    ep_gc_push_root(&three_rhythm);

    ep_gc_maybe_collect();

    landed = landed_period();
    three_rhythm = world_period(3);
    five_rhythm = world_period(5);
    joint = least_common(three_rhythm, five_rhythm);
    ret_val = landed == joint;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
