...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the compact generator: derived forward, traced to the One ===");
    ok = expect_bool((long long)"THE GROUP LAW: chi_a chi_b = chi_(a XOR b) -- all a,b < 8, all n < 64", group_law_holds(8, 64));
    ok = expect_bool((long long)"the subtraction-game span {0,1,2,3} is pairwise distinct", span_is_distinct(1, 2));
    ok = expect_equal((long long)"its generator coefficient: the dyadic 1/2^2", fraction_to_text(generator_coefficient(2)), (long long)"1/4");
    ok = expect_bool((long long)"SUBTRACTION reconstructed exactly over 2^6 states", subtraction_reconstructs_to(64));
    ok = expect_bool((long long)"SUBTRACTION reconstructed exactly over 2^10 states (generator CONSTANT)", subtraction_reconstructs_to(1024));
    ok = expect_bool((long long)"the Nim column span is pairwise distinct (3 heaps, 3 bits)", span_is_distinct(73, 3));
    ok = expect_equal((long long)"the Nim generator length 2^bits at 3 bits", int_to_string(nim_generator_length(3)), (long long)"8");
    ok = expect_bool((long long)"NIM (2 heaps) reconstructed exactly -- field 64, generator 8", nim_reconstructs(2, 3));
    ok = expect_bool((long long)"NIM (3 heaps) reconstructed exactly -- field 512, generator 8", nim_reconstructs(3, 3));
    ok = expect_bool((long long)"NIM (4 heaps) reconstructed exactly -- field 4096, generator STILL 8", nim_reconstructs(4, 3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
