...
long long electroweak_coupling();
long long fermion_mass_share();
long long displaced_vacuum();
long long mass_part_is_vacuum();
long long mass_part_is_proper_preimage();
long long point_is_self_antipodal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the fermion mass-part (mass is vacuum-shaped) ===");
    ok = expect_equal((long long)"the electroweak coupling = 1/2", fraction_to_text(electroweak_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"the fermion mass-part = the shortfall = 1/2", fraction_to_text(fermion_mass_share()), (long long)"1/2");
    ok = expect_bool((long long)"THE IDENTITY: the mass-part equals the displaced vacuum", mass_part_is_vacuum());
    ok = expect_bool((long long)"the shortfall is the fold's unique proper preimage of the One", mass_part_is_proper_preimage());
    ok = expect_bool((long long)"the point is self-antipodal (mass, vacuum, coupling: one point)", point_is_self_antipodal());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fermion_mass_share() {
    ...
    coupling = fraction_from_ratio(1, 2);
    ret_val = fraction_subtract(one, coupling);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long mass_part_is_vacuum() {
    ...
    mass = fermion_mass_share();
    vacuum = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(mass, vacuum) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(mass);
    mass = 0;
...
long long mass_part_is_proper_preimage() {
    ...
    long long folded = 0;
    long long folds_to_one = 0;
    long long is_proper = 0;
    long long mass = 0;
    long long one = 0;
    long long ret_val = 0;
    ...
    mass = fermion_mass_share();
    folded = fold_fraction(mass);
    one = fraction_from_whole_number(1);
    folds_to_one = fraction_compare(folded, one) == 0;
    comparison = fraction_compare(mass, one);
    is_proper = comparison < 0;
    if (folds_to_one) {
    ret_val = is_proper;
    goto L_cleanup;
    }
...
long long point_is_self_antipodal() {
    ...
    mass = fermion_mass_share();
    one = fraction_from_whole_number(1);
    antipode = fraction_subtract(one, mass);
    ret_val = fraction_compare(mass, antipode) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
