...
long long light_and_gravity_share_speed() {
    ...
    ret_val = fold_value_compare(light_speed(), gravitational_wave_speed()) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long signal_speed_is_the_maximum() {
    ...
    full_turn = rotate(fold_signal_speed(), the_one());
    ret_val = fold_value_compare(full_turn, the_one()) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
