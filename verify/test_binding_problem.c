...
long long first_process();
long long second_process();
long long first_folds_to_second();
long long second_folds_to_first();
long long processes_partition_one();
long long lock_threshold();
long long lock_reaches_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the binding problem (two streams, one experience) ===");
    ok = expect_equal((long long)"first process = 1/3", fraction_to_text(first_process()), (long long)"1/3");
    ok = expect_equal((long long)"second process = 2/3", fraction_to_text(second_process()), (long long)"2/3");
    ok = expect_bool((long long)"first folds to second (one shared cycle)", first_folds_to_second());
    ok = expect_bool((long long)"second folds back to first (period 2)", second_folds_to_first());
    ok = expect_equal((long long)"together they partition the One", fraction_to_text(processes_partition_one()), (long long)"1");
    ok = expect_equal((long long)"lock threshold = their balance = 1/2", fraction_to_text(lock_threshold()), (long long)"1/2");
    ok = expect_equal((long long)"the lock folds to unison (one bound whole)", fraction_to_text(lock_reaches_unison()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_folds_to_second() {
    ...
    folded = fold_fraction(first);
    second = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_compare(folded, second) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long second_folds_to_first() {
    ...
    second = fraction_from_ratio(binary_count(), colour_count());
    folded = fold_fraction(second);
    first = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, first) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long lock_reaches_unison() {
    ...
    ret_val = fold_fraction(lock_threshold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
