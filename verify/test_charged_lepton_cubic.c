...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the charged-lepton cubic (forced invariants) ===");
    ok = expect_equal((long long)"roots sum to the One", fraction_to_text(no_loss_partition()), (long long)"1");
    ok = expect_equal((long long)"second invariant = 1/6", fraction_to_text(second_invariant()), (long long)"1/6");
    ok = expect_equal((long long)"third invariant (leading) = 1/485", fraction_to_text(leading_third_invariant()), (long long)"1/485");
    ok = expect_equal((long long)"third invariant (sharpened) = 3/1454", fraction_to_text(third_invariant()), (long long)"3/1454");
    printf("%s\n", (char*)(long long)"--- the neutral channel is the colour count; others do not land ---");
    ok = expect_equal((long long)"colour channel (3) -> 3/1454", fraction_to_text(third_invariant_with_channel(3)), (long long)"3/1454");
    ok = expect_equal((long long)"channel 2 -> 2/969 (rejected)", fraction_to_text(third_invariant_with_channel(2)), (long long)"2/969");
    ok = expect_equal((long long)"channel 4 -> 4/1939 (rejected)", fraction_to_text(third_invariant_with_channel(4)), (long long)"4/1939");
    forced = neutral_channel_is_forced();
    if (forced == 1) {
    printf("%s\n", (char*)(long long)"  ok    neutral channel is forced, not selected");
    } else {
    printf("%s\n", (char*)(long long)"  FAIL  neutral channel uniqueness not enforced");
    }
    ok = expect_equal((long long)"the e2 invariant FORM is unique (forced_unique)", int_to_string(second_invariant_form_is_unique()), (long long)"1");
    ok = expect_equal((long long)"the leading e3 invariant FORM is unique (forced_unique)", int_to_string(leading_third_invariant_form_is_unique()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
