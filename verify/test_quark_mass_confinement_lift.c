...
long long _main() {
    long long corr_down = 0;
    long long corr_up = 0;
    long long down_bare = 0;
    long long down_lift = 0;
    long long down_target = 0;
    long long measured_down = 0;
    long long measured_up = 0;
    long long ok = 0;
    long long up_bare = 0;
    long long up_lift = 0;
    long long up_target = 0;
    long long ret_val = 0;

    ep_gc_push_root(&corr_down);
    ep_gc_push_root(&corr_up);
    ep_gc_push_root(&down_bare);
    ep_gc_push_root(&down_lift);
    ep_gc_push_root(&down_target);
    ep_gc_push_root(&measured_down);
    ep_gc_push_root(&measured_up);
    ep_gc_push_root(&up_bare);
    ep_gc_push_root(&up_lift);
    ep_gc_push_root(&up_target);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the confinement lift (the lightest quark is doubled) ===");
    ok = expect_equal((long long)"the sharpened invariant at d=5: 3/1454", fraction_to_text(sharpened_invariant(5)), (long long)"3/1454");
    ok = expect_equal((long long)"at d=7: 3/13118", fraction_to_text(sharpened_invariant(7)), (long long)"3/13118");
    ok = expect_equal((long long)"the lift factor, COUNTED: the fold's fibre = 2", int_to_string(lift_factor_counted()), (long long)"2");
    down_bare = down_bare_ratio();
    printf("%s\n", (char*)concat((long long)"  info  bare s/d (sharpened) = ", fraction_to_decimal(down_bare, 4)));
    up_bare = up_bare_ratio();
    printf("%s\n", (char*)concat((long long)"  info  bare c/u (sharpened) = ", fraction_to_decimal(up_bare, 4)));
    measured_down = fraction_from_ratio(950, 47);
    corr_down = fraction_from_ratio(1004988, 1000000);
    down_target = fraction_divide(measured_down, corr_down);
    down_lift = fraction_divide(down_bare, down_target);
    printf("%s\n", (char*)concat((long long)"  info  down lift = ", fraction_to_decimal(down_lift, 5)));
    ok = expect_bool((long long)"DOWN: bare/(measured/corr) lands on 2 (the counted fibre)", lands_on_two(down_lift));
    measured_up = fraction_from_ratio(6375, 11);
    corr_up = fraction_from_ratio(11916, 10000);
    up_target = fraction_divide(measured_up, corr_up);
    up_lift = fraction_divide(up_bare, up_target);
    printf("%s\n", (char*)concat((long long)"  info  up lift = ", fraction_to_decimal(up_lift, 5)));
    ok = expect_bool((long long)"UP: bare/(measured/corr) lands on 2 -- the SAME lift, one count", lands_on_two(up_lift));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(10);
    free_struct_Fraction(down_lift);
    down_lift = 0;
    free_struct_Fraction(up_lift);
    up_lift = 0;
    return ret_val;
}
