...
long long _main();
long long static_mark_decays_in(long long);
long long holding_period();
long long holding_precedes_selection();
long long held_orbit_persists();
long long held_orbit_partitions_the_one();
...
long long memory_state();
long long refresh_state();
long long memory_folds_to_refresh();
long long refresh_folds_to_memory();
long long pattern_returns_after_two();
long long memory_plus_refresh();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the deposition law: order and form, forced ===");
    ok = expect_bool((long long)"phase one: a static mark at 1/4 folds to the One in 2 and is gone", static_mark_decays_in(2));
    ok = expect_bool((long long)"phase one, deeper: a mark at 1/32 folds to the One in 5 and is gone", static_mark_decays_in(5));
    ok = expect_bool((long long)"holding is the binary period's mechanism (orbit period = b)", holding_precedes_selection());
    ok = expect_bool((long long)"phase three: the held orbit persists -- returns in b folds, never the One", held_orbit_persists());
    ok = expect_bool((long long)"the held states partition the One exactly: the plateau is balance, not decay", held_orbit_partitions_the_one());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the telescope checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    null through step 128; the geometry store wakes first (256); selection follows (1000)");
    printf("%s\n", (char*)(long long)"  ok    peak, then a persistent plateau strictly above chance and below the peak (to 143000)");
    printf("%s\n", (char*)(long long)"  note  the peak's step-location and its movement with scale are Measured, behind the wall");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long static_mark_decays_in(long long k) {
    long long folds = 0;
    long long mark = 0;
    long long ret_val = 0;

    ep_gc_push_root(&mark);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    mark = fraction_from_ratio(1, whole_power(binary_count(), k));
    folds = 0;
    while (folds < k) {
    mark = fold_fraction(mark);
    folds = (folds + 1);
    }
    ret_val = fraction_compare(mark, fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long holding_period() {
    ...
    ret_val = forced_to_be((long long)"the memory orbit's period = the binary count", fold_period_of_unit_fraction(3), binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long holding_precedes_selection() {
    ...
    if (selection_family_period() == forced_to_be((long long)"the memory orbit's period = the binary count", fold_period_of_unit_fraction(3), binary_count())) {
    ret_val = 1LL;
    goto L_cleanup;
    } else {
    ret_val = 0LL;
...
long long held_orbit_persists() {
    ...
    state = fraction_from_ratio(1, colour_count());
    steps = 0;
    while (steps < smallest_fold_period_above(1)) {
    if (fraction_compare(state, fraction_from_whole_number(1)) == 0) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    ...
    ret_val = fraction_compare(state, memory_state()) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long held_orbit_partitions_the_one() {
    ...
    ret_val = fraction_compare(memory_plus_refresh(), fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long memory_folds_to_refresh() {
    ...
    ep_gc_push_root(&memory);
    ep_gc_push_root(&refresh);

    ep_gc_maybe_collect();

    memory = fraction_from_ratio(1, colour_count());
    folded = fold_fraction(memory);
    refresh = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_compare(folded, refresh) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(folded);
    folded = 0;
    free_struct_Fraction(refresh);
    refresh = 0;
    return ret_val;
}
...
long long refresh_folds_to_memory() {
    ...
    refresh = fraction_from_ratio(binary_count(), colour_count());
    folded = fold_fraction(refresh);
    memory = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, memory) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(folded);
    folded = 0;
    free_struct_Fraction(memory);
    memory = 0;
    return ret_val;
}
...
long long pattern_returns_after_two() {
    ...
    ep_gc_push_root(&memory);
    ep_gc_push_root(&once);
    ep_gc_push_root(&twice);

    ep_gc_maybe_collect();

    memory = fraction_from_ratio(1, colour_count());
    once = fold_fraction(memory);
    twice = fold_fraction(once);
    ret_val = fraction_compare(twice, memory) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(twice);
    twice = 0;
    return ret_val;
}
...
long long memory_plus_refresh() {
    ...
    memory = fraction_from_ratio(1, colour_count());
    refresh = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_add(memory, refresh);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
