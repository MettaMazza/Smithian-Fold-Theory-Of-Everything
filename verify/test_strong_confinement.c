...
long long _main();
long long inner_radius();
long long middle_radius();
long long outer_radius();
long long tube_work_near();
long long tube_work_far();
long long tube_work_grows();
long long coulomb_field_at(long long);
long long flux_is_conserved();
long long coulomb_work_near();
long long coulomb_work_far();
long long coulomb_work_shrinks();
long long riemann_sum(long long, long long, long long, long long, long long);
long long sums_bracket_integrals();
long long deconfinement_gap_is_real();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== confinement as work (the tube binds, the sphere frees) ===");
    ok = expect_equal((long long)"TUBE: work over the near doubling = 1/8", fraction_to_text(tube_work_near()), (long long)"1/8");
    ok = expect_equal((long long)"TUBE: work over the far doubling = 1/4", fraction_to_text(tube_work_far()), (long long)"1/4");
    ok = expect_bool((long long)"CONFINEMENT: the farther interval costs MORE (grows unbounded)", tube_work_grows());
    ok = expect_bool((long long)"COULOMB: flux E*r^2 = 1 conserved exactly at 1/2, 1/4, 1/8", flux_is_conserved());
    ok = expect_equal((long long)"COULOMB: exact work near = 1/a - 1/b = 4", fraction_to_text(coulomb_work_near()), (long long)"4");
    ok = expect_equal((long long)"COULOMB: exact work far = 1/b - 1/c = 2", fraction_to_text(coulomb_work_far()), (long long)"2");
    ok = expect_bool((long long)"DECONFINEMENT: the farther interval costs LESS (bounded total)", coulomb_work_shrinks());
    ok = expect_bool((long long)"the Riemann sums bracket both exact integrals", sums_bracket_integrals());
    ok = expect_bool((long long)"the gap is real: far UPPER bound < near LOWER bound", deconfinement_gap_is_real());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long tube_work_near() {
    ...
    b = fraction_from_ratio(1, 4);
    a = fraction_from_ratio(1, 8);
    ret_val = fraction_subtract(b, a);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long tube_work_far() {
    ...
    c = fraction_from_ratio(1, 2);
    b = fraction_from_ratio(1, 4);
    ret_val = fraction_subtract(c, b);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long tube_work_grows() {
    ...
    near = tube_work_near();
    far = tube_work_far();
    comparison = fraction_compare(far, near);
...
long long coulomb_field_at(long long r) {
    long long one = 0;
    ...
    ep_gc_push_root(&r_twin);
    ep_gc_push_root(&r);

    ep_gc_maybe_collect();

    r_twin = fraction_make(fraction_numerator(r), fraction_denominator(r));
    r_squared = fraction_multiply(r, r_twin);
    one = fraction_from_whole_number(1);
    ret_val = fraction_divide(one, r_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long flux_is_conserved() {
    long long all_conserved = 0;
    long long conserved = 0;
    long long field = 0;
    long long flux = 0;
    long long k = 0;
    long long one = 0;
    ...
    ep_gc_push_root(&field);
    ep_gc_push_root(&flux);
    ep_gc_push_root(&k);
    ep_gc_push_root(&one);
    ep_gc_push_root(&r);
    ep_gc_push_root(&r_again);
    ep_gc_push_root(&r_squared);
    ep_gc_push_root(&r_twin);
    ...
    k = 1;
    while (k < 4) {
    r = fraction_from_ratio(1, whole_power(2, k));
    field = coulomb_field_at(r);
    r_twin = fraction_from_ratio(1, whole_power(2, k));
    r_again = fraction_from_ratio(1, whole_power(2, k));
    r_squared = fraction_multiply(r_twin, r_again);
    flux = fraction_multiply(field, r_squared);
    conserved = fraction_compare(flux, one) == 0;
    if (conserved) {
    ...
    ret_val = all_conserved;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(flux);
    flux = 0;
...
long long coulomb_work_near() {
    long long a = 0;
    long long b = 0;
    long long inverse_a = 0;
    long long inverse_b = 0;
    long long one = 0;
    long long one_again = 0;
    long long ret_val = 0;

    ep_gc_push_root(&a);
    ep_gc_push_root(&b);
    ep_gc_push_root(&inverse_a);
    ep_gc_push_root(&inverse_b);
    ep_gc_push_root(&one);
    ep_gc_push_root(&one_again);
    ...
    a = fraction_from_ratio(1, 8);
    b = fraction_from_ratio(1, 4);
    inverse_a = fraction_divide(one, a);
    one_again = fraction_from_whole_number(1);
    inverse_b = fraction_divide(one_again, b);
    ret_val = fraction_subtract(inverse_a, inverse_b);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long coulomb_work_far() {
    ...
    b = fraction_from_ratio(1, 4);
    c = fraction_from_ratio(1, 2);
    inverse_b = fraction_divide(one, b);
    one_again = fraction_from_whole_number(1);
    inverse_c = fraction_divide(one_again, c);
    ret_val = fraction_subtract(inverse_b, inverse_c);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long coulomb_work_shrinks() {
    ...
    near = coulomb_work_near();
    far = coulomb_work_far();
    comparison = fraction_compare(far, near);
    ret_val = comparison < 0;
    goto L_cleanup;
L_cleanup:
...
long long riemann_sum(long long in_num, long long in_den, long long out_num, long long out_den, long long from_left) {
    long long advance = 0;
    ...
    ep_gc_push_root(&advance);
    ep_gc_push_root(&contribution);
    ep_gc_push_root(&dr);
    ep_gc_push_root(&field);
    ep_gc_push_root(&offset);
    ep_gc_push_root(&offset_fraction);
    ep_gc_push_root(&r_here);
    ep_gc_push_root(&r_in);
    ep_gc_push_root(&r_out);
    ep_gc_push_root(&span);
    ep_gc_push_root(&steps);
    ep_gc_push_root(&total);
    ep_gc_push_root(&in_num);
    ep_gc_push_root(&in_den);
    ep_gc_push_root(&out_num);
    ep_gc_push_root(&out_den);

    ep_gc_maybe_collect();

    r_in = fraction_from_ratio(in_num, in_den);
    r_out = fraction_from_ratio(out_num, out_den);
    span = fraction_subtract(r_out, r_in);
    steps = fraction_from_whole_number(4);
    dr = fraction_divide(span, steps);
    ...
    offset_fraction = fraction_from_whole_number(offset);
    advance = fraction_multiply(offset_fraction, dr);
    r_here = fraction_add(r_in, advance);
    field = coulomb_field_at(r_here);
    contribution = fraction_multiply(field, dr);
    total = fraction_add(total, contribution);
    offset = 1;
    if (from_left) {
    offset = 1;
    } else {
...
long long sums_bracket_integrals() {
    long long far_exact = 0;
    long long far_left = 0;
    long long far_lower = 0;
    long long far_ok = 0;
    long long far_ok_upper = 0;
    long long far_right = 0;
    long long far_upper = 0;
    long long near_exact = 0;
    long long near_left = 0;
    long long near_lower = 0;
    long long near_ok = 0;
    long long near_ok_upper = 0;
    long long near_right = 0;
    long long near_upper = 0;
    long long ret_val = 0;

    ep_gc_push_root(&far_exact);
    ep_gc_push_root(&far_left);
    ep_gc_push_root(&far_right);
    ep_gc_push_root(&near_exact);
    ep_gc_push_root(&near_left);
    ep_gc_push_root(&near_right);

    ep_gc_maybe_collect();

    near_exact = coulomb_work_near();
    near_left = riemann_sum(1, 8, 1, 4, 1LL);
    near_right = riemann_sum(1, 8, 1, 4, 0LL);
    far_exact = coulomb_work_far();
    far_left = riemann_sum(1, 4, 1, 2, 1LL);
    far_right = riemann_sum(1, 4, 1, 2, 0LL);
    near_lower = fraction_compare(near_right, near_exact);
    near_upper = fraction_compare(near_exact, near_left);
    far_lower = fraction_compare(far_right, far_exact);
    far_upper = fraction_compare(far_exact, far_left);
    near_ok = near_lower < 0;
    near_ok_upper = near_upper < 0;
    far_ok = far_lower < 0;
    far_ok_upper = far_upper < 0;
    if (near_ok) {
    if (near_ok_upper) {
    if (far_ok) {
    ret_val = far_ok_upper;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long deconfinement_gap_is_real() {
    ...
    ep_gc_push_root(&far_upper);
    ep_gc_push_root(&near_lower);

    ep_gc_maybe_collect();

    far_upper = riemann_sum(1, 4, 1, 2, 1LL);
    near_lower = riemann_sum(1, 8, 1, 4, 0LL);
    comparison = fraction_compare(far_upper, near_lower);
    ret_val = comparison < 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
