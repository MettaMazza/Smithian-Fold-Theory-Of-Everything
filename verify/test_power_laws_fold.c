...
long long _main();
long long rank_count(long long);
long long unit_law_is_self_similar_to(long long);
long long other_exponents_excluded();
long long unit_exponent();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Zipf and Gutenberg-Richter: the unit power law ===");
    ok = expect_bool((long long)"SELF-SIMILARITY: doubling the rank halves the count, r = 1..32", unit_law_is_self_similar_to(32));
    ok = expect_bool((long long)"EXCLUSION: the square law (1/4 per doubling) and flat law (1) rejected", other_exponents_excluded());
    ok = expect_equal((long long)"THE FORCED EXPONENT: Zipf = the One", fraction_to_text(unit_exponent()), (long long)"1");
    ok = expect_equal((long long)"Gutenberg-Richter b = the same One (a decade per magnitude)", fraction_to_text(unit_exponent()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long rank_count(long long r) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(1, r);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long unit_law_is_self_similar_to(long long bound) {
    ...
    r = 1;
    while (r < (bound + 1)) {
    here = fraction_from_ratio(1, r);
    doubled = fraction_from_ratio(1, (2 * r));
    ratio = fraction_divide(doubled, here);
    half_twin = fraction_from_ratio(1, 2);
    halves = fraction_compare(ratio, half_twin) == 0;
    if (halves) {
    all_hold = all_hold;
    } else {
    all_hold = 0LL;
...
long long other_exponents_excluded() {
    long long flat_doubled = 0;
    long long flat_excluded = 0;
    long long flat_here = 0;
    long long flat_matches = 0;
    long long flat_ratio = 0;
    long long half = 0;
    long long half_twin = 0;
    long long square_doubled = 0;
    long long square_excluded = 0;
    long long square_here = 0;
    long long square_matches = 0;
    long long square_ratio = 0;
    long long ret_val = 0;

    ep_gc_push_root(&flat_doubled);
    ep_gc_push_root(&flat_here);
    ep_gc_push_root(&flat_ratio);
    ep_gc_push_root(&half);
    ep_gc_push_root(&half_twin);
    ep_gc_push_root(&square_doubled);
    ep_gc_push_root(&square_here);
    ep_gc_push_root(&square_ratio);

    ep_gc_maybe_collect();

    half = fraction_from_ratio(1, 2);
    square_here = fraction_from_ratio(1, 9);
    square_doubled = fraction_from_ratio(1, 36);
    square_ratio = fraction_divide(square_doubled, square_here);
    square_matches = fraction_compare(square_ratio, half) == 0;
    square_excluded = 1LL;
    if (square_matches) {
    square_excluded = 0LL;
    }
    flat_here = fraction_from_ratio(1, 7);
    flat_doubled = fraction_from_ratio(1, 7);
    flat_ratio = fraction_divide(flat_doubled, flat_here);
    half_twin = fraction_from_ratio(1, 2);
    flat_matches = fraction_compare(flat_ratio, half_twin) == 0;
    flat_excluded = 1LL;
    if (flat_matches) {
    flat_excluded = 0LL;
    }
    if (square_excluded) {
    ret_val = flat_excluded;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(flat_ratio);
    flat_ratio = 0;
    free_struct_Fraction(half);
    half = 0;
    free_struct_Fraction(half_twin);
    half_twin = 0;
    free_struct_Fraction(square_ratio);
    square_ratio = 0;
    return ret_val;
}
