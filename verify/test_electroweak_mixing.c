...
long long measured_z_scale_mixing();
long long running_crosses_measured_between(long long, long long);
...
long long measured_z_scale_mixing() {
    ...
    ret_val = fraction_from_ratio(23113, 100000);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long running_crosses_measured_between(long long lower_level, long long upper_level) {
    long long at_lower = 0;
    long long at_upper = 0;
    long long measured = 0;
    long long above = 0;
    long long below = 0;
    long long ret_val = 0;

    ep_gc_push_root(&at_lower);
    ep_gc_push_root(&at_upper);
    ep_gc_push_root(&measured);
    ep_gc_push_root(&lower_level);
    ep_gc_push_root(&upper_level);

    ep_gc_maybe_collect();

    at_lower = weak_mixing_at_level(lower_level);
    at_upper = weak_mixing_at_level(upper_level);
    measured = fraction_from_ratio(23113, 100000);
    above = fraction_compare(at_lower, measured);
    below = fraction_compare(at_upper, measured);
    if (above > 0) {
    if (below < 0) {
    ret_val = 1LL;
    goto L_cleanup;
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the electroweak mixing angle ===");
    ok = expect_equal((long long)"unified coupling = 1/2", fraction_to_text(unified_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"sin^2 of mixing = 1/4", fraction_to_text(sin_squared_mixing()), (long long)"1/4");
    ok = expect_equal((long long)"cos^2 of mixing = 3/4", fraction_to_text(cos_squared_mixing()), (long long)"3/4");
    ok = expect_equal((long long)"sin^2 + cos^2 = the One", fraction_to_text(mixing_partition()), (long long)"1");
    printf("%s\n", (char*)(long long)"--- each preimage folds back to the unified coupling ---");
    ok = expect_equal((long long)"fold(1/4) = 1/2", fraction_to_text(lower_preimage_folds_to_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"fold(3/4) = 1/2", fraction_to_text(upper_preimage_folds_to_coupling()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"--- the running mixing, to the measured value (0.23113) ---");
    ok = expect_equal((long long)"bare running mixing = 1/2", fraction_to_text(bare_running_mixing()), (long long)"1/2");
    ok = expect_equal((long long)"level 9 mixing = 121/521", fraction_to_text(weak_mixing_at_level(9)), (long long)"121/521");
    ok = expect_equal((long long)"level 9 decimal = 0.2322", fraction_to_decimal(weak_mixing_at_level(9), 4), (long long)"0.2322");
    ok = expect_equal((long long)"level 10 mixing = 36/157", fraction_to_text(weak_mixing_at_level(10)), (long long)"36/157");
    ok = expect_equal((long long)"level 10 decimal = 0.2292", fraction_to_decimal(weak_mixing_at_level(10), 4), (long long)"0.2292");
    if (running_crosses_measured_between(9, 10)) {
    printf("%s\n", (char*)(long long)"  ok    running crosses the measured value between levels 9 and 10");
    } else {
    printf("%s\n", (char*)(long long)"  FAIL  running does not cross the measured value between levels 9 and 10");
    }
    ok = expect_equal((long long)"the cos^2 partition FORM is unique (forced_unique)", int_to_string(cos_squared_form_is_unique()), (long long)"1");
    ok = expect_equal((long long)"no SIMPLER assembly reaches cos^2 (engine-generated, complete)", int_to_string(cos_squared_assembly_is_minimal()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
