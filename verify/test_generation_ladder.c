...
long long _main();
long long displaced_vacuum();
long long colour_fold(long long);
long long generation_site(long long);
long long first_generation_site();
long long second_generation_site();
long long third_generation_site();
long long site_returns_to_vacuum(long long);
long long generation_count_from_ladder();
long long ladder_site_count();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the generation ladder (three sites = colour-fold preimages of the vacuum) ===");
    ok = expect_equal((long long)"first generation site = 1/6", fraction_to_text(first_generation_site()), (long long)"1/6");
    ok = expect_equal((long long)"second generation site = 1/2 (the vacuum itself)", fraction_to_text(second_generation_site()), (long long)"1/2");
    ok = expect_equal((long long)"third generation site = 5/6", fraction_to_text(third_generation_site()), (long long)"5/6");
    ok = expect_bool((long long)"each site folds back to the vacuum (a genuine preimage)", site_returns_to_vacuum(0));
    ok = expect_equal((long long)"the site count = three generations", int_to_string(generation_count_from_ladder()), (long long)"3");
    ok = expect_equal((long long)"the ladder has binary * colour = 6 sites", int_to_string(ladder_site_count()), (long long)"6");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long colour_fold(long long size) {
    long long scaled = 0;
    long long c = 0;
    long long ret_val = 0;
    ...
    scaled = fraction_multiply(size, c);
...
long long generation_site(long long k) {
    long long shifted = 0;
    long long vacuum = 0;
    long long k_fraction = 0;
    long long colour_fraction = 0;
    long long ret_val = 0;

    ep_gc_push_root(&shifted);
    ep_gc_push_root(&vacuum);
    ep_gc_push_root(&k_fraction);
    ep_gc_push_root(&colour_fraction);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    vacuum = fraction_from_ratio(1, binary_count());
    k_fraction = fraction_from_whole_number(k);
    shifted = fraction_add(vacuum, k_fraction);
    colour_fraction = fraction_from_whole_number(colour_count());
    ret_val = fraction_divide(shifted, colour_fraction);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long third_generation_site() {
    ...
    ret_val = generation_site(2);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long site_returns_to_vacuum(long long k) {
    long long site = 0;
    long long folded = 0;
    long long vacuum = 0;
    long long ret_val = 0;
    ...
    site = generation_site(k);
    folded = colour_fold(site);
    vacuum = fraction_from_ratio(1, binary_count());
    ret_val = fraction_compare(folded, vacuum) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(folded);
    folded = 0;
    free_struct_Fraction(vacuum);
    vacuum = 0;
    return ret_val;
}
...
long long generation_count_from_ladder() {
    ...
    count = 0;
    k = 0;
    while (k < smallest_fold_period_above(binary_count())) {
    if (site_returns_to_vacuum(k)) {
    count = (count + 1);
    }
    k = (k + 1);
    }
