...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold trees and graphs ===");
    example = encode_computation_state(73, 7);
    ok = expect_equal((long long)"depth-seven tree has 255 nodes", int_to_string(fold_tree_node_count(7)), int_to_string(255));
    ok = expect_equal((long long)"depth-seven tree has 254 edges", int_to_string(fold_tree_edge_count(7)), int_to_string(254));
    ok = expect_true((long long)"a generated node has one exact parent", fold_tree_parent_is_unique(example));
    ok = expect_true((long long)"every closing node reaches the root", every_fold_node_reaches_root(7));
    ok = expect_true((long long)"the complete tree and graph census holds", complete_fold_trees_graphs_census());
    ok = expect_true((long long)"a closing self-edge is rejected", closing_graph_self_edge_is_rejected(7));
    ok = expect_true((long long)"an extra parent is rejected", extra_tree_parent_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
