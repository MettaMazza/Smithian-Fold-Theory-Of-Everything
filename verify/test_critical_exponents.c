...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== critical exponents (rational at threshold) ===");
    ok = expect_equal((long long)"critical threshold = (m-1)/m = 1/2 (self-antipodal)", fraction_to_text(critical_threshold()), (long long)"1/2");
    ok = expect_equal((long long)"mean-field order-parameter exponent = 1/m = 1/2 (rational)", fraction_to_text(order_parameter_exponent()), (long long)"1/2");
    ok = expect_bool((long long)"the exponent is rational (whole-number denominator)", exponent_is_rational());
    ok = expect_equal((long long)"threshold folds to the One (phases merge)", fraction_to_text(threshold_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
