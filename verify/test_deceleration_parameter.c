...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the deceleration parameter ===");
    ok = expect_equal((long long)"vacuum share = 2/3", fraction_to_text(vacuum_fraction()), (long long)"2/3");
    ok = expect_equal((long long)"matter share = 1/3", fraction_to_text(matter_fraction()), (long long)"1/3");
    ok = expect_equal((long long)"matter contribution = 1/6", fraction_to_text(matter_half()), (long long)"1/6");
    ok = expect_equal((long long)"deceleration magnitude |q0| = 1/2", fraction_to_text(deceleration_magnitude()), (long long)"1/2");
    ok = expect_bool((long long)"the universe ACCELERATES (q0 < 0)", universe_accelerates());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
