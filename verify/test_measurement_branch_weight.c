...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== measurement branch weight ===");
    ok = expect_equal((long long)"branch depth = colour = 3", int_to_string(branch_depth()), int_to_string(3));
    ok = expect_equal((long long)"step weight = 1/2", fraction_to_text(step_weight()), (long long)"1/2");
    ok = expect_equal((long long)"branch weight = 1/8", fraction_to_text(branch_weight()), (long long)"1/8");
    ok = expect_bool((long long)"branch weight is atomic (denominator = 2^3)", branch_weight_is_atomic());
    ok = expect_equal((long long)"forced denominator = binary^colour = 8", int_to_string(branch_weight_denominator()), int_to_string(8));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
