...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the dark-to-baryon fraction ===");
    ok = expect_equal((long long)"generational volume = 27", int_to_string(generational_volume()), int_to_string(27));
    ok = expect_equal((long long)"covering depth = 5", int_to_string(generational_covering_depth()), int_to_string(5));
    ok = expect_equal((long long)"total binary volume = 32", int_to_string(total_binary_volume()), int_to_string(32));
    ok = expect_equal((long long)"baryon fraction = 5/32", fraction_to_text(baryon_fraction()), (long long)"5/32");
    ok = expect_equal((long long)"dark fraction = 27/32", fraction_to_text(dark_fraction()), (long long)"27/32");
    ok = expect_equal((long long)"baryon + dark = the One", fraction_to_text(matter_partition()), (long long)"1");
    ok = expect_equal((long long)"dark-to-baryon = 27/5", fraction_to_text(dark_to_baryon_ratio()), (long long)"27/5");
    ok = expect_equal((long long)"dark-to-baryon decimal = 5.4", fraction_to_decimal(dark_to_baryon_ratio(), 1), (long long)"5.4");
    printf("%s\n", (char*)(long long)"--- second order, to the measured value (5.3643) ---");
    ok = expect_equal((long long)"generational orbit floor = 2^5 - 1 = 31", int_to_string(generational_orbit_floor()), int_to_string(31));
    ok = expect_equal((long long)"the floor is a genuine period-5 orbit", int_to_string(generational_orbit_floor_is_genuine()), int_to_string(1));
    ok = expect_equal((long long)"second-order ratio = 279/52", fraction_to_text(dark_to_baryon_ratio_second_order()), (long long)"279/52");
    ok = expect_equal((long long)"second-order decimal = 5.3653", dark_to_baryon_ratio_second_order_decimal(4), (long long)"5.3653");
    ok = expect_equal((long long)"the second-order DEEPENING is unique (forced, not fitted)", int_to_string(dark_to_baryon_second_order_is_unique()), int_to_string(1));
    ok = expect_equal((long long)"the ratio FORM is unique among pairwise ratios (forced_unique)", int_to_string(dark_to_baryon_form_is_unique()), (long long)"1");
    ok = expect_equal((long long)"no SIMPLER assembly reaches the ratio (engine-generated, complete)", int_to_string(dark_to_baryon_assembly_is_minimal()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
