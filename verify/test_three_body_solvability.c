...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== three-body solvability ===");
    ok = expect_equal((long long)"fold(1/7) = 2/7 (first advances to second)", fraction_to_text(first_advances_to_second()), (long long)"2/7");
    ok = expect_equal((long long)"fold(2/7) = 4/7 (second advances to third)", fraction_to_text(second_advances_to_third()), (long long)"4/7");
    ok = expect_equal((long long)"fold(4/7) = 1/7 (third returns to first)", fraction_to_text(third_returns_to_first()), (long long)"1/7");
    ok = expect_equal((long long)"joint recurrence period (= colour)", int_to_string(joint_recurrence_period()), int_to_string(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long body_one() {
    ...
    ret_val = fraction_from_ratio(1, 7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long body_two() {
    ...
    ret_val = fraction_from_ratio(2, 7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long body_three() {
    ...
    ret_val = fraction_from_ratio(4, 7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long joint_recurrence_period() {
    ...
    by_orbit = fold_period_of_unit_fraction(7);
    by_colour = smallest_fold_period_above(binary_count());
    ret_val = forced_to_be((long long)"three-body joint period = orbit of 1/7 = colour", by_orbit, by_colour);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
