...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== where the periodic table ends (element 137, forced by 1/alpha) ===");
    ok = expect_equal((long long)"the critical charge = 1/alpha = 34259/250, imported forced", fraction_to_text(critical_charge()), (long long)"34259/250");
    ok = expect_equal((long long)"Z = 137: coupling = 34250/34259 < 1 (bound)", fraction_to_text(binding_coupling(137)), (long long)"34250/34259");
    ok = expect_bool((long long)"the threshold is SHARP: 137 bound, 138 over the One", threshold_is_sharp());
    ok = expect_equal((long long)"THE LAST ELEMENT, counted up: 137", int_to_string(last_element()), (long long)"137");
    ok = expect_bool((long long)"the table ends at the strength of light: one number, one fact", table_ends_at_alpha());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
