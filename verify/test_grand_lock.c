...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Grand Lock: the constants are one object ===");
    ok = expect_equal((long long)"derived down-depth at c=3: cover(27)", int_to_string(lock_down_depth(3)), (long long)"5");
    ok = expect_equal((long long)"derived up-depth at c=3: cover(81)", int_to_string(lock_up_depth(3)), (long long)"7");
    ok = expect_equal((long long)"1/alpha from the generators alone", fraction_to_text(lock_alpha(3)), (long long)"34259/250");
    ok = expect_equal((long long)"lepton third invariant 1/(2*3^5 - 1)", fraction_to_text(lock_lepton_third(3)), (long long)"1/485");
    ok = expect_equal((long long)"up-quark invariant 1/(3*2^7 - 1)", fraction_to_text(lock_quark_up(3)), (long long)"1/383");
    ok = expect_equal((long long)"down-quark invariant 1/(3*2^5 - 1)", fraction_to_text(lock_quark_down(3)), (long long)"1/95");
    ok = expect_equal((long long)"dark-to-baryon ratio 27/5", fraction_to_text(lock_dark_to_baryon(3)), (long long)"27/5");
    ok = expect_equal((long long)"dark fraction 27/32", fraction_to_text(lock_dark_fraction(3)), (long long)"27/32");
    ok = expect_equal((long long)"Hubble calibration ratio 13/12", fraction_to_text(lock_hubble(3)), (long long)"13/12");
    ok = expect_equal((long long)"Planck-hierarchy exponent 127/2", fraction_to_text(lock_planck_exponent(3)), (long long)"127/2");
    ok = expect_equal((long long)"cosmological-constant floor 1/2^20", fraction_to_text(lock_lambda_floor(3)), (long long)"1/1048576");
    ok = expect_equal((long long)"the half-One coupling family", fraction_to_text(lock_half_one()), (long long)"1/2");
    ok = expect_bool((long long)"CROSS-DOMAIN LOCK: alpha and the dark fraction share the counted d_down", alpha_and_dark_share_down_depth());
    ok = expect_equal((long long)"perturbed depths at c=4: cover(64)", int_to_string(lock_down_depth(4)), (long long)"6");
    ok = expect_equal((long long)"perturbed depths at c=4: cover(256)", int_to_string(lock_up_depth(4)), (long long)"8");
    ok = expect_bool((long long)"THE LOCK, BY PERTURBATION: colour 3 -> 4 moves EVERY c-dependent constant", perturbation_moves_all());
    ok = expect_bool((long long)"THE CONTRAST: the half-One coupling stays put (carries no colour)", half_one_is_unmoved());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
