...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold communication and query complexity ===");
    ok = expect_equal((long long)"three observations require three communicated labels", int_to_string(fold_communication_symbol_count(3)), int_to_string(3));
    ok = expect_equal((long long)"three labels distinguish eight histories", int_to_string(fold_communication_history_count(3)), int_to_string(8));
    ok = expect_equal((long long)"three observations require three queries", int_to_string(fold_query_count(3)), int_to_string(3));
    ok = expect_true((long long)"the query answers reconstruct an exact source", fold_queries_reconstruct_source(85, 7, 3));
    ok = expect_equal((long long)"one fewer query leaves b completions", int_to_string(fold_query_remaining_completions(3, 2)), int_to_string(2));
    ok = expect_true((long long)"the complete communication/query census holds", complete_fold_communication_query_census());
    ok = expect_true((long long)"zero-symbol nonzero communication is rejected", zero_symbol_nonzero_communication_is_rejected());
    ok = expect_true((long long)"a shortened query claim is rejected", shortened_query_claim_is_rejected(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
