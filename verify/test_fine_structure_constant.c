...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the fine-structure constant ===");
    ok = expect_equal((long long)"down-depth covers 27", int_to_string(down_depth()), int_to_string(5));
    ok = expect_equal((long long)"up-depth covers 81", int_to_string(up_depth()), int_to_string(7));
    ok = expect_equal((long long)"tower = 2^7", int_to_string(tower()), int_to_string(128));
    ok = expect_equal((long long)"covering volume = 250", int_to_string(covering_volume()), int_to_string(250));
    ok = expect_equal((long long)"sub-correction scale = 175", int_to_string(sub_correction_scale()), int_to_string(175));
    printf("%s\n", (char*)(long long)"--- leading scale ---");
    first = inverse_fine_structure_first_order();
    ok = expect_equal((long long)"1/alpha (first order) exact", fraction_to_text(first), (long long)"34259/250");
    ok = expect_equal((long long)"1/alpha (first order) decimal", inverse_fine_structure_first_order_decimal(3), (long long)"137.036");
    printf("%s\n", (char*)(long long)"--- second, self-similar scale ---");
    second = inverse_fine_structure_second_order();
    ok = expect_equal((long long)"1/alpha (second order) exact", fraction_to_text(second), (long long)"5995462/43751");
    ok = expect_equal((long long)"1/alpha (second order) decimal", inverse_fine_structure_second_order_decimal(9), (long long)"137.035999177");
    printf("%s\n", (char*)(long long)"--- the assembled FORM is unique, not just the ingredients ---");
    ok = expect_equal((long long)"leading assembly is the unique shape (forced_unique)", int_to_string(leading_assembly_is_unique()), int_to_string(1));
