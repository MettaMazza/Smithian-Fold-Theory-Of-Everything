...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold circuit size and depth ===");
    ok = expect_equal((long long)"the deepest layer contains b^7 gates", int_to_string(fold_circuit_layer_size(7)), int_to_string(128));
    ok = expect_equal((long long)"the complete depth-7 circuit contains 254 gates", int_to_string(fold_circuit_size(7)), int_to_string(254));
    ok = expect_equal((long long)"the circuit width is 128", int_to_string(fold_circuit_width(7)), int_to_string(128));
    ok = expect_equal((long long)"the forced circuit depth is seven", int_to_string(fold_circuit_depth(7)), int_to_string(7));
    ok = expect_true((long long)"every circuit layer uses lawful Fold transitions", every_fold_circuit_layer_is_lawful(7));
    ok = expect_true((long long)"omitting a gate is rejected", omitted_fold_circuit_gate_is_rejected(7));
    ok = expect_true((long long)"shortening circuit depth is rejected", shortened_fold_circuit_depth_is_rejected(7));
    ok = expect_true((long long)"the complete Fold circuit census holds", complete_fold_circuit_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
