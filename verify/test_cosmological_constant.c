...
long long vacuum_energy();
long long cosmological_constant_is_positive();
long long vacuum_folds_to_one();
long long cosmological_scale_exponent();
long long single_scale_axis_no_mode_sum();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the cosmological constant ===");
    ok = expect_equal((long long)"vacuum energy = 1/2 (displaced ground)", fraction_to_text(vacuum_energy()), (long long)"1/2");
    ok = expect_bool((long long)"cosmological constant is POSITIVE (1/2 > nothing)", cosmological_constant_is_positive());
    ok = expect_equal((long long)"vacuum folds up to the One (fold(1/2) = 1)", fraction_to_text(vacuum_folds_to_one()), (long long)"1");
    ok = expect_equal((long long)"scale exponent = 127/2 (single axis, absolute scale)", fraction_to_text(cosmological_scale_exponent()), (long long)"127/2");
    ok = expect_bool((long long)"one scale axis, no 10^120 mode-sum", single_scale_axis_no_mode_sum());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long cosmological_constant_is_positive() {
    ...
    vacuum = fraction_from_ratio(1, binary_count());
    one_first = fraction_from_whole_number(1);
    one_second = fraction_from_whole_number(1);
    nothing = fraction_subtract(one_first, one_second);
    order = fraction_compare(vacuum, nothing);
    if (order > 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long vacuum_folds_to_one() {
    ...
    ret_val = fold_fraction(vacuum_energy());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long single_scale_axis_no_mode_sum() {
    ...
    ep_gc_push_root(&absolute);
    ep_gc_push_root(&here);

    ep_gc_maybe_collect();

    here = hierarchy_exponent();
    absolute = hierarchy_exponent();
    ret_val = fraction_compare(here, absolute) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(absolute);
    absolute = 0;
