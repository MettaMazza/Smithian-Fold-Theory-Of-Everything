...
long long subsystem_one_period();
long long subsystem_two_period();
long long subsystems_are_coprime();
long long joint_period();
long long joint_exceeds_the_parts();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== entanglement ===");
    ok = expect_number((long long)"subsystem one period = binary = 2", subsystem_one_period(), 2);
    ok = expect_number((long long)"subsystem two period = colour = 3", subsystem_two_period(), 3);
    ok = expect_bool((long long)"the two subsystems are coprime (gcd = 1)", subsystems_are_coprime());
    ok = expect_number((long long)"joint period = product = 2 * 3 = 6 (tensor product)", joint_period(), 6);
    ok = expect_bool((long long)"the joint holds MORE than the parts (6 > 2+3=5)", joint_exceeds_the_parts());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long subsystems_are_coprime() {
    ...
    a = smallest_fold_period_above(1);
    b = smallest_fold_period_above(binary_count());
    common = whole_gcd(a, b);
    if (common == 1) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long joint_exceeds_the_parts() {
    ...
    product = joint_period();
    sum_of_parts = (smallest_fold_period_above(1) + smallest_fold_period_above(binary_count()));
    if (product > sum_of_parts) {
    ret_val = 1LL;
    goto L_cleanup;
    }
    ret_val = 0LL;
