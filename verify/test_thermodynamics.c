...
long long gap_before();
long long gap_after();
long long expansion_factor();
long long preimages_land_on_target();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the chaotic rate (Lyapunov and entropy, exact) ===");
    ok = expect_equal((long long)"the gap before folding: 1/5 - 1/7 = 2/35", fraction_to_text(gap_before()), (long long)"2/35");
    ok = expect_equal((long long)"the gap after one fold: 4/35", fraction_to_text(gap_after()), (long long)"4/35");
    ok = expect_equal((long long)"the MEASURED expansion factor = 2, exactly", fraction_to_text(expansion_factor()), (long long)"2");
    ok = expect_bool((long long)"the constructed preimages 1/6, 2/3 both land on 1/3", preimages_land_on_target());
    ok = expect_equal((long long)"the branch count = 2 (one bit erased per step)", int_to_string(branch_count()), (long long)"2");
    ok = expect_bool((long long)"three routes, one number: expansion = branches = binary", three_routes_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long gap_before() {
    ...
    x = fraction_from_ratio(1, 5);
    y = fraction_from_ratio(1, 7);
    ret_val = fraction_subtract(x, y);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long gap_after() {
    ...
    ep_gc_push_root(&fold_x);
    ep_gc_push_root(&fold_y);
    ep_gc_push_root(&x);
    ep_gc_push_root(&y);
    ...
    x = fraction_from_ratio(1, 5);
    y = fraction_from_ratio(1, 7);
    fold_x = fold_fraction(x);
    fold_y = fold_fraction(y);
    ret_val = fraction_subtract(fold_x, fold_y);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long expansion_factor() {
    ...
    after = gap_after();
    before = gap_before();
    ret_val = fraction_divide(after, before);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long preimages_land_on_target() {
    ...
    target = fraction_from_ratio(1, 3);
    low = fraction_from_ratio(1, 6);
    high = fraction_from_ratio(2, 3);
    from_low = fold_fraction(low);
    from_high = fold_fraction(high);
    low_lands = fraction_compare(from_low, target) == 0;
    high_lands = fraction_compare(from_high, target) == 0;
    if (low_lands) {
    ret_val = high_lands;
    goto L_cleanup;
    }
...
long long three_routes_agree() {
    ...
    long long branches_match = 0;
    long long expansion = 0;
    long long expansion_matches = 0;
    long long generator = 0;
    long long ret_val = 0;

    ep_gc_push_root(&expansion);
    ep_gc_push_root(&generator);

    ep_gc_maybe_collect();

    expansion = expansion_factor();
    generator = fraction_from_whole_number(binary_count());
    expansion_matches = fraction_compare(expansion, generator) == 0;
    branches = 2;
    branches_match = branches == smallest_fold_period_above(1);
    if (expansion_matches) {
    ret_val = branches_match;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(expansion);
    expansion = 0;
    free_struct_Fraction(generator);
    generator = 0;
    return ret_val;
}
