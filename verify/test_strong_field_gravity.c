...
long long minimal_distance();
long long centre_is_excluded();
long long mass_share();
long long horizon_radius();
long long radius_is_twice_mass();
long long horizon_area();
long long horizon_entropy();
long long entropy_matches_preimage_count();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== strong-field gravity (no singularity, area law, r_s = 2M) ===");
    ok = expect_equal((long long)"the minimal distance = the depth-5 step = 1/32", fraction_to_text(minimal_distance()), (long long)"1/32");
    ok = expect_bool((long long)"the centre r = 0 is excluded (no singularity to reach)", centre_is_excluded());
    ok = expect_equal((long long)"the horizon radius = fold(M) for M = 1/4", fraction_to_text(horizon_radius()), (long long)"1/2");
    ok = expect_bool((long long)"r_s = 2M exactly (the doubling IS the factor two)", radius_is_twice_mass());
    ok = expect_equal((long long)"the horizon area A = 2^5 = 32", int_to_string(horizon_area()), (long long)"32");
    ok = expect_equal((long long)"the entropy S = A/4 = 8", int_to_string(horizon_entropy()), (long long)"8");
    ok = expect_bool((long long)"S equals the ENUMERATED depth-3 preimage count", entropy_matches_preimage_count());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long minimal_distance() {
    ...
    volume = whole_power(binary_count(), 5);
...
long long centre_is_excluded() {
    ...
    floor = minimal_distance();
    ret_val = ({ long long _fap = floor; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long horizon_radius() {
    ...
    mass = fraction_from_ratio(1, 4);
    ret_val = fold_fraction(mass);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long radius_is_twice_mass() {
    ...
    r_s = horizon_radius();
    mass = fraction_from_ratio(1, 4);
    mass_again = fraction_from_ratio(1, 4);
    doubled = fraction_add(mass, mass_again);
    ret_val = fraction_compare(r_s, doubled) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(doubled);
    doubled = 0;
    free_struct_Fraction(r_s);
    r_s = 0;
    return ret_val;
}
...
long long horizon_area() {
    ...
    ret_val = whole_power(binary_count(), 5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long horizon_entropy() {
    ...
    area = whole_power(binary_count(), 5);
    quarter = whole_power(binary_count(), 2);
    ret_val = (area / quarter);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long entropy_matches_preimage_count() {
    ...
    leaves = (leaves * smallest_fold_period_above(1));
    level = 3;
    entropy = horizon_entropy();
    ret_val = leaves == entropy;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
