...
long long _main();
long long tower_at(long long);
long long running_coupling(long long, long long);
long long closed_form_gap(long long, long long);
long long routes_agree_at(long long, long long);
long long routes_agree_everywhere();
long long strong_binds_closer_everywhere();
long long weak_above_em_everywhere();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the order of the forces (strong above weak above EM) ===");
    ok = expect_equal((long long)"the strong coupling at depth 3: (3+8-1)/(3+8) = 10/11", fraction_to_text(running_coupling(3, 3)), (long long)"10/11");
    ok = expect_equal((long long)"the weak coupling at depth 3: 9/10", fraction_to_text(running_coupling(2, 3)), (long long)"9/10");
    ok = expect_bool((long long)"both routes agree at every depth 0..11 (both sectors)", routes_agree_everywhere());
    ok = expect_bool((long long)"the strong binds closer to unison at EVERY depth (no crossing)", strong_binds_closer_everywhere());
    ok = expect_bool((long long)"the weak sits strictly above the flat EM half at every depth", weak_above_em_everywhere());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long tower_at(long long d) {
    long long ret_val = 0;
    ...
    ret_val = whole_power(binary_count(), d);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long running_coupling(long long m, long long d) {
    long long levels = 0;
    long long source = 0;
    long long ret_val = 0;
    ...
    source = (m + levels);
    ret_val = fraction_from_ratio((source - 1), source);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long closed_form_gap(long long m, long long d) {
    long long levels = 0;
    long long ret_val = 0;

    ep_gc_push_root(&levels);
    ep_gc_push_root(&m);
    ep_gc_push_root(&d);
    ...
    levels = whole_power(binary_count(), d);
    ret_val = fraction_from_ratio(1, (m + levels));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long routes_agree_at(long long m, long long d) {
    long long closed = 0;
    long long constructed_gap = 0;
    long long g = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&closed);
    ep_gc_push_root(&constructed_gap);
    ep_gc_push_root(&g);
    ep_gc_push_root(&one);
    ep_gc_push_root(&m);
    ep_gc_push_root(&d);
    ...
    g = running_coupling(m, d);
    constructed_gap = fraction_subtract(one, g);
    closed = closed_form_gap(m, d);
    ret_val = fraction_compare(constructed_gap, closed) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(closed);
    closed = 0;
    free_struct_Fraction(constructed_gap);
    constructed_gap = 0;
    return ret_val;
}
...
long long routes_agree_everywhere() {
    ...
    d = 0;
    while (d < 12) {
    strong_ok = routes_agree_at(3, d);
    weak_ok = routes_agree_at(2, d);
    if (strong_ok) {
    if (weak_ok) {
    all_agree = all_agree;
    } else {
...
long long strong_binds_closer_everywhere() {
    long long all_closer = 0;
    long long comparison = 0;
    long long d = 0;
    long long gap_strong = 0;
    long long gap_weak = 0;
    long long ret_val = 0;

    ep_gc_push_root(&d);
    ep_gc_push_root(&gap_strong);
    ep_gc_push_root(&gap_weak);
    ...
    d = 0;
    while (d < 12) {
    gap_strong = closed_form_gap(3, d);
    gap_weak = closed_form_gap(2, d);
    comparison = fraction_compare(gap_strong, gap_weak);
    if (comparison < 0) {
...
long long weak_above_em_everywhere() {
    ...
    d = 0;
    while (d < 12) {
    g_weak = running_coupling(2, d);
    comparison = fraction_compare(g_weak, half);
    if (comparison > 0) {
    above = above;
    } else {
    above = 0LL;
