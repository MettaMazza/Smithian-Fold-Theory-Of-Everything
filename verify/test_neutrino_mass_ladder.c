...
long long hands_share_lock();
long long dirac_mass();
long long neutrino_hand_count();
long long dirac_route_closed();
long long tower_depth();
long long splitting_ratio();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the neutrino mass ladder (single-handed, on the tower) ===");
    ok = expect_bool((long long)"the two hands 1/4, 3/4 share one lock", hands_share_lock());
    ok = expect_equal((long long)"the Dirac mass = the hands' separation = 1/2", fraction_to_text(dirac_mass()), (long long)"1/2");
    ok = expect_equal((long long)"the neutrino has ONE hand (no right-handed seen)", int_to_string(neutrino_hand_count()), (long long)"1");
    ok = expect_bool((long long)"one hand -> the Dirac route is closed (nearly massless)", dirac_route_closed());
    ok = expect_equal((long long)"the tower depth, counted: minimal cover of 27 = 5", int_to_string(tower_depth()), (long long)"5");
    ok = expect_equal((long long)"the forced splitting ratio = 1/2^5 = 1/32", fraction_to_text(splitting_ratio()), (long long)"1/32");
    measured = fraction_from_ratio(3, 100);
    deviation = fraction_divide(measured, splitting_ratio());
    ok = expect_equal((long long)"measured 3/100 over forced 1/32 = 24/25 (4 percent)", fraction_to_text(deviation), (long long)"24/25");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long hands_share_lock() {
    ...
    ep_gc_push_root(&from_left);
    ep_gc_push_root(&from_right);

    ep_gc_maybe_collect();

    from_left = fold_fraction(left_hand());
    from_right = fold_fraction(right_hand());
    ret_val = fraction_compare(from_left, from_right) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(from_left);
    from_left = 0;
    free_struct_Fraction(from_right);
    from_right = 0;
    return ret_val;
}
...
long long dirac_mass() {
    ...
    ret_val = fraction_subtract(right, left);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
