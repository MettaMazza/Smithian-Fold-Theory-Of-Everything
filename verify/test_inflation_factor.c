...
long long inflation_depth() {
    ...
    generational_volume = whole_power(colour_count(), colour_count());
    by_covering = minimal_binary_cover(generational_volume);
    ret_val = forced_to_be((long long)"inflation depth = binary + colour = cover(colour^colour)", by_relation, by_covering);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long inflation_expansion_factor() {
    ...
    ret_val = whole_power(binary_count(), inflation_depth());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
