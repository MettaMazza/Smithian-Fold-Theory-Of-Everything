...
long long chirality_count();
long long left_handed();
long long right_handed();
long long chiralities_are_antipodes();
long long chiralities_share_one_image();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== chirality ===");
    ok = expect_equal((long long)"chirality count = binary = 2", int_to_string(chirality_count()), int_to_string(2));
    ok = expect_equal((long long)"left-handed = 1/4 (lower preimage)", fraction_to_text(left_handed()), (long long)"1/4");
    ok = expect_equal((long long)"right-handed = 3/4 (upper antipode)", fraction_to_text(right_handed()), (long long)"3/4");
    ok = expect_bool((long long)"the two chiralities are mirror images (antipodes)", chiralities_are_antipodes());
    ok = expect_bool((long long)"both chiralities fold to one image (a particle and its mirror)", chiralities_share_one_image());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long right_handed() {
    ...
    ret_val = fraction_subtract(one, left);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long chiralities_are_antipodes() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&left);
    ep_gc_push_root(&mirror);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    left = fraction_from_ratio(1, 4);
    mirror = fraction_subtract(one, left);
    right = right_handed();
    ret_val = fraction_compare(mirror, right) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(right);
    right = 0;
    free_struct_Fraction(mirror);
    mirror = 0;
    return ret_val;
}
...
long long chiralities_share_one_image() {
    ...
    left_image = fold_fraction(left_handed());
    right_image = fold_fraction(right_handed());
