...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== spacetime dimensions ===");
    ok = expect_number((long long)"spatial dimensions (unique stable = colour)", spatial_dimensions(), 3);
    ok = expect_number((long long)"time dimensions (the fold's one forward direction)", time_dimensions(), 1);
    ok = expect_number((long long)"spacetime dimensions (3+1 = binary^2)", spacetime_dimensions(), 4);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
