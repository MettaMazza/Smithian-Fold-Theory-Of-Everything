...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== crystalline order (the crystallographic restriction) ===");
    ok = expect_number((long long)"phi(5) = 4 (exceeds the binary bound)", euler_totient(5), 4);
    ok = expect_number((long long)"phi(6) = 2 (meets the binary bound)", euler_totient(6), 2);
    ok = expect_bool((long long)"3-fold is crystallographic", rotation_is_crystallographic(3));
    ok = expect_bool((long long)"4-fold is crystallographic", rotation_is_crystallographic(4));
    ok = expect_bool((long long)"6-fold is crystallographic", rotation_is_crystallographic(6));
    ok = expect_bool((long long)"5-fold is FORBIDDEN (quasicrystal)", rotation_is_crystallographic(5) == 0LL);
    ok = expect_bool((long long)"7-fold is FORBIDDEN", rotation_is_crystallographic(7) == 0LL);
    ok = expect_number((long long)"allowed rotation orders: exactly five {1,2,3,4,6}", allowed_rotation_order_count(), 5);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
