...
long long height();
long long gravitational_redshift();
long long acquired_velocity();
long long doppler_shift();
long long equivalence_holds();
long long redshift_folds_linearly();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the equivalence principle (redshift) ===");
    ok = expect_equal((long long)"gravitational redshift z = g*h = 1/4", fraction_to_text(gravitational_redshift()), (long long)"1/4");
    ok = expect_equal((long long)"acquired velocity v = g*h = 1/4", fraction_to_text(acquired_velocity()), (long long)"1/4");
    ok = expect_equal((long long)"Doppler shift z_doppler = v = 1/4", fraction_to_text(doppler_shift()), (long long)"1/4");
    ok = expect_bool((long long)"EQUIVALENCE: gravitational redshift = acceleration Doppler shift", equivalence_holds());
    ok = expect_bool((long long)"weak-field redshift folds linearly in height (fold(gh)=fold(g)h)", redshift_folds_linearly());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long gravitational_redshift() {
    ...
    g = fraction_from_ratio(1, (binary_count() * binary_count()));
    h = fraction_from_whole_number(1);
    ret_val = fraction_multiply(g, h);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long equivalence_holds() {
    long long z_doppler = 0;
    long long z_gravity = 0;
    long long ret_val = 0;

    ep_gc_push_root(&z_doppler);
    ep_gc_push_root(&z_gravity);

    ep_gc_maybe_collect();

    z_gravity = gravitational_redshift();
    z_doppler = acquired_velocity();
    ret_val = fraction_compare(z_gravity, z_doppler) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(z_doppler);
    z_doppler = 0;
    free_struct_Fraction(z_gravity);
    z_gravity = 0;
    return ret_val;
}
...
long long redshift_folds_linearly() {
    ...
    ep_gc_push_root(&h);
    ep_gc_push_root(&fold_gh);
    ep_gc_push_root(&fold_g_times_h);
    ep_gc_push_root(&g);
    ep_gc_push_root(&fold_g);
    ep_gc_push_root(&gh);
    ...
    g = fraction_from_ratio(1, (binary_count() * binary_count()));
    h = fraction_from_whole_number(1);
    gh = fraction_multiply(g, h);
    fold_gh = fold_fraction(gh);
    fold_g = fold_fraction(g);
    fold_g_times_h = fraction_multiply(fold_g, h);
    ret_val = fraction_compare(fold_gh, fold_g_times_h) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(fold_gh);
    fold_gh = 0;
    free_struct_Fraction(fold_g_times_h);
    fold_g_times_h = 0;
    return ret_val;
}
