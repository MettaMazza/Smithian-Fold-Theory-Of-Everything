...
long long _main();
long long representative_constant();
long long denominator_is_positive();
long long rational_polynomial_holds();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the rationality of the constants ===");
    ok = expect_equal((long long)"representative constant 1/alpha = 34259/250", fraction_to_text(representative_constant()), (long long)"34259/250");
    ok = expect_bool((long long)"its denominator is a positive whole number (a ratio p/q)", denominator_is_positive());
    ok = expect_bool((long long)"the rational polynomial holds: q*x = p (250*x = 34259)", rational_polynomial_holds());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long denominator_is_positive() {
    ...
    value = inverse_fine_structure_first_order();
    q = exact_integer_from_sign_and_digits(1, ({ long long _fap = value; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'denominator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->denominator_digits; }));
    if (({ long long _fap = q; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'sign' on 'ExactInteger'\n"); exit(1); } ((EpStruct_ExactInteger*)(_fap))->sign; }) > 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
    ret_val = 0LL;
...
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(value);
    value = 0;
    free_struct_ExactInteger(q);
    q = 0;
    return ret_val;
}
...
long long rational_polynomial_holds() {
    ...
    long long value = 0;
    long long q_int = 0;
    long long q_fraction = 0;
    long long product = 0;
    long long ret_val = 0;

    ep_gc_push_root(&p_int);
    ep_gc_push_root(&p_fraction);
    ep_gc_push_root(&one_int);
    ep_gc_push_root(&value);
    ep_gc_push_root(&q_int);
    ep_gc_push_root(&q_fraction);
    ep_gc_push_root(&product);

    ep_gc_maybe_collect();

    value = inverse_fine_structure_first_order();
    one_int = exact_integer_from_sign_and_digits(1, (long long)"1");
    q_int = exact_integer_from_sign_and_digits(1, ({ long long _fap = value; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'denominator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->denominator_digits; }));
    q_fraction = fraction_make(q_int, one_int);
    product = fraction_multiply(value, q_fraction);
    p_int = exact_integer_from_sign_and_digits(({ long long _fap = value; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }), ({ long long _fap = value; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_digits; }));
    p_fraction = fraction_make(p_int, one_int);
    ret_val = fraction_compare(product, p_fraction) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(p_fraction);
    p_fraction = 0;
    free_struct_Fraction(product);
    product = 0;
    return ret_val;
}
