...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the family signature (Step 316) ===");
    ok = expect_equal((long long)"colour signature low = 1/4 (cos^2)", fraction_to_text(colour_signature_low()), (long long)"1/4");
    ok = expect_equal((long long)"colour signature high = 3/4 (electroweak complement)", fraction_to_text(colour_signature_high()), (long long)"3/4");
    ok = expect_bool((long long)"colour low rests on the CLOSED electroweak sin^2 = 1/4", colour_low_is_closed_value());
    ok = expect_bool((long long)"colour high two routes: cos^2 = 1 - sin^2", colour_high_two_routes());
    ok = expect_bool((long long)"colour signature partitions the One: 1/4 + 3/4 = 1", colour_signature_partitions_one());
    ok = expect_bool((long long)"both colour values are proper fractions in (0,1) -- non-whole", colour_values_not_whole());
    ok = expect_bool((long long)"THE DISCRIMINATOR: family read from the value (whole=binary, {1/4,3/4}=colour), disjoint", family_read_from_value());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    the atlas read family by CONCENTRATION margin (loudest basis) -- the loudness proxy, already in");
    printf("%s\n", (char*)(long long)"  note  registered verification (queued, not run): re-read the recipe map on the value signature --");
    printf("%s\n", (char*)(long long)"  note  each store's law vs the value-sets {whole} and {1/4, 3/4}, not loudest-basis margin");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
