...
long long cooper_pair_fermion_count();
long long pair_is_bosonic();
long long condensate_ground_is_stable();
long long resistance_is_zero();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== superconductivity ===");
    ok = expect_number((long long)"Cooper pair holds binary = 2 fermions", cooper_pair_fermion_count(), 2);
    ok = expect_bool((long long)"a pair (even fermion count) is BOSONIC", pair_is_bosonic());
    ok = expect_bool((long long)"the condensate ground is the shared, stable One", condensate_ground_is_stable());
    ok = expect_bool((long long)"resistance is ZERO (collective lock)", resistance_is_zero());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long pair_is_bosonic() {
    ...
    count = smallest_fold_period_above(1);
    remainder = (count % smallest_fold_period_above(1));
    if (remainder == 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long resistance_is_zero() {
    ...
    bosonic = pair_is_bosonic();
    shared = condensate_ground_is_stable();
    if (bosonic) {
    if (shared) {
    ret_val = 1LL;
    goto L_cleanup;
