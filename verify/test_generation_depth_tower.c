...
long long state_arrives(long long, long long);
long long census_at_depth(long long);
long long census_matches_tower_at(long long);
long long tower_theorem_holds();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the generation depth tower (2^d levels, enumerated) ===");
    ok = expect_bool((long long)"depth 2, state 3/4 arrives: 3/4 -> 1/2 -> 1", state_arrives(3, 2));
    ok = expect_equal((long long)"the census at depth 1 = 2 (every state run)", int_to_string(census_at_depth(1)), (long long)"2");
    ok = expect_equal((long long)"the census at depth 2 = 4", int_to_string(census_at_depth(2)), (long long)"4");
    ok = expect_equal((long long)"the census at depth 3 = 8", int_to_string(census_at_depth(3)), (long long)"8");
    ok = expect_bool((long long)"THE THEOREM: census = 2^d at all three depths (14 states)", tower_theorem_holds());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long state_arrives(long long i, long long d) {
    long long grid = 0;
    long long one = 0;
    long long state = 0;
    long long step = 0;
    ...
    grid = whole_power(binary_count(), d);
...
long long census_at_depth(long long d) {
...
long long census_matches_tower_at(long long d) {
    ...
    counted = census_at_depth(d);
    closed_form = whole_power(binary_count(), d);
...
long long tower_theorem_holds() {
    ...
    at_one = census_matches_tower_at(1);
    at_two = census_matches_tower_at(2);
    at_three = census_matches_tower_at(3);
    if (at_one) {
    if (at_two) {
    ret_val = at_three;
    goto L_cleanup;
