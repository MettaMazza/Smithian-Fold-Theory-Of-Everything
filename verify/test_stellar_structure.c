...
long long hydrostatic_balance();
long long balance_is_self_antipodal();
long long star_is_one_bound_whole();
long long push_plus_pull();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== stellar structure (the self-correcting balance) ===");
    ok = expect_equal((long long)"hydrostatic balance = 1/2 (push = pull)", fraction_to_text(hydrostatic_balance()), (long long)"1/2");
    ok = expect_bool((long long)"the balance is self-antipodal (self-correcting)", balance_is_self_antipodal());
    ok = expect_equal((long long)"the balanced star folds to unison (one bound whole)", fraction_to_text(star_is_one_bound_whole()), (long long)"1");
    ok = expect_equal((long long)"push + pull sum to the One", fraction_to_text(push_plus_pull()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long star_is_one_bound_whole() {
    ...
    ret_val = fold_fraction(hydrostatic_balance());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
