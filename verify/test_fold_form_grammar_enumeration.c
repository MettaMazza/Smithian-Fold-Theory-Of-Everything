...
long long _main();
long long extended_fold_grammar_primitive_count();
long long extended_fold_form_count_at_size(long long);
long long extended_fold_form_count_through(long long);
long long extended_fold_apply_form(long long, long long, long long);
long long extended_fold_form_is_non_injective(long long, long long);
long long extended_fold_form_return_period(long long, long long);
long long extended_fold_form_generates(long long, long long);
long long extended_fold_generators_at_size(long long);
long long extended_fold_minimal_generator_size(long long);
long long extended_fold_minimal_generator_code();
long long extended_fold_grammar_is_complete_through_three();
long long extended_fold_unique_minimal_generator_holds();
long long omitted_extended_form_is_rejected();
long long larger_generator_is_not_minimal();
long long shifted_extended_winner_is_rejected();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== mechanically extended Fold-form grammar ===");
    ok = expect_equal((long long)"the primitive alphabet has four forms", int_to_string(extended_fold_grammar_primitive_count()), int_to_string(4));
    ok = expect_equal((long long)"composition size three has 64 generated forms", int_to_string(extended_fold_form_count_at_size(3)), int_to_string(64));
    ok = expect_equal((long long)"sizes one through three contain 84 forms", int_to_string(extended_fold_form_count_through(3)), int_to_string(84));
    ok = expect_true((long long)"all 84 forms are generated and execute in-domain", extended_fold_grammar_is_complete_through_three());
    ok = expect_equal((long long)"the first generating size is one", int_to_string(extended_fold_minimal_generator_size(3)), int_to_string(1));
    ok = expect_equal((long long)"exactly one size-one form generates", int_to_string(extended_fold_generators_at_size(1)), int_to_string(1));
    ok = expect_equal((long long)"the unique minimal code is the Fold", int_to_string(extended_fold_minimal_generator_code()), int_to_string(3));
    ok = expect_true((long long)"the Fold remains the unique minimal generator", extended_fold_unique_minimal_generator_holds());
    ok = expect_true((long long)"an omitted extended form is rejected", omitted_extended_form_is_rejected());
    ok = expect_true((long long)"larger generators are not minimal rivals", larger_generator_is_not_minimal());
    ok = expect_true((long long)"a shifted winner is rejected", shifted_extended_winner_is_rejected());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long extended_fold_grammar_primitive_count() {
    ...
    ret_val = forced_to_be((long long)"extended Fold grammar alphabet = enumerated primitives", 4, (binary_count() * binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long extended_fold_form_count_at_size(long long size) {
    long long ret_val = 0;
    ...
    ret_val = whole_power(extended_fold_grammar_primitive_count(), size);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long extended_fold_form_count_through(long long maximum_size) {
    long long size = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    total = 0;
    size = 1;
    while (size < (maximum_size + 1)) {
    total = (total + whole_power(extended_fold_grammar_primitive_count(), size));
    size = (size + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long extended_fold_apply_form(long long code, long long size, long long value) {
    long long current = 0;
    long long position = 0;
    long long primitive_tag = 0;
    long long remaining = 0;
    long long ret_val = 0;

    ep_gc_push_root(&current);
    ep_gc_push_root(&primitive_tag);
    ep_gc_push_root(&value);

    ep_gc_maybe_collect();

    current = fraction_make(fraction_numerator(value), fraction_denominator(value));
    remaining = code;
    position = 0;
    while (position < size) {
    primitive_tag = ((remaining % forced_to_be((long long)"extended Fold grammar alphabet = enumerated primitives", 4, (binary_count() * binary_count()))) + 1);
    current = apply_candidate(primitive_tag, current);
    remaining = (remaining / forced_to_be((long long)"extended Fold grammar alphabet = enumerated primitives", 4, (binary_count() * binary_count())));
    position = (position + 1);
    }
    ret_val = current;
    goto L_cleanup;
...
long long extended_fold_form_is_non_injective(long long code, long long size) {
    long long high = 0;
    ...
    low_image = extended_fold_apply_form(code, size, low);
    high_image = extended_fold_apply_form(code, size, high);
...
long long extended_fold_form_return_period(long long code, long long size) {
    long long current = 0;
    long long result = 0;
    long long start = 0;
    long long steps = 0;
    ...
    current = extended_fold_apply_form(code, size, start);
    steps = 1;
    result = 0;
    while (steps < 11) {
    if (fraction_compare(current, start) == 0) {
    if (result < 1) {
    result = steps;
    }
    }
    current = extended_fold_apply_form(code, size, current);
    steps = (steps + 1);
    }
    ret_val = result;
    goto L_cleanup;
...
long long extended_fold_form_generates(long long code, long long size) {
    long long ret_val = 0;
    ...
    if (extended_fold_form_is_non_injective(code, size) == 0LL) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    ret_val = extended_fold_form_return_period(code, size) > 1;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long extended_fold_generators_at_size(long long size) {
    long long code = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    total = 0;
    code = 0;
    while (code < whole_power(extended_fold_grammar_primitive_count(), size)) {
    if (extended_fold_form_generates(code, size)) {
    total = (total + 1);
    }
    code = (code + 1);
    }
...
long long extended_fold_minimal_generator_size(long long maximum_size) {
    long long size = 0;
    long long ret_val = 0;
...
long long extended_fold_minimal_generator_code() {
    ...
    code = 0;
    while (code < whole_power(extended_fold_grammar_primitive_count(), 1)) {
    if (extended_fold_form_generates(code, 1)) {
    ret_val = code;
    goto L_cleanup;
    }
    code = (code + 1);
...
long long extended_fold_grammar_is_complete_through_three() {
    ...
    if (whole_power(extended_fold_grammar_primitive_count(), 1) != 4) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    if (whole_power(extended_fold_grammar_primitive_count(), 2) != 16) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    if (whole_power(extended_fold_grammar_primitive_count(), 3) != 64) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    ...
    code = 0;
    while (code < whole_power(extended_fold_grammar_primitive_count(), size)) {
    image = extended_fold_apply_form(code, size, fraction_from_ratio(2, 5));
    if (fraction_compare(image, fraction_from_whole_number(1)) > 0) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    code = (code + 1);
...
long long extended_fold_unique_minimal_generator_holds() {
    ...
    checked = forced_unique((long long)"minimal generator in mechanically extended composition grammar", 1, (extended_fold_generators_at_size(1) - 1));
    ret_val = 1LL;
    goto L_cleanup;
L_cleanup:
    return ret_val;
...
long long omitted_extended_form_is_rejected() {
    ...
    ret_val = (extended_fold_form_count_through(3) - 1) != 84;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long larger_generator_is_not_minimal() {
    ...
    ret_val = extended_fold_minimal_generator_size(3) < 2;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shifted_extended_winner_is_rejected() {
    ...
    ret_val = extended_fold_minimal_generator_code() != 2;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
