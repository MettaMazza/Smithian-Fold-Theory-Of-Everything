...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold automaton law ===");
    word = encode_computation_state(85, 7);
    ok = expect_equal((long long)"the source configuration expects its first fibre label", int_to_string(fold_automaton_expected_symbol(85, 7)), int_to_string(get_list(word, 0)));
    ok = expect_true((long long)"the reader accepts a generated depth-7 word", fold_automaton_accepts(word));
    ok = expect_true((long long)"one reader accepts the complete depth-7 language", fold_automaton_accepts_complete_language(7));
    ok = expect_equal((long long)"the complete depth-7 reader has 255 configurations", int_to_string(fold_automaton_configuration_count(7)), int_to_string(255));
    ok = expect_equal((long long)"all 254 nonterminal configurations have one transition", int_to_string(fold_automaton_transition_count(7)), int_to_string(254));
    ok = expect_true((long long)"premature acceptance is rejected", premature_fold_automaton_acceptance_is_rejected(7));
    ok = expect_true((long long)"a word paired with the wrong source is rejected", mismatched_fold_word_is_rejected(7));
    ok = expect_true((long long)"the complete Fold automaton census holds", complete_fold_automaton_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
