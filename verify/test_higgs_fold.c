...
long long _main();
long long tower_rung(long long);
long long higgs_vev();
long long higgs_mass_part();
long long higgs_self_coupling();
long long rungs_halve();
long long vacuum_is_displaced();
long long mass_ratio_from_rungs();
long long relation_closes();
long long higgs_mass_from_vev(long long, long long);
...
long long _main() {
    long long forced_mass = 0;
    long long ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&forced_mass);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the Higgs sector: the displaced-vacuum tower's first rungs ===");
    ok = expect_equal((long long)"the VEV: the tower's first rung", fraction_to_text(higgs_vev()), (long long)"1/2");
    ok = expect_equal((long long)"the Higgs mass-part: the second rung", fraction_to_text(higgs_mass_part()), (long long)"1/4");
    ok = expect_equal((long long)"the self-coupling lambda: the third rung", fraction_to_text(higgs_self_coupling()), (long long)"1/8");
    ok = expect_bool((long long)"the rungs halve in sequence (the tower walked)", rungs_halve());
    ok = expect_bool((long long)"the vacuum is displaced: self-antipodal, folds to the One", vacuum_is_displaced());
    ok = expect_equal((long long)"ROUTE A: m_H/v read off the rungs", fraction_to_text(mass_ratio_from_rungs()), (long long)"1/2");
    ok = expect_bool((long long)"ROUTE B: m_H^2 = 2 lambda v^2 closes on the same 1/2", relation_closes());
    forced_mass = higgs_mass_from_vev(24622, 100);
    ok = expect_equal((long long)"forced m_H = v/2 = 123.11 GeV (measured 125.25 -- 1.7%)", fraction_to_decimal(forced_mass, 2), (long long)"123.11");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(forced_mass);
    forced_mass = 0;
    return ret_val;
}

long long tower_rung(long long depth) {
    long long tower = 0;
    long long ret_val = 0;
...
long long rungs_halve() {
    ...
    long long mass_part = 0;
    long long mass_part_twin = 0;
    long long second = 0;
    ...
    ep_gc_push_root(&mass_part);
    ep_gc_push_root(&mass_part_twin);
    ...
    vev = tower_rung(1);
    second = fraction_multiply(vev, half);
    mass_part = tower_rung(2);
    first_step = fraction_compare(second, mass_part) == 0;
    half_twin = fraction_from_ratio(1, 2);
    mass_part_twin = tower_rung(2);
    third = fraction_multiply(mass_part_twin, half_twin);
    coupling = tower_rung(3);
    second_step = fraction_compare(third, coupling) == 0;
    if (first_step) {
    ret_val = second_step;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(coupling);
    coupling = 0;
    free_struct_Fraction(mass_part);
    mass_part = 0;
...
long long vacuum_is_displaced() {
    ...
    long long reaches_one = 0;
    long long reflected = 0;
    long long self_antipodal = 0;
    long long vev = 0;
    long long vev_a = 0;
    long long vev_b = 0;
    long long vev_twin = 0;
    long long ret_val = 0;
    ...
    ep_gc_push_root(&vev);
    ep_gc_push_root(&vev_a);
    ep_gc_push_root(&vev_b);
    ep_gc_push_root(&vev_twin);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    vev = tower_rung(1);
    reflected = fraction_subtract(one, vev);
    vev_twin = tower_rung(1);
    self_antipodal = fraction_compare(reflected, vev_twin) == 0;
    vev_a = tower_rung(1);
    vev_b = tower_rung(1);
    folded = fraction_add(vev_a, vev_b);
    one_twin = fraction_from_whole_number(1);
    reaches_one = fraction_compare(folded, one_twin) == 0;
    if (self_antipodal) {
    ret_val = reaches_one;
    goto L_cleanup;
    }
...
long long mass_ratio_from_rungs() {
    ...
    mass_part = tower_rung(2);
    vev = tower_rung(1);
    ret_val = fraction_divide(mass_part, vev);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long relation_closes() {
    long long coupling = 0;
    long long ratio_a = 0;
    long long ratio_a_twin = 0;
    long long ratio_squared_a = 0;
    long long ratio_squared_b = 0;
    long long two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&coupling);
    ep_gc_push_root(&ratio_a);
    ep_gc_push_root(&ratio_a_twin);
    ep_gc_push_root(&ratio_squared_a);
    ep_gc_push_root(&ratio_squared_b);
    ep_gc_push_root(&two);
    ...
    coupling = tower_rung(3);
    ratio_squared_b = fraction_multiply(two, coupling);
    ratio_a = mass_ratio_from_rungs();
    ratio_a_twin = mass_ratio_from_rungs();
    ratio_squared_a = fraction_multiply(ratio_a, ratio_a_twin);
    ret_val = fraction_compare(ratio_squared_a, ratio_squared_b) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(ratio_squared_a);
    ratio_squared_a = 0;
    free_struct_Fraction(ratio_squared_b);
    ratio_squared_b = 0;
    return ret_val;
}

long long higgs_mass_from_vev(long long vev_top, long long vev_bottom) {
    long long anchor = 0;
    long long ratio = 0;
    long long ret_val = 0;
    ...
    ep_gc_push_root(&vev_top);
    ep_gc_push_root(&vev_bottom);

    ep_gc_maybe_collect();

    anchor = fraction_from_ratio(vev_top, vev_bottom);
    ratio = mass_ratio_from_rungs();
