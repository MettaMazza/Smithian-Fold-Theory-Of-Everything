...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold algebraic algorithms ===");
    ok = expect_true((long long)"zero Fold actions are identity", fold_algebra_identity_holds(73, 7));
    ok = expect_true((long long)"counted actions compose exactly", fold_algebra_composition_holds(73, 7, 3, 4));
    ok = expect_true((long long)"three action blocks regroup exactly", fold_algebra_regrouping_holds(73, 7, 2, 3, 2));
    ok = expect_true((long long)"the recurrent Fold action has its forced inverse", fold_recurrent_inverse_holds());
    ok = expect_true((long long)"the complete algebraic census holds", complete_fold_algebraic_algorithm_census());
    ok = expect_true((long long)"a nonzero identity claim is rejected", false_nonzero_identity_is_rejected(7));
    ok = expect_true((long long)"a shortened recurrent inverse is rejected", shortened_recurrent_inverse_is_rejected());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
