...
long long new_force_lower();
long long new_force_upper();
long long sector_mass_part(long long);
long long sector_colours(long long);
long long sector_confinement_pairs(long long);
long long sector_kinds_pair_to_one(long long, long long);
long long sector_beta_slope(long long);
long long carrier_is_massless(long long);
long long carrier_flux_tube_width();
long long carrier_width_folds_to_one();
long long meson_is_neutral(long long);
long long baryon_is_neutral();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the two new forces (prime sectors 5 and 7) ===");
    ok = expect_equal((long long)"new force lower = binary + colour = 5", int_to_string(new_force_lower()), int_to_string(5));
    ok = expect_equal((long long)"new force upper = deepest depth = 7", int_to_string(new_force_upper()), int_to_string(7));
    printf("%s\n", (char*)(long long)"--- sector 5 ---");
    ok = expect_equal((long long)"mass-part (charge scale) = 1/5", fraction_to_text(sector_mass_part(5)), (long long)"1/5");
    ok = expect_equal((long long)"coupling = (5-1)/5 = 4/5", fraction_to_text(sector_coupling(5)), (long long)"4/5");
    ok = expect_equal((long long)"mediators = 5^2 - 1 = 24", int_to_string(sector_mediator_count(5)), int_to_string(24));
    ok = expect_equal((long long)"colours = 5", int_to_string(sector_colours(5)), int_to_string(5));
    ok = expect_equal((long long)"confinement pairs = (5-1)/2 = 2", int_to_string(sector_confinement_pairs(5)), int_to_string(2));
    ok = expect_equal((long long)"running beta-slope = 5-1 = 4", int_to_string(sector_beta_slope(5)), int_to_string(4));
    ok = expect_bool((long long)"carrier is massless (unbroken, luminal)", carrier_is_massless(5));
    ok = expect_bool((long long)"meson colour-neutral (colour + antipode = One)", meson_is_neutral(5));
    printf("%s\n", (char*)(long long)"--- sector 7 ---");
    ok = expect_equal((long long)"mass-part (charge scale) = 1/7", fraction_to_text(sector_mass_part(7)), (long long)"1/7");
    ok = expect_equal((long long)"coupling = (7-1)/7 = 6/7", fraction_to_text(sector_coupling(7)), (long long)"6/7");
    ok = expect_equal((long long)"mediators = 7^2 - 1 = 48", int_to_string(sector_mediator_count(7)), int_to_string(48));
    ok = expect_equal((long long)"colours = 7", int_to_string(sector_colours(7)), int_to_string(7));
    ok = expect_equal((long long)"confinement pairs = (7-1)/2 = 3", int_to_string(sector_confinement_pairs(7)), int_to_string(3));
    ok = expect_equal((long long)"running beta-slope = 7-1 = 6", int_to_string(sector_beta_slope(7)), int_to_string(6));
    ok = expect_bool((long long)"carrier is massless (unbroken, luminal)", carrier_is_massless(7));
    ok = expect_bool((long long)"meson colour-neutral (colour + antipode = One)", meson_is_neutral(7));
    printf("%s\n", (char*)(long long)"--- carrier and bound states ---");
    ok = expect_equal((long long)"carrier flux-tube width = 1/2", fraction_to_text(carrier_flux_tube_width()), (long long)"1/2");
    ok = expect_bool((long long)"flux-tube width folds to the One (confining)", carrier_width_folds_to_one());
    ok = expect_bool((long long)"baryon colour-neutral (whole group folds to One)", baryon_is_neutral());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long new_force_upper() {
    ...
    ret_val = deepest_covering_depth();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long sector_mass_part(long long p) {
    long long ret_val = 0;
...
long long sector_colours(long long p) {
    long long ret_val = 0;
...
long long sector_confinement_pairs(long long p) {
    long long interior = 0;
    long long ret_val = 0;

    interior = (p - 1);
    ret_val = (interior / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long sector_kinds_pair_to_one(long long p, long long j) {
    long long one = 0;
    long long antipode = 0;
    long long kind = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&antipode);
    ...
    kind = fraction_from_ratio(j, p);
    one = fraction_from_whole_number(1);
    antipode = fraction_subtract(one, kind);
    total = fraction_add(kind, antipode);
...
long long sector_beta_slope(long long p) {
    ...
    expected = fraction_from_whole_number((p - 1));
    matches = fraction_compare(ratio, expected) == 0;
    flag = 0;
    if (matches) {
    flag = 1;
    }
    checked = forced_to_be((long long)"beta-slope = coupling/shortfall = p-1", flag, 1);
    ret_val = (p - 1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
...
long long carrier_is_massless(long long p) {
    long long ret_val = 0;
    ...
    ret_val = sector_kinds_pair_to_one(p, 1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long carrier_width_folds_to_one() {
    ...
    width = fraction_from_ratio(1, binary_count());
    folded = fold_fraction(width);
    one = fraction_from_whole_number(1);
...
long long meson_is_neutral(long long p) {
    long long one = 0;
    long long total = 0;
    long long colour = 0;
    long long anticolour = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&total);
    ep_gc_push_root(&colour);
    ep_gc_push_root(&anticolour);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    colour = fraction_from_ratio(1, p);
    one = fraction_from_whole_number(1);
    anticolour = fraction_subtract(one, colour);
    total = fraction_add(colour, anticolour);
    ret_val = fraction_compare(total, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
...
long long baryon_is_neutral() {
    ...
    ep_gc_push_root(&whole);
    ep_gc_push_root(&folded);

    ep_gc_maybe_collect();

    whole = fraction_from_whole_number(1);
    folded = fold_fraction(whole);
    ret_val = fraction_compare(folded, whole) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(folded);
