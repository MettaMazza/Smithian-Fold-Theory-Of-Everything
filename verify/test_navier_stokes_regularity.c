...
long long _main();
long long lattice_floor();
long long floor_is_strictly_positive();
long long max_vorticity();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Navier-Stokes regularity (no blow-up) ===");
    ok = expect_equal((long long)"lattice floor = 1/32 (depth b+c = 5)", fraction_to_text(lattice_floor()), (long long)"1/32");
    ok = expect_bool((long long)"the floor is strictly positive (no sub-floor eddy)", floor_is_strictly_positive());
    ok = expect_equal((long long)"max vorticity = c/floor = 32 (guard vs 2^5 armed)", int_to_string(max_vorticity()), (long long)"32");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long lattice_floor() {
    ...
    depth = (smallest_fold_period_above(1) + smallest_fold_period_above(binary_count()));
...
long long max_vorticity() {
    long long cap_as_whole = 0;
    long long by_cap = 0;
    long long one = 0;
    long long by_volume = 0;
    long long cap = 0;
    long long floor = 0;
    long long cap_text = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cap_as_whole);
    ep_gc_push_root(&by_cap);
    ep_gc_push_root(&one);
    ep_gc_push_root(&by_volume);
    ep_gc_push_root(&cap);
    ep_gc_push_root(&floor);
    ep_gc_push_root(&cap_text);

    ep_gc_maybe_collect();

    floor = lattice_floor();
    one = fraction_from_whole_number(1);
    cap = fraction_divide(one, floor);
    cap_as_whole = exact_integer_from_sign_and_digits(({ long long _fap = cap; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }), ({ long long _fap = cap; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_digits; }));
    cap_text = exact_integer_to_text(cap_as_whole);
    by_cap = text_to_number(cap_text);
    by_volume = whole_power(binary_count(), (binary_count() + colour_count()));
    ret_val = forced_to_be((long long)"max vorticity = c/floor = the depth-5 binary volume", by_cap, by_volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(cap);
    cap = 0;
    return ret_val;
}
