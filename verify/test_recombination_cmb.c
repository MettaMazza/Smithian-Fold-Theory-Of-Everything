...
long long decoupling_balance();
long long balance_is_self_antipodal();
long long transition_completes_in_one_fold();
long long release_conserves_the_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== recombination and the CMB (light set free in one fold) ===");
    ok = expect_equal((long long)"decoupling balance = 1/2", fraction_to_text(decoupling_balance()), (long long)"1/2");
    ok = expect_bool((long long)"the balance is self-antipodal (neither side preferred)", balance_is_self_antipodal());
    ok = expect_equal((long long)"one fold completes the transition (-> the One)", fraction_to_text(transition_completes_in_one_fold()), (long long)"1");
    ok = expect_equal((long long)"the release conserves the whole (halves sum to One)", fraction_to_text(release_conserves_the_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long balance_is_self_antipodal() {
    ...
    complement = fraction_subtract(one, balance);
    ret_val = fraction_compare(complement, balance) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long transition_completes_in_one_fold() {
    ...
    ret_val = fold_fraction(decoupling_balance());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long release_conserves_the_whole() {
    ...
    share = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(share, share);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
