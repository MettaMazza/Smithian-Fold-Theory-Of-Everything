...
long long mean_thermal_state();
long long one_throw_reaches_unison();
long long two_throws_make_the_whole();
long long cold_floor();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== temperature (the mean throw-rate) ===");
    ok = expect_equal((long long)"mean thermal state = the balance 1/2", fraction_to_text(mean_thermal_state()), (long long)"1/2");
    ok = expect_equal((long long)"one mean throw reaches unison (one whole level)", fraction_to_text(one_throw_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"two mean throws account for the whole", fraction_to_text(two_throws_make_the_whole()), (long long)"1");
    ok = expect_bool((long long)"the cold floor is strictly positive (no absolute zero)", floor_is_strictly_positive());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long one_throw_reaches_unison() {
    ...
    ret_val = fold_fraction(mean_thermal_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_throws_make_the_whole() {
    ...
    mean = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(mean, mean);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long cold_floor() {
    ...
    floor = fraction_from_ratio(1, binary_count());
    ret_val = floor;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
