...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== big-bang nucleosynthesis (primordial helium) ===");
    ok = expect_equal((long long)"deepest depth d_up = 7", int_to_string(up_depth()), int_to_string(7));
    ok = expect_equal((long long)"neutron/proton freeze-out ratio = 1/7", fraction_to_text(freeze_out_ratio()), (long long)"1/7");
    ok = expect_equal((long long)"primordial helium fraction Y = 1/4", fraction_to_text(helium_fraction()), (long long)"1/4");
    ok = expect_bool((long long)"Y = 1/binary^2 (a quarter)", helium_is_one_over_binary_squared());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
