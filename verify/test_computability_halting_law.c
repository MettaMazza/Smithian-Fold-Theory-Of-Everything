...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold halting law ===");
    ok = expect_true((long long)"every depth-7 closing state halts within seven Fold steps", every_closing_fold_state_halts(7));
    ok = expect_equal((long long)"the first depth-7 state halts in exactly seven steps", int_to_string(first_fold_state_halt_time(7)), int_to_string(7));
    ok = expect_true((long long)"the complete recurrent period certifies nonhalting", recurrent_fold_machine_does_not_halt());
    ok = expect_true((long long)"premature halting is rejected", premature_fold_halt_is_rejected(7));
    ok = expect_true((long long)"a recurrent terminal claim is rejected", recurrent_terminal_claim_is_rejected());
    ok = expect_true((long long)"the complete halting census holds", complete_fold_halting_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
