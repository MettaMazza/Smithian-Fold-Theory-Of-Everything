...
long long mixing_share();
long long conversion_is_complete();
long long swap_closes();
long long lopsided_cannot_fully_convert();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== neutrino oscillation (complete conversion) ===");
    ok = expect_equal((long long)"mixing share = 1/2 (each mass state carries half)", fraction_to_text(mixing_share()), (long long)"1/2");
    ok = expect_equal((long long)"conversion is COMPLETE (halves sum to the One)", fraction_to_text(conversion_is_complete()), (long long)"1");
    ok = expect_equal((long long)"the swap closes (folds to unison) and repeats", fraction_to_text(swap_closes()), (long long)"1");
    ok = expect_bool((long long)"a lopsided share cannot fully convert", lopsided_cannot_fully_convert());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long swap_closes() {
    ...
    ret_val = fold_fraction(mixing_share());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long lopsided_cannot_fully_convert() {
    ...
    long long lopsided = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&complement);
    ep_gc_push_root(&lopsided);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    lopsided = fraction_from_ratio(1, colour_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, lopsided);
    balanced = fraction_compare(complement, lopsided) == 0;
    if (balanced) {
    ret_val = 0;
    goto L_cleanup;
    }
