...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the observer resolved (observation is the fold) ===");
    ok = expect_equal((long long)"the branch weight at the counted depth 3 = 1/8", fraction_to_text(branch_weight_here()), (long long)"1/8");
    ok = expect_equal((long long)"the observer's closure state = 1/4", fraction_to_text(observer_closure()), (long long)"1/4");
    ok = expect_bool((long long)"THE JOIN: fold(1/8) = 1/4 -- observation IS the fold", observation_is_the_fold());
    ok = expect_bool((long long)"the chain continues into binding: fold(1/4) = 1/2", chain_reaches_binding());
    ok = expect_bool((long long)"no homunculus: the join is the same one operation", join_uses_no_new_operation());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
