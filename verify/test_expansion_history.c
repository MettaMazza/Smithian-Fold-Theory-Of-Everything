...
long long _main();
long long expansion_vacuum_share();
long long expansion_matter_share();
long long shares_partition_one();
long long expansion_rate_squared(long long);
long long expansion_rate_today();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the expansion history (the Friedmann curve, exact) ===");
    ok = expect_equal((long long)"vacuum share = 2/3", fraction_to_text(expansion_vacuum_share()), (long long)"2/3");
    ok = expect_equal((long long)"matter share = 1/3", fraction_to_text(expansion_matter_share()), (long long)"1/3");
    ok = expect_equal((long long)"flatness: shares partition the One", fraction_to_text(shares_partition_one()), (long long)"1");
    ok = expect_equal((long long)"E^2 today (s=1) = 1 (normalisation IS flatness)", fraction_to_text(expansion_rate_today()), (long long)"1");
    ok = expect_equal((long long)"E^2 at s=2 = 2/3 + 8/3 = 10/3", fraction_to_text(expansion_rate_squared(2)), (long long)"10/3");
    ok = expect_equal((long long)"E^2 at s=3 = 2/3 + 27/3 = 29/3", fraction_to_text(expansion_rate_squared(3)), (long long)"29/3");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long expansion_vacuum_share() {
    ...
    ret_val = fraction_from_ratio((colour_count() - 1), colour_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shares_partition_one() {
    ...
    vac = fraction_from_ratio((colour_count() - 1), colour_count());
    mat = fraction_from_ratio(1, colour_count());
    ret_val = fraction_add(vac, mat);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long expansion_rate_squared(long long stretch) {
    long long cube = 0;
    long long cube_fraction = 0;
    long long matter_term = 0;
    long long vac = 0;
    long long mat = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cube);
    ep_gc_push_root(&cube_fraction);
    ep_gc_push_root(&matter_term);
    ep_gc_push_root(&vac);
    ep_gc_push_root(&mat);
    ep_gc_push_root(&stretch);

    ep_gc_maybe_collect();

    cube = whole_power(stretch, colour_count());
    cube_fraction = fraction_from_whole_number(cube);
    mat = fraction_from_ratio(1, colour_count());
    matter_term = fraction_multiply(mat, cube_fraction);
    vac = fraction_from_ratio((colour_count() - 1), colour_count());
    ret_val = fraction_add(vac, matter_term);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long expansion_rate_today() {
    ...
    ret_val = expansion_rate_squared(1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
