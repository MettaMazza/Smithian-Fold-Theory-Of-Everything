...
long long universal_threshold();
long long threshold_is_self_antipodal();
long long threshold_is_unique();
long long threshold_folds_to_unison();
long long phases_partition_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== universality (one threshold for every system) ===");
    ok = expect_equal((long long)"the universal threshold = 1/2", fraction_to_text(universal_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"the threshold is self-antipodal: p = One - p", threshold_is_self_antipodal());
    ok = expect_bool((long long)"it is UNIQUE (exhaustive check: one hit on the quarter grid)", threshold_is_unique());
    ok = expect_bool((long long)"the single lock folds to unison", threshold_folds_to_unison());
    ok = expect_equal((long long)"the two phase-shares partition the whole", fraction_to_text(phases_partition_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long threshold_is_self_antipodal() {
    ...
    p = fraction_from_ratio(1, 2);
    one = fraction_from_whole_number(1);
    partner = fraction_subtract(one, p);
    ret_val = fraction_compare(p, partner) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long threshold_is_unique() {
    ...
    k = 1;
    while (k < 5) {
    p = fraction_from_ratio(k, 4);
    one = fraction_from_whole_number(1);
    partner = fraction_subtract(one, p);
    balanced = fraction_compare(p, partner) == 0;
    if (balanced) {
    hits = (hits + 1);
    }
    k = (k + 1);
...
long long phases_partition_whole() {
    ...
    p = fraction_from_ratio(1, 2);
    partner = fraction_from_ratio(1, 2);
    ret_val = fraction_add(p, partner);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
