...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold resource law ===");
    ok = expect_equal((long long)"depth-7 exact space resource", int_to_string(space_resource(7)), int_to_string(128));
    ok = expect_equal((long long)"depth-7 exact time resource", int_to_string(time_resource(7)), int_to_string(7));
    ok = expect_equal((long long)"three observations retain 16 distinctions", int_to_string(retained_resource(7, 3)), int_to_string(16));
    ok = expect_equal((long long)"three observations close 8 histories", int_to_string(closed_history_resource(3)), int_to_string(8));
    ok = expect_equal((long long)"retained and closed resources reconstruct 128", int_to_string(retained_closed_accounting(7, 3)), int_to_string(128));
    ok = expect_equal((long long)"depth-7 path resource", int_to_string(path_resource(7)), int_to_string(128));
    ok = expect_true((long long)"one-step-short time is rejected", shortened_time_is_rejected(7));
    ok = expect_true((long long)"nonbinary history growth is rejected", nonbinary_history_is_rejected(7));
    ok = expect_true((long long)"complete resource census holds", complete_resource_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
