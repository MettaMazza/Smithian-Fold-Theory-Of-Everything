...
long long single_spin();
long long aligned_net_moment();
long long is_ferromagnetic();
long long alignment_reaches_unison();
long long curie_threshold();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== magnetism ===");
    ok = expect_equal((long long)"single spin = 1/2", fraction_to_text(single_spin()), (long long)"1/2");
    ok = expect_equal((long long)"aligned net moment = 1/2 + 1/2 = the One", fraction_to_text(aligned_net_moment()), (long long)"1");
    ok = expect_bool((long long)"ferromagnetic: aligned spins reach a net moment", is_ferromagnetic());
    ok = expect_equal((long long)"alignment reaches unison (fold(1/2) = 1)", fraction_to_text(alignment_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"Curie ordering threshold = 1/2 (balance point)", fraction_to_text(curie_threshold()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long is_ferromagnetic() {
    ...
    net = aligned_net_moment();
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(net, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(net);
    net = 0;
...
long long alignment_reaches_unison() {
    ...
    spin = fraction_from_ratio(1, binary_count());
    ret_val = fold_fraction(spin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
