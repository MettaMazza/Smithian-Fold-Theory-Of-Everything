...
long long both_hands_share_coupling();
long long charged_current(long long);
long long charged_current_flips_left();
long long neutral_current(long long);
long long neutral_current_preserves();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== electroweak currents ===");
    ok = expect_equal((long long)"left hand = 1/4", fraction_to_text(left_hand()), (long long)"1/4");
    ok = expect_equal((long long)"right hand = 3/4", fraction_to_text(right_hand()), (long long)"3/4");
    ok = expect_bool((long long)"both hands fold to the shared coupling 1/2", both_hands_share_coupling());
    ok = expect_equal((long long)"charged current (W) flips 1/4 -> 3/4", fraction_to_text(charged_current(left_hand())), (long long)"3/4");
    ok = expect_bool((long long)"the charged current FLIPS the hand (antipode)", charged_current_flips_left());
    ok = expect_bool((long long)"the neutral current (Z) PRESERVES the hand (identity)", neutral_current_preserves());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long both_hands_share_coupling() {
    long long right_image = 0;
    long long left_image = 0;
    long long ret_val = 0;

    ep_gc_push_root(&right_image);
    ep_gc_push_root(&left_image);

    ep_gc_maybe_collect();

    left_image = fold_fraction(left_hand());
    right_image = fold_fraction(right_hand());
    ret_val = fraction_compare(left_image, right_image) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(right_image);
    right_image = 0;
    free_struct_Fraction(left_image);
    left_image = 0;
    return ret_val;
}

long long charged_current(long long hand) {
    long long one = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_subtract(one, hand);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long charged_current_flips_left() {
    ...
    ep_gc_push_root(&flipped);
    ep_gc_push_root(&right);

    ep_gc_maybe_collect();

    flipped = charged_current(left_hand());
    right = fraction_from_ratio(3, 4);
    ret_val = fraction_compare(flipped, right) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(flipped);
    flipped = 0;
...
long long neutral_current(long long hand) {
    long long ret_val = 0;
...
long long neutral_current_preserves() {
    ...
    ep_gc_push_root(&kept);
    ep_gc_push_root(&left);

    ep_gc_maybe_collect();

    kept = fraction_from_ratio(1, 4);
    left = fraction_from_ratio(1, 4);
    ret_val = fraction_compare(kept, left) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(kept);
    kept = 0;
