...
long long _main();
long long initial_disturbance();
long long right_moving_packet();
long long left_moving_packet();
long long split_conserves_disturbance();
long long split_is_even();
long long packet_speed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the d'Alembert wave ===");
    ok = expect_equal((long long)"initial disturbance U0 = 1/2", fraction_to_text(initial_disturbance()), (long long)"1/2");
    ok = expect_equal((long long)"right-moving packet = U0/b = 1/4", fraction_to_text(right_moving_packet()), (long long)"1/4");
    ok = expect_equal((long long)"left-moving packet = 1/4", fraction_to_text(left_moving_packet()), (long long)"1/4");
    ok = expect_bool((long long)"split conserves the disturbance (right + left = U0)", split_conserves_disturbance());
    ok = expect_bool((long long)"split is even (the two halves equal)", split_is_even());
    ok = expect_equal((long long)"each packet travels at the One (signal speed)", fold_value_to_text(packet_speed()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long right_moving_packet() {
    ...
    u0 = fraction_from_ratio(1, binary_count());
    b = fraction_from_whole_number(binary_count());
    ret_val = fraction_divide(u0, b);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long split_conserves_disturbance() {
    ...
    right = right_moving_packet();
    left = left_moving_packet();
    total = fraction_add(right, left);
    u0 = fraction_from_ratio(1, binary_count());
    ret_val = fraction_compare(total, u0) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(total);
    total = 0;
    free_struct_Fraction(u0);
    u0 = 0;
    return ret_val;
}
...
long long split_is_even() {
    ...
    right = right_moving_packet();
    left = left_moving_packet();
    ret_val = fraction_compare(right, left) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
