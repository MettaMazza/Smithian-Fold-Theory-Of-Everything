...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold relative-computation law ===");
    ok = expect_true((long long)"an unassisted observation cannot distinguish a fibre pair", unassisted_fold_observation_is_indistinguishable(1, 7));
    ok = expect_true((long long)"the held Fold label distinguishes the pair", relative_fold_label_distinguishes_pair(1, 7));
    ok = expect_equal((long long)"the held upper label reconstructs the upper source", int_to_string(relative_fold_reconstruct_source(1, 7, 2)), int_to_string(65));
    ok = expect_true((long long)"every admitted source reconstructs relatively", complete_relative_fold_census());
    ok = expect_true((long long)"an outside held label is rejected", outside_relative_label_is_rejected(7));
    ok = expect_true((long long)"changing the held label changes the reconstructed source", changed_relative_label_changes_source(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
