...
long long alpha_phi_coord();
long long alpha_psi_coord();
long long beta_phi_coord();
long long beta_psi_coord();
long long left_alpha_phi_coord();
long long left_alpha_psi_coord();
long long alpha_phi_folds_to_orbit();
long long alpha_psi_folds_to_native();
long long beta_phi_folds_to_orbit();
long long beta_psi_folds_to_native();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== 3D protein folding ===");
    ok = expect_bool((long long)"alpha-helix phi folds to 1/3 (period-2 orbit)", alpha_phi_folds_to_orbit());
    ok = expect_bool((long long)"alpha-helix psi decays to native (1)", alpha_psi_folds_to_native());
    ok = expect_bool((long long)"beta-sheet phi folds to 2/3 (period-2 orbit)", beta_phi_folds_to_orbit());
    ok = expect_bool((long long)"beta-sheet psi decays to native (1)", beta_psi_folds_to_native());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long alpha_phi_coord() {
    ...
    f = fraction_from_ratio(1, 6);
    ((EpStruct_Fraction*)(f))->numerator_sign = -1;
    ep_gc_write_barrier((void*)(f), -1);
    ret_val = f;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long beta_psi_coord() {
    ...
    ret_val = fraction_from_ratio(3, 8);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long left_alpha_phi_coord() {
    ...
    ret_val = fraction_from_ratio(1, 6);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long alpha_phi_folds_to_orbit() {
    ...
    x = alpha_phi_coord();
    ((EpStruct_Fraction*)(x))->numerator_sign = 1;
    ep_gc_write_barrier((void*)(x), 1);
    folded = fold_fraction(x);
...
long long alpha_psi_folds_to_native() {
    ...
    ep_gc_push_root(&step1);
    ep_gc_push_root(&step2);
    ep_gc_push_root(&step3);
    ep_gc_push_root(&target);
    ep_gc_push_root(&x);
    ...
    ((EpStruct_Fraction*)(x))->numerator_sign = 1;
    ep_gc_write_barrier((void*)(x), 1);
    step1 = fold_fraction(x);
    step2 = fold_fraction(step1);
    step3 = fold_fraction(step2);
    target = fraction_from_whole_number(1);
    ret_val = fraction_compare(step3, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(step3);
    step3 = 0;
    free_struct_Fraction(target);
    target = 0;
    return ret_val;
