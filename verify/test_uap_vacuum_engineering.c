...
long long vacuum_displacement();
long long inertia_coupling();
long long exchange_rate();
long long exchange_rate_is_one();
long long both_complete_together();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the vacuum-inertia unity (exchange rate one) ===");
    ok = expect_equal((long long)"the vacuum displacement v = 1/2", fraction_to_text(vacuum_displacement()), (long long)"1/2");
    ok = expect_equal((long long)"the inertia coupling g* = 1/2", fraction_to_text(inertia_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"the exchange rate v/g* = the One, exactly", fraction_to_text(exchange_rate()), (long long)"1");
    ok = expect_bool((long long)"the rate is unity (couple to one = couple to the other)", exchange_rate_is_one());
    ok = expect_bool((long long)"both complete to unison in the same single fold", both_complete_together());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long exchange_rate_is_one() {
    ...
    ret_val = fraction_compare(rate, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long both_complete_together() {
    long long g_folded = 0;
    long long g_ok = 0;
    long long one = 0;
    long long v_folded = 0;
    long long v_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&g_folded);
    ep_gc_push_root(&one);
    ep_gc_push_root(&v_folded);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    v_folded = fold_fraction(vacuum_displacement());
    g_folded = fold_fraction(inertia_coupling());
    v_ok = fraction_compare(v_folded, one) == 0;
    g_ok = fraction_compare(g_folded, one) == 0;
    if (v_ok) {
    ret_val = g_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(g_folded);
    g_folded = 0;
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(v_folded);
    v_folded = 0;
    return ret_val;
}
