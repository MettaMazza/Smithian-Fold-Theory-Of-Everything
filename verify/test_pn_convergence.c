...
long long _main();
long long pn_coupling();
long long pn_source();
long long fixed_point();
long long pn_step(long long);
long long fixed_point_is_consistent();
long long gap_to_fixed_point(long long);
long long iteration_contracts();
long long fixed_point_is_depth_two();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== post-Newtonian convergence (the self-sourcing fixed point) ===");
    ok = expect_equal((long long)"coupling g = 1/2", fraction_to_text(pn_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"matter source M = 7/16", fraction_to_text(pn_source()), (long long)"7/16");
    ok = expect_equal((long long)"fixed point f* = 1/4", fraction_to_text(fixed_point()), (long long)"1/4");
    ok = expect_bool((long long)"f* is self-consistent: g(M + f*^2) = f*", fixed_point_is_consistent());
    ok = expect_bool((long long)"the iteration contracts (gap shrinks) -> the series converges", iteration_contracts());
    ok = expect_bool((long long)"the fixed point is the depth-2 fold scale 1/4 = (1/2)^2", fixed_point_is_depth_two());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long pn_source() {
    ...
    numerator = ((smallest_fold_period_above(1) * smallest_fold_period_above(binary_count())) + 1);
    denominator = whole_power(binary_count(), 4);
...
long long pn_step(long long f) {
    long long f_squared = 0;
    long long g = 0;
    long long m = 0;
    long long source_total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&f_squared);
    ep_gc_push_root(&g);
    ep_gc_push_root(&m);
    ep_gc_push_root(&source_total);
    ep_gc_push_root(&f);

    ep_gc_maybe_collect();

    f_squared = fraction_multiply(f, f);
    m = pn_source();
    source_total = fraction_add(m, f_squared);
    g = fraction_from_ratio(1, binary_count());
    ret_val = fraction_multiply(g, source_total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long fixed_point_is_consistent() {
    ...
    ep_gc_push_root(&f_star);
    ep_gc_push_root(&stepped);

    ep_gc_maybe_collect();

    f_star = fraction_from_ratio(1, (binary_count() * binary_count()));
    stepped = pn_step(f_star);
    ret_val = fraction_compare(stepped, f_star) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(stepped);
    stepped = 0;
    return ret_val;
}

long long gap_to_fixed_point(long long f) {
    long long f_star = 0;
    long long ret_val = 0;
    ...
    f_star = fraction_from_ratio(1, (binary_count() * binary_count()));
    ret_val = fraction_subtract(f, f_star);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long iteration_contracts() {
    ...
    ep_gc_push_root(&gap0);
    ep_gc_push_root(&gap1);
    ep_gc_push_root(&once);

    ep_gc_maybe_collect();

    bare = pn_source();
    gap0 = gap_to_fixed_point(bare);
    once = pn_step(bare);
    gap1 = gap_to_fixed_point(once);
    comparison = fraction_compare(gap1, gap0);
...
long long fixed_point_is_depth_two() {
    long long depth_two = 0;
    long long f_star = 0;
    long long half = 0;
    long long ret_val = 0;

    ep_gc_push_root(&depth_two);
    ep_gc_push_root(&f_star);
    ...
    depth_two = fraction_multiply(half, half);
    ret_val = fraction_compare(f_star, depth_two) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(depth_two);
    depth_two = 0;
    free_struct_Fraction(f_star);
    f_star = 0;
    return ret_val;
}
