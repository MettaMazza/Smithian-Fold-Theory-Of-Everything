...
long long _main();
long long gcd_of(long long, long long);
long long lcm_of(long long, long long);
long long gravity_period();
long long em_period();
long long strong_period();
long long joint_period();
long long joint_is_binary_times_colour();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the master equation (the forces' joint cycle) ===");
    ok = expect_equal((long long)"gravity period = 1 (the fixed point)", int_to_string(gravity_period()), (long long)"1");
    ok = expect_equal((long long)"EM period (1/3) = 2", int_to_string(em_period()), (long long)"2");
    ok = expect_equal((long long)"strong period (1/7) = 3", int_to_string(strong_period()), (long long)"3");
    ok = expect_equal((long long)"joint period = lcm(1,2,3) = 6", int_to_string(joint_period()), (long long)"6");
    ok = expect_bool((long long)"the joint cycle is exactly binary * colour = 6", joint_is_binary_times_colour());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long em_period() {
    ...
    ret_val = fold_period_of_unit_fraction(colour_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long strong_period() {
    ...
    ret_val = fold_period_of_unit_fraction(((binary_count() * colour_count()) + 1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long joint_period() {
    ...
    ep_gc_push_root(&g_em);

    ep_gc_maybe_collect();

    g_em = lcm_of(gravity_period(), em_period());
    ret_val = lcm_of(g_em, strong_period());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long joint_is_binary_times_colour() {
    ...
    ret_val = joint_period() == (smallest_fold_period_above(1) * smallest_fold_period_above(binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
