...
long long pre_lock_state();
long long crosses_the_lock();
long long cycle_locks_in();
long long two_pre_lock_make_the_lock();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the origin of life (autocatalytic ignition) ===");
    ok = expect_equal((long long)"pre-lock state = 1/4 (two folds below unison)", fraction_to_text(pre_lock_state()), (long long)"1/4");
    ok = expect_equal((long long)"first fold crosses the autocatalytic lock (-> 1/2)", fraction_to_text(crosses_the_lock()), (long long)"1/2");
    ok = expect_equal((long long)"second fold locks in the cycle (-> the One)", fraction_to_text(cycle_locks_in()), (long long)"1");
    ok = expect_equal((long long)"two pre-lock shares make one lock", fraction_to_text(two_pre_lock_make_the_lock()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long crosses_the_lock() {
    ...
    ret_val = fold_fraction(pre_lock_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long cycle_locks_in() {
    ...
    ret_val = fold_fraction(crosses_the_lock());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
