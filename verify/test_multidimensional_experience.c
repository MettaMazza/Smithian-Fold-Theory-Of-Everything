...
long long orbit_base();
long long second_state();
long long third_state();
long long orbit_returns_in_three();
long long states_partition_one();
long long qualities_held();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== multidimensional experience (a period-3 orbit) ===");
    ok = expect_equal((long long)"orbit base = 1/7", fraction_to_text(orbit_base()), (long long)"1/7");
    ok = expect_equal((long long)"second state = 2/7", fraction_to_text(second_state()), (long long)"2/7");
    ok = expect_equal((long long)"third state = 4/7", fraction_to_text(third_state()), (long long)"4/7");
    ok = expect_bool((long long)"the orbit is closed with period 3 (returns to the base)", orbit_returns_in_three());
    ok = expect_equal((long long)"the three states partition the One (1/7+2/7+4/7)", fraction_to_text(states_partition_one()), (long long)"1");
    ok = expect_equal((long long)"qualities held at once = colour = 3", int_to_string(qualities_held()), (long long)"3");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long orbit_base() {
    ...
    ep_gc_push_root(&seven);

    ep_gc_maybe_collect();

    seven = ((smallest_fold_period_above(1) * smallest_fold_period_above(binary_count())) + 1);
    ret_val = fraction_from_ratio(1, seven);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long second_state() {
    ...
    ret_val = fold_fraction(orbit_base());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long third_state() {
    ...
    ret_val = fold_fraction(second_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long orbit_returns_in_three() {
    ...
    back = fold_fraction(third_state());
    base = orbit_base();
    ret_val = fraction_compare(back, base) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(back);
    back = 0;
    free_struct_Fraction(base);
    base = 0;
    return ret_val;
}
...
long long states_partition_one() {
    ...
    first = orbit_base();
    second = fold_fraction(orbit_base());
    third = fold_fraction(second_state());
