...
long long screened_charge();
long long charge_up_close();
long long screened_is_below_bare();
long long screen_plus_cloud();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== vacuum polarization (charge runs with distance) ===");
    ok = expect_equal((long long)"screened charge (far, low energy) = 1/2", fraction_to_text(screened_charge()), (long long)"1/2");
    ok = expect_equal((long long)"probe closer: folds up toward the bare One (1/2 -> 1)", fraction_to_text(charge_up_close()), (long long)"1");
    ok = expect_bool((long long)"screened value is below the bare One (the running)", screened_is_below_bare());
    ok = expect_equal((long long)"screen + cloud = the One (cloud carries the other half)", fraction_to_text(screen_plus_cloud()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long charge_up_close() {
    ...
    ret_val = fold_fraction(screened_charge());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long screened_is_below_bare() {
    ...
    comparison = fraction_compare(screened_charge(), one);
...
long long screen_plus_cloud() {
    ...
    ep_gc_push_root(&screen);

    ep_gc_maybe_collect();

    screen = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(screen, screen);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
