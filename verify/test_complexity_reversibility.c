...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold reversibility cost ===");
    ok = expect_equal((long long)"reversing three Fold steps costs three held labels", int_to_string(fold_reversal_information_cost(3)), int_to_string(3));
    ok = expect_equal((long long)"the reverse record distinguishes eight histories", int_to_string(fold_reversal_history_cost(3)), int_to_string(8));
    ok = expect_true((long long)"a complete record reverses an exact source", recorded_fold_process_is_reversible(85, 7, 3));
    ok = expect_true((long long)"one missing reverse label leaves binary ambiguity", one_missing_reverse_label_leaves_binary_ambiguity(3));
    ok = expect_equal((long long)"a complete recurrent cycle has zero reverse cost", int_to_string(recurrent_cycle_reverse_cost()), int_to_string(0));
    ok = expect_true((long long)"the complete reversibility census holds", complete_fold_reversibility_census());
    ok = expect_true((long long)"zero-cost dyadic reversal is rejected", zero_cost_dyadic_reversal_is_rejected());
    ok = expect_true((long long)"an extra reverse cost is not minimal", extra_reverse_cost_is_not_minimal(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
