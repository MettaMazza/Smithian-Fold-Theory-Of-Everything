...
long long total_density();
long long total_is_fold_invariant();
long long curvature_share();
long long curvature_is_excluded_boundary();
long long flatness_forced();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== spatial flatness (the geometry is flat) ===");
    ok = expect_equal((long long)"total physical density = the fold fixed point, the One", fraction_to_text(total_density()), (long long)"1");
    ok = expect_bool((long long)"the total is fold-invariant: fold(1) = 1", total_is_fold_invariant());
    ok = expect_bool((long long)"the curvature share sits at the excluded boundary (no curvature)", curvature_is_excluded_boundary());
    ok = expect_bool((long long)"flatness is forced: fixed-point total AND no curvature share", flatness_forced());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long total_is_fold_invariant() {
    ...
    total = fraction_from_whole_number(1);
    folded = fold_fraction(total);
    ret_val = fraction_compare(folded, total) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long curvature_share() {
    ...
    total = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long curvature_is_excluded_boundary() {
    long long curvature = 0;
    long long ret_val = 0;

    ep_gc_push_root(&curvature);

    ep_gc_maybe_collect();

    curvature = curvature_share();
    ret_val = ({ long long _fap = curvature; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(curvature);
    curvature = 0;
    return ret_val;
}

long long flatness_forced() {
    long long at_fixed_point = 0;
    long long no_curvature = 0;
    long long ret_val = 0;

    at_fixed_point = total_is_fold_invariant();
    no_curvature = curvature_is_excluded_boundary();
    if (at_fixed_point) {
    ret_val = no_curvature;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
