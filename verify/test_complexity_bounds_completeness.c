...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold lower/upper bounds and completeness ===");
    ok = expect_equal((long long)"the depth-7 time lower bound is seven", int_to_string(fold_time_lower_bound(7)), int_to_string(7));
    ok = expect_equal((long long)"the depth-7 time upper bound is seven", int_to_string(fold_time_upper_bound(7)), int_to_string(7));
    ok = expect_true((long long)"the time bound is exact and tight", fold_time_bound_is_tight(7));
    ok = expect_equal((long long)"the space lower bound is 128", int_to_string(fold_space_lower_bound(7)), int_to_string(128));
    ok = expect_equal((long long)"the space upper bound is 128", int_to_string(fold_space_upper_bound(7)), int_to_string(128));
    ok = expect_true((long long)"Fold closure is complete for every admitted source", fold_closure_is_complete(7));
    ok = expect_true((long long)"the complete bounds census holds", complete_fold_bounds_census());
    ok = expect_true((long long)"a shorter upper bound is rejected", false_shorter_upper_bound_is_rejected(7));
    ok = expect_true((long long)"an incomplete reduction is rejected", incomplete_closure_reduction_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
