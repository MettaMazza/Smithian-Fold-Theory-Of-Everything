...
long long _main();
long long holding_activations_cycle();
long long closure_activations_quiet();
long long loud_weights_quiet_activations_is_closure();
long long regime_margin_is_binary_count();
long long which_layer_is_engineering();
long long activation_regime_reads_the_signature();
...
long long the_one_is_invariant();
long long closure_terminates();
long long holding_repeats();
long long closure_repeats();
long long repetition_inequality_holds();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the activation regime (Step 320 / 7.4) ===");
    ok = expect_bool((long long)"holding store: activations cycle (holding repeats b^2-b > 0)", holding_activations_cycle());
    ok = expect_bool((long long)"closure store: activations quiet (closure repeats 0)", closure_activations_quiet());
    ok = expect_bool((long long)"SIGNATURE: loud weights + quiet activations = a closure store", loud_weights_quiet_activations_is_closure());
    ok = expect_bool((long long)"the two regimes separated on the activation axis by b (the binary count)", regime_margin_is_binary_count());
    ok = expect_equal((long long)"which layers close is engineering, kept off the derivation (guard passes)", int_to_string(which_layer_is_engineering()), (long long)"1");
    ok = expect_bool((long long)"THE PICTURE: activation loudness reads the regime; L10-11 = closure signature", activation_regime_reads_the_signature());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    activation spectrometer: GPT-2 L10-11 loud weights, quiet activations (the departure, already in)");
    printf("%s\n", (char*)(long long)"  note  registered verification (rung 7g): per-layer activation repetition vs weight loudness; closure floor near 0");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long loud_weights_quiet_activations_is_closure() {
    ...
    ret_val = closure_activations_quiet();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
...
long long regime_margin_is_binary_count() {
    ...
    ret_val = repetition_inequality_holds();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long which_layer_is_engineering() {
    ...
    ret_val = forbid_target_input((long long)"which layers perform closure (network depth / output position)", 0LL);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long activation_regime_reads_the_signature() {
    ...
    if (repetition_inequality_holds()) {
    if (forbid_target_input((long long)"which layers perform closure (network depth / output position)", 0LL) == 1) {
    ret_val = 1LL;
    goto L_cleanup;
    } else {
    ret_val = 0LL;
...
long long the_one_is_invariant() {
    ...
    ret_val = fraction_compare(fold_fraction(one), one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long closure_terminates() {
    ...
    ret_val = fraction_compare(fold_fraction(balance), fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(balance);
    balance = 0;
    return ret_val;
}
...
long long holding_repeats() {
    ...
    long long repeats = 0;
    long long state = 0;
    long long visits_of_start = 0;
    long long walked = 0;
    long long ret_val = 0;

    ep_gc_push_root(&repeats);
    ep_gc_push_root(&state);
    ep_gc_push_root(&walked);
    ...
    visits_of_start = 0;
    folds = 0;
    while (folds < whole_power(binary_count(), 2)) {
    if (fraction_compare(state, memory_state()) == 0) {
    visits_of_start = (visits_of_start + 1);
    }
    state = fold_fraction(state);
    folds = (folds + 1);
    }
    repeats = (whole_power(binary_count(), 2) - smallest_fold_period_above(1));
    walked = ((visits_of_start - 1) * smallest_fold_period_above(1));
    ret_val = forced_to_be((long long)"holding repeats over b*b folds = b*b - b", walked, repeats);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long closure_repeats() {
    ...
    state = fold_fraction(fraction_from_ratio(1, binary_count()));
    repeats = 0;
    folds = 0;
    while (folds < whole_power(binary_count(), 2)) {
    state = fold_fraction(state);
...
long long repetition_inequality_holds() {
    long long forced_margin = 0;
    long long margin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&margin);

    ep_gc_maybe_collect();

    margin = (holding_repeats() - closure_repeats());
    forced_margin = forced_to_be((long long)"the repetition margin = the binary count", margin, binary_count());
