...
long long _main();
long long branching_one();
long long branching_two();
long long branching_ratios_partition_one();
long long lifetime(long long);
long long wider_lives_shorter();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== decay widths and branching ratios ===");
    ok = expect_equal((long long)"branching ratio one = 1/4", fraction_to_text(branching_one()), (long long)"1/4");
    ok = expect_equal((long long)"branching ratio two = 3/4", fraction_to_text(branching_two()), (long long)"3/4");
    ok = expect_bool((long long)"the branching ratios partition the One (sum = 1)", branching_ratios_partition_one());
    ok = expect_equal((long long)"lifetime of a width-1/2 particle = 2", fraction_to_text(lifetime(fraction_from_ratio(1, 2))), (long long)"2");
    ok = expect_bool((long long)"a wider particle lives shorter (lifetime = 1/width)", wider_lives_shorter());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long branching_ratios_partition_one() {
    ...
    b1 = fraction_from_ratio(1, 4);
    b2 = fraction_from_ratio(3, 4);
    total = fraction_add(b1, b2);
...
long long lifetime(long long width) {
    long long one = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    ret_val = fraction_divide(one, width);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long wider_lives_shorter() {
    long long wide = 0;
    long long narrow = 0;
    long long life_wide = 0;
    long long order = 0;
    long long life_narrow = 0;
    long long ret_val = 0;

    ep_gc_push_root(&wide);
    ep_gc_push_root(&narrow);
    ep_gc_push_root(&life_wide);
    ep_gc_push_root(&life_narrow);

    ep_gc_maybe_collect();

    wide = fraction_from_whole_number(1);
    narrow = fraction_from_ratio(1, 2);
    life_wide = lifetime(wide);
    life_narrow = lifetime(narrow);
    order = fraction_compare(life_wide, life_narrow);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
