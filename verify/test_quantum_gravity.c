...
long long dimension_count();
long long metric_spacing();
long long spacing_is_depth_two();
long long spacing_folds_to_critical();
long long spacing_completes_in_two_folds();
long long dimension_shares_partition_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== quantum gravity (born quantized) ===");
    ok = expect_equal((long long)"spacetime dimension count = 4 = binary^2", int_to_string(dimension_count()), (long long)"4");
    ok = expect_equal((long long)"the metric grid spacing = 1/4", fraction_to_text(metric_spacing()), (long long)"1/4");
    ok = expect_bool((long long)"the spacing is the depth-two step: 1/4 = (1/2)^2", spacing_is_depth_two());
    ok = expect_bool((long long)"one fold lifts gravity to the critical coupling: fold(1/4) = 1/2", spacing_folds_to_critical());
    ok = expect_bool((long long)"two folds complete to unison (same ladder as every sector)", spacing_completes_in_two_folds());
    ok = expect_equal((long long)"the four dimension-shares partition the One", fraction_to_text(dimension_shares_partition_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long dimension_count() {
    ...
    ret_val = whole_power(binary_count(), 2);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long metric_spacing() {
    ...
    dims = whole_power(binary_count(), 2);
    ret_val = fraction_from_ratio(1, dims);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long spacing_is_depth_two() {
    ...
    long long half = 0;
    long long half_again = 0;
    long long spacing = 0;
    long long ret_val = 0;

    ep_gc_push_root(&depth_two);
    ep_gc_push_root(&half);
    ep_gc_push_root(&half_again);
    ...
    half_again = fraction_from_ratio(1, 2);
    depth_two = fraction_multiply(half, half_again);
    ret_val = fraction_compare(spacing, depth_two) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long spacing_folds_to_critical() {
    ...
    spacing = metric_spacing();
    folded = fold_fraction(spacing);
    critical = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(folded, critical) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(critical);
    critical = 0;
...
long long dimension_shares_partition_one() {
    ...
    i = 0;
    while (i < dims) {
    total = fraction_add(total, spacing);
    i = (i + 1);
    }
    ret_val = total;
    goto L_cleanup;
