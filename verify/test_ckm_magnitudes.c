...
long long _main();
long long tripling_fold(long long);
long long mass_basis(long long);
long long channel_basis(long long);
long long basis_distance(long long, long long);
long long alignment_element(long long, long long);
long long diagonal_is_eight_ninths();
long long matrix_is_asymmetric();
long long far_corner_is_smallest();
long long orbit_closure_holds();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the CKM magnitudes (the full 3x3 alignment matrix) ===");
    ok = expect_equal((long long)"V_12 = 5/9 (the upper Cabibbo band)", fraction_to_text(alignment_element(1, 2)), (long long)"5/9");
    ok = expect_equal((long long)"V_21 = 7/9 (the lower band -- different!)", fraction_to_text(alignment_element(2, 1)), (long long)"7/9");
    ok = expect_equal((long long)"V_13 = 2/9 (the far corner)", fraction_to_text(alignment_element(1, 3)), (long long)"2/9");
    ok = expect_equal((long long)"V_31 = 4/9", fraction_to_text(alignment_element(3, 1)), (long long)"4/9");
    ok = expect_bool((long long)"the diagonal is the uniform 8/9 (all three)", diagonal_is_eight_ninths());
    ok = expect_bool((long long)"the matrix is asymmetric (up-leak != down-leak, as measured)", matrix_is_asymmetric());
    ok = expect_bool((long long)"the far corner is the smallest element of the whole matrix", far_corner_is_smallest());
    ok = expect_bool((long long)"ROUTE B: all nine pairs fold onto the orbit with exact complementarity", orbit_closure_holds());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long tripling_fold(long long size) {
    long long doubled = 0;
    long long tripled = 0;
    long long twin_one = 0;
    long long twin_two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&doubled);
    ep_gc_push_root(&tripled);
    ep_gc_push_root(&twin_one);
    ep_gc_push_root(&twin_two);
    ep_gc_push_root(&size);

    ep_gc_maybe_collect();

    twin_one = fraction_make(fraction_numerator(size), fraction_denominator(size));
    twin_two = fraction_make(fraction_numerator(size), fraction_denominator(size));
    doubled = fraction_add(size, twin_one);
    tripled = fraction_add(doubled, twin_two);
    ret_val = cast_out_whole_ones(tripled);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long mass_basis(long long i) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(((3 * i) - 1), 9);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long channel_basis(long long j) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(j, 3);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long basis_distance(long long i, long long j) {
    long long comparison = 0;
    long long mass = 0;
    long long weak = 0;
    long long ret_val = 0;

    ep_gc_push_root(&mass);
    ...
    mass = fraction_from_ratio(((3 * i) - 1), 9);
    weak = fraction_from_ratio(j, 3);
    comparison = fraction_compare(mass, weak);
    if (comparison > 0) {
    ret_val = fraction_subtract(mass, weak);
    goto L_cleanup;
    }
    ret_val = fraction_subtract(weak, mass);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long alignment_element(long long i, long long j) {
    ...
    distance = basis_distance(i, j);
    ret_val = fraction_subtract(one, distance);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long diagonal_is_eight_ninths() {
    ...
    ep_gc_push_root(&element);
    ep_gc_push_root(&i);
    ep_gc_push_root(&target);

    ep_gc_maybe_collect();

    target = fraction_from_ratio(8, 9);
    all_match = 1LL;
    i = 1;
    while (i < 4) {
    element = alignment_element(i, i);
    matches = fraction_compare(element, target) == 0;
    if (matches) {
    all_match = all_match;
    } else {
    all_match = 0LL;
...
long long matrix_is_asymmetric() {
    long long above = 0;
    long long above_ok = 0;
    long long below = 0;
    long long below_ok = 0;
    long long five_ninths = 0;
    long long seven_ninths = 0;
    long long ret_val = 0;

    ep_gc_push_root(&above);
    ep_gc_push_root(&below);
    ep_gc_push_root(&five_ninths);
    ep_gc_push_root(&seven_ninths);

    ep_gc_maybe_collect();

    above = alignment_element(1, 2);
    below = alignment_element(2, 1);
    five_ninths = fraction_from_ratio(5, 9);
    seven_ninths = fraction_from_ratio(7, 9);
    above_ok = fraction_compare(above, five_ninths) == 0;
    below_ok = fraction_compare(below, seven_ninths) == 0;
    if (above_ok) {
    ret_val = below_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(above);
    above = 0;
    free_struct_Fraction(below);
    below = 0;
    free_struct_Fraction(five_ninths);
    five_ninths = 0;
    free_struct_Fraction(seven_ninths);
    seven_ninths = 0;
    return ret_val;
}
...
long long far_corner_is_smallest() {
    long long comparison = 0;
    long long corner = 0;
    long long corner_ok = 0;
    long long element = 0;
    long long i = 0;
    long long j = 0;
    long long same_cell = 0;
    long long smallest = 0;
    long long two_ninths = 0;
    long long ret_val = 0;

    ep_gc_push_root(&corner);
    ep_gc_push_root(&element);
    ep_gc_push_root(&i);
    ep_gc_push_root(&j);
    ep_gc_push_root(&two_ninths);

    ep_gc_maybe_collect();

    corner = alignment_element(1, 3);
    two_ninths = fraction_from_ratio(2, 9);
    corner_ok = fraction_compare(corner, two_ninths) == 0;
    smallest = 1LL;
    i = 1;
    while (i < 4) {
    j = 1;
    ...
    if (same_cell == 0) {
    element = alignment_element(i, j);
    comparison = fraction_compare(corner, element);
    if (comparison < 0) {
    smallest = smallest;
    } else {
    smallest = 0LL;
...
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(two_ninths);
    two_ninths = 0;
    return ret_val;
}
...
long long orbit_closure_holds() {
    ...
    long long d_on_high = 0;
    long long d_on_low = 0;
    long long d_on_orbit = 0;
    long long distance = 0;
    long long folded_distance = 0;
    long long folded_overlap = 0;
    long long i = 0;
    long long j = 0;
    long long one = 0;
    long long overlap = 0;
    ...
    long long third = 0;
    long long two_thirds = 0;
    long long ret_val = 0;

    ep_gc_push_root(&distance);
    ep_gc_push_root(&folded_distance);
    ep_gc_push_root(&folded_overlap);
    ep_gc_push_root(&i);
    ep_gc_push_root(&j);
    ep_gc_push_root(&one);
    ep_gc_push_root(&overlap);
    ep_gc_push_root(&pair_sum);
    ep_gc_push_root(&third);
    ep_gc_push_root(&two_thirds);

    ep_gc_maybe_collect();

    third = fraction_from_ratio(1, 3);
    two_thirds = fraction_from_ratio(2, 3);
    one = fraction_from_whole_number(1);
    all_close = 1LL;
    i = 1;
    while (i < 4) {
    ...
    while (j < 4) {
    distance = basis_distance(i, j);
    overlap = alignment_element(i, j);
    folded_distance = tripling_fold(distance);
    folded_overlap = tripling_fold(overlap);
    d_on_low = fraction_compare(folded_distance, third) == 0;
    d_on_high = fraction_compare(folded_distance, two_thirds) == 0;
    d_on_orbit = 0LL;
    if (d_on_low) {
    d_on_orbit = 1LL;
    }
    ...
    pair_sum = fraction_add(folded_distance, folded_overlap);
    complements = fraction_compare(pair_sum, one) == 0;
    if (d_on_orbit) {
    if (complements) {
    all_close = all_close;
    } else {
...
L_cleanup:
    ...
    one = 0;
    free_struct_Fraction(pair_sum);
    pair_sum = 0;
    free_struct_Fraction(third);
    third = 0;
    free_struct_Fraction(two_thirds);
    two_thirds = 0;
    return ret_val;
}
