...
long long _main();
long long sector_prime(long long);
long long sector_shortfall_of(long long);
long long sector_coupling_of(long long);
long long every_sector_partitions();
long long couplings_strictly_ordered();
long long shortfall_sum();
long long span_invariant();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the unified force law (four primes, one formula) ===");
    ok = expect_bool((long long)"every sector partitions: 1/p + (p-1)/p = 1, all four", every_sector_partitions());
    ok = expect_bool((long long)"the couplings are strictly ordered: 1/2 < 2/3 < 4/5 < 6/7", couplings_strictly_ordered());
    ok = expect_equal((long long)"ROUTE A: the shortfall sum by addition = 247/210", fraction_to_text(shortfall_sum()), (long long)"247/210");
    ok = expect_equal((long long)"ROUTE B: the algebraic span invariant = 247/210", fraction_to_text(span_invariant()), (long long)"247/210");
    ok = expect_bool((long long)"the two routes agree", routes_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long sector_prime(long long index) {
    long long ret_val = 0;
...
long long sector_shortfall_of(long long p) {
    long long ret_val = 0;
...
long long sector_coupling_of(long long p) {
    long long ret_val = 0;
...
long long every_sector_partitions() {
    ...
    total = fraction_add(shortfall, coupling);
    closes = fraction_compare(total, one) == 0;
    if (closes) {
    all_close = all_close;
    } else {
    all_close = 0LL;
...
long long couplings_strictly_ordered() {
    ...
    i = 1;
    while (i < 4) {
    p_here = sector_prime(i);
    p_next = sector_prime((i + 1));
...
long long shortfall_sum() {
    ...
    p = sector_prime(i);
    shortfall = fraction_from_ratio(1, p);
    total = fraction_add(total, shortfall);
    i = (i + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long routes_agree() {
    long long algebraic = 0;
    long long direct = 0;
    long long ret_val = 0;

    ep_gc_push_root(&algebraic);
    ep_gc_push_root(&direct);

    ep_gc_maybe_collect();

    direct = shortfall_sum();
    algebraic = span_invariant();
    ret_val = fraction_compare(direct, algebraic) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(algebraic);
    algebraic = 0;
