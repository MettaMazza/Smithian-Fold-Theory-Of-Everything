...
long long _main();
long long boson_state();
long long fermion_state();
long long fold_fraction(long long);
long long fermion_folds_to_boson();
long long two_fermions_make_boson();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== spin and statistics ===");
    ok = expect_equal((long long)"boson state = the One", fraction_to_text(boson_state()), (long long)"1");
    ok = expect_equal((long long)"fermion state = half-One = 1/2", fraction_to_text(fermion_state()), (long long)"1/2");
    ok = expect_equal((long long)"a single fermion folds to the boson: fold(1/2) = 1", fraction_to_text(fermion_folds_to_boson()), (long long)"1");
    ok = expect_equal((long long)"two fermions make a boson: 1/2 + 1/2 = 1", fraction_to_text(two_fermions_make_boson()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fermion_folds_to_boson() {
    ...
    ep_gc_push_root(&fermion);

    ep_gc_maybe_collect();

    fermion = fraction_from_ratio(1, binary_count());
    ret_val = fold_fraction(fermion);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
