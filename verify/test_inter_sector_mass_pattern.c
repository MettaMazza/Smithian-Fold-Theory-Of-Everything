...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the inter-sector mass pattern (electron, up, down, no neutrino) ===");
    ok = expect_equal((long long)"the electron's mass-part = 1 - 1/2 = 1/2", fraction_to_text(electron_mass_part()), (long long)"1/2");
    ok = expect_equal((long long)"the up quark's mass-part = 1 - 2/3 = 1/3", fraction_to_text(up_mass_part()), (long long)"1/3");
    ok = expect_equal((long long)"the down quark's mass-part = the complement = 2/3", fraction_to_text(down_mass_part()), (long long)"2/3");
    ok = expect_bool((long long)"Route B: the electron's part is the fold preimage of the One", electron_is_preimage());
    ok = expect_bool((long long)"Route B: the quark pair is the period-2 orbit", quark_pair_is_orbit());
    ok = expect_bool((long long)"FORCED: the down outweighs the up (why hydrogen is stable)", down_outweighs_up());
    ok = expect_bool((long long)"the neutrino's part sits at the excluded boundary (unmakeable)", neutrino_part_is_excluded());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
