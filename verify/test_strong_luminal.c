...
long long _main();
long long strong_combination();
long long carrier_is_massless_here();
long long advance_one_tick(long long);
long long carrier_is_luminal_over(long long);
long long carrier_speed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the luminal strong carrier (gluons at c) ===");
    ok = expect_equal((long long)"the strong combination = the One", fraction_to_text(strong_combination()), (long long)"1");
    ok = expect_bool((long long)"the carrier is massless (shortfall at the excluded boundary)", carrier_is_massless_here());
    ok = expect_bool((long long)"walked 8 ticks: the phase returns EXACTLY every tick (no dispersion)", carrier_is_luminal_over(8));
    ok = expect_equal((long long)"the carrier speed = spacing/tick = the One = c", fraction_to_text(carrier_speed()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long strong_combination() {
    ...
    c = fraction_from_ratio(2, 3);
    n = fraction_from_ratio(1, 3);
...
long long advance_one_tick(long long phase) {
    ...
    step = fraction_from_whole_number(1);
    advanced = fraction_add(phase, step);
...
long long carrier_is_luminal_over(long long ticks) {
    long long exact_every_tick = 0;
    long long phase = 0;
    long long returns = 0;
    long long start = 0;
    long long t = 0;
    ...
    start = fraction_from_ratio(1, 5);
    phase = fraction_from_ratio(1, 5);
    exact_every_tick = 1LL;
    t = 0;
    while (t < ticks) {
    phase = advance_one_tick(phase);
    returns = fraction_compare(phase, start) == 0;
    if (returns) {
    exact_every_tick = exact_every_tick;
    } else {
    exact_every_tick = 0LL;
    }
    t = (t + 1);
    }
    ret_val = exact_every_tick;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(start);
    start = 0;
    return ret_val;
}
...
long long carrier_speed() {
    ...
    ep_gc_push_root(&tick);

    ep_gc_maybe_collect();

    spacing = fraction_from_whole_number(1);
    tick = fraction_from_whole_number(1);
    ret_val = fraction_divide(spacing, tick);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
