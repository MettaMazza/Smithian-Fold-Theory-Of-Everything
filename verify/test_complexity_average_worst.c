...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold average and worst cases ===");
    ok = expect_equal((long long)"depth-7 total halt work is 769", int_to_string(fold_total_halt_work(7)), int_to_string(769));
    ok = expect_equal((long long)"the exact depth-7 average is 769/128", fraction_to_text(fold_average_halt_time(7)), (long long)"769/128");
    ok = expect_true((long long)"enumerated and closed-form average routes agree", fold_average_routes_agree(7));
    ok = expect_equal((long long)"the depth-7 worst case is seven", int_to_string(fold_worst_case_halt_time(7)), int_to_string(7));
    ok = expect_true((long long)"the average does not exceed the worst case", fold_average_does_not_exceed_worst(7));
    ok = expect_true((long long)"the complete average/worst census holds", complete_fold_average_worst_census());
    ok = expect_true((long long)"a false average-equals-worst claim is rejected", false_average_equals_worst_is_rejected(7));
    ok = expect_true((long long)"a shorter worst case is rejected", false_shorter_worst_case_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
