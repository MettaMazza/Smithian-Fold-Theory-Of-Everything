...
long long coordination_number();
long long neighbour_weight();
long long neighbour_sum();
long long neighbour_sum_folds_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the cubic lattice ===");
    ok = expect_equal((long long)"coordination number = binary * spatial = 6", int_to_string(coordination_number()), int_to_string(6));
    ok = expect_equal((long long)"each neighbour weight = 1/12", fraction_to_text(neighbour_weight()), (long long)"1/12");
    ok = expect_equal((long long)"six neighbours sum to the balance point 1/2", fraction_to_text(neighbour_sum()), (long long)"1/2");
    ok = expect_equal((long long)"the balance folds up to the One", fraction_to_text(neighbour_sum_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long neighbour_weight() {
    ...
    denominator = (smallest_fold_period_above(1) * (smallest_fold_period_above(1) * spatial_dimensions()));
...
long long neighbour_sum() {
    ...
    count = fraction_from_whole_number(coordination_number());
    weight = neighbour_weight();
    ret_val = fraction_multiply(count, weight);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
