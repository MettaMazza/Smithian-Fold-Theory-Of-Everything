...
long long collapse_threshold();
long long threshold_is_self_antipodal();
long long collapse_completes_in_one_fold();
long long support_plus_claim();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== supernovae (the collapse at the balance) ===");
    ok = expect_equal((long long)"collapse threshold = 1/2 (support = gravity's claim)", fraction_to_text(collapse_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"the threshold is self-antipodal (no margin)", threshold_is_self_antipodal());
    ok = expect_equal((long long)"one fold completes the collapse (-> the One)", fraction_to_text(collapse_completes_in_one_fold()), (long long)"1");
    ok = expect_equal((long long)"support + claim sum to the One at the threshold", fraction_to_text(support_plus_claim()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long threshold_is_self_antipodal() {
    ...
    threshold = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, threshold);
    ret_val = fraction_compare(complement, threshold) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long collapse_completes_in_one_fold() {
    ...
    ret_val = fold_fraction(collapse_threshold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long support_plus_claim() {
    ...
    threshold = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(threshold, threshold);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
