...
long long neighbour_weight();
long long weights_sum();
long long next_centre();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the coupled lattice (presence spreads and is conserved) ===");
    ok = expect_equal((long long)"self-share weight = 1/2", fraction_to_text(self_weight()), (long long)"1/2");
    ok = expect_equal((long long)"neighbour-share weight = 1/4", fraction_to_text(neighbour_weight()), (long long)"1/4");
    ok = expect_equal((long long)"weights sum to One (presence conserved)", fraction_to_text(weights_sum()), (long long)"1");
    ok = expect_equal((long long)"symmetric bump centre relaxes to 3/8", fraction_to_text(next_centre()), (long long)"3/8");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long weights_sum() {
    long long self = 0;
    long long left_plus_right = 0;
    long long nb = 0;
    long long ret_val = 0;

    ep_gc_push_root(&self);
    ep_gc_push_root(&left_plus_right);
    ep_gc_push_root(&nb);

    ep_gc_maybe_collect();

    nb = fraction_from_ratio(1, (binary_count() * binary_count()));
    left_plus_right = fraction_add(nb, nb);
    self = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(self, left_plus_right);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long next_centre() {
    ...
    ep_gc_push_root(&centre);
    ep_gc_push_root(&from_self);
    ep_gc_push_root(&from_left);
    ep_gc_push_root(&sides);
    ep_gc_push_root(&from_right);
    ep_gc_push_root(&neighbour);

    ep_gc_maybe_collect();

    centre = fraction_from_ratio(1, binary_count());
    neighbour = fraction_from_ratio(1, (binary_count() * binary_count()));
    from_self = fraction_multiply(self_weight(), centre);
    from_left = fraction_multiply(neighbour_weight(), neighbour);
    from_right = fraction_multiply(neighbour_weight(), neighbour);
    sides = fraction_add(from_left, from_right);
    ret_val = fraction_add(from_self, sides);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
