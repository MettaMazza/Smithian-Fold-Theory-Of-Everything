...
long long _main() {
    long long forced_density = 0;
    long long forced_density_again = 0;
    long long ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&forced_density);
    ep_gc_push_root(&forced_density_again);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the dark relic: identified particle, counted abundance ===");
    ok = expect_equal((long long)"the generational volume: colour cubed", int_to_string(relic_volume()), (long long)"27");
    ok = expect_equal((long long)"the covering depth: minimal binary cover of 27", int_to_string(relic_depth()), (long long)"5");
    ok = expect_equal((long long)"the forced dark-to-baryon ratio 27/5", fraction_to_text(relic_to_baryon()), (long long)"27/5");
    ok = expect_equal((long long)"the forced dark fraction of matter 27/32", fraction_to_text(relic_fraction_of_matter()), (long long)"27/32");
    ok = expect_bool((long long)"THE RELIC IS IDENTIFIED: the lightest neutral new-sector baryon", relic_particle_identified());
    forced_density = relic_density_from_baryon(224, 10000);
    ok = expect_equal((long long)"forced Omega_dm h^2 = 27/5 x 0.0224 (exact)", fraction_to_text(forced_density), (long long)"378/3125");
    forced_density_again = relic_density_from_baryon(224, 10000);
    ok = expect_equal((long long)"forced Omega_dm h^2 = 0.12096 (measured ~0.120)", fraction_to_decimal(forced_density_again, 5), (long long)"0.12096");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(forced_density);
    forced_density = 0;
    free_struct_Fraction(forced_density_again);
    forced_density_again = 0;
    return ret_val;
}
