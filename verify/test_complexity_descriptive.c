...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold descriptive complexity ===");
    ok = expect_equal((long long)"length seven has capacity 128", int_to_string(fold_description_capacity(7)), int_to_string(128));
    ok = expect_equal((long long)"the minimal depth-seven description length is seven", int_to_string(fold_minimal_description_length(7)), int_to_string(7));
    ok = expect_true((long long)"every state has an exact minimal description", every_fold_state_has_minimal_description(7));
    ok = expect_true((long long)"a length-six description space is insufficient", shorter_fold_description_is_insufficient(7));
    ok = expect_equal((long long)"three closed distinctions need three conditional labels", int_to_string(conditional_fold_description_length(3)), int_to_string(3));
    ok = expect_true((long long)"the complete descriptive census holds", complete_fold_descriptive_census());
    ok = expect_true((long long)"a redundant longer description is not minimal", redundant_longer_description_is_not_minimal(7));
    ok = expect_true((long long)"an outside description symbol is rejected", outside_description_symbol_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
