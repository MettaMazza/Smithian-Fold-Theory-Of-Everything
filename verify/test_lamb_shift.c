...
long long lamb_shift_state();
long long lamb_shift_first_fold();
long long lamb_shift_reaches_unison();
long long lamb_shift_state_doubled();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Lamb shift (alpha^2 order, two folds deep) ===");
    ok = expect_equal((long long)"Lamb shift state = 1/4 = (1/2)^2 (alpha^2 order)", fraction_to_text(lamb_shift_state()), (long long)"1/4");
    ok = expect_equal((long long)"one fold: 1/4 -> 1/2 (the fine-structure level)", fraction_to_text(lamb_shift_first_fold()), (long long)"1/2");
    ok = expect_equal((long long)"second fold: 1/2 -> 1 (unison); two folds total", fraction_to_text(lamb_shift_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"state + state = 1/2 (level one fold up)", fraction_to_text(lamb_shift_state_doubled()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long lamb_shift_first_fold() {
    ...
    ret_val = fold_fraction(lamb_shift_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long lamb_shift_reaches_unison() {
    ...
    ret_val = fold_fraction(lamb_shift_first_fold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
