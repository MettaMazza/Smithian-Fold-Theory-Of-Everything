...
long long _main() {
    long long a_half = 0;
    long long a_third = 0;
    long long neg_third = 0;
    long long ok = 0;
    long long product = 0;
    long long six_quarters = 0;
    long long sum = 0;
    long long three_quarters = 0;
    long long two_thirds = 0;
    long long ret_val = 0;

    ep_gc_push_root(&a_half);
    ep_gc_push_root(&a_third);
    ep_gc_push_root(&neg_third);
    ep_gc_push_root(&product);
    ep_gc_push_root(&six_quarters);
    ep_gc_push_root(&sum);
    ...
    printf("%s\n", (char*)(long long)"=== exact fractions ===");
    six_quarters = fraction_from_ratio(6, 4);
    ok = expect_equal((long long)"6/4 reduces", fraction_to_text(six_quarters), (long long)"3/2");
    neg_third = fraction_from_ratio(3, -9);
    ok = expect_equal((long long)"3/-9 normalises", fraction_to_text(neg_third), (long long)"-1/3");
    a_half = fraction_from_ratio(1, 2);
    a_third = fraction_from_ratio(1, 3);
    sum = fraction_add(a_half, a_third);
    ok = expect_equal((long long)"1/2 + 1/3", fraction_to_text(sum), (long long)"5/6");
    two_thirds = fraction_from_ratio(2, 3);
    three_quarters = fraction_from_ratio(3, 4);
    product = fraction_multiply(two_thirds, three_quarters);
    ok = expect_equal((long long)"2/3 * 3/4", fraction_to_text(product), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(neg_third);
    neg_third = 0;
    free_struct_Fraction(product);
    product = 0;
    free_struct_Fraction(six_quarters);
    six_quarters = 0;
    free_struct_Fraction(sum);
    sum = 0;
    return ret_val;
}
