...
long long _main();
long long tower_step_factor();
long long scale_ratio_at_depth(long long);
long long electroweak_planck_depth();
long long electroweak_planck_ratio();
long long depth_splits_into_rungs();
long long rungs_are_factor_two_apart();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the hierarchy problem (discrete rungs -> no fine-tuning) ===");
    ok = expect_equal((long long)"each tower step is a factor of binary = 2", int_to_string(tower_step_factor()), (long long)"2");
    ok = expect_equal((long long)"electroweak/Planck rung N = 56 (comparison-side)", int_to_string(electroweak_planck_depth()), (long long)"56");
    ok = expect_bool((long long)"2^56 = 2^20 * 2^20 * 2^16 (tower self-consistency)", depth_splits_into_rungs());
    ok = expect_bool((long long)"adjacent rungs are exactly a factor of 2 apart (discrete)", rungs_are_factor_two_apart());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long scale_ratio_at_depth(long long depth) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(1, whole_power(binary_count(), depth));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long electroweak_planck_ratio() {
    ...
    ret_val = fraction_from_ratio(1, whole_power(binary_count(), electroweak_planck_depth()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long depth_splits_into_rungs() {
    ...
    whole = whole_power(binary_count(), electroweak_planck_depth());
    part_a = whole_power(binary_count(), 20);
    part_b = whole_power(binary_count(), 16);
    product = ((part_a * part_a) * part_b);
    ret_val = whole == product;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long rungs_are_factor_two_apart() {
    ...
    ep_gc_push_root(&up_one);
    ep_gc_push_root(&here);
    ep_gc_push_root(&doubled);

    ep_gc_maybe_collect();

    here = fraction_from_ratio(1, whole_power(binary_count(), electroweak_planck_depth()));
    up_one = fraction_from_ratio(1, whole_power(binary_count(), (electroweak_planck_depth() - 1)));
    factor = fraction_from_whole_number(binary_count());
    doubled = fraction_multiply(here, factor);
    ret_val = fraction_compare(doubled, up_one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(up_one);
    up_one = 0;
