...
long long open_shell_state();
long long shell_closes();
long long closure_is_exact();
long long half_shares_sum_to_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the periodic law (why chemistry repeats) ===");
    ok = expect_equal((long long)"a part-filled outer shell = the half-open 1/2", fraction_to_text(open_shell_state()), (long long)"1/2");
    ok = expect_equal((long long)"a closed shell = the fold to unison (a noble gas)", fraction_to_text(shell_closes()), (long long)"1");
    ok = expect_bool((long long)"closure is EXACT, so the recurrence is exact", closure_is_exact());
    ok = expect_equal((long long)"the closing shell's halves sum to the whole", fraction_to_text(half_shares_sum_to_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long shell_closes() {
    ...
    ret_val = fold_fraction(open_shell_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long closure_is_exact() {
    ...
    closed = fold_fraction(open_shell_state());
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(closed, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
