...
long long matter_residue();
long long residue_is_strictly_positive();
long long residue_below_unison();
long long residue_is_self_antipodal();
long long residue_folds_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the baryon asymmetry (why there is matter) ===");
    ok = expect_equal((long long)"the matter residue = 1/2 (smallest positive whole-balance)", fraction_to_text(matter_residue()), (long long)"1/2");
    ok = expect_bool((long long)"the residue is strictly positive (nonzero -- zero forbidden)", residue_is_strictly_positive());
    ok = expect_bool((long long)"the residue is strictly below unison (not everything survived)", residue_below_unison());
    ok = expect_bool((long long)"the residue is self-antipodal (matter vs mirror deficit)", residue_is_self_antipodal());
    ok = expect_equal((long long)"the residue folds to the One (grows into the universe)", fraction_to_text(residue_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long residue_is_strictly_positive() {
    ...
    residue = fraction_from_ratio(1, binary_count());
    ret_val = ({ long long _fap = residue; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long residue_below_unison() {
    ...
    comparison = fraction_compare(residue, one);
...
long long residue_is_self_antipodal() {
    ...
    residue = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, residue);
    ret_val = fraction_compare(complement, residue) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long residue_folds_to_one() {
    ...
    ret_val = fold_fraction(matter_residue());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
