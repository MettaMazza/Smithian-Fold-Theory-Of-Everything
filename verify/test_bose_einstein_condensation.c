...
long long condensate_ground();
long long condensate_ground_folds_to_one();
long long fermion_occupation_ceiling();
long long even_boson_turn_is_identity();
long long boson_occupation_is_uncapped();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Bose-Einstein condensation (bosons pile into one state) ===");
    ok = expect_equal((long long)"shared condensate ground = 1/2", fraction_to_text(condensate_ground()), (long long)"1/2");
    ok = expect_equal((long long)"the ground folds to the One (all share one orbit)", fraction_to_text(condensate_ground_folds_to_one()), (long long)"1");
    ok = expect_equal((long long)"fermion occupation ceiling = 1 (Pauli)", int_to_string(fermion_occupation_ceiling()), (long long)"1");
    ok = expect_bool((long long)"boson occupation is uncapped (even turn is the identity)", boson_occupation_is_uncapped());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long condensate_ground_folds_to_one() {
    ...
    ret_val = fold_fraction(condensate_ground());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long even_boson_turn_is_identity() {
    long long two_half_turns = 0;
    long long ret_val = 0;

    ep_gc_push_root(&two_half_turns);

    ep_gc_maybe_collect();

    two_half_turns = fraction_from_ratio(binary_count(), binary_count());
    ret_val = cast_out_whole_ones(two_half_turns);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long boson_occupation_is_uncapped() {
    ...
    ret_val = fraction_compare(even_boson_turn_is_identity(), one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
