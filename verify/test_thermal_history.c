...
long long _main();
long long radiation_epoch_exponent();
long long matter_epoch_exponent();
long long dark_energy_epoch_exponent();
long long radiation_before_matter();
long long matter_before_dark_energy();
long long timeline_is_ordered();
...
long long matter_dilution_exponent();
long long radiation_dilution_exponent();
long long vacuum_dilution_exponent();
long long radiation_thins_faster_than_matter();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the thermal history ===");
    ok = expect_number((long long)"radiation epoch exponent = 4", radiation_epoch_exponent(), 4);
    ok = expect_number((long long)"matter epoch exponent = 3", matter_epoch_exponent(), 3);
    ok = expect_number((long long)"dark-energy epoch exponent = 0", dark_energy_epoch_exponent(), 0);
    ok = expect_bool((long long)"radiation dominated BEFORE matter (4 > 3)", radiation_before_matter());
    ok = expect_bool((long long)"matter dominated BEFORE dark energy (3 > 0)", matter_before_dark_energy());
    ok = expect_bool((long long)"the timeline is ordered radiation -> matter -> dark energy", timeline_is_ordered());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long radiation_epoch_exponent() {
    ...
    ret_val = radiation_dilution_exponent();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long dark_energy_epoch_exponent() {
    ...
    ret_val = (spatial_dimensions() - spatial_dimensions());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long radiation_before_matter() {
    long long radiation = 0;
    long long matter = 0;
    long long ret_val = 0;

    radiation = radiation_dilution_exponent();
    matter = spatial_dimensions();
    if (radiation > matter) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long matter_before_dark_energy() {
    ...
    matter = spatial_dimensions();
    dark_energy = (spatial_dimensions() - spatial_dimensions());
...
long long timeline_is_ordered() {
    ...
    first_gap = radiation_before_matter();
    second_gap = matter_before_dark_energy();
    if (first_gap) {
    if (second_gap) {
    ret_val = 1LL;
    goto L_cleanup;
...
long long radiation_dilution_exponent() {
    long long by_spacetime = 0;
    long long by_volume_plus_redshift = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_spacetime);
    ep_gc_push_root(&by_volume_plus_redshift);

    ep_gc_maybe_collect();

    by_volume_plus_redshift = (spatial_dimensions() + 1);
    by_spacetime = spacetime_dimensions();
    ret_val = forced_to_be((long long)"radiation exponent = d_space + 1 = spacetime dimensions", by_volume_plus_redshift, by_spacetime);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
