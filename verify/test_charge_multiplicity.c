...
long long _main();
long long fibre_size(long long);
long long charge_states();
long long colour_states();
long long fibre_size_is_multiplicity(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== charge multiplicity (internal states = the fold's fibre size) ===");
    ok = expect_equal((long long)"binary fold fibre = 2 states (charge sign, weak doublet)", int_to_string(charge_states()), (long long)"2");
    ok = expect_equal((long long)"colour fold fibre = 3 states (the three colours)", int_to_string(colour_states()), (long long)"3");
    ok = expect_bool((long long)"the binary fibre size is the binary multiplicity", fibre_size_is_multiplicity(binary_count()));
    ok = expect_bool((long long)"the colour fibre size is the colour multiplicity", fibre_size_is_multiplicity(colour_count()));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fibre_size(long long m) {
    long long shifted = 0;
    long long folded = 0;
    long long count = 0;
    long long k = 0;
    ...
    target = fraction_from_ratio(1, binary_count());
    count = 0;
    k = 0;
    while (k < m) {
    shifted = fraction_add(target, fraction_from_whole_number(k));
    preimage = fraction_divide(shifted, fraction_from_whole_number(m));
    m_fraction = fraction_from_whole_number(m);
    scaled = fraction_multiply(preimage, m_fraction);
    folded = cast_out_whole_ones(scaled);
    if (fraction_compare(folded, target) == 0) {
    count = (count + 1);
    }
    k = (k + 1);
    }
...
long long charge_states() {
    ...
    ret_val = fibre_size(binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long colour_states() {
    ...
    ret_val = fibre_size(colour_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long fibre_size_is_multiplicity(long long m) {
    long long ret_val = 0;
