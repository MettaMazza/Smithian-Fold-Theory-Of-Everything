...
long long primary_coupling();
long long nuclear_residual();
long long residual_folds_to_primary();
long long residual_reaches_unison();
long long two_residuals_make_primary();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the nuclear force (a short-range residual) ===");
    ok = expect_equal((long long)"primary strong coupling = 1/2", fraction_to_text(primary_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"nuclear residual = 1/4 (one fold deeper)", fraction_to_text(nuclear_residual()), (long long)"1/4");
    ok = expect_equal((long long)"residual folds up to the primary (1/4 -> 1/2)", fraction_to_text(residual_folds_to_primary()), (long long)"1/2");
    ok = expect_equal((long long)"second fold reaches unison (1/2 -> 1)", fraction_to_text(residual_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"two residuals make one primary's worth", fraction_to_text(two_residuals_make_primary()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long residual_folds_to_primary() {
    ...
    ret_val = fold_fraction(nuclear_residual());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long residual_reaches_unison() {
    ...
    ret_val = fold_fraction(residual_folds_to_primary());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_residuals_make_primary() {
    ...
    residual = nuclear_residual();
    ret_val = fraction_add(residual, residual);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
