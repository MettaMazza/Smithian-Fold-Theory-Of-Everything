...
long long _main();
long long flux_tube_width();
long long width_is_preimage_of_one();
long long regge_mass_squared(long long, long long, long long, long long, long long);
long long spacing_is_equal(long long, long long, long long, long long);
long long level_multiplicity(long long);
long long multiplicity_doubles_to_ceiling();
...
long long _main() {
    long long first_level = 0;
    long long ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_level);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the excited-state spectrum: linear Regge from the flux tube ===");
    ok = expect_equal((long long)"the flux-tube width: the half-One", fraction_to_text(flux_tube_width()), (long long)"1/2");
    ok = expect_bool((long long)"the width is the fold preimage of the One (doubled = unison)", width_is_preimage_of_one());
    ok = expect_bool((long long)"THE FORCED SHAPE: equal M^2 spacing, J = 0..5 (rho anchors)", spacing_is_equal(59, 100, 110, 100));
    ok = expect_bool((long long)"the shape holds at a DIFFERENT anchor pair (anchor-independent)", spacing_is_equal(7, 3, 5, 4));
    first_level = regge_mass_squared(59, 100, 110, 100, 1);
    ok = expect_equal((long long)"the J=1 rung: 0.59 + 1.10 = 1.69 GeV^2", fraction_to_text(first_level), (long long)"169/100");
    ok = expect_equal((long long)"tower multiplicity at level 3: 2^3 states", int_to_string(level_multiplicity(3)), (long long)"8");
    ok = expect_bool((long long)"the multiplicity doubles level by level to the ceiling 7", multiplicity_doubles_to_ceiling());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(first_level);
    first_level = 0;
    return ret_val;
}
...
long long width_is_preimage_of_one() {
    ...
    long long one = 0;
    long long width = 0;
    long long width_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&doubled);
    ep_gc_push_root(&one);
    ep_gc_push_root(&width);
    ep_gc_push_root(&width_twin);

    ep_gc_maybe_collect();

    width = fraction_from_ratio(1, 2);
    width_twin = fraction_from_ratio(1, 2);
    doubled = fraction_add(width, width_twin);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(doubled, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long regge_mass_squared(long long m0_top, long long m0_bottom, long long sigma_top, long long sigma_bottom, long long spin) {
    long long climb = 0;
    long long ground = 0;
    long long slope = 0;
    long long spin_fraction = 0;
    long long ret_val = 0;

    ep_gc_push_root(&climb);
    ep_gc_push_root(&ground);
    ep_gc_push_root(&slope);
    ep_gc_push_root(&spin_fraction);
    ep_gc_push_root(&m0_top);
    ep_gc_push_root(&m0_bottom);
    ep_gc_push_root(&sigma_top);
    ep_gc_push_root(&sigma_bottom);
    ep_gc_push_root(&spin);

    ep_gc_maybe_collect();

    ground = fraction_from_ratio(m0_top, m0_bottom);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    spin_fraction = fraction_from_whole_number(spin);
    climb = fraction_multiply(slope, spin_fraction);
    ret_val = fraction_add(ground, climb);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    return ret_val;
...
long long spacing_is_equal(long long m0_top, long long m0_bottom, long long sigma_top, long long sigma_bottom) {
    long long all_equal = 0;
    long long here = 0;
    long long j = 0;
    long long matches = 0;
    long long next_level = 0;
    long long slope = 0;
    long long step = 0;
    long long ret_val = 0;

    ep_gc_push_root(&here);
    ep_gc_push_root(&next_level);
    ep_gc_push_root(&slope);
    ...
    all_equal = 1LL;
    here = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 0);
    next_level = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 1);
    step = fraction_subtract(next_level, here);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    matches = fraction_compare(step, slope) == 0;
    if (matches) {
    all_equal = all_equal;
    } else {
    all_equal = 0LL;
    }
    here = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 1);
    next_level = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 2);
    step = fraction_subtract(next_level, here);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    matches = fraction_compare(step, slope) == 0;
    if (matches) {
    ...
    here = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 2);
    next_level = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 3);
    step = fraction_subtract(next_level, here);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    matches = fraction_compare(step, slope) == 0;
    if (matches) {
    ...
    here = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 3);
    next_level = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 4);
    step = fraction_subtract(next_level, here);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    matches = fraction_compare(step, slope) == 0;
    if (matches) {
    ...
    here = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 4);
    next_level = regge_mass_squared(m0_top, m0_bottom, sigma_top, sigma_bottom, 5);
    step = fraction_subtract(next_level, here);
    slope = fraction_from_ratio(sigma_top, sigma_bottom);
    matches = fraction_compare(step, slope) == 0;
    if (matches) {
...
long long level_multiplicity(long long depth) {
    long long ret_val = 0;
