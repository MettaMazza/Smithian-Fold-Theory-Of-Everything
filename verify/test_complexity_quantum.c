...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold quantum complexity ===");
    ok = expect_equal((long long)"depth seven has 128 unresolved quantum branches", int_to_string(fold_quantum_support_count(7)), int_to_string(128));
    ok = expect_equal((long long)"each depth-seven branch has share 1/128", fraction_to_text(fold_quantum_branch_share(7)), (long long)"1/128");
    ok = expect_true((long long)"the complete branch support partitions the One", fold_quantum_branches_partition_one(7));
    ok = expect_equal((long long)"composed binary and colour depths have support 32", int_to_string(fold_quantum_composed_support(2, 3)), int_to_string(32));
    ok = expect_equal((long long)"three observations retain sixteen branches", int_to_string(fold_quantum_support_after_observation(7, 3)), int_to_string(16));
    ok = expect_equal((long long)"quantum transformation depth is seven", int_to_string(fold_quantum_transformation_depth(7)), int_to_string(7));
    ok = expect_true((long long)"the complete quantum-complexity census holds", complete_fold_quantum_complexity_census());
    ok = expect_true((long long)"collapsed support is rejected", collapsed_quantum_support_is_rejected(7));
    ok = expect_true((long long)"shortened quantum depth is rejected", shortened_quantum_depth_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
