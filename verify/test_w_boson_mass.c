...
long long _main() {
    long long measured_mass_squared = 0;
    long long ratio_twin = 0;
    long long ratio = 0;
    long long ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&measured_mass_squared);
    ep_gc_push_root(&ratio_twin);
    ep_gc_push_root(&ratio);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the W-to-Z boson mass ratio ===");
    ok = expect_equal((long long)"(M_W/M_Z)^2 = cos^2(theta_W) = 3/4", fraction_to_text(w_to_z_mass_squared()), (long long)"3/4");
    ok = expect_equal((long long)"bare (M_W/M_Z)^2 = 0.7500", fraction_to_decimal(w_to_z_mass_squared(), 4), (long long)"0.7500");
    printf("%s\n", (char*)(long long)"--- the forced running carries cos^2 to measurement ---");
    ok = expect_equal((long long)"cos^2 at level 9  = 400/521 = 0.7677", fraction_to_text(cos_squared_at_level(9)), (long long)"400/521");
    ok = expect_equal((long long)"cos^2 at level 13 = 784/1009 = 0.7770", fraction_to_text(cos_squared_at_level(13)), (long long)"784/1009");
    ok = expect_equal((long long)"cos^2 at level 13 decimal = 0.7770", fraction_to_decimal(cos_squared_at_level(13), 4), (long long)"0.7770");
    printf("%s\n", (char*)(long long)"--- comparison: measured (M_W/M_Z)^2 from the boson masses ---");
    ratio = fraction_from_ratio(803770, 911876);
    ratio_twin = fraction_from_ratio(803770, 911876);
    measured_mass_squared = fraction_multiply(ratio, ratio_twin);
    ok = expect_equal((long long)"measured (M_W/M_Z)^2 = 0.7769", fraction_to_decimal(measured_mass_squared, 4), (long long)"0.7769");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(measured_mass_squared);
    measured_mass_squared = 0;
    return ret_val;
}
