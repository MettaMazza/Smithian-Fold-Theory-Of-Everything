...
long long coupling_threshold();
long long threshold_is_fold_preimage();
long long growth_multiplier_is_one();
long long difference_is_marginal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the synchronization threshold (coupled oscillators lock) ===");
    ok = expect_equal((long long)"the coupling threshold g_c = 1/2", fraction_to_text(coupling_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"g_c is the fold's preimage of the One: fold(1/2) = 1", threshold_is_fold_preimage());
    ok = expect_bool((long long)"the difference multiplier 2*(1 - g_c) = 1 (marginal)", growth_multiplier_is_one());
    ok = expect_bool((long long)"at g_c the gap is preserved exactly (1/100 -> 1/100): locking", difference_is_marginal());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long threshold_is_fold_preimage() {
    ...
    g_c = fraction_from_ratio(1, 2);
    folded = fold_fraction(g_c);
...
long long growth_multiplier_is_one() {
    ...
    g_c = fraction_from_ratio(1, 2);
    one = fraction_from_whole_number(1);
    uncoupled = fraction_subtract(one, g_c);
    binary = fraction_from_whole_number(binary_count());
    multiplier = fraction_multiply(binary, uncoupled);
    ret_val = fraction_compare(multiplier, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
...
long long difference_is_marginal() {
    ...
    long long from_y = 0;
    long long g_c = 0;
    long long gap_after = 0;
    long long gap_before = 0;
    long long one = 0;
    long long uncoupled = 0;
    long long x = 0;
    long long y = 0;
    ...
    ep_gc_push_root(&from_x);
    ep_gc_push_root(&from_y);
    ep_gc_push_root(&g_c);
    ep_gc_push_root(&gap_after);
    ep_gc_push_root(&gap_before);
    ep_gc_push_root(&one);
    ep_gc_push_root(&uncoupled);
    ep_gc_push_root(&x);
    ep_gc_push_root(&y);
    ep_gc_push_root(&y_next);

    ep_gc_maybe_collect();

    x = fraction_from_ratio(1, 5);
    y = fraction_from_ratio(21, 100);
    gap_before = fraction_subtract(y, x);
    fold_x = fold_fraction(x);
    fold_y = fold_fraction(y);
    g_c = fraction_from_ratio(1, 2);
    one = fraction_from_whole_number(1);
    uncoupled = fraction_subtract(one, g_c);
    from_y = fraction_multiply(uncoupled, fold_y);
    from_x = fraction_multiply(g_c, fold_x);
    y_next = fraction_add(from_y, from_x);
    gap_after = fraction_subtract(y_next, fold_x);
    ret_val = fraction_compare(gap_after, gap_before) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(12);
    free_struct_Fraction(gap_after);
    gap_after = 0;
    free_struct_Fraction(gap_before);
    gap_before = 0;
    return ret_val;
}
