...
long long _main();
long long sweep_bound();
long long sweep_contains(long long, long long);
long long census_by_enumeration();
long long census_by_phi_sum();
long long census_routes_agree();
long long fold_member_lands_inside(long long, long long, long long);
long long closure_is_closed();
long long sweep_partition_holds_to(long long);
long long emergence_second_invariant(long long);
long long emergence_third_invariant(long long);
long long family_lands_lepton_sector();
long long family_predicts_new_sectors();
long long member_equals(long long, long long, long long);
long long sweep_lands_couplings();
long long sweep_lands_mixing();
long long sweep_lands_dark_sector();
long long sweep_lands_cosmic_budget();
long long sweep_composes_reactor_angle();
long long sweep_reaches_the_floor();
long long discovery_sweep_completes();
...
long long relic_volume();
long long relic_depth();
long long relic_to_baryon();
long long relic_fraction_of_matter();
long long relic_particle_identified();
long long relic_density_from_baryon(long long, long long);
long long fibre_member(long long, long long);
long long fibre_folds_to_one(long long);
long long baryon_multiplicity(long long);
long long baryon_charge_sum(long long);
long long baryon_is_neutral_at(long long);
long long meson_multiplicity(long long);
long long standard_model_anchor();
long long dark_hadrons_forced();
long long even_sector_excluded();
...
long long _main() {
    ...
    long long measured_koide = 0;
    long long ok = 0;
    long long swept_koide = 0;
    long long ret_val = 0;

    ep_gc_push_root(&deviation);
    ep_gc_push_root(&measured_koide);
    ep_gc_push_root(&swept_koide);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the discovery engine: the autonomous sweep, exact and deterministic ===");
    ok = expect_equal((long long)"the sweep bound: the tower at the depth ceiling", int_to_string(sweep_bound()), (long long)"128");
    ok = expect_equal((long long)"THE CENSUS: 5,022 members by direct enumeration", int_to_string(census_by_enumeration()), (long long)"5022");
    ok = expect_bool((long long)"the census agrees by two independent routes (enumeration = 1 + phi-sum)", census_routes_agree());
    ok = expect_bool((long long)"CLOSURE PROVEN: every member folded by every sector prime stays inside", closure_is_closed());
    ok = expect_bool((long long)"THE PARTITION: every odd class tiles into phi/ord eternal modes (to 31)", sweep_partition_holds_to(31));
    ok = expect_bool((long long)"THE FAMILY at m=3 IS the lepton sector: e3 = 1/485, Koide = 2/3 (identities)", family_lands_lepton_sector());
    ok = expect_equal((long long)"the family's swept e3 at m=3", fraction_to_text(emergence_third_invariant(3)), (long long)"1/485");
    ok = expect_bool((long long)"the family PREDICTS the new sectors: e3 = 1/6249 (penta), 1/33613 (hepta)", family_predicts_new_sectors());
    ok = expect_bool((long long)"DISCOVERED: all four sector couplings 1/2, 2/3, 4/5, 6/7 (identities)", sweep_lands_couplings());
    ok = expect_bool((long long)"DISCOVERED: the mixing partition 1/4 and 3/4 (identities)", sweep_lands_mixing());
    ok = expect_bool((long long)"DISCOVERED: the dark fraction 27/32 and baryon share 5/32", sweep_lands_dark_sector());
    ok = expect_bool((long long)"DISCOVERED: the cosmic budget 2/3, 1/3, 5/96, 9/32 (identities)", sweep_lands_cosmic_budget());
    ok = expect_bool((long long)"COMPOSED: the reactor angle 1/48 from the sweep's own members", sweep_composes_reactor_angle());
    ok = expect_bool((long long)"the sweep reaches the floor 1/128 at its bound", sweep_reaches_the_floor());
    swept_koide = fraction_from_ratio(2, 3);
    measured_koide = fraction_from_ratio(666661, 1000000);
    deviation = fraction_subtract(swept_koide, measured_koide);
    ok = expect_equal((long long)"the swept Koide vs measured: exactly 17/3000000 (8.5 per million)", fraction_to_text(deviation), (long long)"17/3000000");
    ok = expect_bool((long long)"THE AUTONOMOUS RUN: one call, the whole sweep, every discovery lands", discovery_sweep_completes());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(deviation);
    deviation = 0;
    return ret_val;
}
...
long long sweep_bound() {
    ...
    ret_val = whole_power(2, 7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long sweep_contains(long long p, long long q) {
    long long shared = 0;
    long long ret_val = 0;
...
long long census_routes_agree() {
    ...
    direct = census_by_enumeration();
    by_phi = census_by_phi_sum();
    ret_val = direct == by_phi;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long fold_member_lands_inside(long long p, long long q, long long factor) {
    long long landed_num = 0;
    long long lifted = 0;
    ...
    lifted = (p * factor);
    wraps = ((lifted - 1) / q);
    landed_num = (lifted - (wraps * q));
    shared = common_divisor(landed_num, q);
    reduced_den = (q / shared);
    ret_val = reduced_den < (whole_power(2, 7) + 1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long closure_is_closed() {
    long long all_inside = 0;
    long long by_five = 0;
    long long by_seven = 0;
    long long by_three = 0;
    long long by_two = 0;
    long long p = 0;
    long long q = 0;
    long long shared = 0;
    ...
    shared = common_divisor(p, q);
    if (shared == 1) {
    by_two = fold_member_lands_inside(p, q, 2);
    by_three = fold_member_lands_inside(p, q, 3);
    by_five = fold_member_lands_inside(p, q, 5);
    by_seven = fold_member_lands_inside(p, q, 7);
    if (by_two) {
...
long long sweep_partition_holds_to(long long bound) {
    long long ret_val = 0;

    ep_gc_push_root(&bound);

    ep_gc_maybe_collect();

    ret_val = orbit_count_law_to(bound);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long emergence_second_invariant(long long m) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(1, (2 * m));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long emergence_third_invariant(long long m) {
    long long body = 0;
    long long ret_val = 0;
    ...
    body = whole_power(m, 5);
    ret_val = fraction_from_ratio(1, ((2 * body) - 1));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long family_lands_lepton_sector() {
    long long e3_ok = 0;
    long long forced_e3 = 0;
    long long forced_koide = 0;
    long long koide_ok = 0;
    long long one = 0;
    long long swept_e2 = 0;
    long long swept_e2_twin = 0;
    long long swept_e3 = 0;
    long long swept_koide = 0;
    long long two_e2 = 0;
    long long ret_val = 0;

    ep_gc_push_root(&forced_e3);
    ep_gc_push_root(&forced_koide);
    ep_gc_push_root(&one);
    ep_gc_push_root(&swept_e2);
    ep_gc_push_root(&swept_e2_twin);
    ep_gc_push_root(&swept_e3);
    ep_gc_push_root(&swept_koide);
    ep_gc_push_root(&two_e2);

    ep_gc_maybe_collect();

    swept_e3 = emergence_third_invariant(3);
    forced_e3 = leading_third_invariant();
    e3_ok = fraction_compare(swept_e3, forced_e3) == 0;
    one = fraction_from_whole_number(1);
    swept_e2 = fraction_from_ratio(1, 6);
    swept_e2_twin = fraction_from_ratio(1, 6);
    two_e2 = fraction_add(swept_e2, swept_e2_twin);
    swept_koide = fraction_subtract(one, two_e2);
    forced_koide = koide_ratio();
    koide_ok = fraction_compare(swept_koide, forced_koide) == 0;
    if (e3_ok) {
    ret_val = koide_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(forced_e3);
    forced_e3 = 0;
    free_struct_Fraction(forced_koide);
    forced_koide = 0;
    free_struct_Fraction(swept_e3);
    swept_e3 = 0;
    free_struct_Fraction(swept_koide);
    swept_koide = 0;
    return ret_val;
}
...
long long family_predicts_new_sectors() {
    long long hepta = 0;
    long long hepta_expected = 0;
    long long hepta_ok = 0;
    long long penta = 0;
    long long penta_expected = 0;
    long long penta_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&hepta);
    ep_gc_push_root(&hepta_expected);
    ep_gc_push_root(&penta);
    ep_gc_push_root(&penta_expected);

    ep_gc_maybe_collect();

    penta = emergence_third_invariant(5);
    penta_expected = fraction_from_ratio(1, 6249);
    penta_ok = fraction_compare(penta, penta_expected) == 0;
    hepta = emergence_third_invariant(7);
    hepta_expected = fraction_from_ratio(1, 33613);
    hepta_ok = fraction_compare(hepta, hepta_expected) == 0;
    if (penta_ok) {
    ret_val = hepta_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(hepta);
    hepta = 0;
    free_struct_Fraction(hepta_expected);
    hepta_expected = 0;
    free_struct_Fraction(penta);
    penta = 0;
    free_struct_Fraction(penta_expected);
    penta_expected = 0;
    return ret_val;
}

long long member_equals(long long p, long long q, long long forced) {
    ...
    inside = sweep_contains(p, q);
    candidate = fraction_from_ratio(p, q);
    equal = fraction_compare(candidate, forced) == 0;
    if (inside) {
    ret_val = equal;
    goto L_cleanup;
    }
...
long long sweep_lands_couplings() {
    ...
    strong = member_equals(2, 3, sector_coupling(3));
    balance = member_equals(1, 2, sector_coupling(2));
    penta = member_equals(4, 5, sector_coupling(5));
    hepta = member_equals(6, 7, sector_coupling(7));
    if (strong) {
    if (balance) {
    if (penta) {
    ret_val = hepta;
...
long long sweep_lands_mixing() {
    long long cos_part = 0;
    long long sin_part = 0;
    long long ret_val = 0;

    sin_part = member_equals(1, 4, sin_squared_mixing());
    cos_part = member_equals(3, 4, cos_squared_mixing());
    if (sin_part) {
    ret_val = cos_part;
    goto L_cleanup;
    }
...
long long sweep_lands_dark_sector() {
    long long baryon_side = 0;
    long long dark = 0;
    long long ret_val = 0;

    dark = member_equals(27, 32, relic_fraction_of_matter());
    baryon_side = sweep_contains(5, 32);
    if (dark) {
    ret_val = baryon_side;
    goto L_cleanup;
    }
...
long long sweep_lands_cosmic_budget() {
    ...
    vacuum = member_equals(2, 3, dark_energy_density());
    matter = member_equals(1, 3, matter_density());
    baryon = member_equals(5, 96, baryon_density());
    cold = member_equals(9, 32, cold_dark_matter_density());
    if (vacuum) {
    if (matter) {
    if (baryon) {
    ret_val = cold;
...
long long sweep_composes_reactor_angle() {
    ...
    ep_gc_push_root(&reactor);
    ep_gc_push_root(&solar);
    ep_gc_push_root(&tower);

    ep_gc_maybe_collect();

    atmospheric = fraction_from_ratio(1, 2);
    solar = fraction_from_ratio(1, 3);
    product = fraction_multiply(atmospheric, solar);
    tower = fraction_from_whole_number(8);
    reactor = fraction_divide(product, tower);
    expected = fraction_from_ratio(1, 48);
    equal = fraction_compare(reactor, expected) == 0;
    inside = sweep_contains(1, 48);
    if (equal) {
    ret_val = inside;
    goto L_cleanup;
    }
...
long long discovery_sweep_completes() {
    long long budget_ok = 0;
    long long census_ok = 0;
    long long closed = 0;
    long long couplings_ok = 0;
    long long dark_ok = 0;
    long long family_ok = 0;
    long long floor_ok = 0;
    long long mixing_ok = 0;
    long long partition_ok = 0;
    long long predictions_ok = 0;
    long long reactor_ok = 0;
    long long ret_val = 0;

    census_ok = census_routes_agree();
    closed = closure_is_closed();
    partition_ok = orbit_count_law_to(31);
    family_ok = family_lands_lepton_sector();
    predictions_ok = family_predicts_new_sectors();
    couplings_ok = sweep_lands_couplings();
    mixing_ok = sweep_lands_mixing();
    dark_ok = sweep_lands_dark_sector();
    budget_ok = sweep_lands_cosmic_budget();
    reactor_ok = sweep_composes_reactor_angle();
    floor_ok = sweep_contains(1, 128);
    if (census_ok) {
    if (closed) {
    if (partition_ok) {
    if (family_ok) {
...
long long relic_depth() {
    ...
    volume = relic_volume();
    ret_val = minimal_binary_cover(volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long relic_to_baryon() {
    ...
    volume = relic_volume();
    depth = relic_depth();
    ret_val = fraction_from_ratio(volume, depth);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long relic_fraction_of_matter() {
    ...
    volume = relic_volume();
    depth = relic_depth();
    tower = whole_power(2, depth);
    ret_val = fraction_from_ratio(volume, tower);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long relic_particle_identified() {
    long long exclusion_ok = 0;
    long long hadrons_ok = 0;
    long long ret_val = 0;

    hadrons_ok = dark_hadrons_forced();
    exclusion_ok = even_sector_excluded();
    if (hadrons_ok) {
    ret_val = exclusion_ok;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long relic_density_from_baryon(long long baryon_top, long long baryon_bottom) {
    long long anchor = 0;
    long long ratio = 0;
    long long ret_val = 0;

    ep_gc_push_root(&anchor);
    ep_gc_push_root(&ratio);
    ep_gc_push_root(&baryon_top);
    ep_gc_push_root(&baryon_bottom);

    ep_gc_maybe_collect();

    anchor = fraction_from_ratio(baryon_top, baryon_bottom);
    ratio = relic_to_baryon();
    ret_val = fraction_multiply(ratio, anchor);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long fibre_member(long long p, long long k) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio((1 + k), p);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fibre_folds_to_one(long long p) {
    long long all_fold = 0;
    long long folded = 0;
    long long k = 0;
    long long lands = 0;
    long long lifted = 0;
    ...
    ep_gc_push_root(&k);
    ep_gc_push_root(&lifted);
    ep_gc_push_root(&member);
    ep_gc_push_root(&one);
    ep_gc_push_root(&p_fraction);
    ep_gc_push_root(&p);
    ...
    k = 0;
    while (k < p) {
    member = fraction_from_ratio((1 + k), p);
    p_fraction = fraction_from_whole_number(p);
    lifted = fraction_multiply(p_fraction, member);
    folded = cast_out_whole_ones(lifted);
    lands = fraction_compare(folded, one) == 0;
    if (lands) {
    all_fold = all_fold;
    } else {
    all_fold = 0LL;
...
long long baryon_multiplicity(long long p) {
    long long count = 0;
    long long k = 0;
    long long ret_val = 0;
...
long long baryon_charge_sum(long long p) {
    long long k = 0;
    long long member = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    total = fraction_from_ratio(1, p);
    k = 1;
    while (k < p) {
    member = fraction_from_ratio((1 + k), p);
    total = fraction_add(total, member);
    k = (k + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long baryon_is_neutral_at(long long p) {
    long long bottom = 0;
    long long one_exact = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bottom);
    ep_gc_push_root(&one_exact);
    ep_gc_push_root(&total);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    total = baryon_charge_sum(p);
    bottom = exact_integer_from_sign_and_digits(1, ({ long long _fap = total; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'denominator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->denominator_digits; }));
    one_exact = exact_integer_from_number(1);
    ret_val = exact_integer_compare(bottom, one_exact) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_ExactInteger(bottom);
    bottom = 0;
    free_struct_ExactInteger(one_exact);
    one_exact = 0;
    free_struct_Fraction(total);
    total = 0;
    return ret_val;
...
long long meson_multiplicity(long long p) {
    long long anti = 0;
    long long kind = 0;
    long long one = 0;
    long long pairs = 0;
    ...
    kind = fraction_from_ratio(1, p);
    anti = fraction_from_ratio((p - 1), p);
    total = fraction_add(kind, anti);
    one = fraction_from_whole_number(1);
    pairs = fraction_compare(total, one) == 0;
    if (pairs) {
    ret_val = 2;
    goto L_cleanup;
    }
...
long long standard_model_anchor() {
    long long baryon_ok = 0;
    long long c = 0;
    long long fibre_ok = 0;
    long long meson_ok = 0;
    long long neutral_ok = 0;
    long long ret_val = 0;
    ...
    c = smallest_fold_period_above(binary_count());
    fibre_ok = fibre_folds_to_one(c);
    baryon_ok = baryon_multiplicity(c) == 3;
    meson_ok = meson_multiplicity(c) == 2;
    neutral_ok = baryon_is_neutral_at(c);
    if (fibre_ok) {
    if (baryon_ok) {
    if (meson_ok) {
    ret_val = neutral_ok;
...
long long dark_hadrons_forced() {
    long long hepta_count = 0;
    long long hepta_fibre = 0;
    long long hepta_neutral = 0;
    long long penta_count = 0;
    long long penta_fibre = 0;
    long long penta_neutral = 0;
    long long ret_val = 0;

    penta_fibre = fibre_folds_to_one(5);
    penta_count = baryon_multiplicity(5) == 5;
    penta_neutral = baryon_is_neutral_at(5);
    hepta_fibre = fibre_folds_to_one(7);
    hepta_count = baryon_multiplicity(7) == 7;
    hepta_neutral = baryon_is_neutral_at(7);
    if (penta_fibre) {
    if (penta_count) {
    if (penta_neutral) {
    if (hepta_fibre) {
    if (hepta_count) {
    ret_val = hepta_neutral;
    goto L_cleanup;
    }
    }
    }
...
long long even_sector_excluded() {
    long long neutral_at_four = 0;
    long long neutral_at_two = 0;
    long long ret_val = 0;

    neutral_at_two = baryon_is_neutral_at(2);
    neutral_at_four = baryon_is_neutral_at(4);
    if (neutral_at_two) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long up_depth() {
    ...
    by_covering = minimal_binary_cover(next_volume);
    by_relation = (smallest_fold_period_above(binary_count()) + (smallest_fold_period_above(binary_count()) + 1));
    ret_val = forced_to_be((long long)"deepest covering depth = cover(colour^(colour+1)) = colour+(colour+1)", by_covering, by_relation);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long generational_volume() {
    ...
    ret_val = whole_power(colour, colour);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
