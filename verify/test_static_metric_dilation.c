...
long long metric_coefficient(long long);
long long fold_covariance_at(long long, long long);
long long metric_is_fold_covariant();
long long clock_ratio();
long long ratio_squares_to_coefficient();
long long deep_clock_runs_slower();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the static metric (fold-covariant clock factor) ===");
    ok = expect_equal((long long)"the coefficient at x = 1/8: A = 7/8", fraction_to_text(metric_coefficient(fraction_from_ratio(1, 8))), (long long)"7/8");
    ok = expect_bool((long long)"fold covariance at x = 1/8: fold(A) = A(fold(x))", fold_covariance_at(1, 8));
    ok = expect_bool((long long)"the covariance holds across weak fields (1/8 and 1/16)", metric_is_fold_covariant());
    ok = expect_bool((long long)"the exact clock ratio: (3/4)^2 = 9/16 = A(7/16)", ratio_squares_to_coefficient());
    ok = expect_bool((long long)"the deep clock runs strictly slower (3/4 < 1)", deep_clock_runs_slower());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long metric_coefficient(long long x) {
    long long one = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_subtract(one, x);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fold_covariance_at(long long num, long long den) {
    long long coefficient = 0;
    long long coefficient_of_folded = 0;
    long long folded_coefficient = 0;
    long long folded_x = 0;
    long long x = 0;
    long long ret_val = 0;

    ep_gc_push_root(&coefficient);
    ep_gc_push_root(&coefficient_of_folded);
    ep_gc_push_root(&folded_coefficient);
    ep_gc_push_root(&folded_x);
    ep_gc_push_root(&x);
    ep_gc_push_root(&num);
    ep_gc_push_root(&den);
    ...
    x = fraction_from_ratio(num, den);
    coefficient = metric_coefficient(x);
    folded_coefficient = fold_fraction(coefficient);
    folded_x = fold_fraction(x);
    coefficient_of_folded = metric_coefficient(folded_x);
    ret_val = fraction_compare(folded_coefficient, coefficient_of_folded) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(coefficient_of_folded);
    coefficient_of_folded = 0;
    free_struct_Fraction(folded_coefficient);
    folded_coefficient = 0;
    return ret_val;
}

long long metric_is_fold_covariant() {
    long long at_eighth = 0;
    long long at_sixteenth = 0;
    long long ret_val = 0;

    at_eighth = fold_covariance_at(1, 8);
    at_sixteenth = fold_covariance_at(1, 16);
    if (at_eighth) {
    ret_val = at_sixteenth;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long ratio_squares_to_coefficient() {
    ...
    ratio = fraction_from_ratio(3, 4);
    ratio_twin = fraction_from_ratio(3, 4);
    squared = fraction_multiply(ratio, ratio_twin);
    x = fraction_from_ratio(7, 16);
    coefficient = metric_coefficient(x);
    ret_val = fraction_compare(squared, coefficient) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(coefficient);
    coefficient = 0;
...
long long deep_clock_runs_slower() {
    ...
    ratio = fraction_from_ratio(3, 4);
    one = fraction_from_whole_number(1);
    comparison = fraction_compare(ratio, one);
