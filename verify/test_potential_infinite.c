...
long long rung_reaches_one_in(long long);
long long always_a_deeper_rung(long long);
long long ladder_closes_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the potential infinite (a process, not a thing) ===");
    ok = expect_equal((long long)"rung 5 of the ladder = 1/32", fraction_to_text(ladder_rung(5)), (long long)"1/32");
    ok = expect_bool((long long)"rung 5 is finite and reaches the One in 5 folds", rung_reaches_one_in(5));
    ok = expect_bool((long long)"there is ALWAYS a deeper rung (smaller, never zero)", always_a_deeper_rung(5));
    ok = expect_equal((long long)"the depth-5 ladder closes EXACTLY to the One (a finite whole)", fraction_to_text(ladder_closes_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long ladder_rung(long long k) {
    long long volume = 0;
    long long ret_val = 0;
...
long long rung_reaches_one_in(long long k) {
    long long current = 0;
    long long one = 0;
    long long step = 0;
    long long ret_val = 0;
    ...
    current = ladder_rung(k);
    step = 0;
    while (step < k) {
    current = fold_fraction(current);
    step = (step + 1);
    }
    ret_val = fraction_compare(current, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long always_a_deeper_rung(long long k) {
    ...
    here = ladder_rung(k);
    deeper = ladder_rung((k + 1));
    comparison = fraction_compare(deeper, here);
    smaller = comparison < 0;
    positive = ({ long long _fap = deeper; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    if (smaller) {
    ret_val = positive;
    goto L_cleanup;
    }
...
long long ladder_closes_to_one() {
    ...
    while (k <= depth) {
    rung = ladder_rung(k);
    total = fraction_add(total, rung);
    k = (k + 1);
    }
    boundary = ladder_rung(depth);
    ret_val = fraction_add(total, boundary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
