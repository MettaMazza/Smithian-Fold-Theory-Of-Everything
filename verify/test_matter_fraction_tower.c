...
long long _main();
long long covering_depth();
long long tower_states();
long long pinned_states();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the matter fraction (Omega_m = 5/16) ===");
    ok = expect_equal((long long)"the covering depth, counted (minimal cover of 27) = 5", int_to_string(covering_depth()), (long long)"5");
    ok = expect_equal((long long)"the tower, built by doubling = 32 states", int_to_string(tower_states()), (long long)"32");
    ok = expect_equal((long long)"the pinned states: two per level = 10", int_to_string(pinned_states()), (long long)"10");
    ok = expect_equal((long long)"the vacuum share = 22/32 = 11/16", fraction_to_text(vacuum_fraction()), (long long)"11/16");
    ok = expect_equal((long long)"the matter fraction = 5/16 (Planck: 0.315, forced: 0.3125)", fraction_to_text(matter_fraction()), (long long)"5/16");
    ok = expect_equal((long long)"the two shares partition the One (no third piece)", fraction_to_text(shares_partition_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long pinned_states() {
    ...
    depth = covering_depth();
    ret_val = (smallest_fold_period_above(1) * depth);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long vacuum_fraction() {
    long long free_states = 0;
    long long pinned = 0;
    long long states = 0;
    long long ret_val = 0;

    ep_gc_push_root(&free_states);
    ...
    free_states = (states - pinned);
    ret_val = fraction_from_ratio(free_states, states);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long matter_fraction() {
    ...
    vacuum = vacuum_fraction();
    ret_val = fraction_subtract(one, vacuum);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
