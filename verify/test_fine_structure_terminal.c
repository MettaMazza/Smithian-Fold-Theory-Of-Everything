...
long long _main();
long long ladder_rung(long long);
long long rung_successor_count(long long);
long long ladder_is_counted();
long long ladder_length();
long long effective_cover_at_order(long long);
long long inverse_alpha_at_order(long long);
long long orders_match_the_module();
long long terminal_inverse_alpha();
long long shift_between(long long, long long);
long long convergence_collapses();
...
long long _main() {
    ...
    long long ok = 0;
    long long sigma_fiftieth = 0;
    long long terminal = 0;
    long long terminal_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&codata);
    ep_gc_push_root(&deviation);
    ep_gc_push_root(&sigma_fiftieth);
    ep_gc_push_root(&terminal);
    ep_gc_push_root(&terminal_twin);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the terminal fine structure: alpha read to ALL its orders ===");
    ok = expect_equal((long long)"the ladder's rungs: 5^3", int_to_string(ladder_rung(0)), (long long)"125");
    ok = expect_equal((long long)"one direction promoted: 5^2*7", int_to_string(ladder_rung(1)), (long long)"175");
    ok = expect_equal((long long)"two promoted: 5*7^2", int_to_string(ladder_rung(2)), (long long)"245");
    ok = expect_equal((long long)"all promoted: 7^3 (the last rung)", int_to_string(ladder_rung(3)), (long long)"343");
    ok = expect_bool((long long)"THE LADDER IS COUNTED: one successor per rung, then NONE", ladder_is_counted());
    ok = expect_equal((long long)"the ladder length: colour + 1", int_to_string(ladder_length()), (long long)"4");
    ok = expect_bool((long long)"orders 1 and 2 EQUAL the independent module values (identities)", orders_match_the_module());
    ok = expect_equal((long long)"the third order, exact", fraction_to_text(inverse_alpha_at_order(3)), (long long)"1468922449/10719245");
    ok = expect_equal((long long)"THE TERMINAL VALUE: alpha exact, the ladder closed", fraction_to_text(terminal_inverse_alpha()), (long long)"503846395469/3676744786");
    terminal = inverse_alpha_at_order(4);
    ok = expect_equal((long long)"the terminal decimal: the called digits ...177181", fraction_to_decimal(terminal, 12), (long long)"137.035999177180");
    ok = expect_bool((long long)"convergence collapses self-similarly (>1000x per rung, exact)", convergence_collapses());
    codata = fraction_from_ratio(137035999177, 1000000000);
    terminal_twin = inverse_alpha_at_order(4);
    deviation = fraction_subtract(terminal_twin, codata);
    sigma_fiftieth = fraction_from_ratio(21, 50000000000);
    ok = expect_bool((long long)"vs CODATA: inside a FIFTIETH of the error bar (0.009 sigma)", fraction_compare(deviation, sigma_fiftieth) == -1);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ...
    deviation = 0;
    free_struct_Fraction(sigma_fiftieth);
    sigma_fiftieth = 0;
    free_struct_Fraction(terminal);
    terminal = 0;
    return ret_val;
}

long long ladder_rung(long long sevens) {
    long long down_part = 0;
    long long fives = 0;
    long long up_part = 0;
    long long ret_val = 0;

    ep_gc_push_root(&fives);
    ep_gc_push_root(&sevens);

    ep_gc_maybe_collect();

    fives = (3 - sevens);
    down_part = whole_power(5, fives);
    up_part = whole_power(7, sevens);
    ret_val = (down_part * up_part);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long rung_successor_count(long long sevens) {
...
long long ladder_is_counted() {
    ...
    first = rung_successor_count(0) == 1;
    second = rung_successor_count(1) == 1;
    third = rung_successor_count(2) == 1;
    terminal = rung_successor_count(3) == 0;
    if (first) {
    if (second) {
    if (third) {
    ret_val = terminal;
...
long long effective_cover_at_order(long long order) {
    long long base = 0;
    long long chain = 0;
    long long deepening = 0;
    long long inverse = 0;
    long long level = 0;
    long long one = 0;
    long long one_outer = 0;
    long long rung = 0;
    long long ret_val = 0;

    ep_gc_push_root(&base);
    ep_gc_push_root(&chain);
    ep_gc_push_root(&deepening);
    ep_gc_push_root(&inverse);
    ep_gc_push_root(&level);
    ep_gc_push_root(&one);
    ep_gc_push_root(&one_outer);
    ep_gc_push_root(&rung);
    ep_gc_push_root(&order);

    ep_gc_maybe_collect();

    chain = fraction_from_whole_number(ladder_rung((order - 1)));
    level = (order - 2);
    while (level > 0) {
    one = fraction_from_whole_number(1);
    inverse = fraction_divide(one, chain);
    rung = fraction_from_whole_number(ladder_rung(level));
    chain = fraction_add(rung, inverse);
    level = (level - 1);
    }
    if (order == 1) {
    ret_val = fraction_from_whole_number(250);
    goto L_cleanup;
    }
    one_outer = fraction_from_whole_number(1);
    deepening = fraction_divide(one_outer, chain);
    base = fraction_from_whole_number(250);
    ret_val = fraction_add(base, deepening);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    return ret_val;
...
long long inverse_alpha_at_order(long long order) {
    long long correction = 0;
    long long cov = 0;
    long long cov_twin = 0;
    long long dilated = 0;
    long long nine = 0;
    long long one = 0;
    long long surface_term = 0;
    long long tower = 0;
    long long ret_val = 0;

    ep_gc_push_root(&correction);
    ep_gc_push_root(&cov);
    ep_gc_push_root(&cov_twin);
    ep_gc_push_root(&dilated);
    ep_gc_push_root(&nine);
    ep_gc_push_root(&one);
    ep_gc_push_root(&surface_term);
    ep_gc_push_root(&tower);
    ep_gc_push_root(&order);

    ep_gc_maybe_collect();

    cov = effective_cover_at_order(order);
    one = fraction_from_whole_number(1);
    cov_twin = effective_cover_at_order(order);
    dilated = fraction_add(cov_twin, one);
    correction = fraction_divide(dilated, cov);
    nine = fraction_from_whole_number(9);
    surface_term = fraction_multiply(nine, correction);
    tower = fraction_from_whole_number(128);
    ret_val = fraction_add(tower, surface_term);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    return ret_val;
...
long long orders_match_the_module() {
    long long first_ok = 0;
    long long here_one = 0;
    long long here_two = 0;
    long long module_one = 0;
    long long module_two = 0;
    long long second_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&here_one);
    ep_gc_push_root(&here_two);
    ep_gc_push_root(&module_one);
    ep_gc_push_root(&module_two);

    ep_gc_maybe_collect();

    here_one = inverse_alpha_at_order(1);
    module_one = inverse_fine_structure_first_order();
    first_ok = fraction_compare(here_one, module_one) == 0;
    here_two = inverse_alpha_at_order(2);
    module_two = inverse_fine_structure_second_order();
    second_ok = fraction_compare(here_two, module_two) == 0;
    if (first_ok) {
    ret_val = second_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(here_one);
    here_one = 0;
    free_struct_Fraction(here_two);
    here_two = 0;
    free_struct_Fraction(module_one);
    module_one = 0;
    free_struct_Fraction(module_two);
    module_two = 0;
    return ret_val;
}
...
long long terminal_inverse_alpha() {
    ...
    ret_val = inverse_alpha_at_order(4);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long shift_between(long long lower, long long higher) {
    long long difference = 0;
    long long high_value = 0;
    long long low_value = 0;
    long long zero = 0;
    long long ret_val = 0;

    ep_gc_push_root(&difference);
    ep_gc_push_root(&high_value);
    ep_gc_push_root(&low_value);
    ep_gc_push_root(&zero);
    ep_gc_push_root(&lower);
    ep_gc_push_root(&higher);

    ep_gc_maybe_collect();

    low_value = inverse_alpha_at_order(lower);
    high_value = inverse_alpha_at_order(higher);
    difference = fraction_subtract(high_value, low_value);
    if (({ long long _fap = difference; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 1) {
    ret_val = difference;
    goto L_cleanup;
    }
    zero = fraction_from_whole_number(0);
    ret_val = fraction_subtract(zero, difference);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long convergence_collapses() {
    long long first_dominates = 0;
    long long first_shift = 0;
    long long second_dominates = 0;
    long long second_scaled = 0;
    long long second_shift = 0;
    long long second_shift_twin = 0;
    long long third_scaled = 0;
    long long third_shift = 0;
    long long thousand = 0;
    long long thousand_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_shift);
    ep_gc_push_root(&second_scaled);
    ep_gc_push_root(&second_shift);
    ep_gc_push_root(&second_shift_twin);
    ep_gc_push_root(&third_scaled);
    ep_gc_push_root(&third_shift);
    ep_gc_push_root(&thousand);
    ep_gc_push_root(&thousand_twin);

    ep_gc_maybe_collect();

    thousand = fraction_from_whole_number(1000);
    first_shift = shift_between(1, 2);
    second_shift = shift_between(2, 3);
    second_scaled = fraction_multiply(thousand, second_shift);
    first_dominates = fraction_compare(first_shift, second_scaled) == 1;
    thousand_twin = fraction_from_whole_number(1000);
    second_shift_twin = shift_between(2, 3);
    third_shift = shift_between(3, 4);
    third_scaled = fraction_multiply(thousand_twin, third_shift);
    second_dominates = fraction_compare(second_shift_twin, third_scaled) == 1;
    if (first_dominates) {
    ret_val = second_dominates;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(first_shift);
    first_shift = 0;
    free_struct_Fraction(second_scaled);
    second_scaled = 0;
    free_struct_Fraction(second_shift_twin);
    second_shift_twin = 0;
    free_struct_Fraction(third_scaled);
    third_scaled = 0;
    return ret_val;
}
