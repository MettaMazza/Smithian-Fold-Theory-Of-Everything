...
long long _main();
long long stability_coefficient(long long);
long long orbit_is_stable(long long);
long long maximum_stable_dimension();
long long four_dimensions_is_marginal();
long long colour_period();
long long stable_dimension_is_colour_period();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== orbital stability (why three dimensions) ===");
    ok = expect_equal((long long)"stability coefficient in d=3: S_3 = 1 (stable)", int_to_string(stability_coefficient(3)), (long long)"1");
    ok = expect_bool((long long)"orbits are stable in d = 3", orbit_is_stable(3));
    ok = expect_bool((long long)"d = 4 is exactly marginal (S_4 = 0, no restoring force)", four_dimensions_is_marginal());
    ok = expect_bool((long long)"orbits are UNSTABLE in d = 5", orbit_is_stable(5) == 0LL);
    ok = expect_equal((long long)"the maximum stable dimension, counted over d=1..5: 3", int_to_string(maximum_stable_dimension()), (long long)"3");
    ok = expect_bool((long long)"d_max equals the colour period (two forced counts agree)", stable_dimension_is_colour_period());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long stability_coefficient(long long dims) {
    long long ret_val = 0;
...
long long orbit_is_stable(long long dims) {
    long long s = 0;
    long long ret_val = 0;
...
long long stable_dimension_is_colour_period() {
    ...
    d_max = maximum_stable_dimension();
    period = fold_period_of_unit_fraction(7);
    ret_val = d_max == period;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
