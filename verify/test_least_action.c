...
long long stationary_point();
long long deviations_cancel();
long long off_balance_is_lopsided();
long long stationary_path_completes();
long long half_weights_sum_to_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the principle of least action (the balanced path) ===");
    ok = expect_equal((long long)"the stationary point = 1/2 (self-antipodal)", fraction_to_text(stationary_point()), (long long)"1/2");
    ok = expect_bool((long long)"mirror deviations cancel there (stationarity)", deviations_cancel());
    ok = expect_bool((long long)"an off-balance point is lopsided (not taken)", off_balance_is_lopsided());
    ok = expect_equal((long long)"the balanced path completes (folds to the One)", fraction_to_text(stationary_path_completes()), (long long)"1");
    ok = expect_equal((long long)"two half-weights carry the whole", fraction_to_text(half_weights_sum_to_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long deviations_cancel() {
    ...
    point = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, point);
    ret_val = fraction_compare(complement, point) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long off_balance_is_lopsided() {
    ...
    off = fraction_from_ratio(1, colour_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, off);
    same = fraction_compare(complement, off) == 0;
    if (same) {
    ret_val = 0;
    goto L_cleanup;
    }
...
long long stationary_path_completes() {
    ...
    ret_val = fold_fraction(stationary_point());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long half_weights_sum_to_whole() {
    ...
    point = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(point, point);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
