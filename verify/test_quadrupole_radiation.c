...
long long _main();
long long linear_first_steps_equal();
long long uniform_motion_is_silent();
long long cubic_second_difference_early();
long long cubic_second_difference_late();
long long quadrupole_drive_radiates();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== quadrupole radiation (the first unfrozen moment) ===");
    ok = expect_bool((long long)"uniform motion: first differences all equal", linear_first_steps_equal());
    ok = expect_bool((long long)"so uniform motion is silent (no dipole radiation)", uniform_motion_is_silent());
    ok = expect_equal((long long)"cubic drive: early second difference = 12", int_to_string(cubic_second_difference_early()), (long long)"12");
    ok = expect_equal((long long)"cubic drive: late second difference = 18", int_to_string(cubic_second_difference_late()), (long long)"18");
    ok = expect_bool((long long)"unequal -> the quadrupole drive radiates", quadrupole_drive_radiates());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long linear_first_steps_equal() {
    long long second_pair = 0;
    long long step_c = 0;
    long long step_a = 0;
    long long step_b = 0;
    long long first_pair = 0;
    long long ret_val = 0;

    step_a = 1;
    step_b = 1;
    step_c = 1;
    first_pair = step_a == step_b;
    second_pair = step_b == step_c;
    if (first_pair) {
    ret_val = second_pair;
    goto L_cleanup;
    }
...
long long uniform_motion_is_silent() {
    ...
    ret_val = linear_first_steps_equal();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long quadrupole_drive_radiates() {
    ...
    early = cubic_second_difference_early();
    late = cubic_second_difference_late();
    ret_val = early != late;
    goto L_cleanup;
L_cleanup:
    return ret_val;
