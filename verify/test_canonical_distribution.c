...
long long equilibrium_weight();
long long equilibrium_complement();
long long equilibrium_is_self_antipodal();
long long equilibrium_folds_to_one();
long long weights_normalise_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the canonical distribution (rational equilibrium, no exponential) ===");
    ok = expect_equal((long long)"equilibrium weight = (m-1)/m = 1/2 (rational)", fraction_to_text(equilibrium_weight()), (long long)"1/2");
    ok = expect_bool((long long)"equilibrium is self-antipodal (One - 1/2 = 1/2)", equilibrium_is_self_antipodal());
    ok = expect_equal((long long)"the balance folds to the One (equilibrium fixed point)", fraction_to_text(equilibrium_folds_to_one()), (long long)"1");
    ok = expect_equal((long long)"two equal weights normalise to the One", fraction_to_text(weights_normalise_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long equilibrium_complement() {
    ...
    ret_val = fraction_subtract(one, equilibrium_weight());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long equilibrium_is_self_antipodal() {
    ...
    complement = equilibrium_complement();
    weight = equilibrium_weight();
    ret_val = fraction_compare(complement, weight) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long equilibrium_folds_to_one() {
    ...
    ret_val = fold_fraction(equilibrium_weight());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long weights_normalise_to_one() {
    ...
    w = equilibrium_weight();
    ret_val = fraction_add(w, w);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
