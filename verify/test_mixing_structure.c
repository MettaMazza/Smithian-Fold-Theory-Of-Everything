...
long long mass_state(long long);
long long channel_state(long long);
long long mass_fibre_verified();
long long channel_fibre_verified();
long long fibre_offset(long long);
long long offset_is_uniform_ninth();
long long diagonal_element(long long);
long long diagonal_is_uniform();
long long offset_is_colour_squared_step();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the mixing structure (why the CKM is nearly diagonal) ===");
    ok = expect_bool((long long)"the mass fibre {2/9, 5/9, 8/9} verified (all land on 2/3)", mass_fibre_verified());
    ok = expect_bool((long long)"the channel fibre {1/3, 2/3, 1} verified (all land on the One)", channel_fibre_verified());
    ok = expect_bool((long long)"the fibre offset is a uniform 1/9 across all generations", offset_is_uniform_ninth());
    ok = expect_equal((long long)"the alignment diagonal V_kk = 8/9", fraction_to_text(diagonal_element(1)), (long long)"8/9");
    ok = expect_bool((long long)"the diagonal is UNIFORM (no three tuned angles)", diagonal_is_uniform());
    ok = expect_bool((long long)"the offset is one step of the colour-squared grid (1/9 = 1/3^2)", offset_is_colour_squared_step());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long mass_state(long long k) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio((2 + (3 * k)), 9);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long channel_state(long long k) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio((1 + k), 3);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long mass_fibre_verified() {
    ...
    target = fraction_from_ratio(2, 3);
    all_land = 1LL;
    state = fraction_from_ratio(2, 9);
    tripled = tripling_fold(state);
    lands = fraction_compare(tripled, target) == 0;
    if (lands) {
    all_land = all_land;
    } else {
    all_land = 0LL;
    }
    state = fraction_from_ratio(5, 9);
    tripled = tripling_fold(state);
    lands = fraction_compare(tripled, target) == 0;
    if (lands) {
    all_land = all_land;
...
long long channel_fibre_verified() {
    ...
    tripled = tripling_fold(state);
    lands = fraction_compare(tripled, one) == 0;
    if (lands) {
    all_land = all_land;
    } else {
    all_land = 0LL;
    }
    state = fraction_from_ratio(2, 3);
    tripled = tripling_fold(state);
    lands = fraction_compare(tripled, one) == 0;
    if (lands) {
    all_land = all_land;
...
long long fibre_offset(long long k) {
    long long mass = 0;
    long long weak_state = 0;
    long long ret_val = 0;

    ep_gc_push_root(&mass);
    ep_gc_push_root(&weak_state);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    weak_state = fraction_from_ratio((1 + k), 3);
    mass = fraction_from_ratio((2 + (3 * k)), 9);
    ret_val = fraction_subtract(weak_state, mass);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long offset_is_uniform_ninth() {
    ...
    ep_gc_push_root(&ninth);
    ep_gc_push_root(&offset);

    ep_gc_maybe_collect();

    ninth = fraction_from_ratio(1, 9);
    all_match = 1LL;
    offset = fibre_offset(0);
    matches = fraction_compare(offset, ninth) == 0;
    if (matches) {
    all_match = all_match;
    } else {
    all_match = 0LL;
    }
    offset = fibre_offset(1);
    matches = fraction_compare(offset, ninth) == 0;
    if (matches) {
    all_match = all_match;
    } else {
...
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(ninth);
    ninth = 0;
    free_struct_Fraction(offset);
    offset = 0;
    return ret_val;
}

long long diagonal_element(long long k) {
    long long offset = 0;
    long long one = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    offset = fibre_offset(k);
    ret_val = fraction_subtract(one, offset);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long diagonal_is_uniform() {
    ...
    all_match = 1LL;
    element = diagonal_element(0);
    matches = fraction_compare(element, eight_ninths) == 0;
    if (matches) {
    all_match = all_match;
    } else {
    all_match = 0LL;
    }
    element = diagonal_element(1);
    matches = fraction_compare(element, eight_ninths) == 0;
    if (matches) {
    all_match = all_match;
    } else {
...
long long offset_is_colour_squared_step() {
    ...
    offset = fibre_offset(0);
    grid = whole_power(colour_count(), 2);
    step = fraction_from_ratio(1, grid);
    ret_val = fraction_compare(offset, step) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(offset);
