...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Hubble tension ===");
    ok = expect_equal((long long)"vacuum fraction = 2/3", fraction_to_text(vacuum_fraction()), (long long)"2/3");
    ok = expect_equal((long long)"matter fraction = 1/3", fraction_to_text(matter_fraction()), (long long)"1/3");
    ok = expect_equal((long long)"vacuum + matter = the One", fraction_to_text(flatness_partition()), (long long)"1");
    ok = expect_equal((long long)"covering tower = 8", int_to_string(covering_tower()), int_to_string(8));
    ok = expect_equal((long long)"correction = 1/12", fraction_to_text(calibration_correction()), (long long)"1/12");
    ok = expect_equal((long long)"calibration ratio = 13/12", fraction_to_text(hubble_calibration_ratio()), (long long)"13/12");
    ok = expect_equal((long long)"calibration ratio decimal = 1.083", fraction_to_decimal(hubble_calibration_ratio(), 3), (long long)"1.083");
    printf("%s\n", (char*)(long long)"--- second order, to the measured value (1.0843230) ---");
    ok = expect_equal((long long)"deepest covering depth d_up = 7", int_to_string(up_depth()), int_to_string(7));
    ok = expect_equal((long long)"early orbit floor = 2^7 - 1 = 127", int_to_string(early_orbit_floor()), int_to_string(127));
    ok = expect_equal((long long)"the floor is a genuine period-7 orbit", int_to_string(early_orbit_floor_is_genuine()), int_to_string(1));
    ok = expect_equal((long long)"second-order ratio = 3305/3048", fraction_to_text(hubble_calibration_ratio_second_order()), (long long)"3305/3048");
    ok = expect_equal((long long)"second-order decimal = 1.0843175", hubble_calibration_ratio_second_order_decimal(7), (long long)"1.0843175");
    ok = expect_equal((long long)"the second-order DEEPENING is unique (forced, not fitted)", int_to_string(hubble_second_order_is_unique()), int_to_string(1));
    ok = expect_equal((long long)"the calibration FORM is unique (forced_unique)", int_to_string(hubble_calibration_form_is_unique()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
