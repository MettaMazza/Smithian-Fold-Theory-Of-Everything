...
long long first_hand();
long long second_hand();
long long hands_are_identical_particles();
long long fibre_size();
long long coincident_pair_is_symmetric();
long long distinct_pair_is_antisymmetric();
long long fermion_cap_is_fibre_size();
long long two_statistics_exhaust();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== quantum statistics (Bose and Fermi, no third) ===");
    ok = expect_bool((long long)"the two hands are identical particles (one fold image)", hands_are_identical_particles());
    ok = expect_equal((long long)"the fibre size = binary = 2", int_to_string(fibre_size()), (long long)"2");
    ok = expect_bool((long long)"BOSE: a coincident pair is fixed under exchange (symmetric)", coincident_pair_is_symmetric());
    ok = expect_bool((long long)"FERMI: a distinct pair reverses sign under exchange (antisymmetric)", distinct_pair_is_antisymmetric());
    ok = expect_bool((long long)"the Pauli cap = the fibre size (a third fermion has no preimage)", fermion_cap_is_fibre_size());
    ok = expect_bool((long long)"no third case: the two statistics exhaust the fibre", two_statistics_exhaust());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long hands_are_identical_particles() {
    long long from_first = 0;
    long long from_second = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_first);
    ep_gc_push_root(&from_second);

    ep_gc_maybe_collect();

    from_first = fold_fraction(first_hand());
    from_second = fold_fraction(second_hand());
    ret_val = fraction_compare(from_first, from_second) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(from_first);
    from_first = 0;
    free_struct_Fraction(from_second);
    from_second = 0;
    return ret_val;
}
...
long long coincident_pair_is_symmetric() {
    ...
    ep_gc_push_root(&partner);
    ep_gc_push_root(&separation);

    ep_gc_maybe_collect();

    here = fraction_from_ratio(1, 4);
    partner = fraction_from_ratio(1, 4);
    separation = fraction_subtract(here, partner);
...
long long distinct_pair_is_antisymmetric() {
    ...
    long long backward_negative = 0;
    long long forward = 0;
    long long forward_positive = 0;
    long long high = 0;
    long long low = 0;
    ...
    forward = fraction_subtract(high, low);
    backward = fraction_subtract(low, high);
    forward_positive = ({ long long _fap = forward; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    backward_negative = ({ long long _fap = backward; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) < 0;
    total = fraction_add(forward, backward);
    opposite = ({ long long _fap = total; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    if (forward_positive) {
...
long long fermion_cap_is_fibre_size() {
    ...
    cap = smallest_fold_period_above(1);
    ret_val = cap == 2;
    goto L_cleanup;
L_cleanup:
    return ret_val;
...
long long two_statistics_exhaust() {
    long long antisymmetric = 0;
    long long symmetric = 0;
    long long ret_val = 0;

    symmetric = coincident_pair_is_symmetric();
    antisymmetric = distinct_pair_is_antisymmetric();
    if (symmetric) {
    ret_val = antisymmetric;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
