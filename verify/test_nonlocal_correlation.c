...
long long joint_dimension();
long long shared_origin();
long long shared_origin_folds_connected();
long long is_one_shared_orbit();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== nonlocal correlation (one shared origin, no signal) ===");
    ok = expect_equal((long long)"joint dimension = colour*(colour+binary) = 15", int_to_string(joint_dimension()), (long long)"15");
    ok = expect_equal((long long)"shared origin = 1/15 (one joint state)", fraction_to_text(shared_origin()), (long long)"1/15");
    ok = expect_equal((long long)"it folds connected: fold(1/15) = 2/15", fraction_to_text(shared_origin_folds_connected()), (long long)"2/15");
    ok = expect_bool((long long)"it is ONE shared orbit (not two local parts)", is_one_shared_orbit());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long joint_dimension() {
    ...
    ret_val = (smallest_fold_period_above(binary_count()) * (smallest_fold_period_above(binary_count()) + smallest_fold_period_above(1)));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shared_origin() {
    ...
    ret_val = fraction_from_ratio(1, joint_dimension());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shared_origin_folds_connected() {
    ...
    ret_val = fold_fraction(shared_origin());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long is_one_shared_orbit() {
    ...
    folded = fold_fraction(shared_origin());
    expected = fraction_from_ratio(binary_count(), joint_dimension());
    ret_val = fraction_compare(folded, expected) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
