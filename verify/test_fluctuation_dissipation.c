...
long long _main();
long long equilibrium();
long long fluctuation();
long long dissipation();
long long fluctuation_and_dissipation_are_antipodal();
long long fluctuation_departure();
long long dissipation_departure();
long long fluctuation_equals_dissipation();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the fluctuation-dissipation theorem ===");
    ok = expect_equal((long long)"equilibrium = 1/2 (self-antipodal balance)", fraction_to_text(equilibrium()), (long long)"1/2");
    ok = expect_equal((long long)"fluctuation = 3/4 (a step above)", fraction_to_text(fluctuation()), (long long)"3/4");
    ok = expect_equal((long long)"dissipation = 1/4 (the antipode below)", fraction_to_text(dissipation()), (long long)"1/4");
    ok = expect_bool((long long)"fluctuation + dissipation = the One (antipodal)", fluctuation_and_dissipation_are_antipodal());
    ok = expect_equal((long long)"fluctuation departure = 1/4", fraction_to_text(fluctuation_departure()), (long long)"1/4");
    ok = expect_equal((long long)"dissipation departure = 1/4", fraction_to_text(dissipation_departure()), (long long)"1/4");
    ok = expect_bool((long long)"THE THEOREM: fluctuation = dissipation (equal magnitude)", fluctuation_equals_dissipation());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long dissipation() {
    ...
    ep_gc_push_root(&fluc);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    fluc = fraction_from_ratio(3, 4);
    ret_val = fraction_subtract(one, fluc);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fluctuation_departure() {
    ...
    fluc = fraction_from_ratio(3, 4);
    eq = fraction_from_ratio(1, binary_count());
    ret_val = fraction_subtract(fluc, eq);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long dissipation_departure() {
    ...
    eq = fraction_from_ratio(1, binary_count());
    diss = dissipation();
    ret_val = fraction_subtract(eq, diss);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fluctuation_equals_dissipation() {
    ...
    up = fluctuation_departure();
    down = dissipation_departure();
    ret_val = fraction_compare(up, down) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(down);
    down = 0;
