...
long long degeneracy_limit();
long long limit_folds_to_balance();
long long equilibrium_is_self_antipodal();
long long endpoint_census();
long long census_is_binary_fibre();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== degenerate endpoints (why dead stars are a short list) ===");
    ok = expect_equal((long long)"the degeneracy limit = 3/4 (the exclusion floor loaded full)", fraction_to_text(degeneracy_limit()), (long long)"3/4");
    ok = expect_bool((long long)"one compression step lands on the balance: fold(3/4) = 1/2", limit_folds_to_balance());
    ok = expect_bool((long long)"the remnant equilibrium is self-antipodal (pressure vs gravity)", equilibrium_is_self_antipodal());
    ok = expect_equal((long long)"the endpoint census: exactly 2 families (counted on the grid)", int_to_string(endpoint_census()), (long long)"2");
    ok = expect_bool((long long)"the census equals the binary fibre size (discrete, no continuum)", census_is_binary_fibre());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long limit_folds_to_balance() {
    ...
    ep_gc_push_root(&limit);

    ep_gc_maybe_collect();

    limit = fraction_from_ratio(3, 4);
    folded = fold_fraction(limit);
    balance = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(folded, balance) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long equilibrium_is_self_antipodal() {
    ...
    antipode = fraction_subtract(one, balance);
    ret_val = fraction_compare(balance, antipode) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long endpoint_census() {
    ...
    i = 1;
    while (i < 4) {
    candidate = fraction_from_ratio(i, 4);
    folded = fold_fraction(candidate);
    lands = fraction_compare(folded, balance) == 0;
    if (lands) {
    count = (count + 1);
    }
    i = (i + 1);
...
long long census_is_binary_fibre() {
    ...
    census = endpoint_census();
    ret_val = census == smallest_fold_period_above(1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
