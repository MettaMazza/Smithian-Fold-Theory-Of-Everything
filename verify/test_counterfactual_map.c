...
long long _main();
long long self_antipodal_unique_to(long long);
long long base_one_is_identity();
long long base_two_first_orbit();
long long master_dial_moved_count();
long long sector_label_is_bounded();
long long unit_name_is_physics_free();
...
long long lock_down_depth(long long);
long long lock_up_depth(long long);
long long lock_alpha(long long);
long long lock_lepton_third(long long);
long long lock_quark_up(long long);
long long lock_quark_down(long long);
long long lock_dark_to_baryon(long long);
long long lock_dark_fraction(long long);
long long lock_hubble(long long);
long long lock_planck_exponent(long long);
long long lock_lambda_floor(long long);
long long lock_half_one();
long long alpha_and_dark_share_down_depth();
long long perturbation_moves_all();
long long half_one_is_unmoved();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the counterfactual map: what had to be, what was free ===");
    ok = expect_bool((long long)"NECESSARY: 1/2 is the UNIQUE self-antipode -- exhaustive to denominator 300", self_antipodal_unique_to(300));
    ok = expect_bool((long long)"NECESSARY: base 1 is the identity -- no orbit, no dynamics", base_one_is_identity());
    ok = expect_bool((long long)"NECESSARY: base 2 gives the first real orbit 1/3 -> 2/3 -> 1/3", base_two_first_orbit());
    ok = expect_equal((long long)"NECESSARY: the depths given colour -- cover(27)", int_to_string(lock_down_depth(3)), (long long)"5");
    ok = expect_equal((long long)"NECESSARY: the depths given colour -- cover(81)", int_to_string(lock_up_depth(3)), (long long)"7");
    ok = expect_equal((long long)"THE MASTER DIAL: colour 3 -> 4 moves all NINE c-dependent constants", int_to_string(master_dial_moved_count()), (long long)"9");
    ok = expect_bool((long long)"CONTINGENT: the one freedom is the bounded label m in {2,3,5,7}", sector_label_is_bounded());
    ok = expect_bool((long long)"CONVENTION: the unit name moves no ratio (walked at scale 1000)", unit_name_is_physics_free());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long self_antipodal_unique_to(long long bound) {
    long long all_are_half = 0;
    long long candidate = 0;
    long long candidate_check = 0;
    long long candidate_twin = 0;
    long long fixed = 0;
    long long found = 0;
    long long half = 0;
    long long half_twin = 0;
    long long is_half = 0;
    long long one = 0;
    long long p = 0;
    long long q = 0;
    ...
    ep_gc_push_root(&candidate_check);
    ep_gc_push_root(&candidate_twin);
    ep_gc_push_root(&half);
    ep_gc_push_root(&half_twin);
    ep_gc_push_root(&one);
    ep_gc_push_root(&p);
    ep_gc_push_root(&q);
    ep_gc_push_root(&reflected);
    ...
    reflected = fraction_subtract(one, candidate);
    candidate_twin = fraction_from_ratio(p, q);
    fixed = fraction_compare(reflected, candidate_twin) == 0;
    if (fixed) {
    found = (found + 1);
    candidate_check = fraction_from_ratio(p, q);
    half_twin = fraction_from_ratio(1, 2);
    is_half = fraction_compare(candidate_check, half_twin) == 0;
    if (is_half) {
    all_are_half = all_are_half;
    } else {
    all_are_half = 0LL;
    }
    }
...
L_cleanup:
    ep_gc_pop_roots(9);
    free_struct_Fraction(candidate_check);
    candidate_check = 0;
    free_struct_Fraction(candidate_twin);
    candidate_twin = 0;
    free_struct_Fraction(half);
    half = 0;
    free_struct_Fraction(half_twin);
    half_twin = 0;
    free_struct_Fraction(reflected);
    reflected = 0;
    return ret_val;
}
...
long long base_one_is_identity() {
    long long moved = 0;
    long long one_fraction = 0;
    long long state = 0;
    long long state_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&moved);
    ep_gc_push_root(&one_fraction);
    ep_gc_push_root(&state);
    ep_gc_push_root(&state_twin);

    ep_gc_maybe_collect();

    state = fraction_from_ratio(1, 3);
    one_fraction = fraction_from_whole_number(1);
    moved = fraction_multiply(one_fraction, state);
    state_twin = fraction_from_ratio(1, 3);
    ret_val = fraction_compare(moved, state_twin) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(moved);
    moved = 0;
    free_struct_Fraction(state_twin);
    state_twin = 0;
    return ret_val;
}
...
long long base_two_first_orbit() {
    long long once = 0;
    long long once_again = 0;
    long long once_twin = 0;
    long long really_moved = 0;
    long long returned = 0;
    long long start = 0;
    long long start_check = 0;
    long long start_final = 0;
    long long start_for_step = 0;
    long long stayed = 0;
    long long twice = 0;
    long long twice_raw = 0;
    long long ret_val = 0;

    ep_gc_push_root(&once);
    ep_gc_push_root(&once_again);
    ep_gc_push_root(&once_twin);
    ep_gc_push_root(&start);
    ep_gc_push_root(&start_check);
    ep_gc_push_root(&start_final);
    ep_gc_push_root(&start_for_step);
    ep_gc_push_root(&twice);
    ep_gc_push_root(&twice_raw);

    ep_gc_maybe_collect();

    start = fraction_from_ratio(1, 3);
    start_for_step = fraction_from_ratio(1, 3);
    once = fraction_add(start_for_step, start);
    start_check = fraction_from_ratio(1, 3);
    really_moved = 1LL;
    stayed = fraction_compare(once, start_check) == 0;
    if (stayed) {
    really_moved = 0LL;
    }
    once_twin = fraction_from_ratio(2, 3);
    once_again = fraction_from_ratio(2, 3);
    twice_raw = fraction_add(once_twin, once_again);
    twice = cast_out_whole_ones(twice_raw);
    start_final = fraction_from_ratio(1, 3);
    returned = fraction_compare(twice, start_final) == 0;
    if (really_moved) {
    ret_val = returned;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(9);
    free_struct_Fraction(once);
    once = 0;
    free_struct_Fraction(start_check);
    start_check = 0;
    free_struct_Fraction(start_final);
    start_final = 0;
...
long long master_dial_moved_count() {
    long long alpha3 = 0;
    long long alpha4 = 0;
    long long alpha_same = 0;
    long long dark3 = 0;
    long long dark4 = 0;
    long long dark_same = 0;
    long long down3 = 0;
    long long down4 = 0;
    long long down_same = 0;
    long long floor3 = 0;
    long long floor4 = 0;
    long long floor_same = 0;
    long long hubble3 = 0;
    long long hubble4 = 0;
    long long hubble_same = 0;
    long long lepton3 = 0;
    long long lepton4 = 0;
    long long lepton_same = 0;
    long long moved = 0;
    long long planck3 = 0;
    long long planck4 = 0;
    long long planck_same = 0;
    long long ratio3 = 0;
    long long ratio4 = 0;
    long long ratio_same = 0;
    long long up3 = 0;
    long long up4 = 0;
    long long up_same = 0;
    long long ret_val = 0;

    ep_gc_push_root(&alpha3);
    ep_gc_push_root(&alpha4);
    ep_gc_push_root(&dark3);
    ep_gc_push_root(&dark4);
    ep_gc_push_root(&down3);
    ep_gc_push_root(&down4);
    ep_gc_push_root(&floor3);
    ep_gc_push_root(&floor4);
    ep_gc_push_root(&hubble3);
    ep_gc_push_root(&hubble4);
    ep_gc_push_root(&lepton3);
    ep_gc_push_root(&lepton4);
    ep_gc_push_root(&planck3);
    ep_gc_push_root(&planck4);
    ep_gc_push_root(&ratio3);
    ep_gc_push_root(&ratio4);
    ep_gc_push_root(&up3);
    ep_gc_push_root(&up4);
    ...
    alpha3 = lock_alpha(3);
    alpha4 = lock_alpha(4);
    alpha_same = fraction_compare(alpha3, alpha4) == 0;
    if (alpha_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    lepton3 = lock_lepton_third(3);
    lepton4 = lock_lepton_third(4);
    lepton_same = fraction_compare(lepton3, lepton4) == 0;
    if (lepton_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    ...
    up3 = lock_quark_up(3);
    up4 = lock_quark_up(4);
    up_same = fraction_compare(up3, up4) == 0;
    if (up_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    down3 = lock_quark_down(3);
    down4 = lock_quark_down(4);
    down_same = fraction_compare(down3, down4) == 0;
    if (down_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    ratio3 = lock_dark_to_baryon(3);
    ratio4 = lock_dark_to_baryon(4);
    ratio_same = fraction_compare(ratio3, ratio4) == 0;
    if (ratio_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    dark3 = lock_dark_fraction(3);
    dark4 = lock_dark_fraction(4);
    dark_same = fraction_compare(dark3, dark4) == 0;
    if (dark_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    hubble3 = lock_hubble(3);
    hubble4 = lock_hubble(4);
    hubble_same = fraction_compare(hubble3, hubble4) == 0;
    if (hubble_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    planck3 = lock_planck_exponent(3);
    planck4 = lock_planck_exponent(4);
    planck_same = fraction_compare(planck3, planck4) == 0;
    if (planck_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
    }
    floor3 = lock_lambda_floor(3);
    floor4 = lock_lambda_floor(4);
    floor_same = fraction_compare(floor3, floor4) == 0;
    if (floor_same) {
    moved = moved;
    } else {
    moved = (moved + 1);
...
L_cleanup:
    ep_gc_pop_roots(18);
    free_struct_Fraction(alpha3);
    alpha3 = 0;
    free_struct_Fraction(alpha4);
    alpha4 = 0;
    free_struct_Fraction(dark3);
    dark3 = 0;
    free_struct_Fraction(dark4);
    dark4 = 0;
    free_struct_Fraction(down3);
    down3 = 0;
    free_struct_Fraction(down4);
    down4 = 0;
    free_struct_Fraction(floor3);
    floor3 = 0;
    free_struct_Fraction(floor4);
    floor4 = 0;
    free_struct_Fraction(hubble3);
    hubble3 = 0;
    free_struct_Fraction(hubble4);
    hubble4 = 0;
    free_struct_Fraction(lepton3);
    lepton3 = 0;
    free_struct_Fraction(lepton4);
    lepton4 = 0;
    free_struct_Fraction(planck3);
    planck3 = 0;
    free_struct_Fraction(planck4);
    planck4 = 0;
    free_struct_Fraction(ratio3);
    ratio3 = 0;
    free_struct_Fraction(ratio4);
    ratio4 = 0;
    free_struct_Fraction(up3);
    up3 = 0;
    free_struct_Fraction(up4);
    up4 = 0;
    return ret_val;
}
...
long long unit_name_is_physics_free() {
    long long bare_ratio = 0;
    long long denominator_scaled = 0;
    long long denominator_scaled_base = 0;
    long long denominator_side = 0;
    long long numerator_scaled = 0;
    long long numerator_scaled_base = 0;
    long long numerator_side = 0;
    long long scaled_ratio = 0;
    long long unit = 0;
    long long unit_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bare_ratio);
    ep_gc_push_root(&denominator_scaled);
    ep_gc_push_root(&denominator_scaled_base);
    ep_gc_push_root(&denominator_side);
    ep_gc_push_root(&numerator_scaled);
    ep_gc_push_root(&numerator_scaled_base);
    ep_gc_push_root(&numerator_side);
    ep_gc_push_root(&scaled_ratio);
    ep_gc_push_root(&unit);
    ep_gc_push_root(&unit_twin);

    ep_gc_maybe_collect();

    numerator_side = fraction_from_ratio(27, 1);
    denominator_side = fraction_from_ratio(5, 1);
    bare_ratio = fraction_divide(numerator_side, denominator_side);
    unit = fraction_from_whole_number(1000);
    numerator_scaled_base = fraction_from_ratio(27, 1);
    numerator_scaled = fraction_multiply(numerator_scaled_base, unit);
    unit_twin = fraction_from_whole_number(1000);
    denominator_scaled_base = fraction_from_ratio(5, 1);
    denominator_scaled = fraction_multiply(denominator_scaled_base, unit_twin);
    scaled_ratio = fraction_divide(numerator_scaled, denominator_scaled);
    ret_val = fraction_compare(scaled_ratio, bare_ratio) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(10);
    free_struct_Fraction(bare_ratio);
    bare_ratio = 0;
    free_struct_Fraction(scaled_ratio);
    scaled_ratio = 0;
    return ret_val;
}
...
long long lock_down_depth(long long c) {
    long long volume = 0;
    long long ret_val = 0;
...
long long lock_up_depth(long long c) {
    long long volume = 0;
    long long ret_val = 0;
...
long long lock_alpha(long long c) {
    long long cov = 0;
    long long d_down = 0;
    long long d_up = 0;
    ...
    ep_gc_push_root(&tower_part);
    ep_gc_push_root(&c);

    ep_gc_maybe_collect();

    d_up = lock_up_depth(c);
    d_down = lock_down_depth(c);
    tower = whole_power(2, d_up);
    cov = (2 * whole_power(d_down, 3));
    surface = whole_power(c, 2);
    tower_part = fraction_from_whole_number(tower);
    dilation = fraction_from_ratio((surface * (cov + 1)), cov);
    ret_val = fraction_add(tower_part, dilation);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    return ret_val;
...
long long lock_lepton_third(long long c) {
    long long body = 0;
    long long d_down = 0;
    long long ret_val = 0;
...
long long lock_quark_up(long long c) {
    long long d_up = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    d_up = lock_up_depth(c);
    tower = whole_power(2, d_up);
    ret_val = fraction_from_ratio(1, ((c * tower) - 1));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long lock_quark_down(long long c) {
    long long d_down = 0;
    long long tower = 0;
    long long ret_val = 0;
...
long long lock_dark_to_baryon(long long c) {
    long long d_down = 0;
    long long volume = 0;
    long long ret_val = 0;
    ...
    volume = whole_power(c, 3);
    d_down = lock_down_depth(c);
    ret_val = fraction_from_ratio(volume, d_down);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long lock_dark_fraction(long long c) {
    long long d_down = 0;
    long long tower = 0;
    long long volume = 0;
    long long ret_val = 0;
...
long long lock_hubble(long long c) {
    ...
    one = fraction_from_whole_number(1);
    correction = fraction_from_ratio((c - 1), (c * 8));
...
long long lock_planck_exponent(long long c) {
    long long d_up = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    d_up = lock_up_depth(c);
    tower = whole_power(2, d_up);
    ret_val = fraction_from_ratio((tower - 1), 2);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long lock_lambda_floor(long long c) {
    long long d_down = 0;
    long long tower = 0;
    long long ret_val = 0;
    ...
    d_down = lock_down_depth(c);
    tower = whole_power(2, (4 * d_down));
    ret_val = fraction_from_ratio(1, tower);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long alpha_and_dark_share_down_depth() {
    long long alpha_check = 0;
    long long alpha_cov = 0;
    long long c = 0;
    long long dark_check = 0;
    long long dark_tower = 0;
    long long shared_depth = 0;
    long long ret_val = 0;

    ep_gc_push_root(&c);
    ep_gc_push_root(&shared_depth);

    ep_gc_maybe_collect();

    c = smallest_fold_period_above(binary_count());
    shared_depth = lock_down_depth(c);
    alpha_cov = (2 * whole_power(shared_depth, 3));
    alpha_check = alpha_cov == 250;
    dark_tower = whole_power(2, shared_depth);
    dark_check = dark_tower == 32;
    if (alpha_check) {
    ret_val = dark_check;
    goto L_cleanup;
    }
...
long long perturbation_moves_all() {
    long long alpha3 = 0;
    long long alpha4 = 0;
    long long alpha_moved = 0;
    long long dark3 = 0;
    long long dark4 = 0;
    long long dark_moved = 0;
    long long hubble3 = 0;
    long long hubble4 = 0;
    long long hubble_moved = 0;
    long long lepton3 = 0;
    long long lepton4 = 0;
    long long lepton_moved = 0;
    long long ratio3 = 0;
    long long ratio4 = 0;
    long long ratio_moved = 0;
    long long up3 = 0;
    long long up4 = 0;
    long long up_moved = 0;
    long long ret_val = 0;
    ...
    alpha3 = lock_alpha(3);
    alpha4 = lock_alpha(4);
    alpha_moved = fraction_compare(alpha3, alpha4) == 0 == 0LL;
    lepton3 = lock_lepton_third(3);
    lepton4 = lock_lepton_third(4);
    lepton_moved = fraction_compare(lepton3, lepton4) == 0 == 0LL;
    up3 = lock_quark_up(3);
    up4 = lock_quark_up(4);
    up_moved = fraction_compare(up3, up4) == 0 == 0LL;
    dark3 = lock_dark_fraction(3);
    dark4 = lock_dark_fraction(4);
    dark_moved = fraction_compare(dark3, dark4) == 0 == 0LL;
    ratio3 = lock_dark_to_baryon(3);
    ratio4 = lock_dark_to_baryon(4);
    ratio_moved = fraction_compare(ratio3, ratio4) == 0 == 0LL;
    hubble3 = lock_hubble(3);
    hubble4 = lock_hubble(4);
    hubble_moved = fraction_compare(hubble3, hubble4) == 0 == 0LL;
    if (alpha_moved) {
    if (lepton_moved) {
    if (up_moved) {
    if (dark_moved) {
    if (ratio_moved) {
    ret_val = hubble_moved;
    goto L_cleanup;
    }
    }
    }
...
long long half_one_is_unmoved() {
    ...
    ep_gc_push_root(&at_four);
    ep_gc_push_root(&at_three);

    ep_gc_maybe_collect();

    at_three = fraction_from_ratio(1, 2);
    at_four = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(at_three, at_four) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(at_four);
    at_four = 0;
    free_struct_Fraction(at_three);
    at_three = 0;
    return ret_val;
}
