...
long long _main();
long long running_coupling(long long, long long);
long long built_gap(long long, long long, long long);
long long closed_gap(long long, long long, long long);
long long routes_agree_for_pair(long long, long long);
long long corpus_anchor_holds();
long long all_six_gaps_forced();
long long rises_to_unison(long long);
long long all_four_converge();
long long gaps_shrink_for(long long, long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the new forces run: all four converge to unison ===");
    ok = expect_equal((long long)"the strong coupling at d=0: g_3(1) = 3/4", fraction_to_text(running_coupling(3, 0)), (long long)"3/4");
    ok = expect_equal((long long)"the PENTA coupling at d=0: g_5(1) = 5/6", fraction_to_text(running_coupling(5, 0)), (long long)"5/6");
    ok = expect_equal((long long)"the HEPTA coupling at d=0: g_7(1) = 7/8", fraction_to_text(running_coupling(7, 0)), (long long)"7/8");
    ok = expect_bool((long long)"THE ANCHOR: the corpus strong-EW gap 1/((2+R)(3+R)), d = 0..10", corpus_anchor_holds());
    ok = expect_equal((long long)"the anchor gap at d=0: g_3 - g_2 = 1/12", fraction_to_text(built_gap(2, 3, 0)), (long long)"1/12");
    ok = expect_bool((long long)"ALL SIX sector pairs: built gap = closed form (j-i)/((i+R)(j+R))", all_six_gaps_forced());
    ok = expect_equal((long long)"the NEW-FORCE gap at d=0: g_7 - g_5 = 1/24", fraction_to_text(built_gap(5, 7, 0)), (long long)"1/24");
    ok = expect_equal((long long)"the new-force gap closed form at d=3: 2/((5+8)(7+8))", fraction_to_text(closed_gap(5, 7, 3)), (long long)"2/195");
    ok = expect_bool((long long)"ALL FOUR forces rise strictly toward unison, below the One at every finite scale", all_four_converge());
    ok = expect_bool((long long)"the strong-EW gap shrinks with scale", gaps_shrink_for(2, 3));
    ok = expect_bool((long long)"the new-force gap shrinks with scale", gaps_shrink_for(5, 7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long running_coupling(long long p, long long d) {
    long long scale = 0;
    long long ret_val = 0;
    ...
    scale = whole_power(2, d);
    ret_val = fraction_from_ratio(((p + scale) - 1), (p + scale));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long built_gap(long long i, long long j, long long d) {
    long long g_i = 0;
    long long g_j = 0;
    long long ret_val = 0;
    ...
    g_i = running_coupling(i, d);
    g_j = running_coupling(j, d);
    ret_val = fraction_subtract(g_j, g_i);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long closed_gap(long long i, long long j, long long d) {
    long long scale = 0;
    long long ret_val = 0;
    ...
    scale = whole_power(2, d);
    ret_val = fraction_from_ratio((j - i), ((i + scale) * (j + scale)));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long routes_agree_for_pair(long long i, long long j) {
    long long all_agree = 0;
    long long d = 0;
    long long match = 0;
    long long positive = 0;
    long long route_a = 0;
    long long route_a_twin = 0;
    long long route_b = 0;
    long long ret_val = 0;

    ep_gc_push_root(&d);
    ep_gc_push_root(&route_a);
    ep_gc_push_root(&route_a_twin);
    ep_gc_push_root(&route_b);
    ep_gc_push_root(&i);
    ep_gc_push_root(&j);
    ...
    d = 0;
    while (d < 11) {
    route_a = built_gap(i, j, d);
    route_b = closed_gap(i, j, d);
    match = fraction_compare(route_a, route_b) == 0;
    if (match) {
    all_agree = all_agree;
    } else {
    all_agree = 0LL;
    }
    route_a_twin = built_gap(i, j, d);
    positive = ({ long long _fap = route_a_twin; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 1;
    if (positive) {
    all_agree = all_agree;
    } else {
    all_agree = 0LL;
...
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(route_a);
    route_a = 0;
    free_struct_Fraction(route_a_twin);
    route_a_twin = 0;
    free_struct_Fraction(route_b);
    route_b = 0;
    return ret_val;
}
...
long long corpus_anchor_holds() {
    ...
    gap = built_gap(2, 3, d);
    scale = whole_power(2, d);
    corpus_form = fraction_from_ratio(1, ((2 + scale) * (3 + scale)));
    match = fraction_compare(gap, corpus_form) == 0;
    if (match) {
    all_match = all_match;
    } else {
    all_match = 0LL;
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(corpus_form);
    corpus_form = 0;
...
long long all_six_gaps_forced() {
    ...
    p23 = routes_agree_for_pair(2, 3);
    p25 = routes_agree_for_pair(2, 5);
    p27 = routes_agree_for_pair(2, 7);
    p35 = routes_agree_for_pair(3, 5);
    p37 = routes_agree_for_pair(3, 7);
    p57 = routes_agree_for_pair(5, 7);
    if (p23) {
    if (p25) {
    if (p27) {
    if (p35) {
...
long long rises_to_unison(long long p) {
    long long all_ok = 0;
    long long below = 0;
    long long current = 0;
    long long current_for_bound = 0;
    long long current_for_rise = 0;
    long long d = 0;
    long long one = 0;
    ...
    ep_gc_push_root(&current_for_bound);
    ep_gc_push_root(&current_for_rise);
    ep_gc_push_root(&d);
    ep_gc_push_root(&one);
    ep_gc_push_root(&one_twin);
    ep_gc_push_root(&previous);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    previous = running_coupling(p, 0);
    all_ok = 1LL;
    d = 1;
    while (d < 20) {
    current = running_coupling(p, d);
    current_for_rise = running_coupling(p, d);
    rising = fraction_compare(current_for_rise, previous) == 1;
    if (rising) {
    all_ok = all_ok;
    } else {
    all_ok = 0LL;
    }
    current_for_bound = running_coupling(p, d);
    one_twin = fraction_from_whole_number(1);
    below = fraction_compare(one_twin, current_for_bound) == 1;
    if (below) {
    all_ok = all_ok;
    } else {
    all_ok = 0LL;
    }
    previous = running_coupling(p, d);
    d = (d + 1);
    }
    ret_val = all_ok;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(current);
    current = 0;
    free_struct_Fraction(current_for_bound);
    current_for_bound = 0;
    free_struct_Fraction(current_for_rise);
    current_for_rise = 0;
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(one_twin);
    one_twin = 0;
    free_struct_Fraction(previous);
    previous = 0;
    return ret_val;
}
...
long long all_four_converge() {
    ...
    s2 = rises_to_unison(2);
    s3 = rises_to_unison(3);
    s5 = rises_to_unison(5);
    s7 = rises_to_unison(7);
    if (s2) {
    if (s3) {
    if (s5) {
    ret_val = s7;
...
long long gaps_shrink_for(long long i, long long j) {
    long long narrow = 0;
    long long wide = 0;
    long long ret_val = 0;

    ep_gc_push_root(&narrow);
    ep_gc_push_root(&wide);
    ep_gc_push_root(&i);
    ep_gc_push_root(&j);

    ep_gc_maybe_collect();

    wide = built_gap(i, j, 0);
    narrow = built_gap(i, j, 10);
    ret_val = fraction_compare(wide, narrow) == 1;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(narrow);
    narrow = 0;
    free_struct_Fraction(wide);
    wide = 0;
    return ret_val;
}
