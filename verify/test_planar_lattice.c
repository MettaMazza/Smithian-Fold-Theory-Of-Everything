...
long long neighbour_count();
long long neighbour_share();
long long neighbour_sum();
long long balance_law_holds();
long long neighbourhood_folds_to_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the planar lattice (the line's law extends) ===");
    ok = expect_equal((long long)"the planar neighbour count = 2 * 2 = 4", int_to_string(neighbour_count()), (long long)"4");
    ok = expect_equal((long long)"each neighbour's share = the depth-3 step 1/8", fraction_to_text(neighbour_share()), (long long)"1/8");
    ok = expect_equal((long long)"the neighbour sum, summed explicitly = 1/2", fraction_to_text(neighbour_sum()), (long long)"1/2");
    ok = expect_bool((long long)"the balance law: sum = count * centre share", balance_law_holds());
    ok = expect_bool((long long)"the neighbourhood folds to unison", neighbourhood_folds_to_unison());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long neighbour_count() {
    ...
    ret_val = (smallest_fold_period_above(1) + smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long neighbour_sum() {
    ...
    share = fraction_from_ratio(1, 8);
    count = (smallest_fold_period_above(1) + smallest_fold_period_above(1));
    total = fraction_from_whole_number(0);
    i = 0;
    while (i < count) {
    total = fraction_add(total, share);
    i = (i + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long balance_law_holds() {
    long long centre = 0;
    long long count_fraction = 0;
    long long scaled_centre = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&centre);
    ep_gc_push_root(&count_fraction);
    ep_gc_push_root(&scaled_centre);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    total = neighbour_sum();
    centre = fraction_from_ratio(1, 8);
    count_fraction = fraction_from_whole_number(neighbour_count());
    scaled_centre = fraction_multiply(count_fraction, centre);
    ret_val = fraction_compare(total, scaled_centre) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(scaled_centre);
    scaled_centre = 0;
