...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the neutrino mixing angles (PMNS) ===");
    ok = expect_equal((long long)"atmospheric sin^2(theta23) = 1/2", fraction_to_text(atmospheric_mixing()), (long long)"1/2");
    ok = expect_equal((long long)"solar sin^2(theta12) = 1/3", fraction_to_text(solar_mixing()), (long long)"1/3");
    ok = expect_equal((long long)"lepton sector tower N = 8", int_to_string(lepton_sector_count()), int_to_string(8));
    ok = expect_equal((long long)"reactor sin^2(theta13) = 1/48", fraction_to_text(reactor_mixing()), (long long)"1/48");
    printf("%s\n", (char*)(long long)"--- the fold forces the two large angles ---");
    ok = expect_equal((long long)"fold(1/2) = the One", fraction_to_text(atmospheric_fold()), (long long)"1");
    ok = expect_equal((long long)"One - 1/2 = 1/2 (self-antipodal)", fraction_to_text(atmospheric_antipode()), (long long)"1/2");
    ok = expect_equal((long long)"take(One, fold(1/3)) = 1/3 (tripling)", fraction_to_text(solar_take_of_fold()), (long long)"1/3");
    printf("%s\n", (char*)(long long)"--- forced angle vs measured (forced is exact; measured for comparison) ---");
    ok = expect_equal((long long)"atmospheric 1/2 = 0.5000 (measured ~0.5450)", fraction_to_decimal(atmospheric_mixing(), 4), (long long)"0.5000");
    ok = expect_equal((long long)"solar       1/3 = 0.3333 (measured ~0.3070)", fraction_to_decimal(solar_mixing(), 4), (long long)"0.3333");
    ok = expect_equal((long long)"reactor    1/48 = 0.0208 (measured ~0.0220)", fraction_to_decimal(reactor_mixing(), 4), (long long)"0.0208");
    ok = expect_equal((long long)"the reactor FORM (atm*solar)/N is unique (forced_unique)", int_to_string(reactor_form_is_unique()), (long long)"1");
    ok = expect_equal((long long)"no SIMPLER assembly reaches the reactor angle (engine-generated, complete)", int_to_string(reactor_assembly_is_minimal()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
