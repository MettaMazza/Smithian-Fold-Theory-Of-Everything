...
long long open_shell();
long long shell_closes_to_whole();
long long half_shells_sum_to_closed();
long long first_magic_number();
long long second_magic_number();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the nuclear shell model (magic numbers) ===");
    ok = expect_equal((long long)"a part-filled shell = the half-open balance 1/2", fraction_to_text(open_shell()), (long long)"1/2");
    ok = expect_equal((long long)"closing the shell = the fold to unison", fraction_to_text(shell_closes_to_whole()), (long long)"1");
    ok = expect_equal((long long)"two half-shells sum to the closed whole", fraction_to_text(half_shells_sum_to_closed()), (long long)"1");
    ok = expect_equal((long long)"first magic number = b*1^2 = 2 (helium)", int_to_string(first_magic_number()), (long long)"2");
    ok = expect_equal((long long)"second magic number = b*2^2 = 8 (oxygen)", int_to_string(second_magic_number()), (long long)"8");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long shell_closes_to_whole() {
    ...
    ret_val = fold_fraction(open_shell());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long half_shells_sum_to_closed() {
    long long open_half = 0;
    long long ret_val = 0;

    ep_gc_push_root(&open_half);

    ep_gc_maybe_collect();

    open_half = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(open_half, open_half);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long first_magic_number() {
    ...
    ret_val = (smallest_fold_period_above(1) * whole_power(1, 2));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long second_magic_number() {
    ...
    ret_val = (smallest_fold_period_above(1) * whole_power(2, 2));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
