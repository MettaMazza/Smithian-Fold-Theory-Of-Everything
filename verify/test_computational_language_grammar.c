...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold formal-language and grammar law ===");
    start = create_list();
    ok = expect_equal((long long)"the grammar start is the empty path at the One", int_to_string(length_list(start)), int_to_string(0));
    word = encode_computation_state(85, 7);
    ok = expect_true((long long)"a depth-7 coordinate word is generated", fold_word_is_generated(word, 7));
    ok = expect_true((long long)"the word rebuilds exactly from the grammar start", fold_words_are_equal(word, fold_grammar_rebuild(word)));
    ok = expect_equal((long long)"the depth-7 language contains b^7 words", int_to_string(fold_language_word_count(7)), int_to_string(128));
    ok = expect_equal((long long)"every word has exactly b lawful extensions", int_to_string(fold_grammar_extension_count(word)), int_to_string(2));
    ok = expect_true((long long)"an enlarged alphabet is rejected", enlarged_alphabet_is_rejected(7));
    ok = expect_true((long long)"a shortened derivation is rejected", shortened_grammar_derivation_is_rejected(7));
    ok = expect_true((long long)"the complete language and grammar census holds", complete_fold_language_grammar_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
