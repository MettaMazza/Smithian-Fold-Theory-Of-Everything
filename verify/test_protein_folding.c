...
long long native_state();
long long native_state_is_fixed();
long long starting_conformation();
long long descent_after_one_step();
long long descent_after_two_steps();
long long descent_reaches_native();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== protein folding ===");
    ok = expect_bool((long long)"native state is the fold's fixed point (fold(1) = 1)", native_state_is_fixed());
    ok = expect_equal((long long)"descent step one: fold(3/4) = 1/2", fraction_to_text(descent_after_one_step()), (long long)"1/2");
    ok = expect_equal((long long)"descent step two: fold(1/2) = 1", fraction_to_text(descent_after_two_steps()), (long long)"1");
    ok = expect_bool((long long)"descent reaches native in two steps (not a 10^50 search)", descent_reaches_native());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long native_state_is_fixed() {
    ...
    folded = fold_fraction(native_state());
    ret_val = fraction_compare(folded, native_state()) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long descent_after_one_step() {
    ...
    ret_val = fold_fraction(starting_conformation());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long descent_after_two_steps() {
    ...
    once = fold_fraction(starting_conformation());
...
long long descent_reaches_native() {
    ...
    ep_gc_push_root(&native);

    ep_gc_maybe_collect();

    reached = descent_after_two_steps();
    native = fraction_from_whole_number(1);
    ret_val = fraction_compare(reached, native) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(reached);
    reached = 0;
    free_struct_Fraction(native);
    native = 0;
    return ret_val;
}
