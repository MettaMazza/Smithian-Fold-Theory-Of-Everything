...
long long expect_number(long long label, long long got, long long want) {
    ...
    printf("%s\n", (char*)concat((long long)"    got:  ", int_to_string(got)));
    printf("%s\n", (char*)concat((long long)"    want: ", int_to_string(want)));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the gauge mediator counts ===");
    ok = expect_number((long long)"strong sector = 3", strong_sector(), 3);
    ok = expect_number((long long)"gluons = 3^2 - 1 = 8", gluon_count(), 8);
    ok = expect_number((long long)"next sector = 5", next_sector(), 5);
    ok = expect_number((long long)"next mediators = 5^2 - 1 = 24", next_sector_mediator_count(), 24);
    ok = expect_number((long long)"third sector = 7", third_sector(), 7);
    ok = expect_number((long long)"third mediators = 7^2 - 1 = 48", third_sector_mediator_count(), 48);
    ok = expect_number((long long)"electromagnetic field source = 1", electromagnetic_field_source(), 1);
    ok = expect_number((long long)"strong field source = 3", strong_field_source(), 3);
    ok = expect_number((long long)"the gluon-count FORM is unique (forced_unique)", gluon_count_form_is_unique(), 1);
    ok = expect_number((long long)"no SIMPLER assembly reaches the gluon count (engine-generated, complete)", gluon_assembly_is_minimal(), 1);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
