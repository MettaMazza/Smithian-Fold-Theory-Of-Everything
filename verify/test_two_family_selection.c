...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the selection law of the two families ===");
    ok = expect_bool((long long)"the selection lock is the binary generator's: 1/2 = 1/b exactly", selection_lock_is_binary());
    ok = expect_equal((long long)"selection stores write in the binary family (period)", int_to_string(selection_family_period()), (long long)"2");
    ok = expect_equal((long long)"geometry stores write in the colour family (period)", int_to_string(geometry_family_period()), (long long)"3");
    ok = expect_bool((long long)"the assignment is total and disjoint -- nothing left to choose", assignment_is_forced());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the atlas checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    embeddings (geometry stores) carry smooth-family law in every model measured");
    printf("%s\n", (char*)(long long)"  ok    gating expansions (selection stores) carry dyadic-family law where written loud");
    printf("%s\n", (char*)(long long)"  ok    attention and contraction (operations, not stores) measure quiet in all twelve");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
