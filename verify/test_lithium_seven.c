...
long long _main();
long long primordial_lithium();
long long depletion_factor();
long long observed_lithium();
long long observed_doubled_restores_primordial();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the lithium-7 problem (the deficit is one fold) ===");
    ok = expect_equal((long long)"primordial Li-7 share = 3/16", fraction_to_text(primordial_lithium()), (long long)"3/16");
    ok = expect_equal((long long)"stellar depletion = one fold = 1/2", fraction_to_text(depletion_factor()), (long long)"1/2");
    ok = expect_equal((long long)"observed surface share = 3/32", fraction_to_text(observed_lithium()), (long long)"3/32");
    ok = expect_bool((long long)"observed doubled restores primordial (deficit IS the fold)", observed_doubled_restores_primordial());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long primordial_lithium() {
    long long hypercube = 0;
    long long ret_val = 0;

    ep_gc_push_root(&hypercube);

    ep_gc_maybe_collect();

    hypercube = whole_power(binary_count(), 4);
    ret_val = fraction_from_ratio(colour_count(), hypercube);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long observed_lithium() {
    long long factor = 0;
    long long primordial = 0;
    long long ret_val = 0;

    ep_gc_push_root(&factor);
    ep_gc_push_root(&primordial);

    ep_gc_maybe_collect();

    primordial = primordial_lithium();
    factor = fraction_from_ratio(1, binary_count());
    ret_val = fraction_multiply(primordial, factor);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long observed_doubled_restores_primordial() {
    ...
    observed = observed_lithium();
    doubled = fraction_add(observed, observed);
    primordial = primordial_lithium();
    ret_val = fraction_compare(doubled, primordial) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(doubled);
    doubled = 0;
    free_struct_Fraction(primordial);
    primordial = 0;
    return ret_val;
}
