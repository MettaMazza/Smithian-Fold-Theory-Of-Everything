...
long long vacuum_after_fold();
long long vacuum_energy_density_is_constant();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== dark energy (w = -1) ===");
    ok = expect_equal((long long)"vacuum energy = the One", fraction_to_text(vacuum_energy()), (long long)"1");
    ok = expect_equal((long long)"fold(1) = 1 (the vacuum is fold-invariant)", fraction_to_text(vacuum_after_fold()), (long long)"1");
    ok = expect_true((long long)"vacuum energy density is constant (w = -1)", vacuum_energy_density_is_constant());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long vacuum_energy_density_is_constant() {
    ...
    before = fraction_from_whole_number(1);
    after = vacuum_after_fold();
    ret_val = fraction_compare(before, after) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
