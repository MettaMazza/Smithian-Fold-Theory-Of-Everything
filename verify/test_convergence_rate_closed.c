...
long long _main();
long long rate_coupling(long long, long long);
long long gap_by_subtraction(long long);
long long gap_closed_form(long long);
long long routes_agree_at_all_depths();
long long bare_gap_is_up_invariant();
long long gap_shrinks_everywhere();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the convergence rate, closed (one exact formula) ===");
    ok = expect_equal((long long)"the gap at depth 3: 1/((2+8)(3+8)) = 1/110", fraction_to_text(gap_closed_form(3)), (long long)"1/110");
    ok = expect_bool((long long)"both routes agree at EVERY depth 0..10", routes_agree_at_all_depths());
    ok = expect_bool((long long)"the bare gap = 1/12 -- the up-type invariant reappears", bare_gap_is_up_invariant());
    ok = expect_bool((long long)"the gap shrinks strictly at every step", gap_shrinks_everywhere());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long rate_coupling(long long m, long long d) {
    long long levels = 0;
    long long source = 0;
    long long ret_val = 0;
...
long long gap_by_subtraction(long long d) {
    long long strong = 0;
    long long weak = 0;
    long long ret_val = 0;
    ...
    strong = rate_coupling(3, d);
    weak = rate_coupling(2, d);
...
long long gap_closed_form(long long d) {
...
long long routes_agree_at_all_depths() {
    ...
    d = 0;
    while (d < 11) {
    direct = gap_by_subtraction(d);
    closed = gap_closed_form(d);
    agrees = fraction_compare(direct, closed) == 0;
    if (agrees) {
    all_agree = all_agree;
    } else {
    all_agree = 0LL;
...
long long bare_gap_is_up_invariant() {
    ...
    ep_gc_push_root(&invariant);

    ep_gc_maybe_collect();

    bare = gap_closed_form(0);
    invariant = fraction_from_ratio(1, 12);
    ret_val = fraction_compare(bare, invariant) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(bare);
    bare = 0;
    free_struct_Fraction(invariant);
    invariant = 0;
    return ret_val;
}

long long gap_shrinks_everywhere() {
    long long all_shrink = 0;
    long long comparison = 0;
    long long d = 0;
    long long here = 0;
    long long next_gap = 0;
    long long ret_val = 0;

    ep_gc_push_root(&d);
    ep_gc_push_root(&here);
    ep_gc_push_root(&next_gap);
    ...
    d = 0;
    while (d < 10) {
    here = gap_closed_form(d);
    next_gap = gap_closed_form((d + 1));
    comparison = fraction_compare(next_gap, here);
    if (comparison < 0) {
