...
long long _main();
long long rhythm_one();
long long rhythm_two();
long long beat();
long long silent_beat();
long long unison_is_silent();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== beat frequency ===");
    ok = expect_equal((long long)"rhythm one = 1/3", fold_value_to_text(rhythm_one()), (long long)"1/3");
    ok = expect_equal((long long)"rhythm two = 1/7", fold_value_to_text(rhythm_two()), (long long)"1/7");
    ok = expect_equal((long long)"beat = |1/3 - 1/7| = 4/21", fold_value_to_text(beat()), (long long)"4/21");
    ok = expect_equal((long long)"unison beat = the One (silent)", fold_value_to_text(silent_beat()), (long long)"1");
    ok = expect_bool((long long)"two identical rhythms are silent (beat = One)", unison_is_silent());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long beat() {
    ...
    first = supposed_value(1, 3);
    second = supposed_value(1, 7);
    ret_val = beat_between(first, second);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long unison_is_silent() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&quiet);

    ep_gc_maybe_collect();

    quiet = silent_beat();
    one = the_one();
    ret_val = fold_value_compare(quiet, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_FoldValue(one);
    one = 0;
    free_struct_FoldValue(quiet);
    quiet = 0;
    return ret_val;
}
