...
long long _main();
long long multiplicative_order_of_two(long long);
long long fold_period_matches_order(long long);
long long binary_is_order_of_two_mod_three();
long long colour_is_order_of_two_mod_seven();
...
long long expect_number(long long label, long long got, long long want) {
    ...
    if (got == want) {
    printf("%s\n", (char*)concat((long long)"  ok    ", concat(label, concat((long long)" = ", int_to_string(got)))));
    } else {
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the fold's orbits are the order of two ===");
    printf("%s\n", (char*)(long long)"--- order of 2 mod p, computed by plain arithmetic ---");
    ok = expect_number((long long)"order of 2 mod 3", multiplicative_order_of_two(3), 2);
    ok = expect_number((long long)"order of 2 mod 5", multiplicative_order_of_two(5), 4);
    ok = expect_number((long long)"order of 2 mod 7", multiplicative_order_of_two(7), 3);
    ok = expect_number((long long)"order of 2 mod 9", multiplicative_order_of_two(9), 6);
    ok = expect_number((long long)"order of 2 mod 11", multiplicative_order_of_two(11), 10);
    printf("%s\n", (char*)(long long)"--- the fold-orbit period matches the order (forced_to_be halts otherwise) ---");
    ok = expect_number((long long)"period(1/3) = order (both routes agree)", fold_period_matches_order(3), 2);
    ok = expect_number((long long)"period(1/5) = order", fold_period_matches_order(5), 4);
    ok = expect_number((long long)"period(1/7) = order", fold_period_matches_order(7), 3);
    ok = expect_number((long long)"period(1/9) = order", fold_period_matches_order(9), 6);
    ok = expect_number((long long)"period(1/11) = order", fold_period_matches_order(11), 10);
    printf("%s\n", (char*)(long long)"--- the two generators are the first two values of this order ---");
    ok = expect_number((long long)"binary = order of 2 mod 3", binary_is_order_of_two_mod_three(), 2);
    ok = expect_number((long long)"colour = order of 2 mod 7", colour_is_order_of_two_mod_seven(), 3);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long multiplicative_order_of_two(long long modulus) {
    long long current = 0;
    long long doubled = 0;
    long long doubled_quotient = 0;
    long long order = 0;
    long long quotient = 0;
    long long ret_val = 0;

    quotient = (2 / modulus);
    current = (2 - (quotient * modulus));
    order = 1;
    while (current > 1) {
    doubled = (current + current);
    doubled_quotient = (doubled / modulus);
    current = (doubled - (doubled_quotient * modulus));
    order = (order + 1);
    }
    ret_val = order;
    goto L_cleanup;
...
long long fold_period_matches_order(long long p) {
    long long by_arithmetic = 0;
    long long by_fold = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_arithmetic);
    ep_gc_push_root(&by_fold);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    by_fold = fold_period_of_unit_fraction(p);
    by_arithmetic = multiplicative_order_of_two(p);
    ret_val = forced_to_be((long long)"fold-orbit period of 1/p = order of 2 mod p", by_fold, by_arithmetic);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long binary_is_order_of_two_mod_three() {
    ...
    long long by_order = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_generator);
    ep_gc_push_root(&by_order);

    ep_gc_maybe_collect();

    by_generator = smallest_fold_period_above(1);
    by_order = multiplicative_order_of_two(3);
    ret_val = forced_to_be((long long)"binary = order of 2 mod 3", by_generator, by_order);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long colour_is_order_of_two_mod_seven() {
    ...
    by_generator = smallest_fold_period_above(binary_count());
    by_order = multiplicative_order_of_two(7);
    ret_val = forced_to_be((long long)"colour = order of 2 mod 7", by_generator, by_order);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
