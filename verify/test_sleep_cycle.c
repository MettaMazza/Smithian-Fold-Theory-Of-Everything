...
long long deep_sleep();
long long rem_sleep();
long long deep_folds_to_rem();
long long rem_folds_to_deep();
long long cycle_balance();
long long waking_is_the_fold_to_unison();
long long stages_partition_the_night();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the sleep cycle (a held two-state orbit) ===");
    ok = expect_equal((long long)"deep (slow-wave) sleep = 1/3", fraction_to_text(deep_sleep()), (long long)"1/3");
    ok = expect_equal((long long)"REM sleep = 2/3", fraction_to_text(rem_sleep()), (long long)"2/3");
    ok = expect_bool((long long)"deep folds to REM", deep_folds_to_rem());
    ok = expect_bool((long long)"REM folds back to deep (alternating cycle)", rem_folds_to_deep());
    ok = expect_equal((long long)"the crossing balance = 1/2", fraction_to_text(cycle_balance()), (long long)"1/2");
    ok = expect_equal((long long)"only waking is the fold of the balance to unison", fraction_to_text(waking_is_the_fold_to_unison()), (long long)"1");
    ok = expect_equal((long long)"deep + REM partition the night", fraction_to_text(stages_partition_the_night()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long deep_folds_to_rem() {
    ...
    ep_gc_push_root(&deep);
    ep_gc_push_root(&folded);
    ep_gc_push_root(&rem);

    ep_gc_maybe_collect();

    deep = fraction_from_ratio(1, colour_count());
    folded = fold_fraction(deep);
    rem = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_compare(folded, rem) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(folded);
    folded = 0;
    free_struct_Fraction(rem);
    rem = 0;
    return ret_val;
}
...
long long rem_folds_to_deep() {
    ...
    rem = fraction_from_ratio(binary_count(), colour_count());
    folded = fold_fraction(rem);
    deep = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, deep) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(deep);
    deep = 0;
    free_struct_Fraction(folded);
    folded = 0;
    return ret_val;
...
long long cycle_balance() {
    ...
    whole = fraction_add(deep_sleep(), rem_sleep());
    two = fraction_from_whole_number(binary_count());
    ret_val = fraction_divide(whole, two);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long waking_is_the_fold_to_unison() {
    ...
    ret_val = fold_fraction(cycle_balance());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long stages_partition_the_night() {
    ...
    ret_val = fraction_add(deep_sleep(), rem_sleep());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
