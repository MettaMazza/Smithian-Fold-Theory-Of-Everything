...
long long transition_state();
long long clears_the_barrier();
long long reaction_completes();
long long two_activations_make_barrier();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== reaction kinetics (the activation barrier) ===");
    ok = expect_equal((long long)"transition state = 1/4 (two folds below the product)", fraction_to_text(transition_state()), (long long)"1/4");
    ok = expect_equal((long long)"first fold clears the barrier (-> 1/2)", fraction_to_text(clears_the_barrier()), (long long)"1/2");
    ok = expect_equal((long long)"second fold completes the reaction (-> the One)", fraction_to_text(reaction_completes()), (long long)"1");
    ok = expect_equal((long long)"two activation shares make the barrier", fraction_to_text(two_activations_make_barrier()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long clears_the_barrier() {
    ...
    ret_val = fold_fraction(transition_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long reaction_completes() {
    ...
    ret_val = fold_fraction(clears_the_barrier());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
