...
long long wave_channel();
long long structural_channel();
long long channels_partition_one();
long long channels_cycle();
long long difference_stays_in_cycle();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the channel cycle (no smuggled signal) ===");
    ok = expect_equal((long long)"the two channels partition the One (2/3 + 1/3)", fraction_to_text(channels_partition_one()), (long long)"1");
    ok = expect_bool((long long)"the fold cycles them: fold(1/3) = 2/3, fold(2/3) = 1/3", channels_cycle());
    ok = expect_bool((long long)"CLOSURE: the difference is again a channel -- no third thing", difference_stays_in_cycle());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long channels_partition_one() {
    ...
    ep_gc_push_root(&wave);

    ep_gc_maybe_collect();

    wave = fraction_from_ratio(2, 3);
    structure = fraction_from_ratio(1, 3);
    ret_val = fraction_add(wave, structure);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long channels_cycle() {
    long long down_ok = 0;
    long long from_structure = 0;
    long long from_wave = 0;
    long long structure = 0;
    long long up_ok = 0;
    long long wave = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_structure);
    ep_gc_push_root(&from_wave);
    ep_gc_push_root(&structure);
    ep_gc_push_root(&wave);
    ...
    wave = fraction_from_ratio(2, 3);
    structure = fraction_from_ratio(1, 3);
    from_structure = fold_fraction(structure);
    from_wave = fold_fraction(wave);
    up_ok = fraction_compare(from_structure, wave) == 0;
    down_ok = fraction_compare(from_wave, structure) == 0;
    if (up_ok) {
    ret_val = down_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(from_structure);
    from_structure = 0;
    free_struct_Fraction(from_wave);
    from_wave = 0;
    return ret_val;
}
...
long long difference_stays_in_cycle() {
    ...
    long long folded = 0;
    long long is_member = 0;
    long long returns_to_wave = 0;
    long long structure = 0;
    long long wave = 0;
    long long ret_val = 0;
    ...
    wave = fraction_from_ratio(2, 3);
    structure = fraction_from_ratio(1, 3);
    difference = fraction_subtract(wave, structure);
    is_member = fraction_compare(difference, structural_channel()) == 0;
    folded = fold_fraction(difference);
    returns_to_wave = fraction_compare(folded, wave_channel()) == 0;
    if (is_member) {
    ret_val = returns_to_wave;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
