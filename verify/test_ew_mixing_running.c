...
long long _main();
long long charged_coupling(long long);
long long neutral_coupling();
long long mixing_at(long long);
long long bare_mixing_is_half();
long long running_is_strictly_decreasing();
long long curve_crosses_between_nine_and_ten(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the running weak mixing (1/2 down through the measured value) ===");
    ok = expect_bool((long long)"BARE: the two channels born equal -> sin2 = 1/2 exactly", bare_mixing_is_half());
    ok = expect_equal((long long)"the mixing at level 3: 25/89", fraction_to_text(mixing_at(3)), (long long)"25/89");
    ok = expect_bool((long long)"the running falls STRICTLY at every level through 15", running_is_strictly_decreasing());
    ok = expect_equal((long long)"sin2 at level 9 (exact rational)", fraction_to_text(mixing_at(9)), (long long)"121/521");
    measured = fraction_from_ratio(23113, 100000);
    ok = expect_bool((long long)"the curve CROSSES measured 0.23113 between levels 9 and 10", curve_crosses_between_nine_and_ten(measured));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(measured);
    measured = 0;
    return ret_val;
}

long long charged_coupling(long long k) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio((k + 1), (k + 2));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long mixing_at(long long k) {
    long long c = 0;
    long long c_squared = 0;
    long long c_twin = 0;
    long long n = 0;
    long long n_squared = 0;
    long long n_twin = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&c);
    ep_gc_push_root(&c_squared);
    ep_gc_push_root(&c_twin);
    ep_gc_push_root(&n);
    ep_gc_push_root(&n_squared);
    ep_gc_push_root(&n_twin);
    ep_gc_push_root(&total);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    c = fraction_from_ratio((k + 1), (k + 2));
    c_twin = fraction_from_ratio((k + 1), (k + 2));
    c_squared = fraction_multiply(c, c_twin);
    n = fraction_from_ratio(1, 2);
    n_twin = fraction_from_ratio(1, 2);
    n_squared = fraction_multiply(n, n_twin);
    total = fraction_add(c_squared, n_squared);
    ret_val = fraction_divide(n_squared, total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    return ret_val;
...
long long bare_mixing_is_half() {
    ...
    ret_val = fraction_compare(bare, half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(bare);
    bare = 0;
...
long long running_is_strictly_decreasing() {
    ...
    while (k < 15) {
    here = mixing_at(k);
    next_level = mixing_at((k + 1));
    comparison = fraction_compare(next_level, here);
    if (comparison < 0) {
    falling = falling;
    } else {
    falling = 0LL;
...
long long curve_crosses_between_nine_and_ten(long long target) {
    long long above = 0;
    long long at_nine = 0;
    long long at_ten = 0;
    long long below = 0;
    long long nine_above = 0;
    long long ten_below = 0;
    long long ret_val = 0;

    ep_gc_push_root(&at_nine);
    ep_gc_push_root(&at_ten);
    ...
    at_nine = mixing_at(9);
    at_ten = mixing_at(10);
    above = fraction_compare(at_nine, target);
    below = fraction_compare(at_ten, target);
    nine_above = above > 0;
    ten_below = below < 0;
    if (nine_above) {
    ret_val = ten_below;
