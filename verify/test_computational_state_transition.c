...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold finite-state transition law ===");
    ok = expect_equal((long long)"census depth forced by cover(c^(c+1)) and b+c+b", int_to_string(computation_census_depth()), int_to_string(7));
    ok = expect_equal((long long)"depth 1 has b states", int_to_string(computation_state_count(1)), int_to_string(2));
    ok = expect_equal((long long)"depth 7 has b^7 states", int_to_string(computation_state_count(7)), int_to_string(128));
    ok = expect_true((long long)"complete state, step, preimage, closure, and composition census", complete_computation_transition_census());
    ok = expect_true((long long)"shifted upper-preimage pairing is rejected", shifted_preimage_pairing_is_rejected(7));
    ok = expect_true((long long)"the deepest first state does not close one step early", first_state_does_not_close_early(7));
    ok = expect_true((long long)"a nonbinary branch count is rejected", nonbinary_branch_count_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
