...
long long reactant_state();
long long reaction_completes();
long long below_lock_cannot_complete();
long long activation_energy();
long long enthalpy_released();
long long reversal_repays_exactly();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== reaction thermodynamics (barrier and drop) ===");
    ok = expect_equal((long long)"the reactant state: the balanced bond 1/2", fraction_to_text(reactant_state()), (long long)"1/2");
    ok = expect_bool((long long)"the reaction completes: fold(1/2) = 1 (the closed shell)", reaction_completes());
    ok = expect_bool((long long)"ACTIVATION: below the lock, one step cannot complete", below_lock_cannot_complete());
    ok = expect_equal((long long)"the activation lift = 1/4 (paid, not kept)", fraction_to_text(activation_energy()), (long long)"1/4");
    ok = expect_equal((long long)"the enthalpy released = 1/2 (kept)", fraction_to_text(enthalpy_released()), (long long)"1/2");
    ok = expect_bool((long long)"reversal repays exactly what the forward step released", reversal_repays_exactly());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long reaction_completes() {
    long long one = 0;
    long long product = 0;
    long long reactant = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&product);
    ep_gc_push_root(&reactant);

    ep_gc_maybe_collect();

    reactant = fraction_from_ratio(1, 2);
    product = fold_fraction(reactant);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(product, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long below_lock_cannot_complete() {
    ...
    below = fraction_from_ratio(1, 4);
    folded = fold_fraction(below);
    one = fraction_from_whole_number(1);
    completes = fraction_compare(folded, one) == 0;
    if (completes) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long activation_energy() {
    ...
    below = fraction_from_ratio(1, 4);
    ret_val = fraction_subtract(lock, below);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long enthalpy_released() {
    ...
    one = fraction_from_whole_number(1);
    reactant = fraction_from_ratio(1, 2);
    ret_val = fraction_subtract(one, reactant);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long reversal_repays_exactly() {
    long long cost_back = 0;
    long long lock = 0;
    long long one = 0;
    long long released = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cost_back);
    ep_gc_push_root(&lock);
    ep_gc_push_root(&one);
    ...
    released = enthalpy_released();
    one = fraction_from_whole_number(1);
    lock = fraction_from_ratio(1, 2);
    cost_back = fraction_subtract(one, lock);
    ret_val = fraction_compare(released, cost_back) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(cost_back);
    cost_back = 0;
