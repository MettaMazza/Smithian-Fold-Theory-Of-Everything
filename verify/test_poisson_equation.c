...
long long poisson_coefficient();
long long coefficient_is_six();
long long coefficient_in_dimension(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Poisson equation (nabla^2 = d*m) ===");
    ok = expect_equal((long long)"expansion factor m = binary = 2", int_to_string(expansion_factor()), (long long)"2");
    ok = expect_equal((long long)"spatial dimensions d = colour = 3", int_to_string(spatial_dimensions()), (long long)"3");
    ok = expect_equal((long long)"Poisson coefficient nabla^2 = d*m = 6", int_to_string(poisson_coefficient()), (long long)"6");
    ok = expect_bool((long long)"in three dimensions it is 6 (the coordination number)", coefficient_is_six());
    ok = expect_equal((long long)"scales with dimension: 2D -> 4", int_to_string(coefficient_in_dimension(2)), (long long)"4");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long poisson_coefficient() {
    ...
    ret_val = (smallest_fold_period_above(binary_count()) * smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long coefficient_is_six() {
    ...
    ret_val = (smallest_fold_period_above(binary_count()) * smallest_fold_period_above(1)) == 6;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long coefficient_in_dimension(long long d) {
    long long ret_val = 0;

    ret_val = (d * smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
