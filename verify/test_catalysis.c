...
long long bare_barrier();
long long catalysed_barrier();
long long catalyst_lowers_barrier();
long long catalyst_returns_to_itself();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== catalysis ===");
    ok = expect_equal((long long)"bare barrier = 1/2 (the balance threshold)", fraction_to_text(bare_barrier()), (long long)"1/2");
    ok = expect_equal((long long)"catalysed barrier = 1/4 (a lower path)", fraction_to_text(catalysed_barrier()), (long long)"1/4");
    ok = expect_bool((long long)"the catalyst LOWERS the barrier (1/4 < 1/2)", catalyst_lowers_barrier());
    ok = expect_bool((long long)"the catalyst is conserved (returns to itself, unconsumed)", catalyst_returns_to_itself());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long catalysed_barrier() {
    ...
    bare = fraction_from_ratio(1, binary_count());
    binary = fraction_from_whole_number(binary_count());
    ret_val = fraction_divide(bare, binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long catalyst_lowers_barrier() {
    ...
    low = catalysed_barrier();
    high = fraction_from_ratio(1, binary_count());
    order = fraction_compare(low, high);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long catalyst_returns_to_itself() {
    long long catalyst = 0;
    long long after = 0;
    long long ret_val = 0;

    ep_gc_push_root(&catalyst);
    ep_gc_push_root(&after);

    ep_gc_maybe_collect();

    catalyst = fraction_from_whole_number(1);
    after = fold_fraction(catalyst);
    ret_val = fraction_compare(after, catalyst) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
