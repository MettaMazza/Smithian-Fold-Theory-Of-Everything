...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== self-simulation nesting (the regress halts at two) ===");
    ok = expect_equal((long long)"the closure state = 1/4", fraction_to_text(closure_state()), (long long)"1/4");
    ok = expect_bool((long long)"two nests reach unison (1/4 -> 1/2 -> 1)", nesting_reaches_unison());
    ok = expect_bool((long long)"the THIRD nest is the identity: no deeper level exists", third_nest_is_identity());
    ok = expect_equal((long long)"the nesting depth, walked = 2", int_to_string(nesting_depth()), (long long)"2");
    ok = expect_bool((long long)"the depth equals the lock's denominator (independent count)", depth_matches_lock_denominator());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
