...
long long lower_value_folds_to();
long long upper_value_folds_to();
long long fold_loses_information();
long long entropy_bits_per_step();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the arrow of time ===");
    ok = expect_equal((long long)"fold(1/4) = 1/2", fraction_to_text(lower_value_folds_to()), (long long)"1/2");
    ok = expect_equal((long long)"fold(3/4) = 1/2 (same image, distinct source)", fraction_to_text(upper_value_folds_to()), (long long)"1/2");
    ok = expect_true((long long)"the fold loses information (non-injective)", fold_loses_information());
    ok = expect_equal((long long)"entropy lost per step (bits)", int_to_string(entropy_bits_per_step()), int_to_string(1));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fold_loses_information() {
    long long sources_differ = 0;
    long long images_equal = 0;
    long long three_quarter = 0;
    long long quarter = 0;
    long long from_upper = 0;
    long long from_lower = 0;
    long long ret_val = 0;

    ep_gc_push_root(&three_quarter);
    ...
    from_lower = lower_value_folds_to();
    from_upper = upper_value_folds_to();
    images_equal = fraction_compare(from_lower, from_upper) == 0;
    quarter = fraction_from_ratio(1, 4);
    three_quarter = fraction_from_ratio(3, 4);
    sources_differ = fraction_compare(quarter, three_quarter) == 0;
    if (images_equal) {
    if (sources_differ) {
    ret_val = 0LL;
    goto L_cleanup;
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(three_quarter);
    three_quarter = 0;
