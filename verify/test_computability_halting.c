...
long long fold_repeated(long long, long long);
long long bounded_configuration(long long);
long long halts_after_depth_folds(long long);
long long not_halted_one_step_early(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== computability and halting ===");
    ok = expect_equal((long long)"bounded configuration at depth 4 = 1/16", fraction_to_text(bounded_configuration(4)), (long long)"1/16");
    ok = expect_bool((long long)"a depth-4 config HALTS (reaches the One) after 4 folds", halts_after_depth_folds(4));
    ok = expect_bool((long long)"a depth-6 config halts after exactly 6 folds", halts_after_depth_folds(6));
    ok = expect_bool((long long)"it has NOT halted one step early (depth 4)", not_halted_one_step_early(4));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fold_repeated(long long size, long long repetitions) {
    long long current = 0;
    long long count = 0;
    long long ret_val = 0;
...
long long bounded_configuration(long long depth) {
    ...
    denominator = whole_power(binary_count(), depth);
...
long long halts_after_depth_folds(long long depth) {
    long long config = 0;
    long long final = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&config);
    ...
    config = bounded_configuration(depth);
    final = fold_repeated(config, depth);
...
long long not_halted_one_step_early(long long depth) {
    long long one = 0;
    long long early = 0;
    long long reached = 0;
    long long almost = 0;
    ...
    config = bounded_configuration(depth);
    early = (depth - 1);
    almost = fold_repeated(config, early);
    one = fraction_from_whole_number(1);
    reached = fraction_compare(almost, one) == 0;
    if (reached) {
    ret_val = 0LL;
    goto L_cleanup;
    }
