...
long long _main();
long long built_mass_part(long long);
long long closed_mass_part(long long);
long long mass_part_routes_agree(long long);
long long known_chain_anchors_hold();
long long smithion_to_electron(long long);
long long chain_ordering_holds();
...
long long up_mass_part();
long long down_mass_part();
long long electron_is_preimage();
long long quark_pair_is_orbit();
long long down_outweighs_up();
long long neutrino_part_is_excluded();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Smithion masses on the one fermion chain ===");
    ok = expect_bool((long long)"ROUTE A = ROUTE B at the electroweak sector (1/2)", mass_part_routes_agree(2));
    ok = expect_bool((long long)"ROUTE A = ROUTE B at the strong sector (1/3)", mass_part_routes_agree(3));
    ok = expect_bool((long long)"ROUTE A = ROUTE B at the penta sector (1/5)", mass_part_routes_agree(5));
    ok = expect_bool((long long)"ROUTE A = ROUTE B at the hepta sector (1/7)", mass_part_routes_agree(7));
    ok = expect_bool((long long)"THE ANCHORS: electron 1/2 and up-quark 1/3 are the SAME rule at p=2,3", known_chain_anchors_hold());
    ok = expect_equal((long long)"the PENTA-Smithion mass-part (forced)", fraction_to_text(built_mass_part(5)), (long long)"1/5");
    ok = expect_equal((long long)"the HEPTA-Smithion mass-part (forced)", fraction_to_text(built_mass_part(7)), (long long)"1/7");
    ok = expect_equal((long long)"lightest penta-Smithion / electron = 2/5 (dimensionless, forced)", fraction_to_text(smithion_to_electron(5)), (long long)"2/5");
    ok = expect_equal((long long)"lightest hepta-Smithion / electron = 2/7 (dimensionless, forced)", fraction_to_text(smithion_to_electron(7)), (long long)"2/7");
    ok = expect_bool((long long)"the chain ordering: 1/2 > 1/3 > 1/5 > 1/7, all in the domain", chain_ordering_holds());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long built_mass_part(long long p) {
    ...
    coupling = fraction_subtract(one, shortfall);
    one_twin = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one_twin, coupling);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long closed_mass_part(long long p) {
    long long ret_val = 0;
...
long long mass_part_routes_agree(long long p) {
    long long built = 0;
    long long closed = 0;
    long long ret_val = 0;
    ...
    built = built_mass_part(p);
    closed = fraction_from_ratio(1, p);
    ret_val = fraction_compare(built, closed) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(built);
    built = 0;
...
long long known_chain_anchors_hold() {
    ...
    long long electron_ok = 0;
    long long rule_at_three = 0;
    long long rule_at_two = 0;
    long long up = 0;
    long long up_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&electron);
    ep_gc_push_root(&rule_at_three);
    ep_gc_push_root(&rule_at_two);
    ep_gc_push_root(&up);

    ep_gc_maybe_collect();

    electron = electron_mass_part();
    rule_at_two = built_mass_part(2);
    electron_ok = fraction_compare(electron, rule_at_two) == 0;
    up = up_mass_part();
    rule_at_three = built_mass_part(3);
    up_ok = fraction_compare(up, rule_at_three) == 0;
    if (electron_ok) {
    ret_val = up_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(electron);
    electron = 0;
    free_struct_Fraction(rule_at_three);
    rule_at_three = 0;
    free_struct_Fraction(rule_at_two);
    rule_at_two = 0;
    free_struct_Fraction(up);
    up = 0;
    return ret_val;
}

long long smithion_to_electron(long long p) {
    long long electron = 0;
    long long smithion = 0;
    long long ret_val = 0;

    ep_gc_push_root(&electron);
    ep_gc_push_root(&smithion);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    smithion = built_mass_part(p);
    electron = electron_mass_part();
    ret_val = fraction_divide(smithion, electron);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long chain_ordering_holds() {
    long long electroweak = 0;
    long long first_step = 0;
    long long hepta = 0;
    long long hepta_twin = 0;
    long long penta = 0;
    long long penta_twin = 0;
    long long positive = 0;
    long long second_step = 0;
    long long strong = 0;
    long long strong_twin = 0;
    long long third_step = 0;
    long long ret_val = 0;

    ep_gc_push_root(&electroweak);
    ep_gc_push_root(&hepta);
    ep_gc_push_root(&hepta_twin);
    ep_gc_push_root(&penta);
    ep_gc_push_root(&penta_twin);
    ep_gc_push_root(&strong);
    ep_gc_push_root(&strong_twin);

    ep_gc_maybe_collect();

    electroweak = built_mass_part(2);
    strong = built_mass_part(3);
    first_step = fraction_compare(electroweak, strong) == 1;
    strong_twin = built_mass_part(3);
    penta = built_mass_part(5);
    second_step = fraction_compare(strong_twin, penta) == 1;
    penta_twin = built_mass_part(5);
    hepta = built_mass_part(7);
    third_step = fraction_compare(penta_twin, hepta) == 1;
    hepta_twin = built_mass_part(7);
    positive = ({ long long _fap = hepta_twin; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 1;
    if (first_step) {
    if (second_step) {
    if (third_step) {
    ret_val = positive;
...
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(electroweak);
    electroweak = 0;
    free_struct_Fraction(hepta);
    hepta = 0;
    free_struct_Fraction(hepta_twin);
    hepta_twin = 0;
    free_struct_Fraction(penta);
    penta = 0;
    free_struct_Fraction(penta_twin);
    penta_twin = 0;
    free_struct_Fraction(strong);
    strong = 0;
    free_struct_Fraction(strong_twin);
    strong_twin = 0;
    return ret_val;
}
...
long long electron_mass_part() {
    ...
    ep_gc_push_root(&g_ew);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    g_ew = fraction_from_ratio(1, 2);
    ret_val = fraction_subtract(one, g_ew);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long up_mass_part() {
    ...
    one = fraction_from_whole_number(1);
    g_s = fraction_from_ratio(2, 3);
    ret_val = fraction_subtract(one, g_s);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long down_mass_part() {
    ...
    one = fraction_from_whole_number(1);
    up = up_mass_part();
    ret_val = fraction_subtract(one, up);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long electron_is_preimage() {
    ...
    part = electron_mass_part();
    folded = fold_fraction(part);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(folded, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long quark_pair_is_orbit() {
    long long down = 0;
    long long down_to_up = 0;
    long long from_down = 0;
    long long from_up = 0;
    long long up = 0;
    long long up_to_down = 0;
    long long ret_val = 0;

    ep_gc_push_root(&down);
    ep_gc_push_root(&from_down);
    ep_gc_push_root(&from_up);
    ep_gc_push_root(&up);
    ...
    up = up_mass_part();
    down = down_mass_part();
    from_up = fold_fraction(up);
    from_down = fold_fraction(down);
    up_to_down = fraction_compare(from_up, down) == 0;
    down_to_up = fraction_compare(from_down, up) == 0;
    if (up_to_down) {
    ret_val = down_to_up;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(from_down);
    from_down = 0;
    free_struct_Fraction(from_up);
    from_up = 0;
    return ret_val;
}
...
long long down_outweighs_up() {
    ...
    up = up_mass_part();
    down = down_mass_part();
    comparison = fraction_compare(down, up);
...
long long neutrino_part_is_excluded() {
    long long hand = 0;
    long long same_hand = 0;
    long long separation = 0;
    long long ret_val = 0;

    ep_gc_push_root(&hand);
    ep_gc_push_root(&same_hand);
    ep_gc_push_root(&separation);

    ep_gc_maybe_collect();

    hand = fraction_from_ratio(1, 2);
    same_hand = fraction_from_ratio(1, 2);
    separation = fraction_subtract(hand, same_hand);
    ret_val = ({ long long _fap = separation; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
