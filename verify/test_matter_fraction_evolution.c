...
long long _main();
long long vacuum_today();
long long matter_today();
long long matter_fraction_at(long long);
long long dilution_exponent();
long long today_returns_budget();
long long fraction_grows_into_past();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the matter fraction's history (exact curve) ===");
    ok = expect_equal((long long)"the dilution exponent = the counted d = 3", int_to_string(dilution_exponent()), (long long)"3");
    ok = expect_bool((long long)"today the curve returns the forced budget: Omega_m(1) = 1/3", today_returns_budget());
    ok = expect_equal((long long)"at half scale: Omega_m(2) = 4/5 (matter was 80 percent)", fraction_to_text(matter_fraction_at(2)), (long long)"4/5");
    ok = expect_equal((long long)"at third scale: Omega_m(3) = 27/29", fraction_to_text(matter_fraction_at(3)), (long long)"27/29");
    ok = expect_bool((long long)"the fraction grows strictly into the past (matter domination)", fraction_grows_into_past());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long matter_fraction_at(long long z) {
    long long cube = 0;
    long long matter = 0;
    long long matter_grown = 0;
    long long total = 0;
    long long vacuum = 0;
    long long z_cubed = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cube);
    ep_gc_push_root(&matter);
    ep_gc_push_root(&matter_grown);
    ep_gc_push_root(&total);
    ep_gc_push_root(&vacuum);
    ep_gc_push_root(&z_cubed);

    ep_gc_maybe_collect();

    cube = ((z * z) * z);
    matter = fraction_from_ratio(1, 3);
    z_cubed = fraction_from_whole_number(cube);
    matter_grown = fraction_multiply(matter, z_cubed);
    vacuum = fraction_from_ratio(2, 3);
    total = fraction_add(vacuum, matter_grown);
    ret_val = fraction_divide(matter_grown, total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long today_returns_budget() {
    ...
    ep_gc_push_root(&today);

    ep_gc_maybe_collect();

    today = matter_fraction_at(1);
    budget = fraction_from_ratio(1, 3);
    ret_val = fraction_compare(today, budget) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(budget);
    budget = 0;
    free_struct_Fraction(today);
    today = 0;
    return ret_val;
}
...
long long fraction_grows_into_past() {
    ...
    long long rising_one = 0;
    long long rising_two = 0;
    long long second_step = 0;
    long long ret_val = 0;

    ep_gc_push_root(&at_one);
    ep_gc_push_root(&at_three);
    ep_gc_push_root(&at_two);

    ep_gc_maybe_collect();

    at_one = matter_fraction_at(1);
    at_two = matter_fraction_at(2);
    at_three = matter_fraction_at(3);
    first_step = fraction_compare(at_two, at_one);
    second_step = fraction_compare(at_three, at_two);
    rising_one = first_step > 0;
    rising_two = second_step > 0;
    if (rising_one) {
    ret_val = rising_two;
    goto L_cleanup;
    }
