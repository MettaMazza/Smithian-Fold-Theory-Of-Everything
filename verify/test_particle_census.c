...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the complete particle census: closed and finite ===");
    ok = expect_bool((long long)"KNOWN ANCHORS: 3 weak carriers (W+,W-,Z) and 8 gluons from p^2-1", known_anchors_hold());
    ok = expect_equal((long long)"penta-sector carriers 5^2-1 (PREDICTED)", int_to_string(census_carriers(5)), (long long)"24");
    ok = expect_equal((long long)"hepta-sector carriers 7^2-1 (PREDICTED)", int_to_string(census_carriers(7)), (long long)"48");
    ok = expect_equal((long long)"total gauge carriers 3+8+24+48", int_to_string(total_gauge_carriers()), (long long)"83");
    ok = expect_equal((long long)"still to be discovered: 24+48", int_to_string(undiscovered_carriers()), (long long)"72");
    ok = expect_equal((long long)"the SMITHIONS: 2 kinds x 3 generations x 2 new sectors", int_to_string(smithion_count()), (long long)"12");
    ok = expect_bool((long long)"all four sectors qualify: partition + carry at 2, 3, 5, 7", all_sectors_qualify());
    ok = expect_equal((long long)"the sector count is exactly FOUR", int_to_string(prime_sector_count()), (long long)"4");
    ok = expect_equal((long long)"the first prime beyond the ceiling: 11 > 7", int_to_string(first_prime_beyond_ceiling()), (long long)"11");
    ok = expect_bool((long long)"THE LIST ENDS: no fifth sector -- the census is closed", census_is_closed());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
