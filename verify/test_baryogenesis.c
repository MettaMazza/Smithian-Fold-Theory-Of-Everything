...
long long baryon_number_violated();
long long cp_violated();
long long departed_from_equilibrium();
long long matter_excess_survives();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== baryogenesis (the three Sakharov conditions) ===");
    ok = expect_true((long long)"1. baryon-number violation (fold is two-to-one)", baryon_number_violated());
    ok = expect_true((long long)"2. C and CP violation (opposite-handed preimages)", cp_violated());
    ok = expect_true((long long)"3. departure from equilibrium (non-injective fold)", departed_from_equilibrium());
    ok = expect_true((long long)"=> a matter excess survives (matter, not antimatter)", matter_excess_survives());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long baryon_number_violated() {
    long long upper = 0;
    long long lower_image = 0;
    long long same_source = 0;
    long long same_image = 0;
    long long upper_image = 0;
    long long lower = 0;
    long long ret_val = 0;

    ep_gc_push_root(&upper);
    ep_gc_push_root(&lower_image);
    ep_gc_push_root(&upper_image);
    ep_gc_push_root(&lower);

    ep_gc_maybe_collect();

    lower_image = fold_fraction(fraction_from_ratio(1, 4));
    upper_image = fold_fraction(fraction_from_ratio(3, 4));
    same_image = fraction_compare(lower_image, upper_image) == 0;
    lower = fraction_from_ratio(1, 4);
    upper = fraction_from_ratio(3, 4);
    same_source = fraction_compare(lower, upper) == 0;
    if (same_image) {
    if (same_source) {
    ret_val = 0LL;
    goto L_cleanup;
...
L_cleanup:
    ...
    upper = 0;
    free_struct_Fraction(lower_image);
    lower_image = 0;
    free_struct_Fraction(upper_image);
    upper_image = 0;
...
long long cp_violated() {
    long long lower = 0;
    long long upper = 0;
    long long lower_below = 0;
    long long half_high = 0;
    long long upper_above = 0;
    long long half_low = 0;
    long long ret_val = 0;

    ep_gc_push_root(&lower);
    ep_gc_push_root(&upper);
    ep_gc_push_root(&half_high);
    ep_gc_push_root(&half_low);

    ep_gc_maybe_collect();

    lower = fraction_from_ratio(1, 4);
    half_low = fraction_from_ratio(1, 2);
    lower_below = fraction_compare(lower, half_low);
    upper = fraction_from_ratio(3, 4);
    half_high = fraction_from_ratio(1, 2);
    upper_above = fraction_compare(upper, half_high);
    if (lower_below < 0) {
    if (upper_above > 0) {
    ret_val = 1LL;
    goto L_cleanup;
...
long long departed_from_equilibrium() {
    ...
    lower_image = fold_fraction(fraction_from_ratio(1, 4));
    upper_image = fold_fraction(fraction_from_ratio(3, 4));
    ret_val = fraction_compare(lower_image, upper_image) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(lower_image);
...
long long matter_excess_survives() {
    long long condition_two = 0;
    long long condition_three = 0;
    long long condition_one = 0;
    long long ret_val = 0;

    condition_one = baryon_number_violated();
    condition_two = cp_violated();
    condition_three = departed_from_equilibrium();
    if (condition_one) {
    if (condition_two) {
    if (condition_three) {
    ret_val = 1LL;
