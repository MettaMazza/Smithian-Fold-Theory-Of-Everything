...
long long unintegrated_state();
long long fold_times(long long, long long);
long long never_reaches_unison_within(long long);
long long orbit_returns_in_two();
long long cycle_completes_the_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the introspection limit (a mind cannot fully read itself) ===");
    ok = expect_equal((long long)"the unintegrated state = 1/3", fraction_to_text(unintegrated_state()), (long long)"1/3");
    ok = expect_bool((long long)"its orbit NEVER reaches unison (horizon 12)", never_reaches_unison_within(12));
    ok = expect_bool((long long)"the orbit is closed with period 2", orbit_returns_in_two());
    ok = expect_equal((long long)"conscious + unintegrated = the whole (1/3 + 2/3 = 1)", fraction_to_text(cycle_completes_the_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fold_times(long long size, long long count) {
    long long current = 0;
    long long step = 0;
    long long ret_val = 0;
...
long long never_reaches_unison_within(long long horizon) {
    long long current = 0;
    long long one = 0;
    long long reached = 0;
    long long step = 0;
...
long long orbit_returns_in_two() {
    long long after_two = 0;
    long long start = 0;
    long long ret_val = 0;

    ep_gc_push_root(&after_two);
    ep_gc_push_root(&start);

    ep_gc_maybe_collect();

    start = fraction_from_ratio(1, colour_count());
    after_two = fold_times(start, 2);
    ret_val = fraction_compare(after_two, start) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(after_two);
    after_two = 0;
    return ret_val;
}
...
long long cycle_completes_the_whole() {
    ...
    partner = fold_fraction(state);
    ret_val = fraction_add(state, partner);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
