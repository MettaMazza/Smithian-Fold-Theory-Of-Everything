...
long long _main();
long long replicative_limit(long long);
long long soma_is_mortal_germ_is_immortal();
long long spike_threshold();
long long spike_is_atomic();
long long cancer_is_lost_descent();
long long ecosystem_orbit_is_periodic();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== biology from the fold: aging, the spike, cancer, ecosystems ===");
    ok = expect_equal((long long)"AGING: the replicative limit of soma 1/48 (2-adic valuation)", int_to_string(replicative_limit(48)), (long long)"4");
    ok = expect_equal((long long)"the germ line 1/3 is eternal: zero decay", int_to_string(replicative_limit(3)), (long long)"0");
    ok = expect_bool((long long)"MORTAL soma (1/64 decays) vs IMMORTAL germ (1/3 eternal)", soma_is_mortal_germ_is_immortal());
    ok = expect_equal((long long)"the NEURAL SPIKE threshold: the lock 1/2", fraction_to_text(spike_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"the spike is ATOMIC: all-or-nothing fold to unison", spike_is_atomic());
    ok = expect_bool((long long)"CANCER = lost descent: cycling where a mortal transient was required", cancer_is_lost_descent());
    ok = expect_bool((long long)"ECOSYSTEM: the 3/5 predator-prey orbit is periodic (stable)", ecosystem_orbit_is_periodic());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long replicative_limit(long long q) {
    long long ret_val = 0;
...
long long soma_is_mortal_germ_is_immortal() {
    long long germ = 0;
    long long germ_immortal = 0;
    long long soma = 0;
    long long soma_mortal = 0;
    long long ret_val = 0;

    soma = transient_length(64);
    soma_mortal = soma > 0;
    germ = transient_length(3);
    germ_immortal = germ == 0;
    if (soma_mortal) {
    ret_val = germ_immortal;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long spike_is_atomic() {
    long long below_one = 0;
    long long fired = 0;
    long long one = 0;
    long long reaches = 0;
    long long threshold = 0;
    long long threshold_check = 0;
    long long threshold_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&fired);
    ep_gc_push_root(&one);
    ep_gc_push_root(&threshold);
    ep_gc_push_root(&threshold_check);
    ep_gc_push_root(&threshold_twin);

    ep_gc_maybe_collect();

    threshold = spike_threshold();
    threshold_twin = spike_threshold();
    fired = fraction_add(threshold, threshold_twin);
    one = fraction_from_whole_number(1);
    reaches = fraction_compare(fired, one) == 0;
    threshold_check = spike_threshold();
    below_one = fraction_compare(threshold_check, one) < 0;
    if (reaches) {
    ret_val = below_one;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(fired);
    fired = 0;
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(threshold_check);
    threshold_check = 0;
    return ret_val;
}

long long cancer_is_lost_descent() {
    long long cancer_cycles = 0;
    long long cancerous = 0;
    long long healthy = 0;
    long long healthy_descends = 0;
    long long ret_val = 0;

    healthy = transient_length(8);
    healthy_descends = healthy > 0;
    cancerous = transient_length(3);
    cancer_cycles = cancerous == 0;
    if (healthy_descends) {
    ret_val = cancer_cycles;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long ecosystem_orbit_is_periodic() {
    ...
    period = residue_orbit_length(3, 5);
    ret_val = period > 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
