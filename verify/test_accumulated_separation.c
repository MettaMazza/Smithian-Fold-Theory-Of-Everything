...
long long _main();
long long separation_term(long long);
long long coupling_difference(long long);
long long accumulated_to(long long);
long long differences_to(long long);
long long sums_agree_everywhere();
long long terms_halve_at_least();
long long total_is_bracketed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the accumulated separation (the total disagreement is finite) ===");
    ok = expect_equal((long long)"the first term = 1/12", fraction_to_text(separation_term(0)), (long long)"1/12");
    ok = expect_bool((long long)"both partial sums agree exactly at every N = 0..10", sums_agree_everywhere());
    ok = expect_bool((long long)"CONVERGENCE: from d = 1 on, every next term is under HALF the current", terms_halve_at_least());
    ok = expect_bool((long long)"the total is bracketed forever: 1/12 <= total < 11/60", total_is_bracketed());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long separation_term(long long d) {
    ...
    levels = whole_power(binary_count(), d);
    denominator = ((2 + levels) * (3 + levels));
...
long long coupling_difference(long long d) {
    long long levels = 0;
    long long strong = 0;
    long long strong_source = 0;
    long long weak = 0;
    long long weak_source = 0;
    long long ret_val = 0;

    ep_gc_push_root(&strong);
    ep_gc_push_root(&strong_source);
    ep_gc_push_root(&weak);
    ep_gc_push_root(&weak_source);
    ep_gc_push_root(&d);

    ep_gc_maybe_collect();

    levels = whole_power(binary_count(), d);
    strong_source = (3 + levels);
    weak_source = (2 + levels);
    strong = fraction_from_ratio((strong_source - 1), strong_source);
    weak = fraction_from_ratio((weak_source - 1), weak_source);
    ret_val = fraction_subtract(strong, weak);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long accumulated_to(long long n) {
    long long d = 0;
    long long term = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    d = 0;
    while (d < (n + 1)) {
    term = separation_term(d);
    total = fraction_add(total, term);
    d = (d + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long differences_to(long long n) {
    long long d = 0;
    long long term = 0;
    long long total = 0;
    long long ret_val = 0;
...
long long sums_agree_everywhere() {
    ...
    n = 0;
    while (n < 11) {
    route_a = accumulated_to(n);
    route_b = differences_to(n);
    agrees = fraction_compare(route_a, route_b) == 0;
    if (agrees) {
    all_agree = all_agree;
    } else {
    all_agree = 0LL;
...
long long terms_halve_at_least() {
    long long all_halve = 0;
    long long comparison = 0;
    long long d = 0;
    long long half_here = 0;
    long long here = 0;
    ...
    ep_gc_push_root(&d);
    ep_gc_push_root(&half_here);
    ep_gc_push_root(&here);
    ep_gc_push_root(&next_term);
    ep_gc_push_root(&two);
    ...
    d = 1;
    while (d < 10) {
    here = separation_term(d);
    two = fraction_from_whole_number(2);
    half_here = fraction_divide(here, two);
    next_term = separation_term((d + 1));
    comparison = fraction_compare(next_term, half_here);
    if (comparison < 0) {
    all_halve = all_halve;
    } else {
    all_halve = 0LL;
...
long long total_is_bracketed() {
    ...
    partial = accumulated_to(10);
    lower = fraction_from_ratio(1, 12);
    upper = fraction_from_ratio(11, 60);
    above = fraction_compare(partial, lower);
    below = fraction_compare(partial, upper);
    above_ok = above >= 0;
    below_ok = below < 0;
    if (above_ok) {
    ret_val = below_ok;
