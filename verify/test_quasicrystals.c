...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== quasicrystals (order without a lattice) ===");
    ok = expect_equal((long long)"phi(5) = 4 (> the binary bound 2)", int_to_string(five_fold_totient()), (long long)"4");
    ok = expect_bool((long long)"five-fold PERIODIC order is forbidden (no lattice)", five_fold_lattice_is_forbidden());
    ok = expect_bool((long long)"yet the five-fold HOLDS the balance (a standing mode)", five_fold_holds_the_balance());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
