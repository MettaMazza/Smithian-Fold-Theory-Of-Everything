...
long long _main();
long long first_two_base_boxes();
long long boxes_tile_the_codons();
long long wobble_rung_folds_to_one();
long long subshell_capacity(long long);
long long known_blocks_hold();
long long g_block_capacity();
...
long long nucleotide_base_count();
long long codon_length();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== chemistry and biology: the codon wobble grouping and the g-block ===");
    ok = expect_equal((long long)"the first-two-base boxes: 4^2 = 16", int_to_string(first_two_base_boxes()), (long long)"16");
    ok = expect_bool((long long)"the 64 codons tile the 16 boxes exactly (4 per box, wobble)", boxes_tile_the_codons());
    ok = expect_bool((long long)"the wobble rung folds to unison (the third-base degeneracy)", wobble_rung_folds_to_one());
    ok = expect_bool((long long)"the known blocks s/p/d/f = 2/6/10/14 from 2(2l+1)", known_blocks_hold());
    ok = expect_equal((long long)"the g-block capacity: 2(2*4+1) = 18 (elements 121-137)", int_to_string(g_block_capacity()), (long long)"18");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_two_base_boxes() {
    ...
    ep_gc_push_root(&bases);

    ep_gc_maybe_collect();

    bases = whole_power(binary_count(), 2);
    ret_val = whole_power(bases, 2);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long boxes_tile_the_codons() {
    ...
    boxes = first_two_base_boxes();
    per_box = whole_power(binary_count(), 2);
    total = (boxes * per_box);
    codons = codon_count();
    ret_val = total == codons;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long wobble_rung_folds_to_one() {
    ...
    long long one = 0;
    long long rung = 0;
    long long rung_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&doubled);
    ep_gc_push_root(&one);
    ep_gc_push_root(&rung);
    ep_gc_push_root(&rung_twin);

    ep_gc_maybe_collect();

    rung = fraction_from_ratio(1, 2);
    rung_twin = fraction_from_ratio(1, 2);
    doubled = fraction_add(rung, rung_twin);
...
long long codon_count() {
    ...
    by_alphabet = whole_power(nucleotide_base_count(), codon_length());
    by_binary = whole_power(binary_count(), (binary_count() * colour_count()));
    ret_val = forced_to_be((long long)"codon count = base^codon_length = binary^(binary*colour)", by_alphabet, by_binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
