...
long long _main();
long long stefan_boltzmann_exponent();
long long stefan_boltzmann_exponent_forced();
long long power_ratio_for_doubled_temperature();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== blackbody radiation ===");
    ok = expect_number((long long)"Stefan-Boltzmann exponent = d_space + 1", stefan_boltzmann_exponent(), 4);
    ok = expect_number((long long)"exponent forced = spacetime dimensions", stefan_boltzmann_exponent_forced(), 4);
    ok = expect_number((long long)"doubling temperature multiplies power by 2^4", power_ratio_for_doubled_temperature(), 16);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long stefan_boltzmann_exponent_forced() {
    long long by_modes = 0;
    long long by_spacetime = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_modes);
    ep_gc_push_root(&by_spacetime);

    ep_gc_maybe_collect();

    by_modes = (spatial_dimensions() + 1);
    by_spacetime = spacetime_dimensions();
    ret_val = forced_to_be((long long)"Stefan-Boltzmann exponent = d_space + 1 = spacetime dimensions", by_modes, by_spacetime);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long power_ratio_for_doubled_temperature() {
    ...
    ret_val = whole_power(2, stefan_boltzmann_exponent());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
