...
long long first_channel();
long long second_channel();
long long channels_share_the_lock();
long long lock_folds_to_experience();
long long channels_partition_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== synaesthesia (senses sharing a lock) ===");
    ok = expect_equal((long long)"first channel = 1/4", fraction_to_text(first_channel()), (long long)"1/4");
    ok = expect_equal((long long)"second channel = 3/4", fraction_to_text(second_channel()), (long long)"3/4");
    ok = expect_bool((long long)"both fold to the SAME binding lock 1/2", channels_share_the_lock());
    ok = expect_equal((long long)"the lock folds to one whole experience", fraction_to_text(lock_folds_to_experience()), (long long)"1");
    ok = expect_equal((long long)"the channels partition the One (still one experience)", fraction_to_text(channels_partition_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long channels_share_the_lock() {
    long long first_lock = 0;
    long long second_lock = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_lock);
    ep_gc_push_root(&second_lock);

    ep_gc_maybe_collect();

    first_lock = fold_fraction(first_channel());
    second_lock = fold_fraction(second_channel());
    ret_val = fraction_compare(first_lock, second_lock) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(first_lock);
    first_lock = 0;
    free_struct_Fraction(second_lock);
    second_lock = 0;
    return ret_val;
}
...
long long lock_folds_to_experience() {
    ...
    ret_val = fold_fraction(fold_fraction(first_channel()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long channels_partition_one() {
    ...
    first = fraction_from_ratio(1, (binary_count() * binary_count()));
    second = fraction_from_ratio(colour_count(), (binary_count() * binary_count()));
