...
long long spacing_at_depth(long long);
long long halving_relation_holds(long long);
long long oscillator_level(long long, long long);
long long ground_is_half_spacing(long long);
long long spacing_is_uniform_at(long long, long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== oscillator levels (E_n = (n + 1/2)s) ===");
    ok = expect_equal((long long)"spacing at depth 2 = 1/4", fraction_to_text(spacing_at_depth(2)), (long long)"1/4");
    ok = expect_bool((long long)"halving relation: fold(s_3) = s_2", halving_relation_holds(2));
    ok = expect_equal((long long)"ground level = 1/8 (half a spacing up)", fraction_to_text(oscillator_level(0, 2)), (long long)"1/8");
    ok = expect_bool((long long)"the ground IS the zero-point half-spacing", ground_is_half_spacing(2));
    ok = expect_equal((long long)"levels 1/8, 3/8, 5/8, 7/8: level 3 = 7/8", fraction_to_text(oscillator_level(3, 2)), (long long)"7/8");
    ok = expect_bool((long long)"uniform spacing between levels 0 and 1", spacing_is_uniform_at(0, 2));
    ok = expect_bool((long long)"uniform spacing between levels 2 and 3", spacing_is_uniform_at(2, 2));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spacing_at_depth(long long k) {
    long long volume = 0;
    long long ret_val = 0;
...
long long halving_relation_holds(long long k) {
    long long deeper = 0;
    long long folded = 0;
    long long here = 0;
    long long ret_val = 0;
    ...
    deeper = spacing_at_depth((k + 1));
    folded = fold_fraction(deeper);
    here = spacing_at_depth(k);
    ret_val = fraction_compare(folded, here) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long oscillator_level(long long n, long long k) {
    long long half_spacing = 0;
    long long odd_count = 0;
    long long ret_val = 0;

    ep_gc_push_root(&half_spacing);
    ep_gc_push_root(&odd_count);
    ep_gc_push_root(&n);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    half_spacing = spacing_at_depth((k + 1));
    odd_count = fraction_from_whole_number(((2 * n) + 1));
    ret_val = fraction_multiply(odd_count, half_spacing);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long ground_is_half_spacing(long long k) {
    long long ground = 0;
    long long half_spacing = 0;
    long long ret_val = 0;
    ...
    ground = oscillator_level(0, k);
    half_spacing = spacing_at_depth((k + 1));
    ret_val = fraction_compare(ground, half_spacing) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(ground);
    ground = 0;
    free_struct_Fraction(half_spacing);
    half_spacing = 0;
    return ret_val;
}

long long spacing_is_uniform_at(long long n, long long k) {
    long long gap = 0;
    long long here = 0;
    ...
    here = oscillator_level(n, k);
    next_level = oscillator_level((n + 1), k);
    gap = fraction_subtract(next_level, here);
    s = spacing_at_depth(k);
    ret_val = fraction_compare(gap, s) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(gap);
    gap = 0;
    free_struct_Fraction(s);
    s = 0;
    return ret_val;
}
