...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the genetic code ===");
    ok = expect_number((long long)"nucleotide bases (a two-bit choice, binary^2)", nucleotide_base_count(), 4);
    ok = expect_number((long long)"codon length (a triplet, colour)", codon_length(), 3);
    ok = expect_number((long long)"codons = 4^3 = binary^(binary*colour) (forced two ways)", codon_count(), 64);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
