...
long long _main();
long long scatter_probability();
long long pass_probability();
long long scatter_or_pass_is_certain();
long long mean_free_path(long long);
long long larger_cross_section_shorter_path();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== cross sections ===");
    ok = expect_equal((long long)"scatter probability = 1/2", fraction_to_text(scatter_probability()), (long long)"1/2");
    ok = expect_equal((long long)"pass-through probability = 1/2 (antipode)", fraction_to_text(pass_probability()), (long long)"1/2");
    ok = expect_bool((long long)"scatter or pass is certain (sum = the One)", scatter_or_pass_is_certain());
    ok = expect_equal((long long)"mean free path for cross section 1/2 = 2", fraction_to_text(mean_free_path(fraction_from_ratio(1, 2))), (long long)"2");
    ok = expect_bool((long long)"a larger cross section gives a shorter free path", larger_cross_section_shorter_path());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long pass_probability() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&scatter);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    scatter = fraction_from_ratio(1, binary_count());
    ret_val = fraction_subtract(one, scatter);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long scatter_or_pass_is_certain() {
    ...
    long long one = 0;
    long long scatter = 0;
    long long pass_through = 0;
    long long ret_val = 0;

    ep_gc_push_root(&total);
    ep_gc_push_root(&one);
    ep_gc_push_root(&scatter);
    ep_gc_push_root(&pass_through);

    ep_gc_maybe_collect();

    scatter = fraction_from_ratio(1, binary_count());
    pass_through = pass_probability();
    total = fraction_add(scatter, pass_through);
    one = fraction_from_whole_number(1);
...
long long mean_free_path(long long cross_section) {
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&cross_section);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    ret_val = fraction_divide(one, cross_section);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long larger_cross_section_shorter_path() {
    ...
    long long order = 0;
    long long big = 0;
    long long path_small = 0;
    long long path_big = 0;
    long long ret_val = 0;

    ep_gc_push_root(&small);
    ep_gc_push_root(&big);
    ep_gc_push_root(&path_small);
    ep_gc_push_root(&path_big);

    ep_gc_maybe_collect();

    big = fraction_from_whole_number(1);
    small = fraction_from_ratio(1, 2);
    path_big = mean_free_path(big);
    path_small = mean_free_path(small);
    order = fraction_compare(path_big, path_small);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
