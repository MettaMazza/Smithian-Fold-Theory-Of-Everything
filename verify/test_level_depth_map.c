...
long long _main();
long long scale_axis_at(long long);
long long axis_doubles_everywhere();
long long axis_matches_tower();
long long dimension_scale_ratio(long long);
long long dimension_step_is_eight();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the level-depth map (doubling per step, 8 per dimension) ===");
    ok = expect_equal((long long)"the scale axis starts at R_0 = 1", int_to_string(scale_axis_at(0)), (long long)"1");
    ok = expect_bool((long long)"the axis DOUBLES at every step through k = 10 (walked)", axis_doubles_everywhere());
    ok = expect_bool((long long)"the walked axis equals the tower 2^k at depths 3, 5, 7", axis_matches_tower());
    ok = expect_equal((long long)"the per-dimension ratio S_3 = 2^3 = 8", int_to_string(dimension_scale_ratio(3)), (long long)"8");
    ok = expect_bool((long long)"at the COUNTED d = 3 the dimension step is 8 (the Hubble 8)", dimension_step_is_eight());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long scale_axis_at(long long k) {
    long long step = 0;
    long long value = 0;
    long long ret_val = 0;
...
long long axis_doubles_everywhere() {
    ...
    k = 0;
    while (k < 10) {
    here = scale_axis_at(k);
    next_rung = scale_axis_at((k + 1));
    doubles = next_rung == (2 * here);
    if (doubles) {
...
long long axis_matches_tower() {
    long long at_five = 0;
    long long at_seven = 0;
    long long at_three = 0;
    long long ret_val = 0;

    at_three = scale_axis_at(3) == whole_power(binary_count(), 3);
    at_five = scale_axis_at(5) == whole_power(binary_count(), 5);
    at_seven = scale_axis_at(7) == whole_power(binary_count(), 7);
    if (at_three) {
    if (at_five) {
    ret_val = at_seven;
    goto L_cleanup;
...
long long dimension_scale_ratio(long long d) {
    long long ret_val = 0;
...
long long dimension_step_is_eight() {
    ...
    ratio = whole_power(binary_count(), d);
    ret_val = ratio == 8;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
