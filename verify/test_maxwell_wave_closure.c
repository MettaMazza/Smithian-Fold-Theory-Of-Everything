...
long long _main();
long long spatial_curvature();
long long temporal_curvature();
long long curvature_ratio();
long long wave_speed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Maxwell wave closure ===");
    ok = expect_equal((long long)"spatial curvature = binary * spatial = 6", int_to_string(spatial_curvature()), int_to_string(6));
    ok = expect_equal((long long)"temporal curvature = binary = 2", int_to_string(temporal_curvature()), int_to_string(2));
    ok = expect_equal((long long)"curvature ratio = 6/2 = 3 = d_space (= colour)", int_to_string(curvature_ratio()), int_to_string(3));
    ok = expect_equal((long long)"the closed wave travels at the One (c)", fold_value_to_text(wave_speed()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spatial_curvature() {
    ...
    ret_val = (smallest_fold_period_above(1) * spatial_dimensions());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long curvature_ratio() {
    long long by_dimension = 0;
    long long ratio = 0;
    long long spatial = 0;
    long long temporal = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_dimension);
    ep_gc_push_root(&ratio);

    ep_gc_maybe_collect();

    spatial = (smallest_fold_period_above(1) * spatial_dimensions());
    temporal = smallest_fold_period_above(1);
    ratio = (spatial / temporal);
    by_dimension = spatial_dimensions();
    ret_val = forced_to_be((long long)"Maxwell curvature ratio = spatial/temporal = d_space", ratio, by_dimension);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
