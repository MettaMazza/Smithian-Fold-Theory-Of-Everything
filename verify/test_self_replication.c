...
long long template();
long long copy();
long long template_folds_to_pattern();
long long copy_folds_to_pattern();
long long template_and_copy_partition_one();
long long copies_at_depth(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== self-replication ===");
    ok = expect_bool((long long)"template folds to the pattern (1/4 -> 1/2)", template_folds_to_pattern());
    ok = expect_bool((long long)"copy folds to the same pattern (3/4 -> 1/2)", copy_folds_to_pattern());
    ok = expect_bool((long long)"template + copy partition the One (distinct, sum 1)", template_and_copy_partition_one());
    ok = expect_equal((long long)"copies at depth 0 = 1", int_to_string(copies_at_depth(0)), int_to_string(1));
    ok = expect_equal((long long)"copies at depth 1 = 2", int_to_string(copies_at_depth(1)), int_to_string(2));
    ok = expect_equal((long long)"copies at depth 2 = 4", int_to_string(copies_at_depth(2)), int_to_string(4));
    ok = expect_equal((long long)"copies at depth 3 = 8", int_to_string(copies_at_depth(3)), int_to_string(8));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long template_folds_to_pattern() {
    long long folded = 0;
    long long the_pattern = 0;
    long long the_template = 0;
    long long ret_val = 0;

    ep_gc_push_root(&folded);
    ep_gc_push_root(&the_pattern);
    ep_gc_push_root(&the_template);

    ep_gc_maybe_collect();

    the_template = fraction_from_ratio(1, 4);
    folded = fold_fraction(the_template);
    the_pattern = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(folded, the_pattern) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(folded);
    folded = 0;
    free_struct_Fraction(the_pattern);
    the_pattern = 0;
    return ret_val;
}

long long copy_folds_to_pattern() {
    long long the_copy = 0;
    long long folded = 0;
    long long the_pattern = 0;
    long long ret_val = 0;

    ep_gc_push_root(&the_copy);
    ep_gc_push_root(&folded);
    ep_gc_push_root(&the_pattern);

    ep_gc_maybe_collect();

    the_copy = fraction_from_ratio(3, 4);
    folded = fold_fraction(the_copy);
    the_pattern = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(folded, the_pattern) == 0;
    goto L_cleanup;
L_cleanup:
...
long long template_and_copy_partition_one() {
    ...
    the_template = fraction_from_ratio(1, 4);
    the_copy = fraction_from_ratio(3, 4);
    both = fraction_add(the_template, the_copy);
    one = fraction_from_whole_number(1);
    is_whole = fraction_compare(both, one) == 0;
    distinct = fraction_compare(the_template, the_copy) == 0;
    if (is_whole) {
    if (distinct) {
    ret_val = 0LL;
    goto L_cleanup;
...
long long copies_at_depth(long long depth) {
    long long ret_val = 0;
