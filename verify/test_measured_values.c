...
long long expect_number(long long label, long long got, long long want) {
    ...
    printf("%s\n", (char*)concat((long long)"  FAIL  ", concat(label, concat((long long)" got ", int_to_string(got)))));
    }
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long _main() {
    long long agrees = 0;
    long long codata = 0;
    long long disagrees = 0;
    long long forced = 0;
    long long ok = 0;
    long long tolerance = 0;
    long long wrong_target = 0;
    long long ret_val = 0;

    ep_gc_push_root(&agrees);
    ep_gc_push_root(&codata);
    ep_gc_push_root(&disagrees);
    ep_gc_push_root(&forced);
    ep_gc_push_root(&tolerance);
    ep_gc_push_root(&wrong_target);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== measured values: the sealed comparison boundary ===");
    forced = inverse_fine_structure_second_order();
    codata = ({
    EpStruct_Measured* _s = (EpStruct_Measured*)malloc(sizeof(EpStruct_Measured));
    _s->label = (long long)"CODATA 1/alpha";
    _s->numerator = 137035999177;
    _s->denominator = 1000000000;
    { EpGCObject* _go = ep_gc_register(_s, EP_OBJ_STRUCT); if(_go) _go->num_fields = 3; }
    (long long)_s;
});
    tolerance = fraction_from_ratio(1, 1000000);
    agrees = forced_agrees_with_measured(forced, codata, tolerance);
    ok = expect_bool((long long)"forced 1/alpha AGREES with sealed CODATA target (< 1e-6)", agrees);
    wrong_target = ({
    EpStruct_Measured* _s = (EpStruct_Measured*)malloc(sizeof(EpStruct_Measured));
    _s->label = (long long)"wrong 1/alpha";
    _s->numerator = 137;
    _s->denominator = 1;
    { EpGCObject* _go = ep_gc_register(_s, EP_OBJ_STRUCT); if(_go) _go->num_fields = 3; }
    (long long)_s;
});
    disagrees = forced_agrees_with_measured(forced, wrong_target, tolerance);
    ok = expect_bool((long long)"forced 1/alpha does NOT agree with a wrong target (137)", disagrees == 0LL);
    ok = expect_number((long long)"forbid_target_input passes a forced (non-measured) value", forbid_target_input((long long)"forced value", 0LL), 1);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
