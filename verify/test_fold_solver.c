...
long long _main();
long long tower_above(long long);
long long state_coordinate(long long, long long);
long long coordinates_are_legal(long long);
long long solve_subtraction(long long);
long long certify_subtraction(long long);
long long solve_nim();
long long certify_nim();
...
long long bit_of(long long, long long);
long long bit_and(long long, long long);
long long bit_xor(long long, long long);
long long popcount(long long);
long long chi_sign(long long, long long);
long long group_law_holds(long long, long long);
long long span_frequency(long long, long long, long long);
long long span_is_distinct(long long, long long);
long long generator_coefficient(long long);
long long span_sign_sum(long long, long long, long long);
long long subtraction_reconstructs_to(long long);
long long nim_mask_base(long long, long long);
long long nim_heap_xor(long long, long long, long long);
long long nim_reconstructs(long long, long long);
long long nim_generator_length(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the universal exact solver: the chess engine, off chess ===");
    ok = expect_equal((long long)"the binary tower above 1001 states", int_to_string(tower_above(1001)), (long long)"1024");
    ok = expect_bool((long long)"every state coordinate is a legal fold value (1001 states)", coordinates_are_legal(1001));
    ok = expect_equal((long long)"GAME 1: subtraction, 1001 states vs the mod-4 oracle -- disagreements", int_to_string(certify_subtraction(1000)), (long long)"0");
    ok = expect_equal((long long)"GAME 2: Nim, 512 states vs Bouton's XOR oracle -- disagreements", int_to_string(certify_nim()), (long long)"0");
    ok = expect_bool((long long)"every Nim coordinate is a legal fold value (512 states)", coordinates_are_legal(512));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long tower_above(long long count) {
    long long tower = 0;
    long long ret_val = 0;
...
long long state_coordinate(long long index, long long tower) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio((index + 1), tower);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long coordinates_are_legal(long long count) {
    long long all_legal = 0;
    long long coordinate = 0;
    long long coordinate_twin = 0;
    long long in_domain = 0;
    long long index = 0;
    long long one = 0;
    ...
    ep_gc_push_root(&coordinate_twin);
    ep_gc_push_root(&index);
    ep_gc_push_root(&one);
    ...
    tower = tower_above(count);
    one = fraction_from_whole_number(1);
    all_legal = 1LL;
    index = 0;
    while (index < count) {
    coordinate = fraction_from_ratio((index + 1), tower);
    positive = ({ long long _fap = coordinate; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 1;
    if (positive) {
    all_legal = all_legal;
    } else {
    all_legal = 0LL;
    }
    coordinate_twin = fraction_from_ratio((index + 1), tower);
    one_twin = fraction_from_whole_number(1);
    in_domain = fraction_compare(coordinate_twin, one_twin) < 1;
    if (in_domain) {
    all_legal = all_legal;
    } else {
    all_legal = 0LL;
...
L_cleanup:
    ...
    coordinate = 0;
    free_struct_Fraction(coordinate_twin);
    coordinate_twin = 0;
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(one_twin);
    one_twin = 0;
    return ret_val;
}

long long solve_subtraction(long long bound) {
    long long n = 0;
    long long ok = 0;
    ...
    while (take < 4) {
    if ((n - take) > -1) {
    successor = get_list(values, (n - take));
    if (successor == 0) {
    win = 1;
    }
    }
...
long long certify_subtraction(long long bound) {
    ...
    values = solve_subtraction(bound);
    disagreements = 0;
    n = 0;
    while (n < (bound + 1)) {
    solved = get_list(values, n);
    quarter = (n / 4);
    remainder = (n - ((quarter + quarter) + (quarter + quarter)));
    oracle = 1;
    if (remainder == 0) {
    oracle = 0;
    }
...
long long solve_nim() {
    ...
    long long ok = 0;
    long long successor = 0;
    long long successor_code = 0;
    long long take = 0;
    long long values = 0;
    long long win = 0;
    long long ret_val = 0;

    ep_gc_push_root(&successor_code);
    ep_gc_push_root(&values);
    ep_gc_push_root(&win);
    ...
    code = 0;
    while (code < 512) {
    c_heap = (code - ((code / 8) * 8));
    b_heap = ((code / 8) - ((code / 64) * 8));
    a_heap = (code / 64);
    win = 0;
    take = 1;
    while (take < (a_heap + 1)) {
    successor_code = (code - (take * 64));
    successor = get_list(values, successor_code);
    if (successor == 0) {
    win = 1;
    }
    take = (take + 1);
...
long long certify_nim() {
    ...
    code = 0;
    while (code < 512) {
    solved = get_list(values, code);
    heap_xor = nim_heap_xor(code, 3, 3);
    oracle = 1;
    if (heap_xor == 0) {
    oracle = 0;
    }
...
long long bit_of(long long n, long long k) {
    long long half = 0;
    long long shifted = 0;
    long long ret_val = 0;
    ...
    shifted = (n / whole_power(2, k));
    half = (shifted / 2);
    ret_val = (shifted - (half + half));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long chi_sign(long long s, long long n) {
    long long ones = 0;
    long long overlap = 0;
    long long parity = 0;
    long long ret_val = 0;

    ep_gc_push_root(&overlap);
    ep_gc_push_root(&s);
    ep_gc_push_root(&n);
    ...
    overlap = bit_and(s, n);
    ones = popcount(overlap);
    parity = (ones - ((ones / 2) + (ones / 2)));
    if (parity == 0) {
    ret_val = 1;
    goto L_cleanup;
    }
...
long long group_law_holds(long long freq_bound, long long state_bound) {
    long long a = 0;
    long long ab = 0;
    ...
    n = 0;
    while (n < state_bound) {
    left = (chi_sign(a, n) * chi_sign(b, n));
    right = chi_sign(ab, n);
    if (left == right) {
    all_hold = all_hold;
    } else {
...
long long span_frequency(long long mask_base, long long mask_count, long long selector) {
    ...
    ep_gc_push_root(&frequency);
    ep_gc_push_root(&k);
    ep_gc_push_root(&mask);
    ep_gc_push_root(&selector);
    ...
    k = 0;
    while (k < mask_count) {
    selected = bit_of(selector, k);
    if (selected == 1) {
    mask = (mask_base * whole_power(2, k));
    frequency = bit_xor(frequency, mask);
    }
    k = (k + 1);
    }
    ret_val = frequency;
...
long long span_is_distinct(long long mask_base, long long mask_count) {
    ...
    ep_gc_push_root(&i);
    ep_gc_push_root(&j);
    ep_gc_push_root(&mask_base);
    ep_gc_push_root(&mask_count);

    ep_gc_maybe_collect();

    total = whole_power(2, mask_count);
    all_distinct = 1LL;
    i = 0;
    while (i < total) {
    j = (i + 1);
    while (j < total) {
    freq_i = span_frequency(mask_base, mask_count, i);
    freq_j = span_frequency(mask_base, mask_count, j);
    if (freq_i == freq_j) {
    all_distinct = 0LL;
    }
    j = (j + 1);
...
long long generator_coefficient(long long mask_count) {
    long long total = 0;
    long long ret_val = 0;
    ...
    total = whole_power(2, mask_count);
    ret_val = fraction_from_ratio(1, total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long span_sign_sum(long long mask_base, long long mask_count, long long n) {
    long long frequency = 0;
    ...
    sum = 0;
    selector = 0;
    while (selector < total) {
    frequency = span_frequency(mask_base, mask_count, selector);
    sign = chi_sign(frequency, n);
    sum = (sum + sign);
    selector = (selector + 1);
    }
    ret_val = sum;
...
long long subtraction_reconstructs_to(long long state_bound) {
    long long all_match = 0;
    long long n = 0;
    long long quarter = 0;
    long long remainder = 0;
    long long sum = 0;
...
long long nim_mask_base(long long heaps, long long bits) {
    long long base = 0;
    long long i = 0;
    long long ret_val = 0;

    ep_gc_push_root(&i);
    ep_gc_push_root(&bits);
    ...
    i = 0;
    while (i < heaps) {
    base = (base + whole_power(2, (i * bits)));
    i = (i + 1);
    }
    ret_val = base;
    goto L_cleanup;
...
long long nim_heap_xor(long long code, long long heaps, long long bits) {
    ...
    ep_gc_push_root(&accumulated);
    ep_gc_push_root(&heap);
    ep_gc_push_root(&bits);

    ep_gc_maybe_collect();

    heap_size = whole_power(2, bits);
    accumulated = 0;
    remaining = code;
    i = 0;
    while (i < heaps) {
    heap = (remaining - ((remaining / heap_size) * heap_size));
    accumulated = bit_xor(accumulated, heap);
    remaining = (remaining / heap_size);
    i = (i + 1);
    }
    ret_val = accumulated;
    goto L_cleanup;
...
long long nim_reconstructs(long long heaps, long long bits) {
    long long all_match = 0;
    long long base = 0;
    long long code = 0;
    ...
    ep_gc_push_root(&base);
    ep_gc_push_root(&code);
    ep_gc_push_root(&heaps);
    ep_gc_push_root(&bits);

    ep_gc_maybe_collect();

    base = nim_mask_base(heaps, bits);
    field_size = whole_power(2, (heaps * bits));
    full = whole_power(2, bits);
    all_match = 1LL;
    code = 0;
    while (code < field_size) {
    sum = span_sign_sum(base, bits, code);
    heap_xor = nim_heap_xor(code, heaps, bits);
    if (heap_xor == 0) {
    if (sum == full) {
    all_match = all_match;
    } else {
...
long long nim_generator_length(long long bits) {
    long long ret_val = 0;
