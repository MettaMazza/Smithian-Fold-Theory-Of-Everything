...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold undecidability law ===");
    ok = expect_equal((long long)"outcome one is sent to its Fold partner", int_to_string(self_negating_fold_actual(1)), int_to_string(2));
    ok = expect_equal((long long)"outcome two is sent to its Fold partner", int_to_string(self_negating_fold_actual(2)), int_to_string(1));
    ok = expect_true((long long)"the outcome partner is an involution", fold_outcome_partner_is_involution());
    ok = expect_equal((long long)"the generated self-negating fixed-point count is zero", int_to_string(self_negating_fold_fixed_prediction_count()), int_to_string(0));
    ok = expect_true((long long)"the self-negating total decision is undecidable", fold_self_decision_is_undecidable());
    ok = expect_true((long long)"every candidate total self-prediction is rejected", every_total_self_prediction_is_rejected());
    ok = expect_equal((long long)"the non-negating identity control has b fixed outcomes", int_to_string(nonnegating_identity_fixed_count()), int_to_string(2));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
