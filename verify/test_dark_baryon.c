...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the dark baryon: the colour-singlet rule from the fold's fibre ===");
    ok = expect_bool((long long)"the p=3 fibre folds to the One, member by member", fibre_folds_to_one(3));
    ok = expect_bool((long long)"the p=5 fibre folds to the One, member by member", fibre_folds_to_one(5));
    ok = expect_bool((long long)"the p=7 fibre folds to the One, member by member", fibre_folds_to_one(7));
    ok = expect_equal((long long)"baryon multiplicity at p=3 (the nucleon)", int_to_string(baryon_multiplicity(3)), (long long)"3");
    ok = expect_equal((long long)"baryon multiplicity at p=5 (the PENTA baryon)", int_to_string(baryon_multiplicity(5)), (long long)"5");
    ok = expect_equal((long long)"baryon multiplicity at p=7 (the HEPTA baryon)", int_to_string(baryon_multiplicity(7)), (long long)"7");
    ok = expect_equal((long long)"meson multiplicity is two in EVERY sector (p=3)", int_to_string(meson_multiplicity(3)), (long long)"2");
    ok = expect_equal((long long)"meson multiplicity is two in EVERY sector (p=5)", int_to_string(meson_multiplicity(5)), (long long)"2");
    ok = expect_equal((long long)"meson multiplicity is two in EVERY sector (p=7)", int_to_string(meson_multiplicity(7)), (long long)"2");
    ok = expect_equal((long long)"baryon charge sum at p=3 is whole: (3+1)/2", fraction_to_text(baryon_charge_sum(3)), (long long)"2");
    ok = expect_equal((long long)"baryon charge sum at p=5 is whole: (5+1)/2", fraction_to_text(baryon_charge_sum(5)), (long long)"3");
    ok = expect_equal((long long)"baryon charge sum at p=7 is whole: (7+1)/2", fraction_to_text(baryon_charge_sum(7)), (long long)"4");
    ok = expect_bool((long long)"the Standard-Model anchor: counted colour gives nucleon + pion", standard_model_anchor());
    ok = expect_bool((long long)"the NEW dark hadrons are forced: neutral 5- and 7-quark baryons", dark_hadrons_forced());
    ok = expect_bool((long long)"EXCLUSION: even sectors have no neutral baryon", even_sector_excluded());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
