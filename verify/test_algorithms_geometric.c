...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold geometric algorithms ===");
    ok = expect_equal((long long)"depth-seven spacing is 1/128", fraction_to_text(fold_grid_spacing(7)), fraction_to_text(fraction_from_ratio(1, 128)));
    ok = expect_true((long long)"every adjacent separation is exact", every_fold_grid_adjacency_is_exact(7));
    ok = expect_true((long long)"one Fold scales same-branch distance by b", fold_distance_scales_on_one_branch(7));
    ok = expect_true((long long)"paired preimages are separated by half-One", fold_preimage_separation_is_half(7));
    ok = expect_equal((long long)"exact nearest search returns rank 73", int_to_string(fold_nearest_grid_rank(computation_state(73, 7), 7)), int_to_string(73));
    ok = expect_true((long long)"the complete geometric census holds", complete_fold_geometric_algorithm_census());
    ok = expect_true((long long)"a shifted nearest point is rejected", shifted_nearest_point_is_rejected(73, 7));
    ok = expect_true((long long)"perturbed grid spacing is rejected", perturbed_grid_spacing_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
