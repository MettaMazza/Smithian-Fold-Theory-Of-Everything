...
long long decidability_boundary();
long long descent_reaches_lock();
long long descent_reaches_unison();
long long gap_of(long long);
long long each_step_pays();
long long steps_equal_depth();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the intelligence dividend (why abstraction pays) ===");
    ok = expect_equal((long long)"the decidability boundary = 1/4", fraction_to_text(decidability_boundary()), (long long)"1/4");
    ok = expect_bool((long long)"the descent reaches the lock (the problem grasped whole)", descent_reaches_lock());
    ok = expect_bool((long long)"and closure (mastery, the fixed point)", descent_reaches_unison());
    ok = expect_bool((long long)"THE DIVIDEND: each step leaves a strictly smaller gap", each_step_pays());
    ok = expect_bool((long long)"steps = the depth (logarithmic in the grid, not linear)", steps_equal_depth());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long descent_reaches_lock() {
    ...
    ep_gc_push_root(&boundary);
    ep_gc_push_root(&lock);
    ep_gc_push_root(&target);

    ep_gc_maybe_collect();

    boundary = fraction_from_ratio(1, 4);
    lock = fold_fraction(boundary);
    target = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(lock, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long descent_reaches_unison() {
    ...
    boundary = fraction_from_ratio(1, 4);
    lock = fold_fraction(boundary);
    closure = fold_fraction(lock);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(closure, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long gap_of(long long state) {
    long long one = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, state);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long each_step_pays() {
    ...
    long long closed = 0;
    long long closure = 0;
    long long first_shrinks = 0;
    long long gap_closure = 0;
    long long gap_lock = 0;
    ...
    ep_gc_push_root(&boundary);
    ep_gc_push_root(&closure);
    ep_gc_push_root(&gap_closure);
    ep_gc_push_root(&gap_lock);
    ep_gc_push_root(&gap_start);
    ep_gc_push_root(&lock);

    ep_gc_maybe_collect();

    boundary = fraction_from_ratio(1, 4);
    gap_start = gap_of(boundary);
    lock = fold_fraction(boundary);
    gap_lock = gap_of(lock);
    first_shrinks = fraction_compare(gap_lock, gap_start);
    closure = fold_fraction(lock);
    gap_closure = gap_of(closure);
    closed = ({ long long _fap = gap_closure; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    if (first_shrinks < 0) {
    ret_val = closed;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(gap_closure);
    gap_closure = 0;
    return ret_val;
}
