...
long long _main();
long long proton_channels();
long long coupling_squared();
long long ceiling_fraction();
long long channel_rung_is_traced();
long long cutoff_from_proton(long long, long long);
...
long long _main() {
    long long cutoff = 0;
    long long cutoff_again = 0;
    long long ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&cutoff);
    ep_gc_push_root(&cutoff_again);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== Parker reconnection protons: E = 8 alpha^2 m_p c^2, nothing local ===");
    ok = expect_equal((long long)"the proton's channels: colour^2 - 1 (the eight gluons' count)", int_to_string(proton_channels()), (long long)"8");
    ok = expect_equal((long long)"alpha^2 from the forced 1/alpha = 34259/250", fraction_to_text(coupling_squared()), (long long)"62500/1173679081");
    ok = expect_equal((long long)"THE FORCED FRACTION: 8 alpha^2 of the proton rest energy", fraction_to_text(ceiling_fraction()), (long long)"500000/1173679081");
    ok = expect_bool((long long)"the channel rung 1/8 tiles back to the One", channel_rung_is_traced());
    cutoff = cutoff_from_proton(938272, 1);
    ok = expect_equal((long long)"E = 8 alpha^2 x m_p c^2, exact", fraction_to_text(cutoff), (long long)"469136000000/1173679081");
    cutoff_again = cutoff_from_proton(938272, 1);
    ok = expect_equal((long long)"E = 399.714 keV (Parker: ~400 keV -- within 0.1%)", fraction_to_decimal(cutoff_again, 3), (long long)"399.714");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(cutoff);
    cutoff = 0;
    free_struct_Fraction(cutoff_again);
    cutoff_again = 0;
    return ret_val;
}
...
long long coupling_squared() {
    long long alpha = 0;
    long long alpha_twin = 0;
    long long inverse = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&alpha);
    ep_gc_push_root(&alpha_twin);
    ep_gc_push_root(&inverse);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    inverse = inverse_fine_structure_first_order();
    one = fraction_from_whole_number(1);
    alpha = fraction_divide(one, inverse);
    alpha_twin = fraction_make(fraction_numerator(alpha), fraction_denominator(alpha));
    ret_val = fraction_multiply(alpha, alpha_twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long ceiling_fraction() {
    long long alpha_sq = 0;
    long long channels = 0;
    long long channels_fraction = 0;
    long long ret_val = 0;

    ep_gc_push_root(&alpha_sq);
    ep_gc_push_root(&channels);
    ep_gc_push_root(&channels_fraction);

    ep_gc_maybe_collect();

    channels = proton_channels();
    channels_fraction = fraction_from_whole_number(channels);
    alpha_sq = coupling_squared();
    ret_val = fraction_multiply(channels_fraction, alpha_sq);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long channel_rung_is_traced() {
    ...
    channels = proton_channels();
    rung = fraction_from_ratio(1, channels);
    channels_fraction = fraction_from_whole_number(channels);
    tiled = fraction_multiply(rung, channels_fraction);
    one = fraction_from_whole_number(1);
...
long long cutoff_from_proton(long long rest_top, long long rest_bottom) {
    long long rest_energy = 0;
    long long share = 0;
    long long ret_val = 0;

    ep_gc_push_root(&rest_energy);
    ep_gc_push_root(&share);
    ep_gc_push_root(&rest_top);
    ep_gc_push_root(&rest_bottom);

    ep_gc_maybe_collect();

    rest_energy = fraction_from_ratio(rest_top, rest_bottom);
    share = ceiling_fraction();
    ret_val = fraction_multiply(share, rest_energy);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
