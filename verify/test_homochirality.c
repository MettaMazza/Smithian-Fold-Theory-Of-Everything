...
long long left_hand_target();
long long right_hand_target();
long long hands_share_target();
long long hand_separation();
long long pair_is_degenerate();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== homochirality (why life uses one hand) ===");
    ...
    ok = expect_bool((long long)"both hands fold to the same target (1/2)", hands_share_target());
    ok = expect_equal((long long)"hand separation = 1/2 (equidistant about the balance)", fraction_to_text(hand_separation()), (long long)"1/2");
    ok = expect_bool((long long)"the pair is degenerate (tie broken by forced parity violation)", pair_is_degenerate());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long left_hand_target() {
    ...
    ret_val = fold_fraction(left_hand());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long right_hand_target() {
    ...
    ret_val = fold_fraction(right_hand());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long hands_share_target() {
    ...
    left = fold_fraction(left_hand());
    right = fold_fraction(right_hand());
    ret_val = fraction_compare(left, right) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long pair_is_degenerate() {
    ...
    ret_val = fraction_compare(hand_separation(), half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
