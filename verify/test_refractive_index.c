...
long long phase_state();
long long first_climb();
long long second_climb_restores_c();
long long shares_sum_to_level_above();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the refractive index (light slowed by fold-levels) ===");
    ok = expect_equal((long long)"dense-medium phase state = 1/4 (two levels below c)", fraction_to_text(phase_state()), (long long)"1/4");
    ok = expect_equal((long long)"one exchange repaid: 1/4 -> 1/2", fraction_to_text(first_climb()), (long long)"1/2");
    ok = expect_equal((long long)"second exchange restores c: 1/2 -> 1", fraction_to_text(second_climb_restores_c()), (long long)"1");
    ok = expect_equal((long long)"share + twin = the level above (1/2)", fraction_to_text(shares_sum_to_level_above()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_climb() {
    ...
    ret_val = fold_fraction(phase_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long second_climb_restores_c() {
    ...
    ret_val = fold_fraction(first_climb());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
