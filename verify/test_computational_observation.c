...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold observation law ===");
    ok = expect_equal((long long)"depth-7 observation retains 64 exact classes", int_to_string(retained_observation_distinctions(7)), int_to_string(64));
    ok = expect_equal((long long)"each observation class closes the binary distinction", int_to_string(closed_observation_distinctions_per_class(7)), int_to_string(2));
    ok = expect_equal((long long)"observation classes partition all 128 source states", int_to_string(observation_partition_is_forced(7)), int_to_string(128));
    ok = expect_true((long long)"paired states are distinct before and identical after observation", paired_states_close_under_observation(1, 7));
    ok = expect_true((long long)"singleton observation classes are rejected", singleton_observation_class_is_rejected(7));
    ok = expect_true((long long)"shifted observation targets are rejected", shifted_observation_target_is_rejected(7));
    ok = expect_true((long long)"complete observation census holds through the forced depth", complete_observation_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
