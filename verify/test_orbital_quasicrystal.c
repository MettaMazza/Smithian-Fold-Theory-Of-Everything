...
long long _main();
long long quasicrystal_is_prime_five();
long long modes_stand();
long long orbital_doubling_holds();
long long tully_fisher_exponent();
long long halo_is_the_relic();
...
long long is_standing_mode(long long, long long);
long long count_standing_modes(long long);
long long down_depth_fold();
long long five_fold_standing_mode_count();
long long two_fold_standing_mode_count();
long long half_is_a_standing_mode();
...
long long five_fold(long long);
long long five_fold_totient();
long long five_fold_lattice_is_forbidden();
long long five_fold_holds_the_balance();
long long whole_gcd(long long, long long);
long long euler_totient(long long);
long long rotation_is_crystallographic(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== materials and astrophysics: three orphans, one fold ===");
    ok = expect_bool((long long)"QUASICRYSTAL = PRIME-5: three standing modes AND the periodic lattice forbidden", quasicrystal_is_prime_five());
    ok = expect_bool((long long)"the modes 1/4, 1/2, 3/4 each stand under the five-fold map", modes_stand());
    ok = expect_bool((long long)"PLANETARY DOUBLING: consecutive orbit ratios exactly 2, seven orbits out", orbital_doubling_holds());
    ok = expect_equal((long long)"TULLY-FISHER: the exponent d_space + 1 (measured L ~ v^3.9)", int_to_string(tully_fisher_exponent()), (long long)"4");
    ok = expect_bool((long long)"the halo is the relic: 27/5 to baryons, 27/32 of matter", halo_is_the_relic());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long quasicrystal_is_prime_five() {
    ...
    modes = count_standing_modes(down_depth_fold());
    census_ok = modes == 3;
    forbidden = five_fold_lattice_is_forbidden();
    if (census_ok) {
    ret_val = forbidden;
    goto L_cleanup;
    }
...
long long modes_stand() {
    long long first = 0;
    long long first_stands = 0;
    long long second = 0;
    long long second_stands = 0;
    long long third = 0;
    long long third_stands = 0;
    long long ret_val = 0;
    ...
    first = fraction_from_ratio(1, 4);
    first_stands = fraction_compare(m_fold(first, 5), first) == 0;
    second = fraction_from_ratio(1, 2);
    second_stands = fraction_compare(m_fold(second, 5), second) == 0;
    third = fraction_from_ratio(3, 4);
    third_stands = fraction_compare(m_fold(third, 5), third) == 0;
    if (first_stands) {
    if (second_stands) {
    ret_val = third_stands;
    goto L_cleanup;
    }
    }
    ret_val = 0LL;
...
long long orbital_doubling_holds() {
    long long all_double = 0;
    long long beyond = 0;
    long long beyond_fraction = 0;
    long long doubles = 0;
    long long here = 0;
    long long here_fraction = 0;
    long long n = 0;
    long long ratio = 0;
    long long two = 0;
    long long two_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&beyond);
    ep_gc_push_root(&beyond_fraction);
    ep_gc_push_root(&here);
    ep_gc_push_root(&here_fraction);
    ep_gc_push_root(&ratio);
    ep_gc_push_root(&two);
    ep_gc_push_root(&two_twin);

    ep_gc_maybe_collect();

    two = fraction_from_whole_number(2);
    all_double = 1LL;
    here = whole_power(2, 0);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 1);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    if (doubles) {
    all_double = all_double;
    } else {
    all_double = 0LL;
    }
    here = whole_power(2, 1);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 2);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    ...
    here = whole_power(2, 2);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 3);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    ...
    here = whole_power(2, 3);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 4);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    ...
    here = whole_power(2, 4);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 5);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    ...
    here = whole_power(2, 5);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 6);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
    ...
    here = whole_power(2, 6);
    here_fraction = fraction_from_whole_number(here);
    beyond = whole_power(2, 7);
    beyond_fraction = fraction_from_whole_number(beyond);
    ratio = fraction_divide(beyond_fraction, here_fraction);
    two_twin = fraction_from_whole_number(2);
    doubles = fraction_compare(ratio, two_twin) == 0;
...
L_cleanup:
    ...
    ratio = 0;
    free_struct_Fraction(two);
    two = 0;
    free_struct_Fraction(two_twin);
    two_twin = 0;
    return ret_val;
}
...
long long tully_fisher_exponent() {
    ...
    d_space = smallest_fold_period_above(binary_count());
    ret_val = (d_space + 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
...
long long halo_is_the_relic() {
    ...
    long long ratio = 0;
    long long ratio_ok = 0;
    long long share = 0;
    long long share_expected = 0;
    long long share_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&expected);
    ep_gc_push_root(&ratio);
    ep_gc_push_root(&share);
    ep_gc_push_root(&share_expected);

    ep_gc_maybe_collect();

    ratio = relic_to_baryon();
    expected = fraction_from_ratio(27, 5);
    ratio_ok = fraction_compare(ratio, expected) == 0;
    share = relic_fraction_of_matter();
    share_expected = fraction_from_ratio(27, 32);
    share_ok = fraction_compare(share, share_expected) == 0;
    if (ratio_ok) {
    ret_val = share_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ...
    free_struct_Fraction(ratio);
    ratio = 0;
    free_struct_Fraction(share);
    share = 0;
    free_struct_Fraction(share_expected);
    share_expected = 0;
    return ret_val;
}
...
long long m_fold(long long size, long long m) {
    long long m_fraction = 0;
    long long scaled = 0;
    long long ret_val = 0;

    ep_gc_push_root(&m_fraction);
    ...
    m_fraction = fraction_from_whole_number(m);
    scaled = fraction_multiply(size, m_fraction);
    ret_val = cast_out_whole_ones(scaled);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long is_standing_mode(long long size, long long m) {
    long long ret_val = 0;
    ...
    ret_val = fraction_compare(m_fold(size, m), size) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long count_standing_modes(long long m) {
    long long count = 0;
    long long j = 0;
    long long span = 0;
    long long x = 0;
    ...
    j = 1;
    while (j < span) {
    x = fraction_from_ratio(j, span);
    if (fraction_compare(m_fold(x, m), x) == 0) {
    count = (count + 1);
    }
    j = (j + 1);
    }
...
long long five_fold_standing_mode_count() {
    ...
    ret_val = count_standing_modes(down_depth_fold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_fold_standing_mode_count() {
    ...
    ret_val = count_standing_modes(binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long half_is_a_standing_mode() {
    ...
    ret_val = fraction_compare(m_fold(half, down_depth_fold()), half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long five_fold(long long size) {
    long long five = 0;
    long long scaled = 0;
    long long ret_val = 0;

    ep_gc_push_root(&five);
    ep_gc_push_root(&scaled);
    ep_gc_push_root(&size);

    ep_gc_maybe_collect();

    five = fraction_from_whole_number((binary_count() + colour_count()));
    scaled = fraction_multiply(size, five);
    ret_val = cast_out_whole_ones(scaled);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long five_fold_totient() {
    ...
    ret_val = euler_totient((binary_count() + colour_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long five_fold_lattice_is_forbidden() {
    ...
    allowed = rotation_is_crystallographic((binary_count() + colour_count()));
    if (allowed) {
    ret_val = 0;
    goto L_cleanup;
    }
...
long long five_fold_holds_the_balance() {
    ...
    held = five_fold(balance);
    ret_val = fraction_compare(held, balance) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(held);
    held = 0;
    return ret_val;
}
...
long long rotation_is_crystallographic(long long n) {
    ...
    bound = smallest_fold_period_above(1);
    if (totient <= bound) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long allowed_rotation_order_count() {
    ...
    count = 0;
    limit = whole_power(colour_count(), colour_count());
    n = 1;
    while (n <= limit) {
    ok = rotation_is_crystallographic(n);
    if (ok) {
    count = (count + 1);
    }
    n = (n + 1);
