...
long long neutral_point();
long long acid_share();
long long base_share();
long long conjugate_pair_partitions_one();
long long neutral_is_self_antipodal();
long long neutral_folds_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== acids and bases ===");
    ok = expect_equal((long long)"neutral point = 1/2 (self-antipodal balance)", fraction_to_text(neutral_point()), (long long)"1/2");
    ok = expect_equal((long long)"acid share = 1/3", fraction_to_text(acid_share()), (long long)"1/3");
    ok = expect_equal((long long)"conjugate base share = 2/3", fraction_to_text(base_share()), (long long)"2/3");
    ok = expect_bool((long long)"conjugate pair partitions the One (pKa + pKb = pKw)", conjugate_pair_partitions_one());
    ok = expect_bool((long long)"neutrality is self-antipodal (One - 1/2 = 1/2)", neutral_is_self_antipodal());
    ok = expect_equal((long long)"the neutral point folds up to the One", fraction_to_text(neutral_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long base_share() {
    ...
    ep_gc_push_root(&acid);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    acid = fraction_from_ratio(1, colour_count());
    ret_val = fraction_subtract(one, acid);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long neutral_is_self_antipodal() {
    ...
    one = fraction_from_whole_number(1);
    neutral = fraction_from_ratio(1, binary_count());
    complement = fraction_subtract(one, neutral);
    again = fraction_from_ratio(1, binary_count());
    ret_val = fraction_compare(complement, again) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(again);
    again = 0;
