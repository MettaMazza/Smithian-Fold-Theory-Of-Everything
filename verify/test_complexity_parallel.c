...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold parallel complexity ===");
    ok = expect_equal((long long)"the deepest parallel layer has 128 transitions", int_to_string(fold_parallel_layer_work(7)), int_to_string(128));
    ok = expect_equal((long long)"total parallel work remains 254 transitions", int_to_string(fold_parallel_total_work(7)), int_to_string(254));
    ok = expect_equal((long long)"maximum parallel width is 128", int_to_string(fold_parallel_width(7)), int_to_string(128));
    ok = expect_equal((long long)"dependent parallel depth is seven rounds", int_to_string(fold_parallel_rounds(7)), int_to_string(7));
    ok = expect_true((long long)"parallel layered execution matches serial paths", fold_parallel_execution_matches_serial(7));
    ok = expect_true((long long)"the complete parallel census holds", complete_fold_parallel_census());
    ok = expect_true((long long)"a shortened parallel schedule is rejected", shortened_parallel_schedule_is_rejected(7));
    ok = expect_true((long long)"a nonbinary parallel width is rejected", nonbinary_parallel_width_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
