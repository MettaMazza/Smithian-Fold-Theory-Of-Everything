...
long long _main();
long long force_reach(long long);
long long weak_mediator_mass();
long long weak_reach();
long long lighter_carrier_reaches_farther();
long long weak_force_is_short_range();
long long massless_carrier_is_unbounded();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the weak force range ===");
    ok = expect_equal((long long)"weak mediator mass = 1/3", fraction_to_text(weak_mediator_mass()), (long long)"1/3");
    ok = expect_equal((long long)"reach of the mass-1/3 carrier = 2 (finite)", int_to_string(weak_reach()), int_to_string(2));
    ok = expect_bool((long long)"a lighter carrier reaches farther (range ~ 1/mass)", lighter_carrier_reaches_farther());
    ok = expect_bool((long long)"the weak force is SHORT-range (massive carrier)", weak_force_is_short_range());
    ok = expect_bool((long long)"a massless carrier is unbounded (reach grows without ceiling)", massless_carrier_is_unbounded());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long force_reach(long long mass) {
    ...
    reach = 0;
    order = fraction_compare(intensity, mass);
    while (order > 0) {
    intensity = fraction_subtract(intensity, mass);
    reach = (reach + 1);
    order = fraction_compare(intensity, mass);
    }
    ret_val = reach;
...
long long weak_reach() {
    ...
    mass = fraction_from_ratio(1, colour_count());
    ret_val = force_reach(mass);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long lighter_carrier_reaches_farther() {
    ...
    heavy = weak_reach();
    light_mass = fraction_from_ratio(1, 7);
    light = force_reach(light_mass);
    if (light > heavy) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long massless_carrier_is_unbounded() {
    ...
    long long light = 0;
    long long lighter = 0;
    long long lighter_mass = 0;
    long long ret_val = 0;

    ep_gc_push_root(&light_mass);
    ep_gc_push_root(&lighter_mass);

    ep_gc_maybe_collect();

    light_mass = fraction_from_ratio(1, 7);
    lighter_mass = fraction_from_ratio(1, 15);
    light = force_reach(light_mass);
    lighter = force_reach(lighter_mass);
    if (lighter > light) {
    ret_val = 1LL;
    goto L_cleanup;
    }
