...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== memory persistence (a held orbit) ===");
    ok = expect_equal((long long)"memory state = 1/3, refresh = 2/3", fraction_to_text(memory_state()), (long long)"1/3");
    ok = expect_equal((long long)"refresh state = 2/3", fraction_to_text(refresh_state()), (long long)"2/3");
    ok = expect_bool((long long)"memory folds to refresh", memory_folds_to_refresh());
    ok = expect_bool((long long)"refresh folds back to memory (perpetual cycle)", refresh_folds_to_memory());
    ok = expect_bool((long long)"the held pattern returns after two folds (period 2)", pattern_returns_after_two());
    ok = expect_equal((long long)"memory + refresh partition the One", fraction_to_text(memory_plus_refresh()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
