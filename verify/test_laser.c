...
long long lasing_threshold();
long long threshold_is_balanced();
long long threshold_folds_to_one();
long long gain_plus_loss();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the laser (light above threshold turns coherent) ===");
    ok = expect_equal((long long)"lasing threshold = gain=loss balance = 1/2", fraction_to_text(lasing_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"the threshold is self-antipodal (gain balances loss)", threshold_is_balanced());
    ok = expect_equal((long long)"above threshold: folds to the One (one shared mode)", fraction_to_text(threshold_folds_to_one()), (long long)"1");
    ok = expect_equal((long long)"gain + loss = the One at threshold", fraction_to_text(gain_plus_loss()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long threshold_is_balanced() {
    ...
    complement = fraction_subtract(one, lasing_threshold());
    ret_val = fraction_compare(complement, lasing_threshold()) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long threshold_folds_to_one() {
    ...
    ret_val = fold_fraction(lasing_threshold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long gain_plus_loss() {
    ...
    t = fraction_from_ratio(1, binary_count());
