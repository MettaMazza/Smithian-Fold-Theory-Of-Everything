...
long long antipode(long long);
long long critical_line();
long long line_is_its_own_partner();
long long quarter_pairs_across();
long long third_pairs_across();
long long line_folds_to_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Riemann critical line (the antipode's fixed axis) ===");
    ok = expect_equal((long long)"the critical line = 1/2", fraction_to_text(critical_line()), (long long)"1/2");
    ok = expect_bool((long long)"it is its OWN antipode partner (unique fixed axis)", line_is_its_own_partner());
    ok = expect_bool((long long)"1/4 pairs across to 3/4 (off-axis values pair up)", quarter_pairs_across());
    ok = expect_bool((long long)"1/3 pairs across to 2/3", third_pairs_across());
    ok = expect_equal((long long)"the balance axis folds to unison", fraction_to_text(line_folds_to_unison()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long antipode(long long x) {
    long long one = 0;
    long long ret_val = 0;
...
long long line_is_its_own_partner() {
    ...
    line = fraction_from_ratio(1, binary_count());
    partner = antipode(line);
    ret_val = fraction_compare(partner, line) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(partner);
    partner = 0;
    return ret_val;
}
...
long long quarter_pairs_across() {
    ...
    partner = antipode(quarter);
    expected = fraction_from_ratio(3, 4);
    ret_val = fraction_compare(partner, expected) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long third_pairs_across() {
    ...
    partner = antipode(third);
    expected = fraction_from_ratio(2, 3);
    ret_val = fraction_compare(partner, expected) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long line_folds_to_unison() {
    ...
    ret_val = fold_fraction(critical_line());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
