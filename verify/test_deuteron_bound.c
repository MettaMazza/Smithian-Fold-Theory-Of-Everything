...
long long _main();
long long deuteron_total_spin();
long long triplet_multiplicity();
long long proton_neutron_binds();
long long identical_nucleons_bind();
long long binding_is_spin_dependent();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the deuteron ===");
    ok = expect_number((long long)"deuteron total spin = 1 (triplet)", deuteron_total_spin(), 1);
    ok = expect_number((long long)"triplet multiplicity = 2*spin+1 = 3", triplet_multiplicity(), 3);
    ok = expect_bool((long long)"proton-neutron pair binds (distinguishable)", proton_neutron_binds());
    ok = expect_bool((long long)"di-proton / di-neutron do NOT bind (Pauli)", identical_nucleons_bind() == 0LL);
    ok = expect_bool((long long)"the binding is spin-dependent", binding_is_spin_dependent());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long triplet_multiplicity() {
    ...
    spin = 1;
    ret_val = ((smallest_fold_period_above(1) * spin) + 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
