...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== fold number theory: the integers as orbit dynamics ===");
    ok = expect_equal((long long)"the eternal period of q=7: ord_7(2)", int_to_string(order_of_two(7)), (long long)"3");
    ok = expect_equal((long long)"the eternal period of q=31", int_to_string(order_of_two(31)), (long long)"5");
    ok = expect_bool((long long)"[1] ORBIT-LENGTH LAW: every reduced p/q, every odd q to 99", orbit_length_law_to(99));
    ok = expect_equal((long long)"[2] the orbit census at q=7: two eternal modes", int_to_string(orbit_census(7)), (long long)"2");
    ok = expect_equal((long long)"[2] the orbit census at q=31: six modes", int_to_string(orbit_census(31)), (long long)"6");
    ok = expect_equal((long long)"[2] the orbit census at q=73: eight modes", int_to_string(orbit_census(73)), (long long)"8");
    ok = expect_bool((long long)"[2] ORBIT COUNT: exact tiling for every odd q to 99", orbit_count_law_to(99));
    ok = expect_bool((long long)"[3] ANTIPODES: every pair sums to the One; only 1/2 self-paired (to 99)", antipodal_law_to(99));
    ok = expect_equal((long long)"[4] the transient of q=48: four decay steps", int_to_string(transient_length(48)), (long long)"4");
    ok = expect_equal((long long)"[4] the eternal core of q=48", int_to_string(eternal_core(48)), (long long)"3");
    ok = expect_bool((long long)"[4] ETERNAL vs TRANSIENT: walked for every even q to 64", transient_law_to(64));
    ok = expect_equal((long long)"[5] odd primes to 4000", int_to_string(odd_prime_count_to(4000)), (long long)"549");
    ok = expect_equal((long long)"[5] FULLY ERGODIC: single full-orbit primes to 4000 (density 0.390 vs Artin 0.374)", int_to_string(fully_ergodic_count_to(4000)), (long long)"214");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
