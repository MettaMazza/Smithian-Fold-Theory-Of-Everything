...
long long _main();
long long colour_charge(long long);
long long baryon_colour_sum();
long long baryon_is_neutral();
long long anticolour_of_first();
long long meson_is_neutral();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== colour neutrality (confinement) ===");
    ok = expect_equal((long long)"colour charge 0 = 1/3", fraction_to_text(colour_charge(0)), (long long)"1/3");
    ok = expect_equal((long long)"colour charge 1 = 2/3", fraction_to_text(colour_charge(1)), (long long)"2/3");
    ok = expect_equal((long long)"colour charge 2 = 1 (3/3)", fraction_to_text(colour_charge(2)), (long long)"1");
    ok = expect_equal((long long)"baryon colour sum = 2 (a whole)", fraction_to_text(baryon_colour_sum()), (long long)"2");
    ok = expect_bool((long long)"a baryon (three colours) is colour-neutral", baryon_is_neutral());
    ok = expect_equal((long long)"anticolour of 1/3 = 2/3", fraction_to_text(anticolour_of_first()), (long long)"2/3");
    ok = expect_bool((long long)"a meson (colour + anticolour) is colour-neutral", meson_is_neutral());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long colour_charge(long long k) {
    long long top = 0;
    long long ret_val = 0;
    ...
    top = (1 + k);
    ret_val = fraction_from_ratio(top, colour_count());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long baryon_colour_sum() {
    ...
    first = colour_charge(0);
    second = colour_charge(1);
    third = colour_charge(2);
    partial = fraction_add(first, second);
    ret_val = fraction_add(partial, third);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long baryon_is_neutral() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&cast);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    total = baryon_colour_sum();
    cast = cast_out_whole_ones(total);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(cast, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(cast);
    cast = 0;
    return ret_val;
}
...
long long anticolour_of_first() {
    ...
    one = fraction_from_whole_number(1);
    colour = colour_charge(0);
    ret_val = fraction_subtract(one, colour);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long meson_is_neutral() {
    ...
    colour = colour_charge(0);
    anti = anticolour_of_first();
    total = fraction_add(colour, anti);
    one = fraction_from_whole_number(1);
