...
long long _main();
long long temporal_whole();
long long spatial_excursion();
long long interval_squared();
long long interval_is_timelike();
long long triple_closes();
long long proper_time_kept();
long long proper_time_squares_to_interval();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Minkowski interval (spacetime's causal ruler) ===");
    ok = expect_equal((long long)"spatial excursion dx = 3/5", fraction_to_text(spatial_excursion()), (long long)"3/5");
    ok = expect_equal((long long)"interval ds^2 = 1 - 9/25 = 16/25", fraction_to_text(interval_squared()), (long long)"16/25");
    ok = expect_bool((long long)"the interval is timelike (inside the cone)", interval_is_timelike());
    ok = expect_bool((long long)"the 3-4-5 causal triple closes to the whole tick", triple_closes());
    ok = expect_bool((long long)"kept proper time ds = 4/5 squares to the interval", proper_time_squares_to_interval());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spatial_excursion() {
    ...
    ret_val = fraction_from_ratio(colour_count(), (binary_count() + colour_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long interval_squared() {
    long long dx = 0;
    long long dx_squared = 0;
    long long whole = 0;
    long long ret_val = 0;

    ep_gc_push_root(&dx);
    ep_gc_push_root(&dx_squared);
    ep_gc_push_root(&whole);

    ep_gc_maybe_collect();

    whole = fraction_from_whole_number(1);
    dx = fraction_from_ratio(colour_count(), (binary_count() + colour_count()));
    dx_squared = fraction_multiply(dx, dx);
    ret_val = fraction_subtract(whole, dx_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long interval_is_timelike() {
    long long ds_squared = 0;
    long long ret_val = 0;

    ep_gc_push_root(&ds_squared);

    ep_gc_maybe_collect();

    ds_squared = interval_squared();
    ret_val = ({ long long _fap = ds_squared; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(ds_squared);
    ds_squared = 0;
    return ret_val;
}
...
long long triple_closes() {
    ...
    dx_squared = fraction_multiply(dx, dx);
    ds_squared = interval_squared();
    total = fraction_add(dx_squared, ds_squared);
    whole = fraction_from_whole_number(1);
    ret_val = fraction_compare(total, whole) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(total);
    total = 0;
    free_struct_Fraction(whole);
    whole = 0;
    return ret_val;
}

long long proper_time_kept() {
    long long four_fifths = 0;
    long long ret_val = 0;

    ep_gc_push_root(&four_fifths);

    ep_gc_maybe_collect();

    four_fifths = fraction_from_ratio(4, (binary_count() + colour_count()));
    ret_val = four_fifths;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long proper_time_squares_to_interval() {
    ...
    ep_gc_push_root(&ds);
    ep_gc_push_root(&ds_squared);
    ep_gc_push_root(&interval);

    ep_gc_maybe_collect();

    ds = fraction_from_ratio(4, (binary_count() + colour_count()));
    ds_squared = fraction_multiply(ds, ds);
    interval = interval_squared();
    ret_val = fraction_compare(ds_squared, interval) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(ds_squared);
    ds_squared = 0;
    free_struct_Fraction(interval);
    interval = 0;
    return ret_val;
}
