...
long long left_enantiomer();
long long right_enantiomer();
long long achiral_properties_identical();
long long chiral_probe_distinguishes();
long long racemic_is_the_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== stereochemistry (mirror molecules) ===");
    ok = expect_equal((long long)"left enantiomer = 1/4", fraction_to_text(left_enantiomer()), (long long)"1/4");
    ok = expect_equal((long long)"right enantiomer = 3/4", fraction_to_text(right_enantiomer()), (long long)"3/4");
    ok = expect_bool((long long)"achiral properties identical (same fold image 1/2)", achiral_properties_identical());
    ok = expect_bool((long long)"a chiral probe distinguishes them (different preimages)", chiral_probe_distinguishes());
    ok = expect_equal((long long)"a racemic mixture is the whole (1/4 + 3/4 = 1)", fraction_to_text(racemic_is_the_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long achiral_properties_identical() {
    ...
    left_image = fold_fraction(left_enantiomer());
    right_image = fold_fraction(right_enantiomer());
...
long long chiral_probe_distinguishes() {
    ...
    comparison = fraction_compare(left_enantiomer(), right_enantiomer());
...
long long racemic_is_the_whole() {
    ...
    left = fraction_from_ratio(1, (binary_count() * binary_count()));
    right = fraction_from_ratio(colour_count(), (binary_count() * binary_count()));
    ret_val = fraction_add(left, right);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
