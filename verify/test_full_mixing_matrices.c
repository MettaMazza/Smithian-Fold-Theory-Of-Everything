...
long long channel_state_full(long long);
long long lepton_mass_state(long long);
long long quark_mass_state(long long);
long long lepton_fibre_verified();
long long separation_of(long long, long long);
long long pmns_element(long long, long long);
long long first_row_is_wide();
long long lepton_diagonal_below_quark();
long long pmns_distances_fold_to_balance();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the two mixing matrices (leptons wide, quarks narrow) ===");
    ok = expect_bool((long long)"the lepton fibre {1/6, 1/2, 5/6} verified (all land on 1/2)", lepton_fibre_verified());
    ok = expect_bool((long long)"the first PMNS row is {5/6, 1/2, 1/6} -- WIDE", first_row_is_wide());
    ok = expect_equal((long long)"U_12 = 1/2 (the huge solar element)", fraction_to_text(pmns_element(1, 2)), (long long)"1/2");
    ok = expect_bool((long long)"the lepton diagonal 5/6 sits BELOW the quark 8/9 (leptons leak more)", lepton_diagonal_below_quark());
    ok = expect_bool((long long)"ROUTE B: all nine PMNS distances fold to the balance 1/2", pmns_distances_fold_to_balance());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long channel_state_full(long long j) {
    long long ret_val = 0;
...
long long lepton_mass_state(long long i) {
    long long ret_val = 0;
...
long long quark_mass_state(long long i) {
    long long ret_val = 0;
...
long long lepton_fibre_verified() {
    ...
    lock = fraction_from_ratio(1, 2);
    all_land = 1LL;
    i = 1;
    while (i < 4) {
    state = fraction_from_ratio(((2 * i) - 1), 6);
    tripled = tripling_fold(state);
    lands = fraction_compare(tripled, lock) == 0;
    if (lands) {
    all_land = all_land;
    } else {
    all_land = 0LL;
...
L_cleanup:
    ...
    lock = 0;
    free_struct_Fraction(tripled);
    tripled = 0;
    return ret_val;
}

long long separation_of(long long mass, long long weak) {
...
long long pmns_element(long long i, long long j) {
    ...
    mass = fraction_from_ratio(((2 * i) - 1), 6);
    weak = fraction_from_ratio(j, 3);
    distance = separation_of(mass, weak);
...
long long first_row_is_wide() {
    ...
    ep_gc_push_root(&e12);
    ep_gc_push_root(&e13);
    ep_gc_push_root(&five_sixths);
    ep_gc_push_root(&half);
    ep_gc_push_root(&sixth);

    ep_gc_maybe_collect();

    e11 = pmns_element(1, 1);
    e12 = pmns_element(1, 2);
    e13 = pmns_element(1, 3);
    five_sixths = fraction_from_ratio(5, 6);
    half = fraction_from_ratio(1, 2);
    sixth = fraction_from_ratio(1, 6);
    first_ok = fraction_compare(e11, five_sixths) == 0;
    second_ok = fraction_compare(e12, half) == 0;
    third_ok = fraction_compare(e13, sixth) == 0;
    if (first_ok) {
    if (second_ok) {
    ret_val = third_ok;
    goto L_cleanup;
...
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(e11);
    e11 = 0;
    free_struct_Fraction(e12);
    e12 = 0;
    free_struct_Fraction(e13);
    e13 = 0;
    free_struct_Fraction(five_sixths);
    five_sixths = 0;
    free_struct_Fraction(half);
    half = 0;
    free_struct_Fraction(sixth);
    sixth = 0;
    return ret_val;
}
...
long long lepton_diagonal_below_quark() {
    long long comparison = 0;
    long long lepton_diag = 0;
    long long one = 0;
    long long quark_diag = 0;
    long long quark_distance = 0;
    long long quark_mass = 0;
    long long weak = 0;
    long long ret_val = 0;

    ep_gc_push_root(&lepton_diag);
    ep_gc_push_root(&one);
    ep_gc_push_root(&quark_diag);
    ep_gc_push_root(&quark_distance);
    ep_gc_push_root(&quark_mass);
    ep_gc_push_root(&weak);

    ep_gc_maybe_collect();

    lepton_diag = pmns_element(1, 1);
    one = fraction_from_whole_number(1);
    quark_mass = fraction_from_ratio(2, 9);
    weak = fraction_from_ratio(1, 3);
    quark_distance = separation_of(quark_mass, weak);
    quark_diag = fraction_subtract(one, quark_distance);
    comparison = fraction_compare(lepton_diag, quark_diag);
...
long long pmns_distances_fold_to_balance() {
    ...
    weak = fraction_from_ratio(j, 3);
    distance = separation_of(mass, weak);
    folded = tripling_fold(distance);
    lands = fraction_compare(folded, lock) == 0;
    if (lands) {
    all_land = all_land;
    } else {
    all_land = 0LL;
