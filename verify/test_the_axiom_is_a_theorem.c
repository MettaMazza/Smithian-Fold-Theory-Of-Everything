...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the axiom is a theorem ===");
    printf("%s\n", (char*)(long long)"--- step 1: the fold factor is the smallest period, forced ---");
    ok = expect_equal((long long)"fold factor = binary (smallest fold period)", int_to_string(fold_factor()), int_to_string(2));
    printf("%s\n", (char*)(long long)"--- step 2: the ground is DERIVED, not assumed ---");
    ok = expect_true((long long)"not nothing: the ground is strictly positive", ground_is_not_nothing());
    ok = expect_equal((long long)"ground doubled is the One: 1/2 + 1/2 = 1 (the forcing equation)", fraction_to_text(ground_doubles_to_one()), (long long)"1");
    ok = expect_equal((long long)"the unique solution x = One/binary = 1/2", fraction_to_text(displaced_ground()), (long long)"1/2");
    ok = expect_equal((long long)"equivalently its own antipode: One - 1/2 = 1/2", fraction_to_text(displaced_ground_is_self_antipodal()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"--- step 3: the One is reached and fixed ---");
