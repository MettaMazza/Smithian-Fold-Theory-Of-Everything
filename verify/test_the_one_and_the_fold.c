...
long long _main() {
    long long folded_one = 0;
    long long another_third = 0;
    long long a_third = 0;
    long long three_quarters = 0;
    long long ok = 0;
    long long beat_gap = 0;
    long long folded_twice = 0;
    long long folded_third = 0;
    long long beat_equal = 0;
    long long one = 0;
    long long a_fifth = 0;
    long long one_minus_quarter = 0;
    long long a_quarter = 0;
    long long ret_val = 0;

    ep_gc_push_root(&folded_one);
    ep_gc_push_root(&another_third);
    ep_gc_push_root(&a_third);
    ep_gc_push_root(&three_quarters);
    ep_gc_push_root(&beat_gap);
    ep_gc_push_root(&folded_twice);
    ep_gc_push_root(&folded_third);
    ep_gc_push_root(&beat_equal);
    ep_gc_push_root(&one);
    ep_gc_push_root(&a_fifth);
    ep_gc_push_root(&one_minus_quarter);
    ep_gc_push_root(&a_quarter);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the One and the fold ===");
    one = the_one();
    ok = expect_equal((long long)"the One is 1", fold_value_to_text(one), (long long)"1");
    ok = expect_equal((long long)"the One's derivation", ({ long long _fap = one; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'derivation' on 'FoldValue'\n"); exit(1); } ((EpStruct_FoldValue*)(_fap))->derivation; }), (long long)"ONE");
    folded_one = fold(one);
    ok = expect_equal((long long)"fold(ONE) = 1", fold_value_to_text(folded_one), (long long)"1");
    ok = expect_equal((long long)"period of the One", int_to_string(fold_period(one, 100)), int_to_string(1));
    a_third = supposed_value(1, 3);
    folded_third = fold(a_third);
    ok = expect_equal((long long)"fold(1/3) = 2/3", fold_value_to_text(folded_third), (long long)"2/3");
    folded_twice = fold(folded_third);
    ok = expect_equal((long long)"fold(fold(1/3)) = 1/3", fold_value_to_text(folded_twice), (long long)"1/3");
    ok = expect_equal((long long)"period of 1/3", int_to_string(fold_period(a_third, 100)), int_to_string(2));
    ok = expect_equal((long long)"readable derivation of fold(fold(1/3))", ({ long long _fap = folded_twice; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'derivation' on 'FoldValue'\n"); exit(1); } ((EpStruct_FoldValue*)(_fap))->derivation; }), (long long)"fold(fold(supposed(1/3)))");
    a_quarter = supposed_value(1, 4);
    one_minus_quarter = take(one, a_quarter);
    ok = expect_equal((long long)"take(1, 1/4) = 3/4", fold_value_to_text(one_minus_quarter), (long long)"3/4");
    if (can_take(one, a_quarter)) {
    printf("%s\n", (char*)(long long)"  ok    take(1, 1/4) is permitted");
    } else {
    printf("%s\n", (char*)(long long)"  FAIL  take(1, 1/4) should be permitted");
    }
    if (can_take(a_quarter, one)) {
    printf("%s\n", (char*)(long long)"  FAIL  take(1/4, 1) should be forbidden");
    } else {
    printf("%s\n", (char*)(long long)"  ok    take(1/4, 1) is forbidden");
    }
    a_fifth = supposed_value(1, 5);
    ok = expect_equal((long long)"period of 1/5", int_to_string(fold_period(a_fifth, 100)), int_to_string(4));
    another_third = supposed_value(1, 3);
    beat_equal = beat_between(a_third, another_third);
    ok = expect_equal((long long)"beat(1/3, 1/3) = 1", fold_value_to_text(beat_equal), (long long)"1");
    three_quarters = supposed_value(3, 4);
    beat_gap = beat_between(three_quarters, a_quarter);
    ok = expect_equal((long long)"beat(3/4, 1/4) = 1/2", fold_value_to_text(beat_gap), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(12);
    free_struct_FoldValue(folded_one);
    folded_one = 0;
    free_struct_FoldValue(beat_gap);
    beat_gap = 0;
    free_struct_FoldValue(folded_twice);
    folded_twice = 0;
    free_struct_FoldValue(beat_equal);
    beat_equal = 0;
    free_struct_FoldValue(a_fifth);
    a_fifth = 0;
    free_struct_FoldValue(one_minus_quarter);
    one_minus_quarter = 0;
    return ret_val;
}
