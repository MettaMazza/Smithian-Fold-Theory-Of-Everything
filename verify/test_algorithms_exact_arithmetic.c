...
long long _main() {
    ...
    ep_gc_push_root(&division);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== Fold exact arithmetic algorithms ===");
    division = fold_counted_divide(23, 5);
    ok = expect_equal((long long)"counted addition returns twelve", int_to_string(fold_counted_add(5, 7)), int_to_string(12));
    ok = expect_equal((long long)"counted product returns thirty-five", int_to_string(fold_counted_multiply(5, 7)), int_to_string(35));
    ok = expect_equal((long long)"counted power returns 128", int_to_string(fold_counted_power(2, 7)), int_to_string(128));
    ok = expect_equal((long long)"division quotient is four", int_to_string(({ long long _fap = division; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'quotient' on 'FoldDivision'\n"); exit(1); } ((EpStruct_FoldDivision*)(_fap))->quotient; })), int_to_string(4));
    ok = expect_equal((long long)"division remainder is three", int_to_string(({ long long _fap = division; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'remainder' on 'FoldDivision'\n"); exit(1); } ((EpStruct_FoldDivision*)(_fap))->remainder; })), int_to_string(3));
    ok = expect_true((long long)"the complete exact-arithmetic census holds", complete_fold_exact_arithmetic_census());
    ok = expect_true((long long)"a zero divisor is rejected", zero_divisor_is_rejected(23));
    ok = expect_true((long long)"a perturbed sum is rejected", perturbed_counted_sum_is_rejected(5, 7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_FoldDivision(division);
    division = 0;
    return ret_val;
}
