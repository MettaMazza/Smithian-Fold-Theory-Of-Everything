...
long long composed_chain();
long long intermediates_in_domain();
long long orbit_route();
long long playthroughs_agree();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the unfolding sequence (a derivation is playable) ===");
    ok = expect_equal((long long)"PLAYTHROUGH A: fold(take(One, fold(1/3))) = 2/3 (three moves)", fraction_to_text(composed_chain()), (long long)"2/3");
    ok = expect_bool((long long)"every intermediate is a domain value (positive, bounded)", intermediates_in_domain());
    ok = expect_equal((long long)"PLAYTHROUGH B: the orbit route, one move = 2/3", fraction_to_text(orbit_route()), (long long)"2/3");
    ok = expect_bool((long long)"the two playthroughs agree exactly (one value, two orders)", playthroughs_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long composed_chain() {
    long long first_move = 0;
    long long second_move = 0;
    long long start = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_move);
    ep_gc_push_root(&second_move);
    ep_gc_push_root(&start);

    ep_gc_maybe_collect();

    start = fraction_from_ratio(1, 3);
    first_move = fold_fraction(start);
    second_move = take_from_one(first_move);
    ret_val = fold_fraction(second_move);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long intermediates_in_domain() {
    long long first_bounded = 0;
    long long first_move = 0;
    long long first_positive = 0;
    long long one = 0;
    long long second_bounded = 0;
    long long second_move = 0;
    long long second_positive = 0;
    long long start = 0;
    long long ret_val = 0;
    ...
    second_move = take_from_one(first_move);
    one = fraction_from_whole_number(1);
    first_positive = ({ long long _fap = first_move; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    second_positive = ({ long long _fap = second_move; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    first_bounded = fraction_compare(first_move, one) <= 0;
    second_bounded = fraction_compare(second_move, one) <= 0;
    if (first_positive) {
    if (second_positive) {
    if (first_bounded) {
    ret_val = second_bounded;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
L_cleanup:
    ...
    one = 0;
    free_struct_Fraction(second_move);
    second_move = 0;
    return ret_val;
}
...
long long playthroughs_agree() {
    long long chain = 0;
    long long chain_ok = 0;
    long long orbit = 0;
    long long orbit_ok = 0;
    long long target = 0;
    long long ret_val = 0;

    ep_gc_push_root(&chain);
    ep_gc_push_root(&orbit);
    ...
    chain_ok = fraction_compare(chain, target) == 0;
    orbit_ok = fraction_compare(orbit, target) == 0;
    if (chain_ok) {
    ret_val = orbit_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(chain);
    chain = 0;
    free_struct_Fraction(orbit);
    orbit = 0;
