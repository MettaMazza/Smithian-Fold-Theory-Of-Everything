...
long long period_two_point();
long long point_antipode();
long long double_fold_is_identity();
long long double_fold_identity_on_antipode();
long long orbit_sums_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the one-fold equation (fold^2 = identity) ===");
    ok = expect_equal((long long)"period-2 point = 1/3", fraction_to_text(period_two_point()), (long long)"1/3");
    ok = expect_equal((long long)"its antipode = 2/3", fraction_to_text(point_antipode()), (long long)"2/3");
    ok = expect_bool((long long)"folding TWICE is the identity on the point (fold^2 = id)", double_fold_is_identity());
    ok = expect_bool((long long)"and on its antipode", double_fold_identity_on_antipode());
    ok = expect_equal((long long)"the orbit's two points sum to the One", fraction_to_text(orbit_sums_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long point_antipode() {
    ...
    ret_val = fraction_subtract(one, period_two_point());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long double_fold_is_identity() {
    ...
    point = fraction_from_ratio(1, colour_count());
...
long long orbit_sums_to_one() {
    ...
    point = fraction_from_ratio(1, colour_count());
    antipode = point_antipode();
    ret_val = fraction_add(point, antipode);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
