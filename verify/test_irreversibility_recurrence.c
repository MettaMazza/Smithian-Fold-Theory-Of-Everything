...
long long descent_state();
long long chain_arrives_in_depth();
long long arrival_is_permanent();
long long cycle_returns_exactly();
long long cycle_avoids_fixed_point();
long long preimages_merge();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== irreversibility and recurrence (two timescales, one dynamics) ===");
    ok = expect_equal((long long)"the descent state = 1/4 (depth 2)", fraction_to_text(descent_state()), (long long)"1/4");
    ok = expect_bool((long long)"the chain arrives at the fixed point in its depth (2 folds)", chain_arrives_in_depth());
    ok = expect_bool((long long)"arrival is one-way: fold(1) = 1, the chain never leaves", arrival_is_permanent());
    ok = expect_bool((long long)"the cycle 1/3 <-> 2/3 returns EXACTLY (Poincare recurrence)", cycle_returns_exactly());
    ok = expect_bool((long long)"the cycle never meets the fixed point (no orbit is both)", cycle_avoids_fixed_point());
    ok = expect_bool((long long)"Loschmidt dissolves: 1/4 and 3/4 merge at 1/2 (the arrow is the merging)", preimages_merge());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long cycle_returns_exactly() {
    ...
    point = fraction_from_ratio(1, 3);
    once = fold_fraction(point);
    twice = fold_fraction(once);
    ret_val = fraction_compare(twice, point) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(twice);
...
long long cycle_avoids_fixed_point() {
    long long high = 0;
    long long high_is_one = 0;
    long long low = 0;
    long long low_is_one = 0;
    long long one = 0;
    long long ret_val = 0;
    ...
    low = fraction_from_ratio(1, 3);
    high = fold_fraction(low);
    one = fraction_from_whole_number(1);
    low_is_one = fraction_compare(low, one) == 0;
    high_is_one = fraction_compare(high, one) == 0;
    if (low_is_one) {
    ret_val = 0LL;
    goto L_cleanup;
    }
