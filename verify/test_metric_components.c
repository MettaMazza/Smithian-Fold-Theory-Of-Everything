...
long long _main();
long long spacetime_dimension();
long long metric_components(long long);
long long physical_dof(long long);
long long components_in_spacetime();
long long dof_in_spacetime();
long long dof_in_three_dimensions();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the metric's degrees of freedom (two graviton polarisations) ===");
    ok = expect_equal((long long)"spacetime dimension D = binary^2 = 4", int_to_string(spacetime_dimension()), (long long)"4");
    ok = expect_equal((long long)"metric components D(D+1)/2 = 10", int_to_string(components_in_spacetime()), (long long)"10");
    ok = expect_equal((long long)"physical DOF D(D-3)/2 = 2 (two graviton polarisations)", int_to_string(dof_in_spacetime()), (long long)"2");
    ok = expect_equal((long long)"in 2+1D: components = 6", int_to_string(metric_components(colour_count())), (long long)"6");
    ok = expect_equal((long long)"in 2+1D: physical DOF = 0 (no gravitational waves)", int_to_string(dof_in_three_dimensions()), (long long)"0");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spacetime_dimension() {
    ...
    ret_val = whole_power(binary_count(), binary_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long metric_components(long long d) {
    long long ret_val = 0;

    ret_val = ((d * (d + 1)) / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long physical_dof(long long d) {
    long long ret_val = 0;

    ret_val = ((d * (d - smallest_fold_period_above(binary_count()))) / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long components_in_spacetime() {
    ...
    ret_val = ((whole_power(binary_count(), binary_count()) * (whole_power(binary_count(), binary_count()) + 1)) / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long dof_in_spacetime() {
    ...
    ret_val = ((whole_power(binary_count(), binary_count()) * (whole_power(binary_count(), binary_count()) - smallest_fold_period_above(binary_count()))) / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long dof_in_three_dimensions() {
    ...
    ret_val = ((smallest_fold_period_above(binary_count()) * (smallest_fold_period_above(binary_count()) - smallest_fold_period_above(binary_count()))) / smallest_fold_period_above(1));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
