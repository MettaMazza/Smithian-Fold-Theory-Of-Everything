...
long long mass_gap();
long long strong_self_coupling();
long long gap_is_strictly_positive();
long long gap_folds_to_coupling();
long long coupling_folds_to_gap();
long long gap_plus_coupling();
long long sector_balance_reaches_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Yang-Mills mass gap (gap > 0, forced by the domain) ===");
    ok = expect_equal((long long)"mass gap = 1/3 (the tripling position)", fraction_to_text(mass_gap()), (long long)"1/3");
    ok = expect_bool((long long)"the gap is strictly positive (No-Zero floor)", gap_is_strictly_positive());
    ok = expect_equal((long long)"self-coupling = 2/3", fraction_to_text(strong_self_coupling()), (long long)"2/3");
    ok = expect_bool((long long)"gap folds to coupling (closed cycle)", gap_folds_to_coupling());
    ok = expect_bool((long long)"coupling folds back to gap (period 2)", coupling_folds_to_gap());
    ok = expect_equal((long long)"gap + coupling = the One", fraction_to_text(gap_plus_coupling()), (long long)"1");
    ok = expect_equal((long long)"the sector balance folds to unison", fraction_to_text(sector_balance_reaches_unison()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long gap_is_strictly_positive() {
    ...
    gap = fraction_from_ratio(1, colour_count());
    ret_val = ({ long long _fap = gap; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long gap_folds_to_coupling() {
    ...
    gap = fraction_from_ratio(1, colour_count());
    folded = fold_fraction(gap);
    coupling = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_compare(folded, coupling) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long coupling_folds_to_gap() {
    ...
    coupling = fraction_from_ratio(binary_count(), colour_count());
    folded = fold_fraction(coupling);
    gap = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, gap) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(folded);
...
long long gap_plus_coupling() {
    ...
    gap = fraction_from_ratio(1, colour_count());
    coupling = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_add(gap, coupling);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long sector_balance_reaches_unison() {
    ...
    whole = gap_plus_coupling();
    two = fraction_from_whole_number(binary_count());
    balance = fraction_divide(whole, two);
