...
long long fixed_point_count();
long long initial_condition_is_one();
long long step_merges_two_into_one();
long long bits_lost_per_step();
long long inflation_depth();
long long expansion_factor();
long long expansion_matches_tower();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the cosmological timeline (beginning, arrow, inflation) ===");
    ok = expect_equal((long long)"BEGINNING: fixed points on the eighth grid (exhaustive) = 1", int_to_string(fixed_point_count()), (long long)"1");
    ok = expect_bool((long long)"the unique fixed point is the One (the initial condition)", initial_condition_is_one());
    ok = expect_bool((long long)"ARROW: each step merges two states into one (1/4, 3/4 -> 1/2)", step_merges_two_into_one());
    ok = expect_equal((long long)"one bit lost per step (the KS entropy)", int_to_string(bits_lost_per_step()), (long long)"1");
    ok = expect_equal((long long)"INFLATION: the depth, counted (minimal cover of 27) = 5", int_to_string(inflation_depth()), (long long)"5");
    ok = expect_equal((long long)"the expansion factor, enumerated = 32", int_to_string(expansion_factor()), (long long)"32");
    ok = expect_bool((long long)"the enumeration equals the closed form 2^5", expansion_matches_tower());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fixed_point_count() {
    ...
    k = 1;
    while (k < 9) {
    p = fraction_from_ratio(k, 8);
    folded = fold_fraction(p);
    fixed = fraction_compare(folded, p) == 0;
    if (fixed) {
    hits = (hits + 1);
    }
    k = (k + 1);
...
long long step_merges_two_into_one() {
    ...
    long long from_high = 0;
    long long from_low = 0;
    long long high = 0;
    long long low = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_high);
    ep_gc_push_root(&from_low);
    ...
    distinct = fraction_compare(low, high) == 0;
    if (distinct) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    from_low = fold_fraction(low);
    from_high = fold_fraction(high);
    ret_val = fraction_compare(from_low, from_high) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(from_high);
    from_high = 0;
    free_struct_Fraction(from_low);
    from_low = 0;
    return ret_val;
}

long long bits_lost_per_step() {
    long long preimages_per_state = 0;
    long long ret_val = 0;

    preimages_per_state = smallest_fold_period_above(1);
    ret_val = (preimages_per_state - 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long expansion_factor() {
    ...
    depth = inflation_depth();
    leaves = 1;
    level = 0;
    while (level < depth) {
    leaves = (leaves * smallest_fold_period_above(1));
    level = (level + 1);
    }
    ret_val = leaves;
    goto L_cleanup;
...
long long expansion_matches_tower() {
    ...
    enumerated = expansion_factor();
    closed_form = whole_power(binary_count(), inflation_depth());
    ret_val = enumerated == closed_form;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
