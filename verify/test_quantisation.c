...
long long grid_state(long long, long long);
long long state_count(long long);
long long state_returns_within(long long, long long);
long long whole_grid_returns(long long);
long long gaps_are_uniform(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== quantisation (the depth-k grid) ===");
    ok = expect_equal((long long)"depth-2 state count = 2^2 = 4", int_to_string(state_count(2)), (long long)"4");
    ok = expect_equal((long long)"grid state 3/4 at depth 2", fraction_to_text(grid_state(3, 2)), (long long)"3/4");
    ok = expect_bool((long long)"the WHOLE depth-2 grid returns to the One within 2 folds", whole_grid_returns(2));
    ok = expect_bool((long long)"the whole depth-3 grid returns within 3 folds", whole_grid_returns(3));
    ok = expect_bool((long long)"adjacent gaps are uniform at depth 2 (steps, not a ramp)", gaps_are_uniform(2));
    ok = expect_bool((long long)"adjacent gaps are uniform at depth 3", gaps_are_uniform(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long grid_state(long long i, long long k) {
    long long volume = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(i, volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long state_count(long long k) {
    long long ret_val = 0;
    ...
    ret_val = whole_power(binary_count(), k);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long state_returns_within(long long i, long long k) {
    long long current = 0;
    long long one = 0;
    long long reached = 0;
    long long step = 0;
    ...
    current = grid_state(i, k);
    step = 0;
    reached = 0;
    while (step < k) {
    current = fold_fraction(current);
    if (fraction_compare(current, one) == 0) {
    reached = 1;
    }
    step = (step + 1);
    }
...
long long whole_grid_returns(long long k) {
    long long all_return = 0;
    long long count = 0;
    long long i = 0;
    long long ret_val = 0;
    ...
    count = whole_power(binary_count(), k);
    i = 1;
    all_return = 1;
    while (i <= count) {
    if (state_returns_within(i, k)) {
...
long long gaps_are_uniform(long long k) {
    long long all_uniform = 0;
    long long count = 0;
    long long gap = 0;
    long long here = 0;
    long long i = 0;
    ...
    count = whole_power(binary_count(), k);
    spacing = fraction_from_ratio(1, count);
    i = 1;
    all_uniform = 1;
    while (i < count) {
    here = grid_state(i, k);
    next_state = grid_state((i + 1), k);
    gap = fraction_subtract(next_state, here);
    if (fraction_compare(gap, spacing) == 0) {
