...
long long _main();
long long carrier_types();
long long electron_carrier();
long long hole_carrier();
long long hole_is_electron_absence();
long long junction_balances_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== semiconductors ===");
    ok = expect_equal((long long)"carrier types = binary = 2 (electron, hole)", int_to_string(carrier_types()), int_to_string(2));
    ok = expect_equal((long long)"electron carrier = 1/4 (filled state)", fraction_to_text(electron_carrier()), (long long)"1/4");
    ok = expect_equal((long long)"hole carrier = 3/4 (electron's absence)", fraction_to_text(hole_carrier()), (long long)"3/4");
    ok = expect_bool((long long)"the hole is the electron's absence (One - electron)", hole_is_electron_absence());
    ok = expect_bool((long long)"a p-n junction balances to the One (electron + hole)", junction_balances_to_one());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long hole_carrier() {
    ...
    electron = fraction_from_ratio(1, 4);
    ret_val = fraction_subtract(one, electron);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long hole_is_electron_absence() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&absence);
    ep_gc_push_root(&hole);
    ep_gc_push_root(&electron);
    ...
    one = fraction_from_whole_number(1);
    electron = fraction_from_ratio(1, 4);
    absence = fraction_subtract(one, electron);
    hole = hole_carrier();
    ret_val = fraction_compare(absence, hole) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(absence);
    absence = 0;
    free_struct_Fraction(hole);
    hole = 0;
    return ret_val;
}
...
long long junction_balances_to_one() {
    ...
    electron = fraction_from_ratio(1, 4);
    hole = hole_carrier();
    total = fraction_add(electron, hole);
    one = fraction_from_whole_number(1);
