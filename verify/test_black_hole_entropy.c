...
long long _main();
long long one_halving();
long long entropy_area_coefficient();
long long entropy_coefficient_is_quarter();
long long entropy_coefficient_denominator();
long long horizon_entropy(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== black-hole entropy ===");
    ok = expect_equal((long long)"entropy-area coefficient = 1/4 (two binary halvings)", fraction_to_text(entropy_area_coefficient()), (long long)"1/4");
    ok = expect_bool((long long)"coefficient is exactly one quarter", entropy_coefficient_is_quarter());
    ok = expect_equal((long long)"coefficient denominator = binary^2 = 4 (forced)", int_to_string(entropy_coefficient_denominator()), int_to_string(4));
    ok = expect_equal((long long)"entropy of area 1/2: S = (1/4)(1/2) = 1/8", fraction_to_text(horizon_entropy(fraction_from_ratio(1, 2))), (long long)"1/8");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long entropy_area_coefficient() {
    ...
    first = fraction_from_ratio(1, binary_count());
    second = fraction_from_ratio(1, binary_count());
    ret_val = fraction_multiply(first, second);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long entropy_coefficient_is_quarter() {
    ...
    coefficient = entropy_area_coefficient();
    quarter = fraction_from_ratio(1, 4);
    ret_val = fraction_compare(coefficient, quarter) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long entropy_coefficient_denominator() {
    ...
    built = (smallest_fold_period_above(1) * smallest_fold_period_above(1));
    counted = whole_power(binary_count(), 2);
    ret_val = forced_to_be((long long)"horizon entropy coefficient denominator = binary^2", built, counted);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long horizon_entropy(long long area) {
    long long ret_val = 0;

    ep_gc_push_root(&area);

    ep_gc_maybe_collect();

    ret_val = fraction_multiply(entropy_area_coefficient(), area);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
