...
long long colour_state(long long);
long long anticolour_of(long long);
long long single_quark_closures();
long long quark_pair_closures();
long long meson_closures();
long long baryon_sum();
long long census_is_two_families();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the hadron census (mesons and baryons only) ===");
    ok = expect_equal((long long)"SIZE 1: single-colour closures (no free quark)", int_to_string(single_quark_closures()), (long long)"0");
    ok = expect_equal((long long)"SIZE 2 qq: pair closures, all pairs enumerated (no diquark hadron)", int_to_string(quark_pair_closures()), (long long)"0");
    ok = expect_equal((long long)"SIZE 2 q-antiq: meson closures, one per colour", int_to_string(meson_closures()), (long long)"3");
    ok = expect_equal((long long)"SIZE 3: the whole cycle closes (the baryon)", fraction_to_text(baryon_sum()), (long long)"1");
    ok = expect_bool((long long)"the census: exactly two minimal hadron families", census_is_two_families());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long colour_state(long long k) {
    ...
    numerator = whole_power(binary_count(), k);
    ret_val = fraction_from_ratio(numerator, 7);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long anticolour_of(long long c) {
    long long one = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_subtract(one, c);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long single_quark_closures() {
    ...
    c = colour_state(0);
    one = fraction_from_whole_number(1);
    closes = fraction_compare(c, one) == 0;
    if (closes) {
    hits = (hits + 1);
    }
    c = colour_state(1);
...
long long quark_pair_closures() {
    ...
    j = 1;
    while (j < 3) {
    first = colour_state(0);
    second = colour_state(j);
    total = fraction_add(first, second);
    one = fraction_from_whole_number(1);
...
long long meson_closures() {
    ...
    hits = 0;
    c = colour_state(0);
    anti = anticolour_of(c);
    total = fraction_add(c, anti);
    one = fraction_from_whole_number(1);
    closes = fraction_compare(total, one) == 0;
    if (closes) {
    hits = (hits + 1);
...
long long baryon_sum() {
    ...
    first = colour_state(0);
    second = colour_state(1);
    third = colour_state(2);
...
long long census_is_two_families() {
    long long mesons = 0;
    long long mesons_exist = 0;
    long long no_diquark = 0;
    long long no_free_quark = 0;
    long long pairs = 0;
    long long singles = 0;
    long long ret_val = 0;

    singles = single_quark_closures();
    pairs = quark_pair_closures();
    mesons = meson_closures();
    no_free_quark = singles == 0;
    no_diquark = pairs == 0;
    mesons_exist = mesons == 3;
    if (no_free_quark) {
    if (no_diquark) {
    ret_val = mesons_exist;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
