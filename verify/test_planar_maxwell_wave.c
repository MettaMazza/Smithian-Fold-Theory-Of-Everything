...
long long axis_curvature();
long long spatial_laplacian();
long long temporal_curvature();
long long wave_ratio();
long long fold_expansion();
long long wave_closes();
long long wave_speed_squared();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the planar light wave (Maxwell closes in 2D) ===");
    ok = expect_equal((long long)"the spatial Laplacian: two axes of exact 2 = 4", fraction_to_text(spatial_laplacian()), (long long)"4");
    ok = expect_equal((long long)"the temporal curvature: one axis = 2", fraction_to_text(temporal_curvature()), (long long)"2");
    ok = expect_equal((long long)"the wave ratio = 2 = the planar dimension", fraction_to_text(wave_ratio()), (long long)"2");
    ok = expect_bool((long long)"the ratio equals the measured fold expansion (the wave closes)", wave_closes());
    ok = expect_equal((long long)"the wave speed squared = 1 (light at c in the plane)", fraction_to_text(wave_speed_squared()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spatial_laplacian() {
    ...
    first_axis = fraction_from_whole_number(2);
    second_axis = fraction_from_whole_number(2);
    ret_val = fraction_add(first_axis, second_axis);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long wave_ratio() {
    long long spatial = 0;
    long long temporal = 0;
    long long ret_val = 0;

    ep_gc_push_root(&spatial);
    ep_gc_push_root(&temporal);

    ep_gc_maybe_collect();

    spatial = spatial_laplacian();
    temporal = fraction_from_whole_number(2);
    ret_val = fraction_divide(spatial, temporal);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long wave_closes() {
    ...
    ratio = wave_ratio();
    m = fold_expansion();
    ret_val = fraction_compare(ratio, m) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(m);
    m = 0;
...
long long wave_speed_squared() {
    long long dims = 0;
    long long scaled_temporal = 0;
    long long spatial = 0;
    long long temporal = 0;
    long long ret_val = 0;

    ep_gc_push_root(&dims);
    ep_gc_push_root(&scaled_temporal);
    ep_gc_push_root(&spatial);
    ep_gc_push_root(&temporal);
    ...
    spatial = spatial_laplacian();
    temporal = fraction_from_whole_number(2);
    dims = fraction_from_whole_number(2);
    scaled_temporal = fraction_multiply(dims, temporal);
    ret_val = fraction_divide(spatial, scaled_temporal);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
