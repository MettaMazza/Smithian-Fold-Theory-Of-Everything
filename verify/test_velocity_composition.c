...
long long _main();
long long causal_ceiling();
long long compose_speeds(long long, long long);
long long light_is_fixed_point();
long long sublight_stays_sublight();
long long half_with_half();
long long tenth_with_tenth();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== relativistic velocity composition ===");
    ok = expect_bool((long long)"light is the fixed point: compose(1, v) = 1", light_is_fixed_point());
    ok = expect_bool((long long)"sublight stays sublight (1/2 with 2/3 < 1)", sublight_stays_sublight());
    ok = expect_equal((long long)"compose(1/2, 1/2) = 4/5 (not 1)", fraction_to_text(half_with_half()), (long long)"4/5");
    ok = expect_equal((long long)"compose(1/10, 1/10) = 20/101 (Galileo recovered)", fraction_to_text(tenth_with_tenth()), (long long)"20/101");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long compose_speeds(long long u, long long v) {
    long long overlap = 0;
    long long one = 0;
    long long divisor = 0;
    ...
    ep_gc_push_root(&naive_sum);
    ep_gc_push_root(&u);
    ep_gc_push_root(&v);

    ep_gc_maybe_collect();

    naive_sum = fraction_add(u, v);
    overlap = fraction_multiply(u, v);
    one = fraction_from_whole_number(1);
    divisor = fraction_add(one, overlap);
    ret_val = fraction_divide(naive_sum, divisor);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long light_is_fixed_point() {
    ...
    c = fraction_from_whole_number(1);
    v = fraction_from_ratio(1, 2);
    composed = compose_speeds(c, v);
    ret_val = fraction_compare(composed, c) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long sublight_stays_sublight() {
    ...
    u = fraction_from_ratio(1, 2);
    v = fraction_from_ratio(2, 3);
    composed = compose_speeds(u, v);
    c = fraction_from_whole_number(1);
    comparison = fraction_compare(composed, c);
...
long long tenth_with_tenth() {
    ...
    u = fraction_from_ratio(1, 10);
    v = fraction_from_ratio(1, 10);
    ret_val = compose_speeds(u, v);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
