...
long long _main();
long long s12_squared();
long long s23_value();
long long s23_squared();
long long s13_squared();
long long cosine_squared_of(long long);
long long jarlskog_squared();
long long baryon_to_photon();
long long within_tolerance_of(long long, long long, long long, long long);
...
long long _main() {
    long long eta = 0;
    long long eta_twin = 0;
    long long half = 0;
    long long j_squared = 0;
    long long j_squared_twin = 0;
    long long measured_eta = 0;
    long long measured_j_squared = 0;
    long long ok = 0;
    long long s23 = 0;
    long long wrong_eta = 0;
    long long ret_val = 0;

    ep_gc_push_root(&eta);
    ep_gc_push_root(&eta_twin);
    ep_gc_push_root(&half);
    ep_gc_push_root(&j_squared);
    ep_gc_push_root(&j_squared_twin);
    ep_gc_push_root(&measured_eta);
    ep_gc_push_root(&measured_j_squared);
    ep_gc_push_root(&s23);
    ep_gc_push_root(&wrong_eta);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the baryon-to-photon ratio (eta from the quark masses) ===");
    s23 = s23_value();
    ok = expect_bool((long long)"s23 is strictly positive (the two ladder slopes differ)", ({ long long _fap = s23; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0);
    printf("%s\n", (char*)concat((long long)"  info  s23 = ", fraction_to_decimal(s23, 6)));
    j_squared = jarlskog_squared();
    printf("%s\n", (char*)concat((long long)"  info  J^2 = ", fraction_to_decimal(j_squared, 12)));
    measured_j_squared = fraction_from_ratio(961, 1000000000000);
    ok = expect_bool((long long)"J^2 lands within 25% of measured (3.1e-5)^2 (PDG)", within_tolerance_of(j_squared, measured_j_squared, 1, 4));
    half = fraction_from_ratio(1, 2);
    j_squared_twin = fraction_make(fraction_numerator(j_squared), fraction_denominator(j_squared));
    eta = fraction_multiply(j_squared_twin, half);
    printf("%s\n", (char*)concat((long long)"  info  eta = J^2/2 = ", fraction_to_decimal(eta, 12)));
    measured_eta = fraction_from_ratio(61, 100000000000);
    ok = expect_bool((long long)"eta = J^2/2 lands within 25% of measured 6.1e-10 (Planck/BBN)", within_tolerance_of(eta, measured_eta, 1, 4));
    wrong_eta = fraction_from_ratio(61, 1000000000);
    eta_twin = fraction_make(fraction_numerator(eta), fraction_denominator(eta));
    ok = expect_bool((long long)"and does NOT land on a 10x wrong target (discriminating)", within_tolerance_of(eta_twin, wrong_eta, 1, 4) == 0LL);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    free_struct_Fraction(eta);
    eta = 0;
    free_struct_Fraction(eta_twin);
    eta_twin = 0;
    free_struct_Fraction(j_squared);
    j_squared = 0;
    free_struct_Fraction(measured_eta);
    measured_eta = 0;
    free_struct_Fraction(measured_j_squared);
    measured_j_squared = 0;
    free_struct_Fraction(s23);
    s23 = 0;
    free_struct_Fraction(wrong_eta);
    wrong_eta = 0;
    return ret_val;
}
...
long long s12_squared() {
    ...
    ratio = bare_s_over_d();
    ret_val = fraction_divide(one, ratio);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long s23_value() {
    long long ct = 0;
    long long down_central = 0;
    long long down_heavy = 0;
    long long sb = 0;
    long long up_central = 0;
    long long up_heavy = 0;
    long long ret_val = 0;

    ep_gc_push_root(&ct);
    ep_gc_push_root(&down_central);
    ep_gc_push_root(&down_heavy);
    ep_gc_push_root(&sb);
    ep_gc_push_root(&up_central);
    ep_gc_push_root(&up_heavy);

    ep_gc_maybe_collect();

    down_central = down_root(2);
    down_heavy = down_root(3);
    sb = fraction_divide(down_central, down_heavy);
    up_central = up_root(2);
    up_heavy = up_root(3);
    ct = fraction_divide(up_central, up_heavy);
    ret_val = fraction_subtract(sb, ct);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long s23_squared() {
    ...
    ep_gc_push_root(&s);
    ep_gc_push_root(&s_twin);

    ep_gc_maybe_collect();

    s = s23_value();
    s_twin = fraction_make(fraction_numerator(s), fraction_denominator(s));
    ret_val = fraction_multiply(s, s_twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long s13_squared() {
    long long first = 0;
    long long joint_period = 0;
    long long product = 0;
    long long second = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first);
    ep_gc_push_root(&joint_period);
    ...
    first = s12_squared();
    second = s23_squared();
    product = fraction_multiply(first, second);
    joint_period = fraction_from_whole_number(6);
    ret_val = fraction_divide(product, joint_period);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long cosine_squared_of(long long sine_squared) {
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&sine_squared);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, sine_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long jarlskog_squared() {
    ...
    long long c = 0;
    long long c_c = 0;
    long long c_c_squared = 0;
    long long c_c_twin = 0;
    long long first = 0;
    long long joint_period = 0;
    long long one_a = 0;
    long long one_b = 0;
    ...
    ep_gc_push_root(&a);
    ep_gc_push_root(&a_c);
    ep_gc_push_root(&a_twin);
    ep_gc_push_root(&ab);
    ep_gc_push_root(&b);
    ep_gc_push_root(&b_c);
    ep_gc_push_root(&b_twin);
    ep_gc_push_root(&c);
    ep_gc_push_root(&c_c);
    ep_gc_push_root(&c_c_squared);
    ep_gc_push_root(&c_c_twin);
    ep_gc_push_root(&first);
    ep_gc_push_root(&joint_period);
    ...
    a_c = fraction_subtract(one_a, a);
    first = fraction_multiply(a, a_c);
    b = s23_squared();
    one_b = fraction_from_whole_number(1);
    b_c = fraction_subtract(one_b, b);
    second = fraction_multiply(b, b_c);
    a_twin = fraction_make(fraction_numerator(a), fraction_denominator(a));
    b_twin = fraction_make(fraction_numerator(b), fraction_denominator(b));
    ab = fraction_multiply(a_twin, b_twin);
    joint_period = fraction_from_whole_number(6);
    c = fraction_divide(ab, joint_period);
    one_c = fraction_from_whole_number(1);
    c_c = fraction_subtract(one_c, c);
    c_c_twin = fraction_make(fraction_numerator(c_c), fraction_denominator(c_c));
    c_c_squared = fraction_multiply(c_c, c_c_twin);
    third = fraction_multiply(c, c_c_squared);
    partial = fraction_multiply(first, second);
    ret_val = fraction_multiply(partial, third);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(19);
    return ret_val;
...
long long baryon_to_photon() {
    ...
    ep_gc_push_root(&imbalance);
    ep_gc_push_root(&j_squared);

    ep_gc_maybe_collect();

    j_squared = jarlskog_squared();
    imbalance = fraction_from_ratio(1, 2);
    ret_val = fraction_multiply(j_squared, imbalance);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long within_tolerance_of(long long value, long long target, long long tol_num, long long tol_den) {
    long long allowed = 0;
    ...
    ep_gc_push_root(&tol_num);
    ep_gc_push_root(&tol_den);
    ...
    tolerance = fraction_from_ratio(tol_num, tol_den);
    allowed = fraction_multiply(tolerance, target);
