...
long long peak_binding();
long long light_nucleus();
long long after_fusion_one_step();
long long after_fusion_two_steps();
long long fusion_reaches_peak();
long long binding_increases_toward_peak();
long long reaction_barrier();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== fission and fusion ===");
    ok = expect_equal((long long)"peak of stability = the One (max binding)", fraction_to_text(peak_binding()), (long long)"1");
    ok = expect_equal((long long)"light nucleus = 1/4 (below the peak)", fraction_to_text(light_nucleus()), (long long)"1/4");
    ok = expect_equal((long long)"after one fusion step = 1/2", fraction_to_text(after_fusion_one_step()), (long long)"1/2");
    ok = expect_equal((long long)"after two fusion steps = the peak (1)", fraction_to_text(after_fusion_two_steps()), (long long)"1");
    ok = expect_bool((long long)"fusion reaches the peak of stability", fusion_reaches_peak());
    ok = expect_bool((long long)"binding increases toward the peak (energy released)", binding_increases_toward_peak());
    ok = expect_equal((long long)"reaction barrier = 1/2 (Coulomb barrier)", fraction_to_text(reaction_barrier()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long after_fusion_one_step() {
    ...
    ret_val = fold_fraction(light_nucleus());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long after_fusion_two_steps() {
    ...
    once = fold_fraction(light_nucleus());
    ret_val = fold_fraction(once);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long fusion_reaches_peak() {
    ...
    ep_gc_push_root(&reached);
    ep_gc_push_root(&peak);

    ep_gc_maybe_collect();

    reached = after_fusion_two_steps();
    peak = fraction_from_whole_number(1);
    ret_val = fraction_compare(reached, peak) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(reached);
    reached = 0;
    free_struct_Fraction(peak);
    peak = 0;
    return ret_val;
}
...
long long binding_increases_toward_peak() {
    ...
    start = fraction_from_ratio(1, (binary_count() * binary_count()));
    stepped = fold_fraction(light_nucleus());
    order = fraction_compare(stepped, start);
    if (order > 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
