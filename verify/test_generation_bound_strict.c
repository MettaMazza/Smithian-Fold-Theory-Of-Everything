...
long long generation_candidate(long long);
long long three_generations_verified();
long long fourth_candidate_exits_domain();
long long generation_count_strict();
long long count_matches_colour_period();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the strict generation bound (three, no fourth possible) ===");
    ok = expect_bool((long long)"the three constructed generations all triple to the One", three_generations_verified());
    ok = expect_equal((long long)"the fourth candidate = 4/3 (outside the domain)", fraction_to_text(generation_candidate(3)), (long long)"4/3");
    ok = expect_bool((long long)"THE BOUND IS STRICT: the fourth candidate exits the domain", fourth_candidate_exits_domain());
    ok = expect_equal((long long)"the generation count = 3", int_to_string(generation_count_strict()), (long long)"3");
    ok = expect_bool((long long)"Route B: the count equals the colour period (one structure)", count_matches_colour_period());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long generation_candidate(long long k) {
    long long ret_val = 0;
...
long long three_generations_verified() {
    ...
    all_land = 1LL;
    candidate = fraction_from_ratio(1, 3);
    tripled = tripling_fold(candidate);
    ...
    candidate = fraction_from_ratio(2, 3);
    tripled = tripling_fold(candidate);
    lands = fraction_compare(tripled, one) == 0;
    if (lands) {
    all_land = all_land;
    ...
    candidate = fraction_from_ratio(3, 3);
    tripled = tripling_fold(candidate);
    lands = fraction_compare(tripled, one) == 0;
    if (lands) {
    all_land = all_land;
...
long long fourth_candidate_exits_domain() {
    ...
    ep_gc_push_root(&fourth);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    fourth = fraction_from_ratio(4, 3);
    one = fraction_from_whole_number(1);
    comparison = fraction_compare(fourth, one);
...
long long count_matches_colour_period() {
    ...
    count = 3;
    period = fold_period_of_unit_fraction(7);
    ret_val = count == period;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
