...
long long _main();
long long unifying_center();
long long sector_fold(long long, long long);
long long center_is_standing_in(long long);
long long binary_carries_center_to_one();
long long is_self_antipodal(long long);
long long center_is_uniquely_self_antipodal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the half-One unifying center (one point, every sector) ===");
    ok = expect_equal((long long)"the center = 1/2", fraction_to_text(unifying_center()), (long long)"1/2");
    ok = expect_bool((long long)"the center is self-antipodal", is_self_antipodal(unifying_center()));
    ok = expect_bool((long long)"standing mode of the colour sector (m=3)", center_is_standing_in(3));
    ok = expect_bool((long long)"standing mode of the down-depth sector (m=5)", center_is_standing_in(5));
    ok = expect_bool((long long)"standing mode of the up-period sector (m=7)", center_is_standing_in(7));
    ok = expect_equal((long long)"the binary fold carries it UP to unison", fraction_to_text(binary_carries_center_to_one()), (long long)"1");
    ok = expect_equal((long long)"uniquely self-antipodal (alternatives all fail; guard armed)", int_to_string(center_is_uniquely_self_antipodal()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long sector_fold(long long size, long long m) {
...
long long center_is_standing_in(long long m) {
    long long folded = 0;
    long long center = 0;
    long long ret_val = 0;

    ep_gc_push_root(&folded);
    ep_gc_push_root(&center);
    ep_gc_push_root(&m);

    ep_gc_maybe_collect();

    center = fraction_from_ratio(1, binary_count());
    folded = sector_fold(center, m);
    ret_val = fraction_compare(folded, center) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long binary_carries_center_to_one() {
    ...
    center = fraction_from_ratio(1, binary_count());
    ret_val = sector_fold(center, binary_count());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long is_self_antipodal(long long x) {
    ...
    antipode = fraction_subtract(one, x);
    ret_val = fraction_compare(antipode, x) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(antipode);
    antipode = 0;
    return ret_val;
}
...
long long center_is_uniquely_self_antipodal() {
    ...
    if (is_self_antipodal(third)) {
    alternatives_matching = (alternatives_matching + 1);
    }
    two_thirds = fraction_from_ratio(2, 3);
    if (is_self_antipodal(two_thirds)) {
    alternatives_matching = (alternatives_matching + 1);
    }
    quarter = fraction_from_ratio(1, 4);
    if (is_self_antipodal(quarter)) {
    alternatives_matching = (alternatives_matching + 1);
    }
    three_quarters = fraction_from_ratio(3, 4);
    if (is_self_antipodal(three_quarters)) {
    alternatives_matching = (alternatives_matching + 1);
    }
    ret_val = forced_unique((long long)"the self-antipodal center", 1, alternatives_matching);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    ...
    two_thirds = 0;
    free_struct_Fraction(three_quarters);
    three_quarters = 0;
