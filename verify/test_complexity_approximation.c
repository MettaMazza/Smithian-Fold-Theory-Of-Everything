...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold approximation complexity ===");
    ok = expect_equal((long long)"three observations retain sixteen approximation classes", int_to_string(fold_approximation_class_count(7, 3)), int_to_string(16));
    ok = expect_equal((long long)"each class represents eight exact sources", int_to_string(fold_approximation_class_size(3)), int_to_string(8));
    ok = expect_equal((long long)"class accounting reconstructs all 128 sources", int_to_string(fold_approximation_accounting(7, 3)), int_to_string(128));
    ok = expect_equal((long long)"the approximation retains four distinctions", int_to_string(fold_approximation_retained_precision(7, 3)), int_to_string(4));
    ok = expect_true((long long)"one refinement divides ambiguity by b", fold_approximation_refinement_is_binary(3));
    ok = expect_true((long long)"zero observation loss is exact", fold_approximation_is_exact_at_zero(7));
    ok = expect_true((long long)"the complete approximation census holds", complete_fold_approximation_census());
    ok = expect_true((long long)"a false singleton coarse class is rejected", false_singleton_coarse_class_is_rejected(3));
    ok = expect_true((long long)"perturbed approximation accounting is rejected", perturbed_approximation_accounting_is_rejected(7, 3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
