...
long long allowed_transition();
long long transition_is_balanced();
long long emits_a_whole_photon();
long long loss_plus_gain();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== selection rules (allowed = balanced hand-off) ===");
    ok = expect_equal((long long)"allowed transition = the balanced one-unit hand-off 1/2", fraction_to_text(allowed_transition()), (long long)"1/2");
    ok = expect_bool((long long)"it is self-antipodal (giver = taker; conservation)", transition_is_balanced());
    ok = expect_equal((long long)"it emits a whole photon (folds to the One)", fraction_to_text(emits_a_whole_photon()), (long long)"1");
    ok = expect_equal((long long)"electron's loss + photon's gain = the One", fraction_to_text(loss_plus_gain()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long transition_is_balanced() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&transition);

    ep_gc_maybe_collect();

    transition = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, transition);
    ret_val = fraction_compare(complement, transition) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long emits_a_whole_photon() {
    ...
    ret_val = fold_fraction(allowed_transition());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long loss_plus_gain() {
    ...
    transition = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(transition, transition);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
