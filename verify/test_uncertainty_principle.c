...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the uncertainty principle (count bound) ===");
    ok = expect_number((long long)"total states at depth 3 (N = 2^3)", total_states(3), 8);
    ok = expect_number((long long)"uncertainty bound at depth 3", uncertainty_bound(3), 8);
    ok = expect_number((long long)"minimum-uncertainty support product (2 * 4)", minimum_uncertainty_product(3), 8);
    ok = expect_number((long long)"fully-localized frequency support (forced to N)", fully_localized_frequency_support(3), 8);
    printf("%s\n", (char*)(long long)"--- deeper: the bound grows as 2^k ---");
    ok = expect_number((long long)"bound at depth 5", uncertainty_bound(5), 32);
    ok = expect_number((long long)"minimum product at depth 5 (2 * 16)", minimum_uncertainty_product(5), 32);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
