...
long long _main();
long long spatial_step(long long);
long long tick_step(long long);
long long limit_speed(long long);
long long speed_is_one_at_depth(long long);
long long speed_invariant_across_depths();
long long spacing_halves_per_depth(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== scale invariance (the limit speed at every scale) ===");
    ok = expect_equal((long long)"limit speed at depth 3 = 1 (s_3 / dt_3)", fraction_to_text(limit_speed(3)), (long long)"1");
    ok = expect_bool((long long)"the speed is the One at depth 5", speed_is_one_at_depth(5));
    ok = expect_bool((long long)"the speed is invariant across depths 1..7", speed_invariant_across_depths());
    ok = expect_bool((long long)"refining the scale halves the spatial step: s_4 * 2 = s_3", spacing_halves_per_depth(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spatial_step(long long depth) {
    long long volume = 0;
    long long ret_val = 0;
...
long long tick_step(long long depth) {
    long long volume = 0;
    long long ret_val = 0;
...
long long limit_speed(long long depth) {
    long long dt_k = 0;
    long long s_k = 0;
    long long ret_val = 0;

    ep_gc_push_root(&dt_k);
    ep_gc_push_root(&s_k);
    ep_gc_push_root(&depth);

    ep_gc_maybe_collect();

    s_k = spatial_step(depth);
    dt_k = tick_step(depth);
    ret_val = fraction_divide(s_k, dt_k);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long speed_is_one_at_depth(long long depth) {
    long long c_k = 0;
    long long one = 0;
    long long ret_val = 0;
    ...
    c_k = limit_speed(depth);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(c_k, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(c_k);
    c_k = 0;
...
long long spacing_halves_per_depth(long long depth) {
    long long binary = 0;
    long long doubled = 0;
    long long s_k = 0;
    long long s_next = 0;
    ...
    ep_gc_push_root(&s_k);
    ep_gc_push_root(&s_next);
    ep_gc_push_root(&depth);

    ep_gc_maybe_collect();

    s_k = spatial_step(depth);
    s_next = spatial_step((depth + 1));
    binary = fraction_from_whole_number(binary_count());
    doubled = fraction_multiply(s_next, binary);
    ret_val = fraction_compare(doubled, s_k) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(doubled);
    doubled = 0;
    free_struct_Fraction(s_k);
    s_k = 0;
    return ret_val;
}
