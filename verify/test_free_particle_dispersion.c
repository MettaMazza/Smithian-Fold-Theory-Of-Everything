...
long long momentum();
long long phase();
long long kinetic_dispersion();
long long momentum_doubled();
long long dispersion_equals_two_steps();
long long phase_after_step();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== free-particle dispersion (de Broglie) ===");
    ok = expect_equal((long long)"kinetic dispersion = fold(1/4) = 1/2", fold_value_to_text(kinetic_dispersion()), (long long)"1/2");
    ok = expect_equal((long long)"momentum doubled = 1/4 + 1/4 = 1/2", fold_value_to_text(momentum_doubled()), (long long)"1/2");
    ok = expect_bool((long long)"dispersion equals momentum doubled (fold(p) = p + p)", dispersion_equals_two_steps());
    ok = expect_equal((long long)"phase after one step = 1/3 + 1/2 = 5/6", fold_value_to_text(phase_after_step()), (long long)"5/6");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long momentum_doubled() {
    long long p_second = 0;
    long long p_first = 0;
    long long ret_val = 0;

    ep_gc_push_root(&p_second);
    ep_gc_push_root(&p_first);

    ep_gc_maybe_collect();

    p_first = supposed_value(1, 4);
    p_second = supposed_value(1, 4);
    ret_val = rotate(p_first, p_second);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long dispersion_equals_two_steps() {
    ...
    ep_gc_push_root(&kinetic);
    ep_gc_push_root(&doubled);

    ep_gc_maybe_collect();

    kinetic = kinetic_dispersion();
    doubled = momentum_doubled();
    ret_val = fold_value_compare(kinetic, doubled) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_FoldValue(kinetic);
    kinetic = 0;
    free_struct_FoldValue(doubled);
    doubled = 0;
    return ret_val;
}

long long phase_after_step() {
    long long the_phase = 0;
    long long step = 0;
    long long ret_val = 0;

    ep_gc_push_root(&the_phase);
    ep_gc_push_root(&step);

    ep_gc_maybe_collect();

    the_phase = supposed_value(1, 3);
    step = kinetic_dispersion();
    ret_val = rotate(the_phase, step);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
