...
long long unperturbed_level();
long long lower_component();
long long upper_component();
long long components_are_degenerate();
long long split_count();
long long split_is_symmetric();
long long shifts_are_equal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== field splitting (Zeeman and Stark from the fibre) ===");
    ok = expect_bool((long long)"the pair {1/4, 3/4} is degenerate: one line without a field", components_are_degenerate());
    ok = expect_equal((long long)"the split census: exactly binary = 2 components (a doublet)", int_to_string(split_count()), (long long)"2");
    ok = expect_bool((long long)"the split is SYMMETRIC: the pair's mean is the unperturbed level", split_is_symmetric());
    ok = expect_bool((long long)"the shifts are equal and opposite (1/4 up, 1/4 down)", shifts_are_equal());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long components_are_degenerate() {
    ...
    long long level = 0;
    long long lower_ok = 0;
    long long upper_ok = 0;
    long long ret_val = 0;
    ...
    level = fraction_from_ratio(1, 2);
    from_lower = fold_fraction(lower_component());
    from_upper = fold_fraction(upper_component());
    lower_ok = fraction_compare(from_lower, level) == 0;
    upper_ok = fraction_compare(from_upper, level) == 0;
    if (lower_ok) {
    ret_val = upper_ok;
    goto L_cleanup;
    }
...
long long split_is_symmetric() {
    ...
    mean = fraction_divide(total, two);
    level = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(mean, level) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_Fraction(level);
    level = 0;
    free_struct_Fraction(mean);
    mean = 0;
    return ret_val;
}

long long shifts_are_equal() {
    long long down_shift = 0;
    long long level = 0;
    long long lower = 0;
    long long up_shift = 0;
    long long upper = 0;
    long long ret_val = 0;

    ep_gc_push_root(&down_shift);
    ep_gc_push_root(&level);
    ep_gc_push_root(&lower);
    ep_gc_push_root(&up_shift);
    ep_gc_push_root(&upper);
    ...
    lower = fraction_from_ratio(1, 4);
    upper = fraction_from_ratio(3, 4);
    down_shift = fraction_subtract(level, lower);
    up_shift = fraction_subtract(upper, level);
    ret_val = fraction_compare(down_shift, up_shift) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(down_shift);
    down_shift = 0;
    free_struct_Fraction(up_shift);
    up_shift = 0;
    return ret_val;
}
