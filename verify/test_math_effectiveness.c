...
long long orbit_low();
long long orbit_high();
long long orbit_period_is_arithmetic();
long long states_partition_one();
long long balance_folds_to_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the effectiveness of mathematics (one structure, two readings) ===");
    ok = expect_equal((long long)"the orbit, physical reading: 1/3", fraction_to_text(orbit_low()), (long long)"1/3");
    ok = expect_equal((long long)"its partner: 2/3", fraction_to_text(orbit_high()), (long long)"2/3");
    ok = expect_bool((long long)"the SAME orbit, mathematical reading: period = ord_3(2) = 2", orbit_period_is_arithmetic());
    ok = expect_equal((long long)"both readings agree on the whole (1/3 + 2/3 = 1)", fraction_to_text(states_partition_one()), (long long)"1");
    ok = expect_equal((long long)"their balance folds to unison (the shared centre)", fraction_to_text(balance_folds_to_unison()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long orbit_period_is_arithmetic() {
    ...
    period = fold_period_of_unit_fraction(colour_count());
    ret_val = period == smallest_fold_period_above(1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
