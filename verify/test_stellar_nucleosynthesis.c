...
long long ignition_share();
long long ignition_crosses_barrier();
long long burn_completes();
long long two_shares_make_the_barrier();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== stellar nucleosynthesis (the two-fold ignition) ===");
    ok = expect_equal((long long)"ignition share = 1/4 (two folds below unison)", fraction_to_text(ignition_share()), (long long)"1/4");
    ok = expect_equal((long long)"first fold crosses the Coulomb barrier (-> 1/2)", fraction_to_text(ignition_crosses_barrier()), (long long)"1/2");
    ok = expect_equal((long long)"second fold completes the burn (-> the One)", fraction_to_text(burn_completes()), (long long)"1");
    ok = expect_equal((long long)"two ignition shares make one barrier", fraction_to_text(two_shares_make_the_barrier()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long ignition_crosses_barrier() {
    ...
    ret_val = fold_fraction(ignition_share());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long burn_completes() {
    ...
    ret_val = fold_fraction(ignition_crosses_barrier());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
