...
long long _main();
long long force_prime(long long);
long long carry_closes(long long);
long long all_kinds_pair(long long);
long long sector_is_a_force(long long);
long long all_four_qualify();
long long couplings_rise_strictly();
long long fifth_force_coupling();
long long seventh_force_coupling();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the force criterion (two new forces predicted) ===");
    ok = expect_bool((long long)"CARRY closes at p = 5: (1/5) * 5 = the One", carry_closes(5));
    ok = expect_bool((long long)"CONFINEMENT: all 4 kinds of the 5-sector pair to the One", all_kinds_pair(5));
    ok = expect_bool((long long)"all 6 kinds of the 7-sector pair (exhaustive)", all_kinds_pair(7));
    ok = expect_bool((long long)"ALL FOUR sectors pass the identical criterion", all_four_qualify());
    ok = expect_bool((long long)"the couplings rise strictly: 1/2 < 2/3 < 4/5 < 6/7", couplings_rise_strictly());
    ok = expect_equal((long long)"the PREDICTED fifth-force coupling = 4/5", fraction_to_text(fifth_force_coupling()), (long long)"4/5");
    ok = expect_equal((long long)"the PREDICTED seventh-force coupling = 6/7", fraction_to_text(seventh_force_coupling()), (long long)"6/7");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long force_prime(long long index) {
    long long ret_val = 0;
...
long long carry_closes(long long p) {
    long long carried = 0;
    long long one = 0;
    ...
    size = fraction_from_whole_number(p);
    carried = fraction_multiply(shortfall, size);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(carried, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(carried);
    carried = 0;
...
long long all_kinds_pair(long long p) {
    ...
    antipode = fraction_from_ratio((p - j), p);
...
long long sector_is_a_force(long long p) {
    long long carry_ok = 0;
    long long pairing_ok = 0;
    long long ret_val = 0;
    ...
    carry_ok = carry_closes(p);
    pairing_ok = all_kinds_pair(p);
    if (carry_ok) {
    ret_val = pairing_ok;
    goto L_cleanup;
    }
...
long long couplings_rise_strictly() {
    ...
    ep_gc_push_root(&g_here);
    ep_gc_push_root(&g_next);
    ep_gc_push_root(&i);
    ep_gc_push_root(&p_here);
    ep_gc_push_root(&p_next);
    ...
    i = 1;
    while (i < 4) {
    p_here = force_prime(i);
    p_next = force_prime((i + 1));
    g_here = fraction_from_ratio((p_here - 1), p_here);
    g_next = fraction_from_ratio((p_next - 1), p_next);
    comparison = fraction_compare(g_here, g_next);
    if (comparison < 0) {
    rising = rising;
    } else {
    rising = 0LL;
...
long long seventh_force_coupling() {
    ...
    ret_val = fraction_from_ratio(6, 7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
