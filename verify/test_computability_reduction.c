...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold reduction law ===");
    ok = expect_true((long long)"state-to-word reduction preserves a process prefix", state_to_word_reduction_preserves_process(85, 7, 4));
    ok = expect_true((long long)"word-to-state reduction is the exact inverse", word_to_state_reduction_is_inverse(85, 7));
    ok = expect_true((long long)"observation reduction preserves the Fold transition", observation_reduction_preserves_transition(85, 7));
    ok = expect_true((long long)"Fold reductions compose at every split", fold_reduction_composes(85, 7));
    ok = expect_true((long long)"the complete reduction census holds", complete_fold_reduction_census());
    ok = expect_true((long long)"a changed reduction symbol is rejected", changed_reduction_symbol_is_rejected(7));
    ok = expect_true((long long)"a shortened reduction process is rejected", shortened_reduction_process_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
