...
long long potential_at(long long);
long long lattice_curvature(long long);
long long curvature_is_exact_two();
long long planar_laplacian();
long long fold_expansion();
long long laplacian_is_expansion_squared();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== planar gravity (Laplacian = expansion squared) ===");
    ok = expect_equal((long long)"the 1D lattice curvature of x^2 at spacing 1/8 = 2 EXACTLY", fraction_to_text(lattice_curvature(3)), (long long)"2");
    ok = expect_bool((long long)"the curvature is exactly 2 at every spacing (depths 3, 5)", curvature_is_exact_two());
    ok = expect_equal((long long)"the planar Laplacian: two axes = 4", fraction_to_text(planar_laplacian()), (long long)"4");
    ok = expect_equal((long long)"the fold expansion, MEASURED on 1/5 and 1/7 = 2", fraction_to_text(fold_expansion()), (long long)"2");
    ok = expect_bool((long long)"the Laplacian equals the expansion squared: 4 = m*m", laplacian_is_expansion_squared());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long potential_at(long long x) {
    long long x_twin = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_multiply(x, x_twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long lattice_curvature(long long k) {
    ...
    f_right = potential_at(right);
    f_left = potential_at(left);
    f_centre = potential_at(x);
    f_centre_again = potential_at(fraction_from_ratio(1, 2));
...
long long curvature_is_exact_two() {
    ...
    at_three = lattice_curvature(3);
    at_five = lattice_curvature(5);
    two = fraction_from_whole_number(2);
    three_ok = fraction_compare(at_three, two) == 0;
    five_ok = fraction_compare(at_five, two) == 0;
    if (three_ok) {
    ret_val = five_ok;
    goto L_cleanup;
    }
...
long long planar_laplacian() {
    long long axis_one = 0;
    long long axis_two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&axis_one);
    ep_gc_push_root(&axis_two);

    ep_gc_maybe_collect();

    axis_one = lattice_curvature(3);
    axis_two = lattice_curvature(3);
    ret_val = fraction_add(axis_one, axis_two);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long fold_expansion() {
    ...
    ret_val = fraction_divide(fold_gap, gap);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long laplacian_is_expansion_squared() {
    ...
    ep_gc_push_root(&m);
    ep_gc_push_root(&m_squared);
    ep_gc_push_root(&m_twin);

    ep_gc_maybe_collect();

    laplacian = planar_laplacian();
    m = fold_expansion();
    m_twin = fold_expansion();
    m_squared = fraction_multiply(m, m_twin);
    ret_val = fraction_compare(laplacian, m_squared) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(laplacian);
    laplacian = 0;
    free_struct_Fraction(m_squared);
    m_squared = 0;
    return ret_val;
}
