...
long long maximal_mismatch();
long long mismatch_is_self_antipodal();
long long halves_complete_to_bright();
long long dark_plus_bright();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== wave optics (bright and dark fringes) ===");
    ok = expect_equal((long long)"maximal mismatch = 1/2 (fully out of step)", fraction_to_text(maximal_mismatch()), (long long)"1/2");
    ok = expect_bool((long long)"the darkest fringe is self-antipodal (exact cancellation)", mismatch_is_self_antipodal());
    ok = expect_equal((long long)"brought into step, halves complete to bright (-> the One)", fraction_to_text(halves_complete_to_bright()), (long long)"1");
    ok = expect_equal((long long)"dark + bright shares sum to the One (energy conserved)", fraction_to_text(dark_plus_bright()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long mismatch_is_self_antipodal() {
    ...
    ep_gc_push_root(&mismatch);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    mismatch = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, mismatch);
    ret_val = fraction_compare(complement, mismatch) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long halves_complete_to_bright() {
    ...
    ret_val = fold_fraction(maximal_mismatch());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long dark_plus_bright() {
    ...
    mismatch = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(mismatch, mismatch);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
