...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold strings and sequences ===");
    first = encode_computation_state(3, 2);
    second = encode_computation_state(5, 3);
    joined = fold_sequence_join(first, second);
    ok = expect_equal((long long)"joined sequence length is five", int_to_string(length_list(joined)), int_to_string(5));
    ok = expect_true((long long)"joined generated paths remain generated", fold_sequence_join_is_generated(first, second));
    ok = expect_true((long long)"every split rejoins exactly", fold_sequence_split_rejoins(joined, 2));
    ok = expect_equal((long long)"joined choice count is thirty-two", int_to_string(fold_sequence_product_count(2, 3)), int_to_string(32));
    ok = expect_true((long long)"the complete sequence census holds", complete_fold_strings_sequences_census());
    ok = expect_true((long long)"a changed sequence is distinguished", changed_sequence_is_rejected(7));
    ok = expect_true((long long)"an outside sequence symbol is rejected", outside_sequence_symbol_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
