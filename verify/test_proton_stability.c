...
long long _main();
long long quark_fibre();
long long lepton_fibre();
long long fibres_are_distinct();
long long proton_baryon_number();
long long baryon_number_is_conserved();
long long proton_is_stable();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== proton stability ===");
    ok = expect_equal((long long)"quark fibre = colour = 3", int_to_string(quark_fibre()), int_to_string(3));
    ok = expect_equal((long long)"lepton fibre = binary = 2", int_to_string(lepton_fibre()), int_to_string(2));
    ok = expect_bool((long long)"the fibres are distinct (3 != 2, no crossing)", fibres_are_distinct());
    ok = expect_equal((long long)"proton baryon number = 3 * 1/3 = 1 (conserved whole)", fraction_to_text(proton_baryon_number()), (long long)"1");
    ok = expect_bool((long long)"baryon number is conserved (= the One)", baryon_number_is_conserved());
    ok = expect_bool((long long)"the proton is STABLE (no decay path to leptons)", proton_is_stable());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fibres_are_distinct() {
    ...
    quarks = smallest_fold_period_above(binary_count());
    leptons = smallest_fold_period_above(1);
    if (quarks == leptons) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long proton_baryon_number() {
    long long per_quark = 0;
    long long colour = 0;
    long long ret_val = 0;

    ep_gc_push_root(&per_quark);
    ep_gc_push_root(&colour);

    ep_gc_maybe_collect();

    per_quark = fraction_from_ratio(1, colour_count());
    colour = fraction_from_whole_number(colour_count());
    ret_val = fraction_multiply(colour, per_quark);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long baryon_number_is_conserved() {
    ...
    ep_gc_push_root(&number);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    number = proton_baryon_number();
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(number, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(number);
    number = 0;
...
long long proton_is_stable() {
    ...
    distinct = fibres_are_distinct();
    conserved = baryon_number_is_conserved();
    if (distinct) {
    if (conserved) {
    ret_val = 1LL;
    goto L_cleanup;
