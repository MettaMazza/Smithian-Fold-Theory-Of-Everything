...
long long light_nucleus_binding();
long long first_fusion_step();
long long second_step_reaches_peak();
long long two_light_fuse_to_next_level();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== nuclear binding (the valley of stability) ===");
    ok = expect_equal((long long)"light-nucleus binding = 1/4 (two folds below the peak)", fraction_to_text(light_nucleus_binding()), (long long)"1/4");
    ok = expect_equal((long long)"first fusion step doubles the share (-> 1/2)", fraction_to_text(first_fusion_step()), (long long)"1/2");
    ok = expect_equal((long long)"second step reaches the peak (the One, iron)", fraction_to_text(second_step_reaches_peak()), (long long)"1");
    ok = expect_equal((long long)"two light nuclei fuse to the next level", fraction_to_text(two_light_fuse_to_next_level()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_fusion_step() {
    ...
    ret_val = fold_fraction(light_nucleus_binding());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long second_step_reaches_peak() {
    ...
    ret_val = fold_fraction(first_fusion_step());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
