...
long long _main();
long long nim_two_field(long long);
long long subtraction_field(long long);
long long spectral_coefficient(long long, long long);
long long swap_blocks(long long);
long long nim_field_is_swap_invariant();
long long twin_pair_law_holds();
long long subtraction_field_is_mirror_invariant();
long long vanishing_law_holds();
long long vanished_class_is_half();
long long nim_spectrum_collapses();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the certified game theorems: the chess campaign's laws, off chess ===");
    ok = expect_bool((long long)"T-1 precondition: the Nim field is swap-invariant (64 states)", nim_field_is_swap_invariant());
    ok = expect_bool((long long)"T-1 THE TWIN-PAIR LAW: all 64 coefficients equal their swapped twins", twin_pair_law_holds());
    ok = expect_bool((long long)"the twin structure visible: c_s = 8 iff the blocks agree, 0 otherwise", nim_spectrum_collapses());
    ok = expect_bool((long long)"T-2 precondition: the subtraction field is mirror-invariant (mask 60)", subtraction_field_is_mirror_invariant());
    ok = expect_bool((long long)"T-2 THE VANISHING LAW: all odd-overlap coefficients exactly zero", vanishing_law_holds());
    ok = expect_equal((long long)"THE STORAGE COROLLARY: exactly half the spectrum vanishes (32 of 64)", int_to_string(vanished_class_is_half()), (long long)"32");
    ok = expect_equal((long long)"a surviving coefficient, exact: c_0 of the subtraction field", int_to_string(spectral_coefficient(0, 0)), (long long)"16");
    ok = expect_equal((long long)"a vanished coefficient, exact: c_4 (odd overlap with the mirror)", int_to_string(spectral_coefficient(0, 4)), (long long)"0");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long nim_two_field(long long code) {
    long long high_heap = 0;
    long long low_heap = 0;
    long long ret_val = 0;

    low_heap = (code - ((code / 8) * 8));
    high_heap = (code / 8);
...
long long subtraction_field(long long n) {
    long long quarter = 0;
...
long long spectral_coefficient(long long field_kind, long long s) {
    long long n = 0;
    long long sign = 0;
    long long total = 0;
    long long value = 0;
    ...
    value = 0;
    if (field_kind == 1) {
    value = nim_two_field(n);
    } else {
    value = subtraction_field(n);
    }
    if (value == 1) {
    sign = chi_sign(s, n);
    total = (total + sign);
...
long long swap_blocks(long long s) {
    long long high_block = 0;
    long long low_block = 0;
    long long ret_val = 0;

    low_block = (s - ((s / 8) * 8));
    high_block = (s / 8);
    ret_val = ((low_block * 8) + high_block);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long nim_field_is_swap_invariant() {
    ...
    n = 0;
    while (n < 64) {
    here = nim_two_field(n);
    swapped = nim_two_field(swap_blocks(n));
    if (here == swapped) {
    all_match = all_match;
    } else {
    all_match = 0LL;
...
long long twin_pair_law_holds() {
    ...
    s = 0;
    while (s < 64) {
    here = spectral_coefficient(1, s);
    twin = spectral_coefficient(1, swap_blocks(s));
    if (here == twin) {
    all_twin = all_twin;
    } else {
    all_twin = 0LL;
...
long long subtraction_field_is_mirror_invariant() {
    ...
    n = 0;
    while (n < 64) {
    here = subtraction_field(n);
    mirrored = subtraction_field(bit_xor(n, 60));
    if (here == mirrored) {
    all_match = all_match;
    } else {
    all_match = 0LL;
...
long long vanishing_law_holds() {
    ...
    s = 0;
    while (s < 64) {
    overlap = bit_and(s, 60);
    ones = popcount(overlap);
    parity = (ones - ((ones / 2) + (ones / 2)));
    if (parity == 1) {
    coefficient = spectral_coefficient(0, s);
    if (coefficient == 0) {
...
long long nim_spectrum_collapses() {
    ...
    low_block = (s - ((s / 8) * 8));
    high_block = (s / 8);
    coefficient = spectral_coefficient(1, s);
