...
long long _main();
long long up_depth();
long long fock_count();
long long massive_states();
long long gravitational_coupling();
long long hierarchy_exponent();
long long planck_to_proton_squared();
long long hierarchy_exponent_form_is_unique();
...
long long _main() {
    long long forced_squared = 0;
    long long gap = 0;
    long long measured_ratio = 0;
    long long measured_ratio_squared = 0;
    long long ok = 0;
    long long ratio_bottom = 0;
    long long ratio_top = 0;
    long long relative = 0;
    long long tolerance = 0;
    long long ret_val = 0;

    ep_gc_push_root(&forced_squared);
    ep_gc_push_root(&gap);
    ep_gc_push_root(&measured_ratio);
    ep_gc_push_root(&measured_ratio_squared);
    ep_gc_push_root(&ratio_bottom);
    ep_gc_push_root(&ratio_top);
    ep_gc_push_root(&relative);
    ep_gc_push_root(&tolerance);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the absolute scale (proton-to-Planck hierarchy) ===");
    ok = expect_equal((long long)"deepest covering depth d_up = 7", int_to_string(up_depth()), int_to_string(7));
    ok = expect_equal((long long)"Fock count 2^7 = 128", int_to_string(fock_count()), int_to_string(128));
    ok = expect_equal((long long)"massive states 128 - 1 = 127", int_to_string(massive_states()), int_to_string(127));
    ok = expect_equal((long long)"gravitational coupling = 1/2", fraction_to_text(gravitational_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"hierarchy exponent = 127/2", fraction_to_text(hierarchy_exponent()), (long long)"127/2");
    ok = expect_equal((long long)"(Planck/proton)^2 = 2^127", exact_integer_to_text(planck_to_proton_squared()), (long long)"170141183460469231731687303715884105728");
    printf("%s\n", (char*)(long long)"--- comparison to measurement (squared, so no irrational root) ---");
    ratio_top = exact_integer_from_messy_digits(1, (long long)"2176434000000000000000000000");
    ratio_bottom = exact_integer_from_messy_digits(1, (long long)"167262192");
    measured_ratio = fraction_make(ratio_top, ratio_bottom);
    measured_ratio_squared = fraction_multiply(measured_ratio, measured_ratio);
    forced_squared = fraction_make(planck_to_proton_squared(), exact_integer_one());
    gap = fraction_subtract(forced_squared, measured_ratio_squared);
    relative = fraction_divide(gap, measured_ratio_squared);
    printf("%s\n", (char*)concat((long long)"  relative gap (squared) = ", fraction_to_decimal(relative, 5)));
    tolerance = fraction_from_ratio(1, 200);
    if (fraction_compare(relative, tolerance) < 0) {
    printf("%s\n", (char*)(long long)"  ok    forced 2^127 matches measured (Planck/proton)^2 within 1/200");
    } else {
    printf("%s\n", (char*)(long long)"  FAIL  forced 2^127 outside 1/200 of measured (Planck/proton)^2");
    }
    ok = expect_equal((long long)"the hierarchy-exponent FORM is unique (forced_unique)", int_to_string(hierarchy_exponent_form_is_unique()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(relative);
    relative = 0;
    free_struct_Fraction(tolerance);
    tolerance = 0;
    return ret_val;
}
...
long long massive_states() {
    ...
    ret_val = (whole_power(binary_count(), up_depth()) - 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long hierarchy_exponent() {
    ...
    states = fraction_from_whole_number(massive_states());
    coupling = fraction_from_ratio(1, binary_count());
    ret_val = fraction_multiply(states, coupling);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long planck_to_proton_squared() {
    ...
    base = exact_integer_from_number(binary_count());
    ret_val = exact_integer_power(base, massive_states());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long hierarchy_exponent_form_is_unique() {
    ...
    long long form_d = 0;
    long long form_e = 0;
    long long states_a = 0;
    long long states_b = 0;
    long long states_c = 0;
    long long states_d = 0;
    long long states_e = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    ep_gc_push_root(&states_a);
    ep_gc_push_root(&states_b);
    ep_gc_push_root(&states_c);
    ep_gc_push_root(&states_d);
    ep_gc_push_root(&states_e);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    total = 0;
    states_a = fraction_from_whole_number(massive_states());
    form_a = fraction_multiply(states_a, gravitational_coupling());
    total = count_form_match(total, form_a, hierarchy_exponent());
    states_b = fraction_from_whole_number(massive_states());
    form_b = fraction_divide(states_b, gravitational_coupling());
    total = count_form_match(total, form_b, hierarchy_exponent());
    states_c = fraction_from_whole_number(massive_states());
    form_c = fraction_add(states_c, gravitational_coupling());
    total = count_form_match(total, form_c, hierarchy_exponent());
    states_d = fraction_from_whole_number(massive_states());
    form_d = fraction_subtract(states_d, gravitational_coupling());
    total = count_form_match(total, form_d, hierarchy_exponent());
    states_e = fraction_from_whole_number(massive_states());
    form_e = fraction_divide(gravitational_coupling(), states_e);
    total = count_form_match(total, form_e, hierarchy_exponent());
    ret_val = forbid_form_selection((long long)"the hierarchy exponent form", total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(11);
    return ret_val;
