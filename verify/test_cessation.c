...
long long held_lock();
long long lock_is_self_antipodal();
long long lock_completes();
long long lock_is_releasable();
long long anchor_persists();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== cessation (the lock ends, the anchor cannot) ===");
    ok = expect_bool((long long)"the lock is self-antipodal (balanced against itself)", lock_is_self_antipodal());
    ok = expect_bool((long long)"the lock completes to unison in one fold", lock_completes());
    ok = expect_bool((long long)"THE RELEASE: occupation can end (the lock is one state among many)", lock_is_releasable());
    ok = expect_bool((long long)"THE ANCHOR: the unison persists under eight further folds", anchor_persists());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long lock_is_self_antipodal() {
    ...
    ret_val = fraction_compare(lock, antipode) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long lock_is_releasable() {
    ...
    ep_gc_push_root(&elsewhere);
    ep_gc_push_root(&lock);

    ep_gc_maybe_collect();

    lock = fraction_from_ratio(1, 2);
    elsewhere = fraction_from_ratio(1, 4);
    same = fraction_compare(elsewhere, lock) == 0;
    if (same) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(elsewhere);
    elsewhere = 0;
...
long long anchor_persists() {
    ...
    one = fraction_from_whole_number(1);
    state = fraction_from_whole_number(1);
    held = 1LL;
    state = fold_fraction(state);
    still = fraction_compare(state, one) == 0;
    if (still) {
    held = held;
    } else {
    held = 0LL;
