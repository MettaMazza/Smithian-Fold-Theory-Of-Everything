...
long long horizon();
long long hawking_temperature();
long long temperature_is_strictly_positive();
long long temperature_folds_to_horizon();
long long temperature_reaches_unison();
long long two_temperatures_make_horizon();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== black holes (Hawking radiation, not a perfect trap) ===");
    ok = expect_equal((long long)"the horizon = the gravitational balance 1/2", fraction_to_text(horizon()), (long long)"1/2");
    ok = expect_equal((long long)"Hawking temperature = 1/4 (second-order, one fold below)", fraction_to_text(hawking_temperature()), (long long)"1/4");
    ok = expect_bool((long long)"the temperature is strictly positive (no perfect trap)", temperature_is_strictly_positive());
    ok = expect_equal((long long)"one fold lifts it to the horizon (1/4 -> 1/2)", fraction_to_text(temperature_folds_to_horizon()), (long long)"1/2");
    ok = expect_equal((long long)"a second fold reaches unison (1/2 -> 1)", fraction_to_text(temperature_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"two temperatures make the horizon balance", fraction_to_text(two_temperatures_make_horizon()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long hawking_temperature() {
    ...
    ret_val = fraction_multiply(half, half);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long temperature_is_strictly_positive() {
    ...
    ep_gc_push_root(&temp);

    ep_gc_maybe_collect();

    temp = hawking_temperature();
    ret_val = ({ long long _fap = temp; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(temp);
    temp = 0;
    return ret_val;
}
...
long long temperature_folds_to_horizon() {
    ...
    ret_val = fold_fraction(hawking_temperature());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long temperature_reaches_unison() {
    ...
    ret_val = fold_fraction(temperature_folds_to_horizon());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_temperatures_make_horizon() {
    ...
    temp = hawking_temperature();
    ret_val = fraction_add(temp, temp);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
