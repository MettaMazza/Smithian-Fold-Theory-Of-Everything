...
long long _main() {
    long long absolute_scale = 0;
    long long big_square = 0;
    long long ok = 0;
    long long outcome = 0;
    long long top = 0;
    long long two = 0;
    long long two_fifty = 0;
    long long ret_val = 0;

    ep_gc_push_root(&absolute_scale);
    ep_gc_push_root(&big_square);
    ep_gc_push_root(&outcome);
    ep_gc_push_root(&top);
    ep_gc_push_root(&two);
    ep_gc_push_root(&two_fifty);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== exact integers ===");
    two = exact_integer_from_number(2);
    absolute_scale = exact_integer_power(two, 127);
    ok = expect_equal((long long)"2^127", exact_integer_to_text(absolute_scale), (long long)"170141183460469231731687303715884105728");
    ok = expect_equal((long long)"1000000007 round-trip", exact_integer_to_text(exact_integer_from_number(1000000007)), (long long)"1000000007");
    top = exact_integer_from_number(34259);
    two_fifty = exact_integer_from_number(250);
    ok = expect_equal((long long)"34259 * 250", exact_integer_to_text(exact_integer_multiply(top, two_fifty)), (long long)"8564750");
    ok = expect_equal((long long)"250 - 34259", exact_integer_to_text(exact_integer_subtract(two_fifty, top)), (long long)"-34009");
    ok = expect_equal((long long)"34259 + (-34259)", exact_integer_to_text(exact_integer_add(top, exact_integer_negate(top))), (long long)"0");
    big_square = exact_integer_multiply(absolute_scale, absolute_scale);
    ok = expect_equal((long long)"2^254", exact_integer_to_text(big_square), (long long)"28948022309329048855892746252171976963317496166410141009864396001978282409984");
    outcome = exact_integer_divide(top, two_fifty);
    ok = expect_equal((long long)"34259 / 250 quotient", ({ long long _fap = outcome; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'quotient_digits' on 'DivisionOutcome'\n"); exit(1); } ((EpStruct_DivisionOutcome*)(_fap))->quotient_digits; }), (long long)"137");
    ok = expect_equal((long long)"34259 / 250 remainder", ({ long long _fap = outcome; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'remainder_digits' on 'DivisionOutcome'\n"); exit(1); } ((EpStruct_DivisionOutcome*)(_fap))->remainder_digits; }), (long long)"9");
    ok = expect_equal((long long)"gcd(8564750, 62500)", exact_integer_to_text(exact_integer_greatest_common_divisor(exact_integer_from_number(8564750), exact_integer_from_number(62500))), (long long)"250");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    free_struct_ExactInteger(big_square);
    big_square = 0;
