...
long long descent_of(long long, long long);
long long longest_descent(long long);
long long order_is_log_of_complexity();
long long doubling_adds_one_step();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== order from complexity (exponential states, linear paths) ===");
    ok = expect_equal((long long)"COMPLEXITY at depth 4: 16 states, counted", int_to_string(state_count(4)), (long long)"16");
    ok = expect_equal((long long)"ORDER at depth 4: longest descent = 4, walked over all 16", int_to_string(longest_descent(4)), (long long)"4");
    ok = expect_bool((long long)"order is the LOG of complexity (depths 2, 3, 4 all walked)", order_is_log_of_complexity());
    ok = expect_bool((long long)"each doubling of complexity adds exactly ONE step", doubling_adds_one_step());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long descent_of(long long i, long long k) {
    long long arrived = 0;
    long long grid = 0;
    long long one = 0;
    long long state = 0;
    ...
    grid = whole_power(binary_count(), k);
...
long long longest_descent(long long k) {
    long long grid = 0;
    long long i = 0;
    long long length = 0;
    long long longest = 0;
...
long long order_is_log_of_complexity() {
    ...
    at_two = longest_descent(2) == 2;
    at_three = longest_descent(3) == 3;
    at_four = longest_descent(4) == 4;
    if (at_two) {
    if (at_three) {
    ret_val = at_four;
    goto L_cleanup;
...
long long doubling_adds_one_step() {
    long long path_grows = 0;
    long long states_grow = 0;
    long long ret_val = 0;

    states_grow = whole_power(binary_count(), 4) == (2 * whole_power(binary_count(), 3));
    path_grows = longest_descent(4) == (longest_descent(3) + 1);
    if (states_grow) {
    ret_val = path_grows;
    goto L_cleanup;
    }
