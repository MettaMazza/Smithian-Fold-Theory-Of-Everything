...
long long floor_scale();
long long first_climb();
long long reaches_the_whole();
long long shares_make_the_balance();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== renormalization without infinities ===");
    ok = expect_equal((long long)"floor-scale state = 1/4 (finitely below unison)", fraction_to_text(floor_scale()), (long long)"1/4");
    ok = expect_equal((long long)"first climb: 1/4 -> 1/2", fraction_to_text(first_climb()), (long long)"1/2");
    ok = expect_equal((long long)"reaches the whole in two folds (no infinity)", fraction_to_text(reaches_the_whole()), (long long)"1");
    ok = expect_equal((long long)"two floor shares make the balance", fraction_to_text(shares_make_the_balance()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_climb() {
    ...
    ret_val = fold_fraction(floor_scale());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shares_make_the_balance() {
    ...
    scale = floor_scale();
    ret_val = fraction_add(scale, scale);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
