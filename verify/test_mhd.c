...
long long alfven_state();
long long alfven_folds_to_balance();
long long balance_is_self_antipodal();
long long two_alfven_shares();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== magnetohydrodynamics (Alfven waves) ===");
    ok = expect_equal((long long)"Alfven wave state = 3/4 (field tension high)", fraction_to_text(alfven_state()), (long long)"3/4");
    ok = expect_equal((long long)"it folds to the tension-inertia balance 1/2", fraction_to_text(alfven_folds_to_balance()), (long long)"1/2");
    ok = expect_bool((long long)"the balance is self-antipodal (tension = inertia)", balance_is_self_antipodal());
    ok = expect_equal((long long)"two Alfven shares -> 3/2 casting to 1/2", fraction_to_text(two_alfven_shares()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long alfven_folds_to_balance() {
    ...
    ret_val = fold_fraction(alfven_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long balance_is_self_antipodal() {
    ...
    balance = fold_fraction(alfven_state());
...
long long two_alfven_shares() {
    ...
    state = fraction_from_ratio(colour_count(), (binary_count() * binary_count()));
    doubled = fraction_add(state, state);
    ret_val = cast_out_whole_ones(doubled);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
