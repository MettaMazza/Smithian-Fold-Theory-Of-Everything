...
long long valence_share();
long long bond_completes_the_whole();
long long half_closes_by_one_fold();
long long electrons_per_bond();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the molecular bond (two halves complete a whole) ===");
    ok = expect_equal((long long)"each atom's valence share = 1/2 (half-open)", fraction_to_text(valence_share()), (long long)"1/2");
    ok = expect_equal((long long)"the two halves complete the whole (the bond)", fraction_to_text(bond_completes_the_whole()), (long long)"1");
    ok = expect_equal((long long)"each half is one fold from unison through the shared state", fraction_to_text(half_closes_by_one_fold()), (long long)"1");
    ok = expect_equal((long long)"electrons per bond = the binary fibre = 2", int_to_string(electrons_per_bond()), (long long)"2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long half_closes_by_one_fold() {
    ...
    ret_val = fold_fraction(valence_share());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
