...
long long bond_share();
long long bond_completes_in_two_folds();
long long bonded_pair_balances();
long long slip_preserves_image();
long long slip_changes_position();
long long bond_share_is_positive();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== mechanical properties (elastic, plastic, fracture) ===");
    ok = expect_equal((long long)"the lattice-bond share = 1/4 (depth 2)", fraction_to_text(bond_share()), (long long)"1/4");
    ok = expect_bool((long long)"ELASTIC: the bond completes to unison in two folds (restoring chain)", bond_completes_in_two_folds());
    ok = expect_bool((long long)"two bonded neighbours balance at the half-One (1/4 + 1/4 = 1/2)", bonded_pair_balances());
    ok = expect_bool((long long)"PLASTIC: the twin 3/4 has the identical fold image (slip, same strength)", slip_preserves_image());
    ok = expect_bool((long long)"the twin is a different position (permanent shape change)", slip_changes_position());
    ok = expect_bool((long long)"FRACTURE: a present bond is strictly positive -> loss is discrete", bond_share_is_positive());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long bond_completes_in_two_folds() {
    ...
    bond = fraction_from_ratio(1, 4);
    once = fold_fraction(bond);
    twice = fold_fraction(once);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(twice, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long bonded_pair_balances() {
    long long bond = 0;
    long long half = 0;
    long long neighbour = 0;
    long long pair = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bond);
    ep_gc_push_root(&half);
    ep_gc_push_root(&neighbour);
    ep_gc_push_root(&pair);

    ep_gc_maybe_collect();

    bond = fraction_from_ratio(1, 4);
    neighbour = fraction_from_ratio(1, 4);
    pair = fraction_add(bond, neighbour);
    half = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(pair, half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(half);
    half = 0;
    free_struct_Fraction(pair);
    pair = 0;
    return ret_val;
}
...
long long slip_preserves_image() {
    ...
    long long image = 0;
    long long twin = 0;
    long long twin_image = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bond);
    ep_gc_push_root(&image);
    ep_gc_push_root(&twin);
    ep_gc_push_root(&twin_image);

    ep_gc_maybe_collect();

    bond = fraction_from_ratio(1, 4);
    twin = fraction_from_ratio(3, 4);
    image = fold_fraction(bond);
    twin_image = fold_fraction(twin);
    ret_val = fraction_compare(image, twin_image) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(image);
    image = 0;
    free_struct_Fraction(twin_image);
    twin_image = 0;
    return ret_val;
}
...
long long slip_changes_position() {
    ...
    bond = fraction_from_ratio(1, 4);
    twin = fraction_from_ratio(3, 4);
    same = fraction_compare(bond, twin) == 0;
    if (same) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(bond);
    bond = 0;
    free_struct_Fraction(twin);
    twin = 0;
    return ret_val;
}
...
long long bond_share_is_positive() {
    ...
    bond = fraction_from_ratio(1, 4);
    ret_val = ({ long long _fap = bond; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(bond);
