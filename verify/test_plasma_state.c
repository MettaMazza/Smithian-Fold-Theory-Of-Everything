...
long long shielding_balance();
long long shielding_is_balanced();
long long screening_completes_in_one_fold();
long long opposition_plus_field();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the plasma state (screening by balance-and-fold) ===");
    ok = expect_equal((long long)"shielding balance = 1/2 (cloud matches field)", fraction_to_text(shielding_balance()), (long long)"1/2");
    ok = expect_bool((long long)"the shielding is self-antipodal (opposition = source)", shielding_is_balanced());
    ok = expect_equal((long long)"one fold completes the screening (-> the One)", fraction_to_text(screening_completes_in_one_fold()), (long long)"1");
    ok = expect_equal((long long)"opposition + field sum to the One (nothing leaks)", fraction_to_text(opposition_plus_field()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long screening_completes_in_one_fold() {
    ...
    ret_val = fold_fraction(shielding_balance());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long opposition_plus_field() {
    ...
    balance = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(balance, balance);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
