...
long long _main();
long long electric_force();
long long test_speed();
long long magnetic_piece();
long long lorentz_force();
long long pieces_partition_electric();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Lorentz force (motion trims the electric force) ===");
    ok = expect_equal((long long)"electric force fe = 1/4", fraction_to_text(electric_force()), (long long)"1/4");
    ok = expect_equal((long long)"test speed beta = 1/2", fraction_to_text(test_speed()), (long long)"1/2");
    ok = expect_equal((long long)"magnetic piece = fe*beta^2 = 1/16", fraction_to_text(magnetic_piece()), (long long)"1/16");
    ok = expect_equal((long long)"net Lorentz force = fe(1 - beta^2) = 3/16", fraction_to_text(lorentz_force()), (long long)"3/16");
    ok = expect_bool((long long)"net + magnetic = electric (partition, nothing lost)", pieces_partition_electric());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long magnetic_piece() {
    long long beta = 0;
    long long beta_squared = 0;
    long long fe = 0;
    long long ret_val = 0;

    ep_gc_push_root(&beta);
    ep_gc_push_root(&beta_squared);
    ep_gc_push_root(&fe);
    ...
    beta_squared = fraction_multiply(beta, beta);
    ret_val = fraction_multiply(fe, beta_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long lorentz_force() {
    ...
    ep_gc_push_root(&fe);
    ep_gc_push_root(&trim);

    ep_gc_maybe_collect();

    fe = electric_force();
    trim = magnetic_piece();
    ret_val = fraction_subtract(fe, trim);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long pieces_partition_electric() {
    ...
    net = lorentz_force();
    trim = magnetic_piece();
    total = fraction_add(net, trim);
    fe = electric_force();
    ret_val = fraction_compare(total, fe) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(fe);
    fe = 0;
