...
long long _main();
long long advance_tick(long long);
long long fields_stay_locked();
long long light_speed();
long long energy_shares_equal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the coupled light wave (E and B in lockstep at c) ===");
    ok = expect_bool((long long)"E and B locked and returning at every one of 8 ticks", fields_stay_locked());
    ok = expect_equal((long long)"the wave speed = displacement/tick = the One = c", fraction_to_text(light_speed()), (long long)"1");
    ok = expect_bool((long long)"the energy shares split the whole evenly (E = B)", energy_shares_equal());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long advance_tick(long long phase) {
    long long moved = 0;
    long long step = 0;
    long long ret_val = 0;
    ...
    moved = fraction_add(phase, step);
    ret_val = cast_out_whole_ones(moved);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long fields_stay_locked() {
    ...
    ep_gc_push_root(&b_field);
    ep_gc_push_root(&e_field);
    ep_gc_push_root(&start);

    ep_gc_maybe_collect();

    start = fraction_from_ratio(1, 5);
    e_field = fraction_from_ratio(1, 5);
    b_field = fraction_from_ratio(1, 5);
    locked = 1LL;
    e_field = advance_tick(e_field);
    b_field = advance_tick(b_field);
    equal_now = fraction_compare(e_field, b_field) == 0;
    returned = fraction_compare(e_field, start) == 0;
    if (equal_now) {
    if (returned) {
    locked = locked;
    } else {
...
long long light_speed() {
    ...
    ep_gc_push_root(&displacement);
    ep_gc_push_root(&tick);

    ep_gc_maybe_collect();

    displacement = fraction_from_whole_number(1);
    tick = fraction_from_whole_number(1);
    ret_val = fraction_divide(displacement, tick);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long energy_shares_equal() {
    ...
    ep_gc_push_root(&b_share);
    ep_gc_push_root(&e_share);
    ep_gc_push_root(&one);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    e_share = fraction_from_ratio(1, 2);
    b_share = fraction_from_ratio(1, 2);
    total = fraction_add(e_share, b_share);
    one = fraction_from_whole_number(1);
    even = fraction_compare(e_share, b_share) == 0;
    whole = fraction_compare(total, one) == 0;
    if (even) {
    ret_val = whole;
    goto L_cleanup;
    }
