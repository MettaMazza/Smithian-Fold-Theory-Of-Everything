...
long long vacuum_density();
long long density_is_fold_invariant();
long long equation_of_state();
long long w_is_negative_unison();
long long non_diluting_condition();
long long dilution_exponent_vanishes();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the vacuum equation of state (w = -1) ===");
    ok = expect_equal((long long)"the vacuum density = the One (non-diluting)", fraction_to_text(vacuum_density()), (long long)"1");
    ok = expect_bool((long long)"the density is fold-invariant: fold(1) = 1", density_is_fold_invariant());
    ok = expect_equal((long long)"the equation of state w = -1", fraction_to_text(equation_of_state()), (long long)"-1");
    ok = expect_bool((long long)"w is the negative of unison (-1)", w_is_negative_unison());
    ok = expect_bool((long long)"non-diluting: w + 1 = 0 (pressure = -density)", non_diluting_condition());
    ok = expect_bool((long long)"the dilution exponent 3(1 + w) vanishes", dilution_exponent_vanishes());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long density_is_fold_invariant() {
    ...
    rho = fraction_from_whole_number(1);
    folded = fold_fraction(rho);
    ret_val = fraction_compare(folded, rho) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long equation_of_state() {
    ...
    ret_val = fraction_subtract(zero, one);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long w_is_negative_unison() {
    long long is_negative = 0;
    long long one = 0;
    long long restored = 0;
    long long restores_to_boundary = 0;
    long long w = 0;
    long long ret_val = 0;
    ...
    w = equation_of_state();
    is_negative = ({ long long _fap = w; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) < 0;
    one = fraction_from_whole_number(1);
    restored = fraction_add(w, one);
    restores_to_boundary = ({ long long _fap = restored; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    if (is_negative) {
    ret_val = restores_to_boundary;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long non_diluting_condition() {
    ...
    w = equation_of_state();
    one = fraction_from_whole_number(1);
    total = fraction_add(w, one);
    ret_val = ({ long long _fap = total; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long dilution_exponent_vanishes() {
    ...
    long long one = 0;
    long long one_plus_w = 0;
    long long w = 0;
    long long ret_val = 0;

    ep_gc_push_root(&dims);
    ep_gc_push_root(&exponent);
    ep_gc_push_root(&one);
    ep_gc_push_root(&one_plus_w);
    ep_gc_push_root(&w);
    ...
    w = equation_of_state();
    one = fraction_from_whole_number(1);
    one_plus_w = fraction_add(w, one);
    dims = fraction_from_whole_number(3);
    exponent = fraction_multiply(dims, one_plus_w);
    ret_val = ({ long long _fap = exponent; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(exponent);
    exponent = 0;
    return ret_val;
}
