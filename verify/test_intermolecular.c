...
long long primary_bond();
long long intermolecular_residual();
long long residual_folds_to_bond();
long long two_residuals_make_a_bond();
long long residual_reaches_unison();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== intermolecular forces (a residual, one fold deeper) ===");
    ok = expect_equal((long long)"primary bond = 1/2", fraction_to_text(primary_bond()), (long long)"1/2");
    ok = expect_equal((long long)"intermolecular residual = 1/4 = (1/2)^2", fraction_to_text(intermolecular_residual()), (long long)"1/4");
    ok = expect_equal((long long)"residual folds up to the bond level (1/4 -> 1/2)", fraction_to_text(residual_folds_to_bond()), (long long)"1/2");
    ok = expect_equal((long long)"two residuals make a bond (1/4 + 1/4 = 1/2)", fraction_to_text(two_residuals_make_a_bond()), (long long)"1/2");
    ok = expect_equal((long long)"it reaches unison in two folds (second-order mark)", fraction_to_text(residual_reaches_unison()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long residual_folds_to_bond() {
    ...
    ret_val = fold_fraction(intermolecular_residual());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_residuals_make_a_bond() {
    ...
    r = intermolecular_residual();
    ret_val = fraction_add(r, r);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long residual_reaches_unison() {
    ...
    ret_val = fold_fraction(residual_folds_to_bond());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
