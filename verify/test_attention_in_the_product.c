...
long long _main();
long long attention_writes_no_weight_family();
long long attention_selects_at_the_lock();
long long live_selection_on_cascade();
long long lock_is_the_cascade_head();
long long attention_law_is_in_the_product();
...
long long cascade_share(long long);
long long cascade_closes_at(long long);
long long cascade_closes_through_cover();
long long localization_beats_uniform();
long long capacity_reused();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== attention is in the product (Step 315) ===");
    ok = expect_bool((long long)"attention writes no weight-family (both families taken by the two stores)", attention_writes_no_weight_family());
    ok = expect_bool((long long)"attention selects at the unit-capacity lock 1/2", attention_selects_at_the_lock());
    ok = expect_bool((long long)"the live selection lies on the dyadic cascade, never uniform", live_selection_on_cascade());
    ok = expect_bool((long long)"one lock, two roles: focus_lock = cascade_share(1) = 1/2", lock_is_the_cascade_head());
    ok = expect_bool((long long)"THE PICTURE: attention's law is in the product, not the weights", attention_law_is_in_the_product());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    attention WEIGHTS near the null in all 11 atlas models (the quiet class, already in)");
    printf("%s\n", (char*)(long long)"  note  registered verification (Rung 7c, queued, not run): per-head QK^T spectra + attention");
    printf("%s\n", (char*)(long long)"  note  distributions vs the cascade shares through the checkpoint telescope -- gate 4b");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long attention_writes_no_weight_family() {
    ...
    if (harmonic_family_count() == smallest_fold_period_above(1)) {
    ret_val = assignment_is_forced();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
...
long long attention_selects_at_the_lock() {
    ...
    ret_val = single_focus_is_complete();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long live_selection_on_cascade() {
    ...
    ret_val = cascade_closes_through_cover();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
...
long long lock_is_the_cascade_head() {
    ...
    ret_val = fraction_compare(focus_lock(), cascade_share(1)) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long cascade_share(long long rank) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(1, whole_power(binary_count(), rank));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long cascade_closes_at(long long depth) {
    long long rank = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    total = fraction_from_ratio(1, 1);
    total = fraction_from_ratio(1, whole_power(binary_count(), depth));
    rank = 1;
    while (rank <= depth) {
    total = fraction_add(total, cascade_share(rank));
    rank = (rank + 1);
    }
    ret_val = fraction_compare(total, fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
...
long long localization_beats_uniform() {
    ...
    ep_gc_push_root(&gap);
    ep_gc_push_root(&top);
    ep_gc_push_root(&uniform);
    ep_gc_push_root(&width);

    ep_gc_maybe_collect();

    width = (smallest_fold_period_above(1) + 1);
    limit = whole_power(binary_count(), (binary_count() + colour_count()));
    while (width <= limit) {
    uniform = fraction_from_ratio(1, width);
    top = fraction_from_ratio(1, binary_count());
    if (fraction_compare(top, uniform) == 0) {
    ret_val = 0LL;
    goto L_cleanup;
    }
    gap = fraction_add(uniform, fraction_from_ratio(1, 1));
    width = (width + 1);
    }
    ret_val = 1LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(gap);
    gap = 0;
    free_struct_Fraction(top);
    top = 0;
    return ret_val;
}
...
long long capacity_reused() {
    ...
    if (single_focus_is_complete()) {
    ret_val = split_focus_incomplete();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
