...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== three of everything ===");
    ok = expect_number((long long)"colour count (period of 1/7)", colour_count(), 3);
    ok = expect_number((long long)"integers in stability window (2,4)", count_in_stability_window(), 1);
    ok = expect_number((long long)"spatial dimension (unique stable = colour)", spatial_dimension(), 3);
    ok = expect_number((long long)"generation count (tripling fibre = colour)", generation_count(), 3);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
