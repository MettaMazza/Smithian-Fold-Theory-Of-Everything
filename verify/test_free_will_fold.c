...
long long _main();
long long self_observation_closure();
long long forward_is_determined();
long long backward_is_two_to_one();
long long one_bit_is_lost();
long long free_will_is_ruled_out();
...
long long branch_weight_here();
long long observer_closure();
long long observation_is_the_fold();
long long chain_reaches_binding();
long long join_uses_no_new_operation();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== free will: fully determined, and self-opaque ===");
    ok = expect_equal((long long)"the self-observation closure (observation IS the fold)", fraction_to_text(self_observation_closure()), (long long)"1/4");
    ok = expect_bool((long long)"FORWARD: the fold is a function -- one state, one successor", forward_is_determined());
    ok = expect_bool((long long)"BACKWARD: 1/4 and 3/4 both fold to the half-One (2-to-1)", backward_is_two_to_one());
    ok = expect_bool((long long)"THE LOST BIT: distinct pasts, equal presents -- unrecoverable", one_bit_is_lost());
    ok = expect_bool((long long)"THE VERDICT: libertarian free will RULED OUT -- determinism + forced self-opacity", free_will_is_ruled_out());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long forward_is_determined() {
    ...
    long long second_run = 0;
    long long state = 0;
    long long state_again = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_run);
    ep_gc_push_root(&second_run);
    ep_gc_push_root(&state);
    ep_gc_push_root(&state_again);

    ep_gc_maybe_collect();

    state = fraction_from_ratio(3, 8);
    first_run = fold_fraction(state);
    state_again = fraction_from_ratio(3, 8);
    second_run = fold_fraction(state_again);
    ret_val = fraction_compare(first_run, second_run) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(first_run);
    first_run = 0;
    free_struct_Fraction(second_run);
    second_run = 0;
    return ret_val;
}

long long backward_is_two_to_one() {
    long long from_lower = 0;
    long long from_upper = 0;
    long long lower_hand = 0;
    long long upper_hand = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_lower);
    ep_gc_push_root(&from_upper);
    ep_gc_push_root(&lower_hand);
    ep_gc_push_root(&upper_hand);

    ep_gc_maybe_collect();

    lower_hand = fraction_from_ratio(1, 4);
    from_lower = fold_fraction(lower_hand);
    upper_hand = fraction_from_ratio(3, 4);
    from_upper = fold_fraction(upper_hand);
    ret_val = fraction_compare(from_lower, from_upper) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(from_lower);
    from_lower = 0;
    free_struct_Fraction(from_upper);
    from_upper = 0;
    return ret_val;
}

long long one_bit_is_lost() {
    long long hands_distinct = 0;
    long long hands_equal = 0;
    long long images_collapse = 0;
    long long lower_hand = 0;
    long long upper_hand = 0;
    long long ret_val = 0;
    ...
    lower_hand = fraction_from_ratio(1, 4);
    upper_hand = fraction_from_ratio(3, 4);
    hands_equal = fraction_compare(lower_hand, upper_hand) == 0;
    hands_distinct = 1LL;
    if (hands_equal) {
    hands_distinct = 0LL;
    }
    images_collapse = backward_is_two_to_one();
    if (hands_distinct) {
    ret_val = images_collapse;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(lower_hand);
    lower_hand = 0;
    free_struct_Fraction(upper_hand);
    upper_hand = 0;
    return ret_val;
}

long long free_will_is_ruled_out() {
    long long determined = 0;
    long long opaque = 0;
    long long ret_val = 0;

    determined = forward_is_determined();
    opaque = one_bit_is_lost();
    if (determined) {
    ret_val = opaque;
    goto L_cleanup;
    }
...
long long branch_weight_here() {
    ...
    depth = smallest_fold_period_above(binary_count());
...
long long observation_is_the_fold() {
    ...
    ep_gc_push_root(&closure);
    ep_gc_push_root(&observed);
    ep_gc_push_root(&weight);

    ep_gc_maybe_collect();

    weight = branch_weight_here();
    observed = fold_fraction(weight);
    closure = fraction_from_ratio(1, 4);
    ret_val = fraction_compare(observed, closure) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(closure);
    closure = 0;
    free_struct_Fraction(observed);
    observed = 0;
    return ret_val;
}
...
long long chain_reaches_binding() {
    ...
    closure = fraction_from_ratio(1, 4);
    lock = fold_fraction(closure);
    half = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(lock, half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long join_uses_no_new_operation() {
    ...
    ep_gc_push_root(&via_fold);
    ep_gc_push_root(&weight);

    ep_gc_maybe_collect();

    weight = branch_weight_here();
    twin = fraction_from_ratio(1, 8);
    doubled = fraction_add(weight, twin);
    direct = cast_out_whole_ones(doubled);
    via_fold = fold_fraction(branch_weight_here());
    ret_val = fraction_compare(direct, via_fold) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(direct);
    direct = 0;
    free_struct_Fraction(via_fold);
    via_fold = 0;
    return ret_val;
}
