...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold admissible-computation limit law ===");
    word = encode_computation_state(85, 7);
    ok = expect_true((long long)"a lawful counted Fold prefix is admissible", finite_fold_prefix_is_admissible(word, 4));
    ok = expect_true((long long)"every generated process prefix is admitted", complete_admitted_prefix_census());
    ok = expect_true((long long)"premature completion is rejected", premature_completion_claim_is_rejected(7));
    ok = expect_true((long long)"recurrent completion at the One is rejected", recurrent_completion_claim_is_rejected());
    ok = expect_true((long long)"relative information is exactly accounted", relative_information_is_accounted(7));
    ok = expect_true((long long)"a negative step count is rejected", negative_step_count_is_rejected(7));
    ok = expect_true((long long)"an outside result symbol is rejected", outside_result_symbol_is_rejected(7));
    ok = expect_true((long long)"a total self-negating outcome claim is rejected", total_self_negating_claim_is_rejected());
    ok = expect_true((long long)"the complete admissible-limit census holds", complete_admissible_computation_limit_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
