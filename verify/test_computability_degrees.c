...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold degrees of computability ===");
    ok = expect_equal((long long)"degree three distinguishes b^3 histories", int_to_string(fold_degree_distinction_count(3)), int_to_string(8));
    ok = expect_equal((long long)"depth seven at degree three retains sixteen observed classes", int_to_string(fold_degree_observed_class_count(7, 3)), int_to_string(16));
    ok = expect_equal((long long)"degree accounting reconstructs all depth-seven states", int_to_string(fold_degree_accounting(7, 3)), int_to_string(128));
    ok = expect_true((long long)"degree three reconstructs an exact source", fold_degree_reconstruction_holds(85, 7, 3));
    ok = expect_true((long long)"adjacent degrees refine by exactly b", adjacent_fold_degrees_are_nested(3));
    ok = expect_true((long long)"the complete degree hierarchy census holds", complete_fold_degree_census());
    ok = expect_true((long long)"a degree beyond the description is rejected", degree_beyond_description_is_rejected(7));
    ok = expect_true((long long)"changing a held degree label changes the source", changed_degree_label_changes_source(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
