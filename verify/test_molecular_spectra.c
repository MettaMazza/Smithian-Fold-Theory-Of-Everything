...
long long electronic_transition();
long long molecular_transition();
long long molecular_folds_to_electronic();
long long molecular_reaches_unison();
long long two_molecular_make_electronic();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== molecular spectra (deeper fold-levels) ===");
    ok = expect_equal((long long)"electronic transition = 1/2 (gross level)", fraction_to_text(electronic_transition()), (long long)"1/2");
    ok = expect_equal((long long)"molecular transition = 1/4 (one fold deeper)", fraction_to_text(molecular_transition()), (long long)"1/4");
    ok = expect_equal((long long)"molecular folds up to electronic (1/4 -> 1/2)", fraction_to_text(molecular_folds_to_electronic()), (long long)"1/2");
    ok = expect_equal((long long)"second fold reaches unison (1/2 -> 1)", fraction_to_text(molecular_reaches_unison()), (long long)"1");
    ok = expect_equal((long long)"two molecular quanta = one electronic level", fraction_to_text(two_molecular_make_electronic()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long molecular_transition() {
    long long electronic = 0;
    long long ret_val = 0;

    ep_gc_push_root(&electronic);

    ep_gc_maybe_collect();

    electronic = fraction_from_ratio(1, binary_count());
    ret_val = fraction_multiply(electronic, electronic);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long molecular_folds_to_electronic() {
    ...
    ret_val = fold_fraction(molecular_transition());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long molecular_reaches_unison() {
    ...
    ret_val = fold_fraction(molecular_folds_to_electronic());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long two_molecular_make_electronic() {
    long long molecular = 0;
    long long ret_val = 0;

    ep_gc_push_root(&molecular);

    ep_gc_maybe_collect();

    molecular = molecular_transition();
    ret_val = fraction_add(molecular, molecular);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
