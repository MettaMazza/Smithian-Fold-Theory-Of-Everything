...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== cosmic dilution exponents ===");
    ok = expect_number((long long)"matter exponent = d_space = 3 (a^-3)", matter_dilution_exponent(), 3);
    ok = expect_number((long long)"radiation exponent = d_space + 1 = spacetime = 4 (a^-4)", radiation_dilution_exponent(), 4);
    ok = expect_number((long long)"vacuum exponent = 0 (a^0, non-diluting)", vacuum_dilution_exponent(), 0);
    ok = expect_bool((long long)"radiation thins faster than matter (4 > 3)", radiation_thins_faster_than_matter());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
