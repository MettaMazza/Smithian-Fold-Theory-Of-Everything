...
long long alignment_state();
long long max_violation_state();
long long alignment_is_fold_invariant();
long long antipode_folds_to_alignment();
long long states_are_distinct();
long long antipode_doubles_to_alignment();
long long strong_lands_at_alignment();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the strong-CP problem (alignment is the fixed point) ===");
    ok = expect_equal((long long)"alignment (theta = 0) = the fold fixed point, the One", fraction_to_text(alignment_state()), (long long)"1");
    ok = expect_equal((long long)"maximal violation = the antipode 1/2", fraction_to_text(max_violation_state()), (long long)"1/2");
    ok = expect_bool((long long)"alignment is fold-invariant: fold(1) = 1", alignment_is_fold_invariant());
    ok = expect_bool((long long)"the antipode folds to alignment: fold(1/2) = 1", antipode_folds_to_alignment());
    ok = expect_bool((long long)"the two distinguished CP positions are distinct", states_are_distinct());
    ok = expect_bool((long long)"the antipode's halves sum to alignment (1/2 + 1/2 = 1)", antipode_doubles_to_alignment());
    ok = expect_bool((long long)"the vectorial strong phase can only sit at alignment -> theta = 0", strong_lands_at_alignment());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long alignment_is_fold_invariant() {
    ...
    ep_gc_push_root(&aligned);
    ep_gc_push_root(&folded);

    ep_gc_maybe_collect();

    aligned = fraction_from_whole_number(1);
    folded = fold_fraction(aligned);
    ret_val = fraction_compare(folded, aligned) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long antipode_folds_to_alignment() {
    ...
    antipode = fraction_from_ratio(1, 2);
    folded = fold_fraction(antipode);
    aligned = fraction_from_whole_number(1);
    ret_val = fraction_compare(folded, aligned) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(aligned);
    aligned = 0;
    free_struct_Fraction(folded);
    folded = 0;
    return ret_val;
...
long long states_are_distinct() {
    ...
    aligned = fraction_from_whole_number(1);
    antipode = fraction_from_ratio(1, 2);
    same = fraction_compare(aligned, antipode) == 0;
    if (same) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long antipode_doubles_to_alignment() {
    long long aligned = 0;
    long long antipode = 0;
    long long antipode_again = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&aligned);
    ep_gc_push_root(&antipode);
    ep_gc_push_root(&antipode_again);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    antipode = fraction_from_ratio(1, 2);
    antipode_again = fraction_from_ratio(1, 2);
    total = fraction_add(antipode, antipode_again);
    aligned = fraction_from_whole_number(1);
    ret_val = fraction_compare(total, aligned) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(aligned);
...
long long strong_lands_at_alignment() {
    long long aligned = 0;
    long long at_alignment = 0;
    long long invariant = 0;
    long long strong_phase = 0;
    long long ret_val = 0;

    ep_gc_push_root(&aligned);
    ep_gc_push_root(&strong_phase);

    ep_gc_maybe_collect();

    invariant = alignment_is_fold_invariant();
    aligned = fraction_from_whole_number(1);
    strong_phase = fraction_from_whole_number(1);
    at_alignment = fraction_compare(strong_phase, aligned) == 0;
    if (invariant) {
    ret_val = at_alignment;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ...
    free_struct_Fraction(aligned);
    aligned = 0;
    free_struct_Fraction(strong_phase);
    strong_phase = 0;
    return ret_val;
}
