...
long long _main();
long long composite_fold(long long);
long long world_shadow(long long, long long);
long long world_fold(long long, long long);
long long commutes_at(long long, long long);
long long bridge_commutes_everywhere();
long long anchor_is_universal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the composite bridge (the fold commutes with the prime worlds) ===");
    ok = expect_equal((long long)"the composite fold: 2/15 doubles to 4/15", int_to_string(composite_fold(2)), (long long)"4");
    ok = expect_bool((long long)"commutation at 2/15 in the 3-world", commutes_at(2, 3));
    ok = expect_bool((long long)"THE CENSUS: all 15 states commute in BOTH worlds (30 checks)", bridge_commutes_everywhere());
    ok = expect_bool((long long)"the anchor is universal: the One fixed in every world", anchor_is_universal());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long composite_fold(long long i) {
    long long doubled = 0;
    long long reduced = 0;
    long long ret_val = 0;
...
long long world_shadow(long long i, long long p) {
    long long shadow = 0;
    long long ret_val = 0;
...
long long world_fold(long long s, long long p) {
    long long doubled = 0;
    long long reduced = 0;
    long long ret_val = 0;
...
long long commutes_at(long long i, long long p) {
    long long folded_whole = 0;
    long long projected_first = 0;
    long long then_folded = 0;
    long long then_projected = 0;
    long long ret_val = 0;

    ep_gc_push_root(&folded_whole);
    ep_gc_push_root(&projected_first);
    ep_gc_push_root(&i);
    ep_gc_push_root(&p);

    ep_gc_maybe_collect();

    folded_whole = composite_fold(i);
    then_projected = world_shadow(folded_whole, p);
    projected_first = world_shadow(i, p);
    then_folded = world_fold(projected_first, p);
    ret_val = then_projected == then_folded;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long bridge_commutes_everywhere() {
    long long all_commute = 0;
    long long i = 0;
    long long in_five = 0;
    long long in_three = 0;
    long long ret_val = 0;
    ...
    i = 1;
    while (i < 16) {
    in_three = commutes_at(i, 3);
    in_five = commutes_at(i, 5);
    if (in_three) {
    if (in_five) {
...
long long anchor_is_universal() {
    long long composite_ok = 0;
    long long five_ok = 0;
    long long three_ok = 0;
    long long ret_val = 0;

    composite_ok = composite_fold(15) == 15;
    three_ok = world_fold(3, 3) == 3;
    five_ok = world_fold(5, 5) == 5;
    if (composite_ok) {
    if (three_ok) {
    ret_val = five_ok;
    goto L_cleanup;
