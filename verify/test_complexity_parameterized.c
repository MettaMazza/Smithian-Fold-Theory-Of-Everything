...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold parameterized complexity ===");
    ok = expect_equal((long long)"parameter three has eight active states", int_to_string(fold_parameter_active_states(3)), int_to_string(8));
    ok = expect_equal((long long)"depth seven parameter three has sixteen residual histories", int_to_string(fold_parameter_residual_histories(7, 3)), int_to_string(16));
    ok = expect_equal((long long)"parameter accounting reconstructs 128 states", int_to_string(fold_parameter_accounting(7, 3)), int_to_string(128));
    ok = expect_equal((long long)"parameter-three closure time is three", int_to_string(fold_parameter_time(3)), int_to_string(3));
    ok = expect_true((long long)"adjacent parameters scale by exactly b", adjacent_fold_parameters_scale_by_binary(7, 3));
    ok = expect_true((long long)"the complete parameterized census holds", complete_fold_parameterized_census());
    ok = expect_true((long long)"outside parameter values are rejected", outside_fold_parameter_is_rejected(7));
    ok = expect_true((long long)"nonbinary parameter scaling is rejected", nonbinary_parameter_scaling_is_rejected(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
