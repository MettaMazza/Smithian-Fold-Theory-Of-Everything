...
long long strong_field();
long long kerr_self_action();
long long kerr_shifts_the_field();
long long input_wave();
long long third_harmonic();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== nonlinear optics (the Kerr effect) ===");
    ok = expect_equal((long long)"strong field = 3/4", fraction_to_text(strong_field()), (long long)"3/4");
    ok = expect_equal((long long)"Kerr self-action: fold(3/4) = 1/2", fraction_to_text(kerr_self_action()), (long long)"1/2");
    ok = expect_bool((long long)"the Kerr effect shifts the field (self-action, not linear)", kerr_shifts_the_field());
    ok = expect_equal((long long)"input wave f = 1/6", fraction_to_text(input_wave()), (long long)"1/6");
    ok = expect_equal((long long)"third harmonic = 3 * f = 1/2 (odd Kerr harmonic)", fraction_to_text(third_harmonic()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long kerr_self_action() {
    ...
    ret_val = fold_fraction(strong_field());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long kerr_shifts_the_field() {
    ...
    before = fraction_from_ratio(3, 4);
    after = fold_fraction(strong_field());
    same = fraction_compare(before, after) == 0;
    if (same) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long third_harmonic() {
    ...
    ret_val = fraction_multiply(three, f);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
