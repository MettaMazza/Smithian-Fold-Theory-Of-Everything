...
long long dirac_generator();
long long generator_folds_to_unison();
long long generator_count();
long long generator_squared();
long long dispersion_sum();
long long square_of(long long);
long long polarization_sum();
long long routes_agree();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the full Dirac structure (the 3+1 dispersion closes) ===");
    ok = expect_bool((long long)"each generator (1/2) folds to unison on its own", generator_folds_to_unison());
    ok = expect_equal((long long)"the generator count = 3 + 1 = 4", int_to_string(generator_count()), (long long)"4");
    ok = expect_equal((long long)"each square = 1/4", fraction_to_text(generator_squared()), (long long)"1/4");
    ok = expect_equal((long long)"ROUTE A: the dispersion sum = the One", fraction_to_text(dispersion_sum()), (long long)"1");
    ok = expect_equal((long long)"ROUTE B: the full polarization identity = the One", fraction_to_text(polarization_sum()), (long long)"1");
    ok = expect_bool((long long)"the two routes agree (no mutated arithmetic)", routes_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long generator_folds_to_unison() {
    ...
    g = fraction_from_ratio(1, 2);
    folded = fold_fraction(g);
...
long long generator_count() {
    ...
    ret_val = (smallest_fold_period_above(binary_count()) + 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long generator_squared() {
    ...
    ep_gc_push_root(&g);
    ep_gc_push_root(&g_twin);

    ep_gc_maybe_collect();

    g = fraction_from_ratio(1, 2);
    g_twin = fraction_from_ratio(1, 2);
    ret_val = fraction_multiply(g, g_twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long dispersion_sum() {
    ...
    square = generator_squared();
    count = (smallest_fold_period_above(binary_count()) + 1);
    total = fraction_from_whole_number(0);
    i = 0;
    while (i < count) {
    total = fraction_add(total, square);
    i = (i + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long square_of(long long value) {
    long long twin = 0;
    long long ret_val = 0;
    ...
    twin = fraction_make(fraction_numerator(value), fraction_denominator(value));
    ret_val = fraction_multiply(value, twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long polarization_sum() {
    long long first_difference = 0;
    long long first_difference_squared = 0;
    long long first_sum = 0;
    long long first_sum_squared = 0;
    long long m = 0;
    long long m_again = 0;
    long long p1 = 0;
    long long p1_again = 0;
    long long p2 = 0;
    long long p2_again = 0;
    long long p3 = 0;
    long long p3_again = 0;
    long long partial_one = 0;
    long long partial_two = 0;
    long long second_difference = 0;
    long long second_difference_squared = 0;
    long long second_sum = 0;
    long long second_sum_squared = 0;
    long long total = 0;
    long long two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_difference);
    ep_gc_push_root(&first_difference_squared);
    ep_gc_push_root(&first_sum);
    ep_gc_push_root(&first_sum_squared);
    ep_gc_push_root(&m);
    ep_gc_push_root(&m_again);
    ep_gc_push_root(&p1);
    ep_gc_push_root(&p1_again);
    ep_gc_push_root(&p2);
    ep_gc_push_root(&p2_again);
    ep_gc_push_root(&p3);
    ep_gc_push_root(&p3_again);
    ep_gc_push_root(&partial_one);
    ep_gc_push_root(&partial_two);
    ep_gc_push_root(&second_difference);
    ep_gc_push_root(&second_difference_squared);
    ep_gc_push_root(&second_sum);
    ep_gc_push_root(&second_sum_squared);
    ep_gc_push_root(&total);
    ep_gc_push_root(&two);

    ep_gc_maybe_collect();

    p1 = fraction_from_ratio(1, 2);
    p2 = fraction_from_ratio(1, 2);
    first_sum = fraction_add(p1, p2);
    first_sum_squared = square_of(first_sum);
    p1_again = fraction_from_ratio(1, 2);
    p2_again = fraction_from_ratio(1, 2);
    first_difference = fraction_subtract(p1_again, p2_again);
    first_difference_squared = square_of(first_difference);
    p3 = fraction_from_ratio(1, 2);
    m = fraction_from_ratio(1, 2);
    second_sum = fraction_add(p3, m);
    second_sum_squared = square_of(second_sum);
    p3_again = fraction_from_ratio(1, 2);
    m_again = fraction_from_ratio(1, 2);
    second_difference = fraction_subtract(p3_again, m_again);
    second_difference_squared = square_of(second_difference);
    partial_one = fraction_add(first_sum_squared, first_difference_squared);
    partial_two = fraction_add(second_sum_squared, second_difference_squared);
    total = fraction_add(partial_one, partial_two);
    two = fraction_from_whole_number(2);
    ret_val = fraction_divide(total, two);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(20);
    return ret_val;
...
long long routes_agree() {
    long long direct = 0;
    long long polarized = 0;
    long long ret_val = 0;

    ep_gc_push_root(&direct);
    ep_gc_push_root(&polarized);

    ep_gc_maybe_collect();

    direct = dispersion_sum();
    polarized = polarization_sum();
    ret_val = fraction_compare(direct, polarized) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(direct);
    direct = 0;
    free_struct_Fraction(polarized);
    polarized = 0;
    return ret_val;
}
