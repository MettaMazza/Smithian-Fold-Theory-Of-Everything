...
long long _main();
long long pair_multiplier();
long long multiplier_is_branching_ratio();
long long pair_contracts();
long long collatz_move(long long);
long long odd_move_lands_even_to(long long);
long long eternal_cycle_closes();
long long descent_steps(long long);
long long all_orbits_descend_to(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Collatz as a fold contraction ===");
    ok = expect_equal((long long)"the odd-even pair multiplier (3/2)(1/2)", fraction_to_text(pair_multiplier()), (long long)"3/4");
    ok = expect_bool((long long)"the multiplier IS the branching ratio (m-1)/m at m=4 (Kleiber's 3/4)", multiplier_is_branching_ratio());
    ok = expect_bool((long long)"the pair CONTRACTS: 3/4 < the One", pair_contracts());
    ok = expect_bool((long long)"the odd move 3n+1 lands EVEN for every odd n to 10001", odd_move_lands_even_to(10001));
    ok = expect_bool((long long)"THE FLOOR: the eternal cycle 1 -> 4 -> 2 -> 1 closes in three", eternal_cycle_closes());
    ok = expect_equal((long long)"the walked descent of 27 (the famous long orbit): 111 steps", int_to_string(descent_steps(27)), (long long)"111");
    ok = expect_bool((long long)"THE DESCENT: every start 1 .. 100000 falls to the 1-cycle", all_orbits_descend_to(100000));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long pair_multiplier() {
    long long forced_halving = 0;
    long long odd_move = 0;
    long long ret_val = 0;

    ep_gc_push_root(&forced_halving);
    ep_gc_push_root(&odd_move);

    ep_gc_maybe_collect();

    odd_move = fraction_from_ratio(3, 2);
    forced_halving = fraction_from_ratio(1, 2);
    ret_val = fraction_multiply(odd_move, forced_halving);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long multiplier_is_branching_ratio() {
    long long branching = 0;
    long long multiplier = 0;
    long long ret_val = 0;

    ep_gc_push_root(&branching);
    ep_gc_push_root(&multiplier);

    ep_gc_maybe_collect();

    multiplier = pair_multiplier();
    branching = fraction_from_ratio(3, 4);
    ret_val = fraction_compare(multiplier, branching) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(branching);
    branching = 0;
    free_struct_Fraction(multiplier);
    multiplier = 0;
    return ret_val;
}
...
long long pair_contracts() {
    ...
    multiplier = pair_multiplier();
    ret_val = fraction_compare(one, multiplier) == 1;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(multiplier);
...
long long collatz_move(long long n) {
    long long half = 0;
    long long remainder = 0;
    long long ret_val = 0;

    half = (n / 2);
    remainder = (n - (half + half));
    if (remainder == 0) {
    ret_val = half;
    goto L_cleanup;
    }
...
long long odd_move_lands_even_to(long long bound) {
    long long all_even = 0;
    long long half = 0;
    long long moved = 0;
    long long n = 0;
    long long remainder = 0;
...
long long eternal_cycle_closes() {
    ...
    first = collatz_move(1);
    second = collatz_move(first);
    third = collatz_move(second);
    rises = first == 4;
    falls = second == 2;
    lands = third == 1;
    if (rises) {
...
long long descent_steps(long long start) {
    long long n = 0;
    long long steps = 0;
    long long ret_val = 0;
...
long long all_orbits_descend_to(long long bound) {
