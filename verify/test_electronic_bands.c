...
long long partly_filled_band();
long long filled_band();
long long partly_filled_conducts();
long long filled_band_insulates();
long long conductor_insulator_split();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== electronic bands ===");
    ok = expect_equal((long long)"partly-filled band = 1/2 (mobile, below the One)", fraction_to_text(partly_filled_band()), (long long)"1/2");
    ok = expect_equal((long long)"filled band = the One (locked)", fraction_to_text(filled_band()), (long long)"1");
    ok = expect_bool((long long)"a partly-filled band CONDUCTS (below the One)", partly_filled_conducts());
    ok = expect_bool((long long)"a filled band INSULATES (locked at the One)", filled_band_insulates());
    ok = expect_bool((long long)"conductor / insulator split", conductor_insulator_split());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long partly_filled_conducts() {
    ...
    ep_gc_push_root(&band);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    band = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    order = fraction_compare(band, one);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long filled_band_insulates() {
    ...
    band = fraction_from_whole_number(1);
    folded = fold_fraction(band);
    ret_val = fraction_compare(folded, band) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long conductor_insulator_split() {
    long long insulates = 0;
    long long conducts = 0;
    long long ret_val = 0;

    conducts = partly_filled_conducts();
    insulates = filled_band_insulates();
    if (conducts) {
    if (insulates) {
    ret_val = 1LL;
    goto L_cleanup;
