...
long long first_state();
long long second_state();
long long first_advances_to_second();
long long second_returns_to_first();
long long orbit_partitions_one();
long long orbit_period();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== self-organisation ===");
    ok = expect_equal((long long)"fold(1/3) = 2/3 (first advances to second)", fraction_to_text(first_advances_to_second()), (long long)"2/3");
    ok = expect_equal((long long)"fold(2/3) = 1/3 (second returns to first)", fraction_to_text(second_returns_to_first()), (long long)"1/3");
    ok = expect_bool((long long)"orbit partitions the One (1/3 + 2/3 = 1)", orbit_partitions_one());
    ok = expect_equal((long long)"orbit period (= period(1/3) = binary)", int_to_string(orbit_period()), int_to_string(2));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long first_advances_to_second() {
    ...
    ret_val = fold_fraction(first_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long orbit_partitions_one() {
    ...
    first = fraction_from_ratio(1, 3);
    second = fraction_from_ratio(2, 3);
    both = fraction_add(first, second);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(both, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long orbit_period() {
    long long by_orbit = 0;
    long long by_binary = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_orbit);
    ep_gc_push_root(&by_binary);

    ep_gc_maybe_collect();

    by_orbit = fold_period_of_unit_fraction(3);
    by_binary = smallest_fold_period_above(1);
    ret_val = forced_to_be((long long)"self-organisation orbit period = period(1/3) = binary", by_orbit, by_binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
