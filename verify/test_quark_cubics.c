...
long long _main() {
    long long bs = 0;
    long long bs_above = 0;
    long long central = 0;
    long long first_ordered = 0;
    long long heavy = 0;
    long long light = 0;
    long long ok = 0;
    long long one = 0;
    long long sd = 0;
    long long sd_above = 0;
    long long second_ordered = 0;
    long long tc = 0;
    long long tc_above = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bs);
    ep_gc_push_root(&bs_above);
    ep_gc_push_root(&central);
    ep_gc_push_root(&first_ordered);
    ep_gc_push_root(&heavy);
    ep_gc_push_root(&light);
    ep_gc_push_root(&one);
    ep_gc_push_root(&sd);
    ep_gc_push_root(&sd_above);
    ep_gc_push_root(&second_ordered);
    ep_gc_push_root(&tc);
    ep_gc_push_root(&tc_above);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the quark cubics (the dual mass equations) ===");
    ok = expect_equal((long long)"down second invariant: the dual 1/(3*2^7 - 1) = 1/383", fraction_to_text(down_second_invariant()), (long long)"1/383");
    ok = expect_equal((long long)"up second invariant: the dual 1/(3*2^10 - 1) = 1/3071", fraction_to_text(up_second_invariant()), (long long)"1/3071");
    ok = expect_bool((long long)"all three brackets carry genuine sign changes", brackets_carry_sign_changes());
    light = down_root(1);
    central = down_root(2);
    heavy = down_root(3);
    first_ordered = fraction_compare(light, central);
    second_ordered = fraction_compare(central, heavy);
    ok = expect_bool((long long)"the down roots are ordered (light < central < heavy)", first_ordered < 0);
    ok = expect_bool((long long)"and central < heavy", second_ordered < 0);
    one = fraction_from_whole_number(1);
    sd = bare_s_over_d();
    bs = bare_b_over_s();
    tc = bare_t_over_c();
    sd_above = fraction_compare(sd, one);
    bs_above = fraction_compare(bs, one);
    tc_above = fraction_compare(tc, one);
    ok = expect_bool((long long)"the bare ratios all exceed the One (mass grows up the ladder)", sd_above > 0);
    ok = expect_bool((long long)"b/s bare > 1", bs_above > 0);
    ok = expect_bool((long long)"t/c bare > 1", tc_above > 0);
    printf("%s\n", (char*)concat((long long)"  info  bare s/d = ", fraction_to_decimal(sd, 4)));
    printf("%s\n", (char*)concat((long long)"  info  bare b/s = ", fraction_to_decimal(bs, 4)));
    printf("%s\n", (char*)concat((long long)"  info  bare t/c = ", fraction_to_decimal(tc, 4)));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
