...
long long _main();
long long up_hand_count();
long long apex_squared();
long long apex_is_inverse_hand_count();
long long v_ub_squared();
...
long long _main() {
    long long apex = 0;
    long long measured_vub_squared = 0;
    long long ok = 0;
    long long vub2 = 0;
    long long ret_val = 0;

    ep_gc_push_root(&apex);
    ep_gc_push_root(&measured_vub_squared);
    ep_gc_push_root(&vub2);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the third CKM entry closed (the apex is the hand count) ===");
    ok = expect_equal((long long)"the up-hand count N_up = 6", int_to_string(up_hand_count()), (long long)"6");
    apex = apex_squared();
    ok = expect_equal((long long)"the apex squared = 1/6 EXACTLY (the closure identity)", fraction_to_text(apex), (long long)"1/6");
    ok = expect_bool((long long)"apex^2 = 1/N_up (the third entry adds no information)", apex_is_inverse_hand_count());
    vub2 = s13_squared();
    printf("%s\n", (char*)concat((long long)"  info  V_ub^2 = ", fraction_to_decimal(vub2, 9)));
    measured_vub_squared = fraction_from_ratio(1369, 100000000);
    ok = expect_bool((long long)"V_ub^2 lands within 25% of measured (0.0037)^2 (PDG)", within_tolerance_of(vub2, measured_vub_squared, 1, 4));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(apex);
    apex = 0;
    free_struct_Fraction(measured_vub_squared);
    measured_vub_squared = 0;
    free_struct_Fraction(vub2);
    vub2 = 0;
    return ret_val;
}
...
long long apex_squared() {
    ...
    ret_val = fraction_divide(third, product);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long apex_is_inverse_hand_count() {
    ...
    apex = apex_squared();
    target = fraction_from_ratio(1, up_hand_count());
    ret_val = fraction_compare(apex, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(apex);
