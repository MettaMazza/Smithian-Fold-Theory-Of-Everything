...
long long _main();
long long cascade_mass_through(long long);
long long band_interior();
long long loud_head_reaches_band_interior();
long long loud_head_depth();
long long quiet_tail();
long long tail_is_final_rung();
long long head_and_tail_partition_one();
long long member_count_is_engineering();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== expert quantization (Step 318 / A4) ===");
    ok = expect_equal((long long)"band interior = 31/32", fraction_to_text(band_interior()), (long long)"31/32");
    ok = expect_bool((long long)"the loud head (top b+c) carries the band interior 31/32", loud_head_reaches_band_interior());
    ok = expect_equal((long long)"loud-head depth = b+c = 5 (two routes agree)", int_to_string(loud_head_depth()), (long long)"5");
    ok = expect_equal((long long)"the quiet tail = 1/32", fraction_to_text(quiet_tail()), (long long)"1/32");
    ok = expect_bool((long long)"the tail is the band's final rung 1/band", tail_is_final_rung());
    ok = expect_bool((long long)"loud head + quiet tail partition the One (31/32 + 1/32 = 1)", head_and_tail_partition_one());
    ok = expect_equal((long long)"member count is engineering, kept off the derivation (guard passes)", int_to_string(member_count_is_engineering()), (long long)"1");
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    expert-axis instrument: individual experts loud beside ~1x neighbours (Qwen3-235B gates)");
    printf("%s\n", (char*)(long long)"  note  registered verification (rung 7d): rank experts by law-mass; loud head vs top-(b+c) cascade, tail vs 1/32");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long cascade_mass_through(long long depth) {
    long long rank = 0;
    long long total = 0;
    long long ret_val = 0;
    ...
    total = fraction_from_whole_number(0);
    rank = 1;
    while (rank <= depth) {
...
long long band_interior() {
    ...
    ret_val = fraction_from_ratio((functional_band() - 1), functional_band());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long loud_head_reaches_band_interior() {
    ...
    ret_val = fraction_compare(cascade_mass_through((binary_count() + colour_count())), band_interior()) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long loud_head_depth() {
    ...
    depth = 1;
    while (depth <= functional_band()) {
    if (fraction_compare(cascade_mass_through(depth), band_interior()) == 0) {
    ret_val = forced_to_be((long long)"loud-head depth = covering depth b+c", depth, (binary_count() + colour_count()));
    goto L_cleanup;
    }
    depth = (depth + 1);
    }
    ret_val = halt_violation((long long)"the cascade never reaches the band interior");
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long quiet_tail() {
    ...
    ret_val = fraction_subtract(one, cascade_mass_through((binary_count() + colour_count())));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long tail_is_final_rung() {
    ...
    ret_val = fraction_compare(quiet_tail(), fraction_from_ratio(1, functional_band())) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long head_and_tail_partition_one() {
    ...
    ep_gc_push_root(&head);

    ep_gc_maybe_collect();

    head = cascade_mass_through((binary_count() + colour_count()));
    ret_val = fraction_compare(fraction_add(head, quiet_tail()), fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(head);
    head = 0;
    return ret_val;
}
...
long long member_count_is_engineering() {
    ...
    ret_val = forbid_target_input((long long)"MoE member (expert) count (architecture width)", 0LL);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
