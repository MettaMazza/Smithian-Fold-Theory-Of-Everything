...
long long occupation_state_count();
long long max_particles_per_mode();
long long empty_occupation();
long long occupied_occupation();
long long occupations_share_one_mode();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== fermionic occupation (Pauli exclusion) ===");
    ok = expect_equal((long long)"occupation states = binary = 2 (empty, occupied)", int_to_string(occupation_state_count()), int_to_string(2));
    ok = expect_equal((long long)"max particles per mode = 1 (Pauli)", int_to_string(max_particles_per_mode()), int_to_string(1));
    ok = expect_equal((long long)"empty occupation = 1/4", fraction_to_text(empty_occupation()), (long long)"1/4");
    ok = expect_equal((long long)"occupied occupation = 3/4", fraction_to_text(occupied_occupation()), (long long)"3/4");
    ok = expect_bool((long long)"both occupations are preimages of one mode (fold to 1/2)", occupations_share_one_mode());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long max_particles_per_mode() {
    ...
    ret_val = (smallest_fold_period_above(1) - 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long occupations_share_one_mode() {
    long long empty_image = 0;
    long long occupied_image = 0;
    long long ret_val = 0;

    ep_gc_push_root(&empty_image);
    ep_gc_push_root(&occupied_image);

    ep_gc_maybe_collect();

    empty_image = fold_fraction(empty_occupation());
    occupied_image = fold_fraction(occupied_occupation());
    ret_val = fraction_compare(empty_image, occupied_image) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(empty_image);
    empty_image = 0;
    free_struct_Fraction(occupied_image);
    occupied_image = 0;
    return ret_val;
}
