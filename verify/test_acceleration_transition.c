...
long long vacuum_matter_ratio();
long long equality_scale_cube();
long long acceleration_scale_cube();
long long onset_precedes_today();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the acceleration transition (when the universe sped up) ===");
    ok = expect_equal((long long)"vacuum 2/3, matter 1/3 -> ratio = 2", fraction_to_text(vacuum_matter_ratio()), (long long)"2");
    ok = expect_equal((long long)"matter-vacuum equality at a^3 = 1/2", fraction_to_text(equality_scale_cube()), (long long)"1/2");
    ok = expect_equal((long long)"acceleration onset (q=0) at a^3 = 1/4 (z ~ 0.6)", fraction_to_text(acceleration_scale_cube()), (long long)"1/4");
    ok = expect_bool((long long)"onset precedes today (smaller scale-cube)", onset_precedes_today());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long vacuum_matter_ratio() {
    ...
    ret_val = fraction_divide(vac, mat);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long equality_scale_cube() {
    ...
    mat = fraction_from_ratio(1, colour_count());
    vac = fraction_from_ratio((colour_count() - 1), colour_count());
    ret_val = fraction_divide(mat, vac);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long acceleration_scale_cube() {
    long long mat = 0;
    long long vac = 0;
    long long twice_vacuum = 0;
    long long two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&mat);
    ep_gc_push_root(&vac);
    ep_gc_push_root(&twice_vacuum);
    ep_gc_push_root(&two);
    ...
    twice_vacuum = fraction_multiply(two, vac);
    mat = fraction_from_ratio(1, colour_count());
    ret_val = fraction_divide(mat, twice_vacuum);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long onset_precedes_today() {
    ...
    ep_gc_push_root(&onset);
    ep_gc_push_root(&equality);

    ep_gc_maybe_collect();

    onset = acceleration_scale_cube();
    equality = equality_scale_cube();
    comparison = fraction_compare(onset, equality);
