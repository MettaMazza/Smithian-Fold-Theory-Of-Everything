...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold search and order ===");
    target = encode_computation_state(73, 7);
    ok = expect_equal((long long)"ordered search finds rank 73", int_to_string(fold_ordered_search_rank(target, 7)), int_to_string(73));
    ok = expect_equal((long long)"ordered search uses 73 exact comparisons", int_to_string(fold_ordered_search_comparisons(73, 7)), int_to_string(73));
    ok = expect_equal((long long)"guided search decodes the same rank", int_to_string(fold_guided_search_rank(target, 7)), int_to_string(73));
    ok = expect_equal((long long)"guided search uses seven forced labels", int_to_string(fold_guided_search_queries(7)), int_to_string(7));
    ok = expect_true((long long)"the complete coordinate order is strict", fold_order_is_exact(7));
    ok = expect_true((long long)"every target is found in the complete census", complete_fold_search_order_census());
    ok = expect_true((long long)"an outside word is rejected", outside_search_word_is_rejected(7));
    ok = expect_true((long long)"reversed adjacent order is rejected", reversed_adjacent_order_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
