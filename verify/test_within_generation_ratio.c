...
long long generation_position(long long);
long long positions_triple_to_one();
long long mass_part(long long);
long long first_to_second_ratio();
long long light_pair_is_period_two();
long long heaviest_is_fixed_point();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== within-generation ratios (mass ratios are position ratios) ===");
    ok = expect_bool((long long)"all three positions triple to the One (the fibre, verified)", positions_triple_to_one());
    ok = expect_equal((long long)"mass-part of generation 1 = 2/3", fraction_to_text(mass_part(1)), (long long)"2/3");
    ok = expect_equal((long long)"mass-part of generation 2 = 1/3", fraction_to_text(mass_part(2)), (long long)"1/3");
    ok = expect_equal((long long)"mass-part of generation 3 = the whole", fraction_to_text(mass_part(3)), (long long)"1");
    ok = expect_equal((long long)"the between-generation ratio m1/m2 = 2 (position ratio)", fraction_to_text(first_to_second_ratio()), (long long)"2");
    ok = expect_bool((long long)"ROUTE B: the light pair is the doubling period-2 orbit", light_pair_is_period_two());
    ok = expect_bool((long long)"the heaviest generation sits at the fixed point", heaviest_is_fixed_point());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long generation_position(long long k) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(k, 3);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long positions_triple_to_one() {
    ...
    k = 1;
    while (k < 4) {
    position = fraction_from_ratio(k, 3);
    tripled = tripling_fold(position);
...
long long mass_part(long long k) {
    long long one = 0;
    long long position = 0;
    ...
    one = fraction_from_whole_number(1);
    position = fraction_from_ratio(k, 3);
    shortfall = fraction_subtract(one, position);
    if (({ long long _fap = shortfall; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0) {
...
long long light_pair_is_period_two() {
    ...
    long long from_m2 = 0;
    long long m1 = 0;
    long long m1_to_m2 = 0;
    long long m2 = 0;
    long long m2_to_m1 = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_m1);
    ep_gc_push_root(&from_m2);
    ep_gc_push_root(&m1);
    ep_gc_push_root(&m2);
    ...
    m1 = mass_part(1);
    m2 = mass_part(2);
    from_m1 = fold_fraction(m1);
    from_m2 = fold_fraction(m2);
    m1_to_m2 = fraction_compare(from_m1, m2) == 0;
    m2_to_m1 = fraction_compare(from_m2, m1) == 0;
    if (m1_to_m2) {
    ret_val = m2_to_m1;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(from_m1);
    from_m1 = 0;
    free_struct_Fraction(from_m2);
    from_m2 = 0;
    return ret_val;
}
...
long long heaviest_is_fixed_point() {
    ...
    m3 = mass_part(3);
    folded = fold_fraction(m3);
    ret_val = fraction_compare(folded, m3) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
