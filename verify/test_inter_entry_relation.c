...
long long _main();
long long ckm_first_row(long long);
long long pmns_first_row(long long);
long long ckm_row_sum();
long long pmns_row_sum();
long long ckm_residue_is_strong_coupling();
long long pmns_residue_is_electroweak_coupling();
long long locks_are_recoverable();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the inter-entry relation (each row casts to its coupling) ===");
    ok = expect_equal((long long)"the first CKM row sums to 5/3", fraction_to_text(ckm_row_sum()), (long long)"5/3");
    ok = expect_equal((long long)"the first PMNS row sums to 3/2", fraction_to_text(pmns_row_sum()), (long long)"3/2");
    ok = expect_bool((long long)"cast_out(5/3) = 2/3: the STRONG coupling recovered", ckm_residue_is_strong_coupling());
    ok = expect_bool((long long)"cast_out(3/2) = 1/2: the ELECTROWEAK coupling recovered", pmns_residue_is_electroweak_coupling());
    ok = expect_bool((long long)"the seal: each matrix hands back its own generating lock", locks_are_recoverable());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long ckm_first_row(long long j) {
...
long long pmns_first_row(long long j) {
    long long comparison = 0;
    long long distance = 0;
    long long mass = 0;
    long long one = 0;
...
long long ckm_row_sum() {
    ...
    first = ckm_first_row(1);
    second = ckm_first_row(2);
    third = ckm_first_row(3);
...
long long pmns_row_sum() {
    ...
    first = pmns_first_row(1);
    second = pmns_first_row(2);
    third = pmns_first_row(3);
    pair = fraction_add(first, second);
    ret_val = fraction_add(pair, third);
    goto L_cleanup;
L_cleanup:
...
long long ckm_residue_is_strong_coupling() {
    ...
    total = ckm_row_sum();
    residue = cast_out_whole_ones(total);
    target = fraction_from_ratio(2, 3);
    ret_val = fraction_compare(residue, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(residue);
    residue = 0;
...
long long pmns_residue_is_electroweak_coupling() {
    ...
    total = pmns_row_sum();
    residue = cast_out_whole_ones(total);
    target = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(residue, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long locks_are_recoverable() {
    ...
    ckm_ok = ckm_residue_is_strong_coupling();
    pmns_ok = pmns_residue_is_electroweak_coupling();
    if (ckm_ok) {
    ret_val = pmns_ok;
    goto L_cleanup;
    }
