...
long long _main();
long long sample_point();
long long square_at(long long);
long long lattice_curvature(long long);
long long continuum_curvature();
long long lattice_matches_continuum_at_quarter();
long long lattice_matches_continuum_at_eighth();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the continuum limit ===");
    ok = expect_equal((long long)"continuum curvature of x^2 = 2", fraction_to_text(continuum_curvature()), (long long)"2");
    ok = expect_equal((long long)"lattice curvature at spacing 1/4 = 2", fraction_to_text(lattice_curvature(fraction_from_ratio(1, 4))), (long long)"2");
    ok = expect_equal((long long)"lattice curvature at spacing 1/8 = 2", fraction_to_text(lattice_curvature(fraction_from_ratio(1, 8))), (long long)"2");
    ok = expect_bool((long long)"lattice matches continuum at spacing 1/4 (exact)", lattice_matches_continuum_at_quarter());
    ok = expect_bool((long long)"lattice matches continuum at spacing 1/8 (exact at every spacing)", lattice_matches_continuum_at_eighth());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long square_at(long long x) {
    long long x_again = 0;
    long long ret_val = 0;

    ep_gc_push_root(&x_again);
    ep_gc_push_root(&x);

    ep_gc_maybe_collect();

    x_again = fraction_make(fraction_numerator(x), fraction_denominator(x));
    ret_val = fraction_multiply(x, x_again);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long lattice_curvature(long long spacing) {
    long long outer_sum = 0;
    long long two_centre = 0;
    long long x_minus = 0;
    long long numerator = 0;
    long long x_plus = 0;
    long long spacing_again = 0;
    long long spacing_squared = 0;
    long long x = 0;
    long long f_minus = 0;
    long long f_plus = 0;
    ...
    ep_gc_push_root(&outer_sum);
    ep_gc_push_root(&two_centre);
    ep_gc_push_root(&x_minus);
    ep_gc_push_root(&numerator);
    ep_gc_push_root(&x_plus);
    ep_gc_push_root(&spacing_again);
    ep_gc_push_root(&spacing_squared);
    ep_gc_push_root(&x);
    ep_gc_push_root(&f_minus);
    ep_gc_push_root(&f_plus);
    ...
    x_plus = fraction_add(x, spacing);
    x_minus = fraction_subtract(x, spacing);
    f_plus = square_at(x_plus);
    f_minus = square_at(x_minus);
    f_centre = square_at(x);
    two_centre = fraction_add(f_centre, f_centre);
    outer_sum = fraction_add(f_plus, f_minus);
    numerator = fraction_subtract(outer_sum, two_centre);
    spacing_again = fraction_make(fraction_numerator(spacing), fraction_denominator(spacing));
    spacing_squared = fraction_multiply(spacing, spacing_again);
    ret_val = fraction_divide(numerator, spacing_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(12);
    return ret_val;
...
long long continuum_curvature() {
    ...
    ret_val = fraction_from_whole_number(2);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long lattice_matches_continuum_at_quarter() {
    ...
    ep_gc_push_root(&continuum);
    ep_gc_push_root(&lattice);

    ep_gc_maybe_collect();

    lattice = lattice_curvature(fraction_from_ratio(1, 4));
    continuum = fraction_from_whole_number(2);
    ret_val = fraction_compare(lattice, continuum) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(continuum);
    continuum = 0;
    free_struct_Fraction(lattice);
    lattice = 0;
    return ret_val;
}
...
long long lattice_matches_continuum_at_eighth() {
    ...
    lattice = lattice_curvature(fraction_from_ratio(1, 8));
    continuum = fraction_from_whole_number(2);
    ret_val = fraction_compare(lattice, continuum) == 0;
    goto L_cleanup;
L_cleanup:
