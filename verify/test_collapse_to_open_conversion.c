...
long long _main();
long long absolute_size(long long);
long long enclosure_tolerance();
long long lands_on(long long, long long);
long long roots_sum_to_one();
long long pair_sum_is_second_invariant();
long long product_is_third_invariant();
long long conversion_closes();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Vieta cross-check (roots pinned from both sides) ===");
    ok = expect_bool((long long)"VIETA 1: the roots sum to the One (no share lost)", roots_sum_to_one());
    ok = expect_bool((long long)"VIETA 2: the pair-sum lands on the invariant 1/6", pair_sum_is_second_invariant());
    ok = expect_bool((long long)"VIETA 3: the product lands on the invariant 1/485", product_is_third_invariant());
    ok = expect_bool((long long)"the conversion closes: bisection and Vieta agree on all three", conversion_closes());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long absolute_size(long long value) {
    long long zero = 0;
    long long ret_val = 0;
    ...
    ret_val = fraction_subtract(zero, value);
    goto L_cleanup;
    }
    ret_val = value;
    goto L_cleanup;
...
long long enclosure_tolerance() {
    ...
    ret_val = fraction_from_ratio(1, 1000000000);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long lands_on(long long value, long long target) {
    ...
    size = absolute_size(gap);
    tolerance = fraction_from_ratio(1, 1000000000);
    comparison = fraction_compare(size, tolerance);
...
long long roots_sum_to_one() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&pair);
    ep_gc_push_root(&tau);
    ep_gc_push_root(&total);

    ep_gc_maybe_collect();

    electron = lepton_root_in_bracket(1, 1000, 1, 20, 40);
    muon = lepton_root_in_bracket(1, 10, 3, 10, 40);
    tau = lepton_root_in_bracket(7, 10, 9, 10, 40);
    pair = fraction_add(electron, muon);
    total = fraction_add(pair, tau);
    one = fraction_from_whole_number(1);
    ret_val = lands_on(total, one);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long pair_sum_is_second_invariant() {
    ...
    long long e_tau = 0;
    long long electron = 0;
    long long electron_again = 0;
    long long mu_tau = 0;
    long long muon = 0;
    long long muon_again = 0;
    long long partial = 0;
    long long target = 0;
    long long tau = 0;
    long long tau_again = 0;
    long long tau_third = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&e_mu);
    ep_gc_push_root(&e_tau);
    ep_gc_push_root(&electron);
    ep_gc_push_root(&electron_again);
    ep_gc_push_root(&mu_tau);
    ep_gc_push_root(&muon);
    ep_gc_push_root(&muon_again);
    ep_gc_push_root(&partial);
    ep_gc_push_root(&target);
    ep_gc_push_root(&tau);
    ep_gc_push_root(&tau_again);
    ep_gc_push_root(&tau_third);
    ep_gc_push_root(&total);
    ...
    muon = lepton_root_in_bracket(1, 10, 3, 10, 40);
    tau = lepton_root_in_bracket(7, 10, 9, 10, 40);
    e_mu = fraction_multiply(electron, muon);
    electron_again = lepton_root_in_bracket(1, 1000, 1, 20, 40);
    tau_again = lepton_root_in_bracket(7, 10, 9, 10, 40);
    e_tau = fraction_multiply(electron_again, tau_again);
    muon_again = lepton_root_in_bracket(1, 10, 3, 10, 40);
    tau_third = lepton_root_in_bracket(7, 10, 9, 10, 40);
    mu_tau = fraction_multiply(muon_again, tau_third);
    partial = fraction_add(e_mu, e_tau);
    total = fraction_add(partial, mu_tau);
    target = second_invariant();
    ret_val = lands_on(total, target);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(13);
    free_struct_Fraction(tau);
    tau = 0;
    return ret_val;
}
...
long long product_is_third_invariant() {
    ...
    muon = lepton_root_in_bracket(1, 10, 3, 10, 40);
    tau = lepton_root_in_bracket(7, 10, 9, 10, 40);
    pair = fraction_multiply(electron, muon);
    total = fraction_multiply(pair, tau);
    target = leading_third_invariant();
    ret_val = lands_on(total, target);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
...
long long conversion_closes() {
    ...
    first = roots_sum_to_one();
    second = pair_sum_is_second_invariant();
    third = product_is_third_invariant();
    if (first) {
    if (second) {
    ret_val = third;
    goto L_cleanup;
