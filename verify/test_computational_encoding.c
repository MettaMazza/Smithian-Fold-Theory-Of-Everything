...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold encoding law ===");
    ok = expect_equal((long long)"the derived alphabet has b symbols", int_to_string(encoding_symbol_count()), int_to_string(2));
    ok = expect_equal((long long)"depth-7 state 128 decodes from its codeword", int_to_string(decode_computation_codeword(encode_computation_state(128, 7))), int_to_string(128));
    ok = expect_true((long long)"all depth-7 states encode and decode exactly", all_states_round_trip_at_depth(7));
    ok = expect_equal((long long)"depth-7 codeword count is forced", int_to_string(codeword_count_is_forced(7)), int_to_string(128));
    ok = expect_true((long long)"observation retains every depth-7 codeword suffix", every_observation_retains_suffix(7));
    ok = expect_equal((long long)"each observed suffix has b-1 redundant alternatives", int_to_string(redundant_codeword_alternatives(7)), int_to_string(1));
    ok = expect_true((long long)"an out-of-alphabet symbol is rejected", out_of_alphabet_symbol_is_rejected(7));
    ok = expect_true((long long)"changing one lawful symbol changes the decoded state", changed_symbol_changes_state(7));
    ok = expect_true((long long)"complete encoding census holds", complete_encoding_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
