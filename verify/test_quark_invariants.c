...
long long _main();
long long up_channel_count();
long long down_channel_count();
long long up_first_invariant();
long long down_first_invariant();
long long up_invariant_structural();
long long down_invariant_structural();
long long invariant_routes_agree();
long long up_depth();
long long down_depth();
long long up_depth_by_period();
long long down_depth_by_period();
long long depth_routes_agree();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the quark first invariants (1/12, 1/8, depths 7 and 5) ===");
    ok = expect_equal((long long)"up channels: colour 3 + unbroken 3 = 6", int_to_string(up_channel_count()), (long long)"6");
    ok = expect_equal((long long)"down channels: colour 3 + broken neutral 1 = 4", int_to_string(down_channel_count()), (long long)"4");
    ok = expect_equal((long long)"I1_up = 1/(2*6) = 1/12", fraction_to_text(up_first_invariant()), (long long)"1/12");
    ok = expect_equal((long long)"I1_down = 1/(2*4) = 1/8", fraction_to_text(down_first_invariant()), (long long)"1/8");
    ok = expect_bool((long long)"Route B products agree: (1/4)(1/3) and (1/4)(1/2)", invariant_routes_agree());
    ok = expect_equal((long long)"d_up, counted: minimal cover of 81 = 7", int_to_string(up_depth()), (long long)"7");
    ok = expect_equal((long long)"d_down, counted: minimal cover of 27 = 5", int_to_string(down_depth()), (long long)"5");
    ok = expect_bool((long long)"Route B periods agree: period(1/127) = 7, period(1/31) = 5", depth_routes_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long up_channel_count() {
    ...
    ret_val = (smallest_fold_period_above(binary_count()) + smallest_fold_period_above(binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long up_first_invariant() {
    ...
    channels = (smallest_fold_period_above(binary_count()) + smallest_fold_period_above(binary_count()));
    ret_val = fraction_from_ratio(1, (binary_count() * channels));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long down_first_invariant() {
    ...
    channels = (smallest_fold_period_above(binary_count()) + 1);
    ret_val = fraction_from_ratio(1, (binary_count() * channels));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long up_invariant_structural() {
    ...
    long long tripling_share = 0;
    long long ret_val = 0;

    ep_gc_push_root(&lower_hand);
    ep_gc_push_root(&tripling_share);

    ep_gc_maybe_collect();

    lower_hand = fraction_from_ratio(1, 4);
    tripling_share = fraction_from_ratio(1, 3);
    ret_val = fraction_multiply(lower_hand, tripling_share);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long down_invariant_structural() {
    ...
    ep_gc_push_root(&critical);
    ep_gc_push_root(&lower_hand);

    ep_gc_maybe_collect();

    lower_hand = fraction_from_ratio(1, 4);
    critical = fraction_from_ratio(1, 2);
    ret_val = fraction_multiply(lower_hand, critical);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long invariant_routes_agree() {
    long long down_a = 0;
    long long down_b = 0;
    long long down_match = 0;
    long long up_a = 0;
    long long up_b = 0;
    long long up_match = 0;
    long long ret_val = 0;

    ep_gc_push_root(&down_a);
    ep_gc_push_root(&down_b);
    ep_gc_push_root(&up_a);
    ep_gc_push_root(&up_b);

    ep_gc_maybe_collect();

    up_a = up_first_invariant();
    up_b = up_invariant_structural();
    down_a = down_first_invariant();
    down_b = down_invariant_structural();
    up_match = fraction_compare(up_a, up_b) == 0;
    down_match = fraction_compare(down_a, down_b) == 0;
    if (up_match) {
    ret_val = down_match;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(down_a);
    down_a = 0;
    free_struct_Fraction(down_b);
    down_b = 0;
    free_struct_Fraction(up_a);
    up_a = 0;
    free_struct_Fraction(up_b);
    up_b = 0;
    return ret_val;
}
...
long long up_depth() {
    ...
    volume = whole_power(colour_count(), 4);
...
long long down_depth() {
    ...
    volume = whole_power(colour_count(), 3);
    ret_val = minimal_binary_cover(volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long up_depth_by_period() {
    long long mersenne = 0;
    long long ret_val = 0;

    ep_gc_push_root(&mersenne);

    ep_gc_maybe_collect();

    mersenne = (whole_power(binary_count(), 7) - 1);
    ret_val = fold_period_of_unit_fraction(mersenne);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long down_depth_by_period() {
    ...
    mersenne = (whole_power(binary_count(), 5) - 1);
    ret_val = fold_period_of_unit_fraction(mersenne);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long depth_routes_agree() {
    ...
    up_a = up_depth();
    up_b = up_depth_by_period();
    down_a = down_depth();
    down_b = down_depth_by_period();
    up_match = up_a == up_b;
    down_match = down_a == down_b;
    if (up_match) {
    ret_val = down_match;
    goto L_cleanup;
    }
