...
long long _main();
long long integer_plateau(long long);
long long fractional_plateau();
long long fractional_plateau_is_one_over_colour();
long long plateaus_differ_by_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the quantum Hall effect ===");
    ok = expect_equal((long long)"integer plateau 1 = 1 unit", int_to_string(integer_plateau(1)), int_to_string(1));
    ok = expect_equal((long long)"integer plateau 2 = 2 units", int_to_string(integer_plateau(2)), int_to_string(2));
    ok = expect_equal((long long)"integer plateau 3 = 3 units", int_to_string(integer_plateau(3)), int_to_string(3));
    ok = expect_equal((long long)"primary fractional plateau nu = 1/colour = 1/3", fraction_to_text(fractional_plateau()), (long long)"1/3");
    ok = expect_bool((long long)"fractional plateau is exactly 1/colour", fractional_plateau_is_one_over_colour());
    ok = expect_bool((long long)"plateaus are quantized (differ by exactly one unit)", plateaus_differ_by_one());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long integer_plateau(long long n) {
    long long ret_val = 0;
...
long long fractional_plateau_is_one_over_colour() {
    ...
    ep_gc_push_root(&plateau);

    ep_gc_maybe_collect();

    plateau = fraction_from_ratio(1, colour_count());
    one_over_colour = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(plateau, one_over_colour) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(one_over_colour);
    one_over_colour = 0;
    free_struct_Fraction(plateau);
    plateau = 0;
    return ret_val;
}
