...
long long _main();
long long longitudinal_branches();
long long transverse_branches();
long long acoustic_branches();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== phonons (lattice vibrations) ===");
    ok = expect_number((long long)"longitudinal branches = 1", longitudinal_branches(), 1);
    ok = expect_number((long long)"transverse branches = d_space - 1 = 2", transverse_branches(), 2);
    ok = expect_number((long long)"acoustic branches = d_space = 3 (forced two ways)", acoustic_branches(), 3);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long acoustic_branches() {
    ...
    by_sum = (1 + (spatial_dimensions() - 1));
    by_dimension = spatial_dimensions();
    ret_val = forced_to_be((long long)"acoustic phonon branches = longitudinal + transverse = d_space", by_sum, by_dimension);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
