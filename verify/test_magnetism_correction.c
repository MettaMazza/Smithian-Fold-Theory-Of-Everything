...
long long speed_squared(long long, long long);
long long correction_factor(long long, long long);
long long covariance_at(long long, long long);
long long correction_is_fold_covariant();
long long moving_charge_feels_less();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== magnetism as relativity (the fold-covariant correction) ===");
    ok = expect_equal((long long)"at beta = 1/2: C = 1 - 1/4 = 3/4", fraction_to_text(correction_factor(1, 2)), (long long)"3/4");
    ok = expect_bool((long long)"fold covariance at beta = 1/2: fold(C) = 1 - fold(beta^2)", covariance_at(1, 2));
    ok = expect_bool((long long)"the covariance holds across speeds (1/2 and 1/4)", correction_is_fold_covariant());
    ok = expect_bool((long long)"a moving charge feels strictly less force (C < 1)", moving_charge_feels_less());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long speed_squared(long long num, long long den) {
    long long beta = 0;
    long long twin = 0;
    long long ret_val = 0;
    ...
    beta = fraction_from_ratio(num, den);
    twin = fraction_from_ratio(num, den);
    ret_val = fraction_multiply(beta, twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long correction_factor(long long num, long long den) {
    long long beta_sq = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&beta_sq);
    ep_gc_push_root(&one);
    ep_gc_push_root(&num);
    ep_gc_push_root(&den);
    ...
    beta_sq = speed_squared(num, den);
    ret_val = fraction_subtract(one, beta_sq);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long covariance_at(long long num, long long den) {
    long long beta_sq = 0;
    long long c = 0;
    long long folded_c = 0;
    long long folded_sq = 0;
    long long one = 0;
    long long target = 0;
    long long ret_val = 0;
    ...
    ep_gc_push_root(&beta_sq);
    ep_gc_push_root(&c);
    ep_gc_push_root(&folded_c);
    ep_gc_push_root(&folded_sq);
    ep_gc_push_root(&one);
    ...
    c = correction_factor(num, den);
    folded_c = fold_fraction(c);
    beta_sq = speed_squared(num, den);
    folded_sq = fold_fraction(beta_sq);
    one = fraction_from_whole_number(1);
    target = fraction_subtract(one, folded_sq);
    ret_val = fraction_compare(folded_c, target) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_Fraction(folded_c);
    folded_c = 0;
...
long long correction_is_fold_covariant() {
    long long at_half = 0;
    long long at_quarter = 0;
    long long ret_val = 0;

    at_half = covariance_at(1, 2);
    at_quarter = covariance_at(1, 4);
    if (at_half) {
    ret_val = at_quarter;
    goto L_cleanup;
    }
...
long long moving_charge_feels_less() {
    ...
    c = correction_factor(1, 2);
    one = fraction_from_whole_number(1);
    comparison = fraction_compare(c, one);
