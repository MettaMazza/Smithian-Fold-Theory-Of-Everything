...
long long _main();
long long momentum_share();
long long mass_share_two();
long long square_of_share(long long);
long long dispersion_closes();
long long polarization_route();
long long routes_agree_on_one();
long long triple_is_generated();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the two-component dispersion (motion plus substance = the One) ===");
    ok = expect_equal((long long)"the momentum share p = 3/5", fraction_to_text(momentum_share()), (long long)"3/5");
    ok = expect_equal((long long)"the mass share m = 4/5", fraction_to_text(mass_share_two()), (long long)"4/5");
    ok = expect_equal((long long)"ROUTE A: p^2 + m^2 = 9/25 + 16/25 = the One", fraction_to_text(dispersion_closes()), (long long)"1");
    ok = expect_equal((long long)"ROUTE B: the polarization identity gives the same One", fraction_to_text(polarization_route()), (long long)"1");
    ok = expect_bool((long long)"the two routes agree (no single-computation artifact)", routes_agree_on_one());
    ok = expect_bool((long long)"the triple is generated: 3 = colour, 4 = binary^2, 5 = 2+3", triple_is_generated());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long momentum_share() {
    ...
    ret_val = fraction_from_ratio(3, 5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long mass_share_two() {
    ...
    ret_val = fraction_from_ratio(4, 5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long square_of_share(long long value) {
    long long twin = 0;
    long long ret_val = 0;
...
long long dispersion_closes() {
    ...
    long long p = 0;
    long long p_squared = 0;
    long long ret_val = 0;

    ep_gc_push_root(&m);
    ep_gc_push_root(&m_squared);
    ep_gc_push_root(&p);
    ep_gc_push_root(&p_squared);

    ep_gc_maybe_collect();

    p = fraction_from_ratio(3, 5);
    m = fraction_from_ratio(4, 5);
    p_squared = square_of_share(p);
    m_squared = square_of_share(m);
    ret_val = fraction_add(p_squared, m_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long polarization_route() {
    ...
    long long difference_squared = 0;
    long long m = 0;
    long long m_again = 0;
    long long p = 0;
    long long p_again = 0;
    long long total = 0;
    long long total_squared = 0;
    long long two = 0;
    long long ret_val = 0;

    ep_gc_push_root(&both);
    ep_gc_push_root(&difference);
    ep_gc_push_root(&difference_squared);
    ep_gc_push_root(&m);
    ep_gc_push_root(&m_again);
    ep_gc_push_root(&p);
    ep_gc_push_root(&p_again);
    ep_gc_push_root(&total);
    ep_gc_push_root(&total_squared);
    ep_gc_push_root(&two);
    ...
    p = fraction_from_ratio(3, 5);
    m = fraction_from_ratio(4, 5);
    total = fraction_add(p, m);
    total_squared = square_of_share(total);
    p_again = fraction_from_ratio(3, 5);
    m_again = fraction_from_ratio(4, 5);
    difference = fraction_subtract(m_again, p_again);
    difference_squared = square_of_share(difference);
    both = fraction_add(total_squared, difference_squared);
    two = fraction_from_whole_number(2);
    ret_val = fraction_divide(both, two);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(10);
    return ret_val;
...
long long routes_agree_on_one() {
    long long direct = 0;
    long long direct_ok = 0;
    long long one = 0;
    long long polarized = 0;
    long long polarized_ok = 0;
    long long ret_val = 0;
    ...
    direct = dispersion_closes();
    polarized = polarization_route();
    one = fraction_from_whole_number(1);
    direct_ok = fraction_compare(direct, one) == 0;
    polarized_ok = fraction_compare(polarized, one) == 0;
    if (direct_ok) {
    ret_val = polarized_ok;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
...
long long triple_is_generated() {
    ...
    three_ok = smallest_fold_period_above(binary_count()) == 3;
    four_ok = (smallest_fold_period_above(1) * smallest_fold_period_above(1)) == 4;
    five_ok = (smallest_fold_period_above(1) + smallest_fold_period_above(binary_count())) == 5;
    if (three_ok) {
    if (four_ok) {
    ret_val = five_ok;
    goto L_cleanup;
