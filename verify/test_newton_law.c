...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Newton's law (the inverse-square field from flux) ===");
    ok = expect_equal((long long)"source ms = 1/4", fraction_to_text(gravity_source()), (long long)"1/4");
    r_half = fraction_from_ratio(1, 2);
    ok = expect_equal((long long)"potential at r=1/2: Phi = 1/2", fraction_to_text(potential_at(r_half)), (long long)"1/2");
    ok = expect_equal((long long)"field at r=1/2: g = ms/r^2 = 1", fraction_to_text(field_at(r_half)), (long long)"1");
    r_one = fraction_from_whole_number(1);
    ok = expect_equal((long long)"field at r=1: g = 1/4", fraction_to_text(field_at(r_one)), (long long)"1/4");
    ok = expect_bool((long long)"Gauss: the flux is the same at every radius", flux_is_conserved());
    ok = expect_bool((long long)"the flux constant equals the source", flux_equals_source());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long potential_at(long long radius) {
    long long one = 0;
    long long ms = 0;
    long long claim = 0;
    long long ret_val = 0;
    ...
    ms = gravity_source();
    claim = fraction_divide(ms, radius);
...
long long field_at(long long radius) {
    ...
    ret_val = fraction_divide(ms, r_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long flux_at(long long radius) {
    long long g = 0;
    long long r_squared = 0;
    long long ret_val = 0;
    ...
    g = field_at(radius);
    r_squared = fraction_multiply(radius, radius);
    ret_val = fraction_multiply(r_squared, g);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long flux_is_conserved() {
    long long r_half = 0;
    long long r_one = 0;
    long long flux_far = 0;
    long long flux_near = 0;
    long long ret_val = 0;

    ep_gc_push_root(&r_half);
    ep_gc_push_root(&r_one);
    ep_gc_push_root(&flux_far);
    ep_gc_push_root(&flux_near);

    ep_gc_maybe_collect();

    r_half = fraction_from_ratio(1, 2);
    flux_near = flux_at(r_half);
    r_one = fraction_from_whole_number(1);
    flux_far = flux_at(r_one);
    ret_val = fraction_compare(flux_near, flux_far) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(flux_far);
    flux_far = 0;
    free_struct_Fraction(flux_near);
    flux_near = 0;
    return ret_val;
}
...
long long flux_equals_source() {
    ...
    flux = flux_at(r_one);
    ms = gravity_source();
    ret_val = fraction_compare(flux, ms) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(ms);
    ms = 0;
