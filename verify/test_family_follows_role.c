...
long long _main();
long long distinct_storing_role_generators();
long long role_family_bijection();
long long family_invariant_per_role();
long long role_is_engineering();
long long family_follows_role_not_recipe();
...
long long selection_lock_is_binary();
long long selection_family_period();
long long geometry_family_period();
long long assignment_is_forced();
...
long long further_generators_in_window();
long long harmonic_family_count();
long long colour_cos_squared();
long long third_turn_identity_holds();
long long colour_family_meets_electroweak();
long long families_distinct();
...
long long focus_lock();
long long single_focus_is_complete();
long long focus_folds_to_unison();
long long split_focus();
long long split_focus_incomplete();
...
long long colour_signature_low();
long long colour_signature_high();
long long colour_low_is_closed_value();
long long colour_high_two_routes();
long long colour_signature_partitions_one();
long long is_proper_fraction(long long);
long long colour_values_not_whole();
long long family_read_from_value();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== family follows the role, not the recipe (Step 317) ===");
    ok = expect_equal((long long)"two distinct storing-role generators (selection!=geometry)", int_to_string(distinct_storing_role_generators()), (long long)"2");
    ok = expect_equal((long long)"role<->family bijection: distinct role-generators = family count", int_to_string(role_family_bijection()), (long long)"2");
    ok = expect_bool((long long)"family invariant per role (bijection is invertible)", family_invariant_per_role());
    ok = expect_equal((long long)"the role is engineering, kept off the derivation side (guard passes)", int_to_string(role_is_engineering()), (long long)"1");
    ok = expect_bool((long long)"ingredient-forced (NOT closed -- form guards pending): family follows the role, not the recipe", family_follows_role_not_recipe());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    atlas store-to-family readings across recipes = a ROLE map, not a family choice");
    printf("%s\n", (char*)(long long)"  note  registered verification (queued, not run): the role census -- does each store hold");
    printf("%s\n", (char*)(long long)"  note  geometry, gate a selection, or both -- read alongside its value-signature family (S316)");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long distinct_storing_role_generators() {
    ...
    if (selection_family_period() == forced_to_be((long long)"geometry store family = spatial dimension's generator = colour", spatial_dimension(), colour_count())) {
    ret_val = 1;
    goto L_cleanup;
    } else {
    ret_val = 2;
...
long long role_family_bijection() {
    ...
    ret_val = forced_to_be((long long)"distinct storing-role generators = family count", distinct_storing_role_generators(), harmonic_family_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long family_invariant_per_role() {
    ...
    if (forced_to_be((long long)"distinct storing-role generators = family count", distinct_storing_role_generators(), harmonic_family_count()) == harmonic_family_count()) {
    if (assignment_is_forced()) {
    ret_val = family_read_from_value();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
...
long long role_is_engineering() {
    ...
    ret_val = forbid_target_input((long long)"which role a store plays (architecture)", 0LL);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long family_follows_role_not_recipe() {
    ...
    if (forbid_target_input((long long)"which role a store plays (architecture)", 0LL) == 1) {
    ret_val = 1LL;
    goto L_cleanup;
    } else {
    ret_val = 0LL;
...
long long selection_lock_is_binary() {
    ...
    ret_val = fraction_compare(focus_lock(), fraction_from_ratio(1, binary_count())) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long selection_family_period() {
    ...
    if (fraction_compare(focus_lock(), fraction_from_ratio(1, binary_count())) == 0) {
    ret_val = smallest_fold_period_above(1);
    goto L_cleanup;
    } else {
    ret_val = halt_violation((long long)"the selection lock is not the binary generator's");
    goto L_cleanup;
    }
L_cleanup:
    return ret_val;
...
long long geometry_family_period() {
    ...
    ret_val = forced_to_be((long long)"geometry store family = spatial dimension's generator = colour", spatial_dimension(), colour_count());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long harmonic_family_count() {
    ...
    long long by_window = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_generators);
    ep_gc_push_root(&by_window);

    ep_gc_maybe_collect();

    by_generators = 0;
    if (smallest_fold_period_above(1) == smallest_fold_period_above(binary_count())) {
    by_generators = 1;
    } else {
    by_generators = 2;
    }
    by_window = (1 + further_generators_in_window());
    ret_val = forced_to_be((long long)"harmonic family count = generator count", by_generators, by_window);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long colour_cos_squared() {
    ...
    ret_val = fraction_from_ratio(1, whole_power(binary_count(), 2));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long third_turn_identity_holds() {
    ...
    ep_gc_push_root(&four);
    ep_gc_push_root(&product);

    ep_gc_maybe_collect();

    four = fraction_from_whole_number(whole_power(binary_count(), 2));
    product = fraction_multiply(four, colour_cos_squared());
    ret_val = fraction_compare(product, fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long colour_family_meets_electroweak() {
    ...
    total = fraction_add(colour_cos_squared(), cos_squared_mixing());
    ret_val = fraction_compare(total, fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long single_focus_is_complete() {
    ...
    ep_gc_push_root(&complement);
    ep_gc_push_root(&lock);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    lock = fraction_from_ratio(1, binary_count());
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, lock);
    ret_val = fraction_compare(complement, lock) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(complement);
    complement = 0;
    return ret_val;
}
...
long long focus_folds_to_unison() {
    ...
    ret_val = fold_fraction(focus_lock());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long split_focus() {
    ...
    ret_val = fraction_divide(lock, two);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long split_focus_incomplete() {
    ...
    split = split_focus();
    one = fraction_from_whole_number(1);
    complement = fraction_subtract(one, split);
    same = fraction_compare(complement, split) == 0;
    if (same) {
    ret_val = 0;
    goto L_cleanup;
    }
...
long long colour_low_is_closed_value() {
    ...
    ret_val = fraction_compare(colour_signature_low(), sin_squared_mixing()) == 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long colour_high_two_routes() {
    long long by_complement = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_complement);

    ep_gc_maybe_collect();

    by_complement = fraction_subtract(fraction_from_whole_number(1), colour_signature_low());
    ret_val = fraction_compare(colour_signature_high(), by_complement) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    free_struct_Fraction(by_complement);
    by_complement = 0;
    return ret_val;
}
...
long long colour_signature_partitions_one() {
    ...
    total = fraction_add(colour_signature_low(), colour_signature_high());
    ret_val = fraction_compare(total, fraction_from_whole_number(1)) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long is_proper_fraction(long long value) {
    long long one = 0;
    long long zero = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    if (fraction_compare(value, zero) > 0) {
    if (fraction_compare(value, one) < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
    }
...
long long colour_values_not_whole() {
    ...
    ret_val = is_proper_fraction(colour_signature_high());
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
...
long long family_read_from_value() {
    ...
    if (fraction_compare(colour_signature_low(), sin_squared_mixing()) == 0) {
    if (colour_high_two_routes()) {
    if (colour_signature_partitions_one()) {
    ret_val = colour_values_not_whole();
    goto L_cleanup;
    } else {
    ret_val = 0LL;
    goto L_cleanup;
