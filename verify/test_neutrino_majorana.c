...
long long _main();
long long left_hand_m();
long long right_hand_m();
long long dirac_coupling();
long long single_hand_count();
long long dirac_impossible();
long long lock_is_own_antipode();
long long generic_hand_excluded();
long long neutrino_is_majorana();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the neutrino is Majorana (0vbb must occur) ===");
    ok = expect_equal((long long)"the Dirac coupling of the two hands = 1/2", fraction_to_text(dirac_coupling()), (long long)"1/2");
    ok = expect_bool((long long)"the neutrino's single hand makes a Dirac mass IMPOSSIBLE", dirac_impossible());
    ok = expect_bool((long long)"the lock is its own antipode (the self-coupling exists)", lock_is_own_antipode());
    ok = expect_bool((long long)"EXCLUSION: a generic hand is not self-paired (only the lock)", generic_hand_excluded());
    ok = expect_bool((long long)"THE VERDICT: the neutrino is MAJORANA -- 0vbb MUST occur", neutrino_is_majorana());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long lock_is_own_antipode() {
    ...
    antipode = fraction_subtract(one, lock);
    ret_val = fraction_compare(antipode, lock) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
...
long long generic_hand_excluded() {
    ...
    hand = fraction_from_ratio(1, 4);
    one = fraction_from_whole_number(1);
    antipode = fraction_subtract(one, hand);
    self_paired = fraction_compare(antipode, hand) == 0;
    if (self_paired) {
    ret_val = 0LL;
    goto L_cleanup;
    }
...
long long neutrino_is_majorana() {
    long long no_dirac = 0;
    long long self_coupling_exists = 0;
    long long unique = 0;
    long long ret_val = 0;

    no_dirac = dirac_impossible();
    self_coupling_exists = lock_is_own_antipode();
    unique = generic_hand_excluded();
    if (no_dirac) {
