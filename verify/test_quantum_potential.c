...
long long _main();
long long phase_rotate(long long, long long);
long long start_phase();
long long kinetic_step();
long long potential_step();
long long step_k_then_v();
long long step_by_sum();
long long phases_add();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the quantum phase (phases add) ===");
    ok = expect_equal((long long)"start phase = 1/3", fraction_to_text(start_phase()), (long long)"1/3");
    ok = expect_equal((long long)"kinetic step K = 1/8", fraction_to_text(kinetic_step()), (long long)"1/8");
    ok = expect_equal((long long)"potential step V = 1/4", fraction_to_text(potential_step()), (long long)"1/4");
    ok = expect_equal((long long)"step K then V = 17/24", fraction_to_text(step_k_then_v()), (long long)"17/24");
    ok = expect_equal((long long)"step by the sum K+V = 17/24", fraction_to_text(step_by_sum()), (long long)"17/24");
    ok = expect_bool((long long)"phases ADD: the two agree", phases_add());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long phase_rotate(long long phase, long long amount) {
...
long long kinetic_step() {
    ...
    ret_val = fraction_from_ratio(1, ((binary_count() * binary_count()) * binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long step_k_then_v() {
    ...
    ep_gc_push_root(&after_k);

    ep_gc_maybe_collect();

    after_k = phase_rotate(start_phase(), kinetic_step());
    ret_val = phase_rotate(after_k, potential_step());
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long step_by_sum() {
    ...
    total = fraction_add(kinetic_step(), potential_step());
    ret_val = phase_rotate(start_phase(), total);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long phases_add() {
    ...
    long long separate = 0;
    long long ret_val = 0;

    ep_gc_push_root(&combined);
    ep_gc_push_root(&separate);

    ep_gc_maybe_collect();

    separate = step_k_then_v();
    combined = step_by_sum();
    ret_val = fraction_compare(separate, combined) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(combined);
    combined = 0;
    free_struct_Fraction(separate);
    separate = 0;
    return ret_val;
}
