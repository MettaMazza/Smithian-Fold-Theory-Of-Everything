...
long long _main();
long long inventory_tower_rungs();
long long floor_is_walked();
long long element_cap();
long long species_are_finite();
long long inventory_is_finite();
...
long long critical_charge();
long long binding_coupling(long long);
long long element_is_bound(long long);
long long last_element();
long long threshold_is_sharp();
long long table_ends_at_alpha();
...
long long census_prime(long long);
long long census_carriers(long long);
long long total_gauge_carriers();
long long undiscovered_carriers();
long long smithion_count();
long long known_anchors_hold();
long long all_sectors_qualify();
long long census_is_closed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the finite inventory: everything bounded, nothing infinite ===");
    ok = expect_equal((long long)"[1] the tower: 2^7 rungs from the One to the floor", int_to_string(inventory_tower_rungs()), (long long)"128");
    ok = expect_bool((long long)"the floor 1/128 walked down in seven halvings, still positive", floor_is_walked());
    ok = expect_equal((long long)"[2] the element cap: the table's last element (imported threshold)", int_to_string(element_cap()), (long long)"137");
    ok = expect_bool((long long)"[3] the species are finite: 83 carriers, 12 Smithions, ladder sealed", species_are_finite());
    ok = expect_bool((long long)"THE INVENTORY IS FINITE: every factor bounded, the total a number", inventory_is_finite());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long floor_is_walked() {
    ...
    ep_gc_push_root(&survivor);

    ep_gc_maybe_collect();

    rung = fraction_from_whole_number(1);
    half = fraction_from_ratio(1, 2);
    rung = fraction_multiply(rung, half);
    half = fraction_from_ratio(1, 2);
    rung = fraction_multiply(rung, half);
    half = fraction_from_ratio(1, 2);
    rung = fraction_multiply(rung, half);
    ...
    rung = fraction_multiply(rung, half);
    step = 7;
    expected = fraction_from_ratio(1, 128);
    lands = fraction_compare(rung, expected) == 0;
    survivor = fraction_from_ratio(1, 128);
    alive = ({ long long _fap = survivor; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 1;
    if (lands) {
    ret_val = alive;
    goto L_cleanup;
    }
...
long long species_are_finite() {
    long long carriers_ok = 0;
    long long sealed = 0;
    long long smithions_ok = 0;
    long long ret_val = 0;

    carriers_ok = total_gauge_carriers() == 83;
    smithions_ok = smithion_count() == 12;
    sealed = census_is_closed();
    if (carriers_ok) {
    if (smithions_ok) {
    ret_val = sealed;
    goto L_cleanup;
...
long long inventory_is_finite() {
    long long elements_end = 0;
    long long ladders_end = 0;
    long long species_end = 0;
    long long ret_val = 0;

    ladders_end = floor_is_walked();
    elements_end = last_element() == 137;
    species_end = species_are_finite();
    if (ladders_end) {
    if (elements_end) {
    ret_val = species_end;
    goto L_cleanup;
...
long long critical_charge() {
    ...
    ret_val = inverse_fine_structure_first_order();
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long binding_coupling(long long z) {
    long long inv_alpha = 0;
    long long z_fraction = 0;
    long long ret_val = 0;

    ep_gc_push_root(&inv_alpha);
    ep_gc_push_root(&z_fraction);
    ep_gc_push_root(&z);

    ep_gc_maybe_collect();

    inv_alpha = inverse_fine_structure_first_order();
    z_fraction = fraction_from_whole_number(z);
    ret_val = fraction_divide(z_fraction, inv_alpha);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long element_is_bound(long long z) {
    ...
    coupling = binding_coupling(z);
    one = fraction_from_whole_number(1);
    comparison = fraction_compare(coupling, one);
...
long long threshold_is_sharp() {
    ...
    below = element_is_bound(137);
    above = element_is_bound(138);
    if (below) {
    if (above) {
    ret_val = 0LL;
    goto L_cleanup;
...
long long census_prime(long long index) {
    long long ret_val = 0;
...
long long census_carriers(long long p) {
    long long ret_val = 0;
    ...
    ret_val = sector_mediator_count(p);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long total_gauge_carriers() {
    ...
    while (i < 5) {
    p = census_prime(i);
    carriers = sector_mediator_count(p);
...
long long undiscovered_carriers() {
    ...
    penta = sector_mediator_count(5);
    hepta = sector_mediator_count(7);
    ret_val = (penta + hepta);
    goto L_cleanup;
L_cleanup:
    return ret_val;
...
long long smithion_count() {
    ...
    long long kinds = 0;
    long long new_sectors = 0;
    long long ret_val = 0;

    kinds = 2;
    generations = smallest_fold_period_above(binary_count());
    new_sectors = 2;
    ret_val = ((kinds * generations) * new_sectors);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long known_anchors_hold() {
    long long strong_ok = 0;
    long long weak_ok = 0;
    long long ret_val = 0;

    weak_ok = sector_mediator_count(2) == 3;
    strong_ok = sector_mediator_count(3) == 8;
    if (weak_ok) {
    ret_val = strong_ok;
    goto L_cleanup;
    }
...
long long all_sectors_qualify() {
    ...
    while (i < 5) {
    p = census_prime(i);
    partition_ok = sector_partition_holds(p);
    if (partition_ok) {
    all_ok = all_ok;
    } else {
    all_ok = 0LL;
    }
    carry_ok = sector_carry_closes(p);
    if (carry_ok) {
    all_ok = all_ok;
    } else {
    all_ok = 0LL;
...
long long census_is_closed() {
    ...
    count_ok = prime_sector_count() == 4;
    ceiling = deepest_covering_depth();
    beyond = first_prime_beyond_ceiling();
    sealed = beyond > ceiling;
    if (count_ok) {
    ret_val = sealed;
    goto L_cleanup;
    }
