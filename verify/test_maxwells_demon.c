...
long long memory_slow();
long long memory_fast();
long long slow_resets_to();
long long fast_resets_to();
long long reset_erases_the_bit();
long long books_balance_at_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Maxwell's demon (the ledger pays the bit back) ===");
    ok = expect_equal((long long)"memory states: slow = 1/4", fraction_to_text(memory_slow()), (long long)"1/4");
    ok = expect_equal((long long)"memory states: fast = 3/4", fraction_to_text(memory_fast()), (long long)"3/4");
    ok = expect_equal((long long)"reset sends slow to the ready state 1/2", fraction_to_text(slow_resets_to()), (long long)"1/2");
    ok = expect_equal((long long)"reset sends fast to the SAME ready state", fraction_to_text(fast_resets_to()), (long long)"1/2");
    ok = expect_bool((long long)"the reset erases exactly the held bit (irreversible)", reset_erases_the_bit());
    ok = expect_equal((long long)"demon + gas books balance at the One", fraction_to_text(books_balance_at_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long slow_resets_to() {
    ...
    ret_val = fold_fraction(memory_slow());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long fast_resets_to() {
    ...
    ret_val = fold_fraction(memory_fast());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long reset_erases_the_bit() {
    long long fast_reset = 0;
    long long slow_reset = 0;
    long long ret_val = 0;

    ep_gc_push_root(&fast_reset);
    ep_gc_push_root(&slow_reset);

    ep_gc_maybe_collect();

    slow_reset = fold_fraction(memory_slow());
    fast_reset = fold_fraction(memory_fast());
    ret_val = fraction_compare(slow_reset, fast_reset) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(fast_reset);
    fast_reset = 0;
    free_struct_Fraction(slow_reset);
    slow_reset = 0;
    return ret_val;
}
...
long long books_balance_at_one() {
    ...
    ret_val = fold_fraction(slow_resets_to());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
