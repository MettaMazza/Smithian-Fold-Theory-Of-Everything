...
long long _main();
long long mode_dimensions();
long long mode_volume();
long long mode_spacing();
long long modes_partition_one();
long long volume_seeds_down_depth();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== what string theory got right (mode spacing 1/27) ===");
    ok = expect_equal((long long)"the mode dimensions = the counted 3 (period of 1/7)", int_to_string(mode_dimensions()), (long long)"3");
    ok = expect_equal((long long)"the mode volume = 3^3 = 27", int_to_string(mode_volume()), (long long)"27");
    ok = expect_equal((long long)"the mode spacing = 1/27", fraction_to_text(mode_spacing()), (long long)"1/27");
    ok = expect_equal((long long)"the 27 modes partition the One (summed explicitly)", fraction_to_text(modes_partition_one()), (long long)"1");
    ok = expect_bool((long long)"the same 27 seeds the covering depth 5 (the cross-link)", volume_seeds_down_depth());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long mode_volume() {
    ...
    ret_val = whole_power(colour_count(), d);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long mode_spacing() {
    ...
    ret_val = fraction_from_ratio(1, mode_volume());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long modes_partition_one() {
    ...
    spacing = fraction_from_ratio(1, mode_volume());
    count = mode_volume();
