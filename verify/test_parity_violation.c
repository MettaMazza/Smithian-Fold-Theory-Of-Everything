...
long long _main();
long long vacuum_image();
long long lower_preimage();
long long upper_preimage();
long long fold_fraction(long long);
long long lower_preimage_folds_to_image();
long long upper_preimage_folds_to_image();
long long lower_is_left_handed();
long long upper_is_right_handed();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== parity violation ===");
    ok = expect_equal((long long)"lower preimage (left hand) = 1/4", fraction_to_text(lower_preimage()), (long long)"1/4");
    ok = expect_equal((long long)"upper preimage (right hand) = 3/4", fraction_to_text(upper_preimage()), (long long)"3/4");
    ok = expect_equal((long long)"fold(1/4) = 1/2 (folds to the image)", fraction_to_text(lower_preimage_folds_to_image()), (long long)"1/2");
    ok = expect_equal((long long)"fold(3/4) = 1/2 (folds to the image)", fraction_to_text(upper_preimage_folds_to_image()), (long long)"1/2");
    ok = expect_true((long long)"lower preimage is left-handed (below 1/2)", lower_is_left_handed());
    ok = expect_true((long long)"upper preimage is right-handed (above 1/2)", upper_is_right_handed());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long lower_preimage() {
    ...
    image = fraction_from_ratio(1, binary_count());
    binary = fraction_from_whole_number(binary_count());
    ret_val = fraction_divide(image, binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long upper_preimage() {
    ...
    shifted = fraction_add(image, one);
    binary = fraction_from_whole_number(binary_count());
    ret_val = fraction_divide(shifted, binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long lower_is_left_handed() {
    ...
    comparison = fraction_compare(lower, half);
    if (comparison < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long upper_is_right_handed() {
    ...
    upper = upper_preimage();
    half = fraction_from_ratio(1, binary_count());
    comparison = fraction_compare(upper, half);
    if (comparison > 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
