...
long long rung_reaches_unison_in(long long);
long long ladder_sum_to_down_depth();
long long ladder_plus_boundary();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the continuum ladder (discrete rungs do everything) ===");
    ok = expect_equal((long long)"rung 5 = 1/32", fraction_to_text(ladder_rung(5)), (long long)"1/32");
    ok = expect_bool((long long)"rung 1 reaches unison in 1 fold", rung_reaches_unison_in(1));
    ok = expect_bool((long long)"rung 3 reaches unison in 3 folds", rung_reaches_unison_in(3));
    ok = expect_bool((long long)"rung 5 reaches unison in 5 folds", rung_reaches_unison_in(5));
    ok = expect_equal((long long)"ladder to down-depth 5 sums to 31/32", fraction_to_text(ladder_sum_to_down_depth()), (long long)"31/32");
    ok = expect_equal((long long)"plus the boundary rung = the One, exactly", fraction_to_text(ladder_plus_boundary()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long ladder_rung(long long k) {
    ...
    denominator = whole_power(binary_count(), k);
...
long long rung_reaches_unison_in(long long k) {
    long long current = 0;
    long long one = 0;
    long long step = 0;
    long long ret_val = 0;
...
long long ladder_plus_boundary() {
    ...
    partial = ladder_sum_to_down_depth();
    depth = (smallest_fold_period_above(1) + smallest_fold_period_above(binary_count()));
    boundary = ladder_rung(depth);
    ret_val = fraction_add(partial, boundary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
