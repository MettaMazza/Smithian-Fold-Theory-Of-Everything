...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold rewriting law ===");
    ok = expect_true((long long)"one rewrite equals one Fold observation", fold_rewrite_matches_observation(85, 7));
    word = encode_computation_state(85, 7);
    suffix = fold_rewrite_once(word);
    ok = expect_equal((long long)"one rewrite closes exactly one symbol", int_to_string(length_list(suffix)), int_to_string(6));
    ok = expect_equal((long long)"every suffix has exactly b predecessors", int_to_string(fold_rewrite_preimage_count(suffix)), int_to_string(2));
    ok = expect_true((long long)"all depth-7 words reach the empty normal form", every_fold_word_reaches_normal_form(7));
    ok = expect_true((long long)"rewrite composition holds at every split", fold_rewrite_composition_holds(85, 7));
    ok = expect_true((long long)"a shifted suffix is rejected", shifted_rewrite_suffix_is_rejected(7));
    ok = expect_true((long long)"an early normal form is rejected", early_rewrite_normal_form_is_rejected(7));
    ok = expect_true((long long)"the complete Fold rewriting census holds", complete_fold_rewriting_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
