...
long long _main();
long long sample_radius_in_horizon_radii();
long long well_depth();
long long time_dilation_factor();
long long time_runs_slow();
long long horizon_factor();
long long horizon_reaches_forbidden_zero();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== gravitational time dilation ===");
    ok = expect_equal((long long)"well depth x = r_s/r = 1/4 (at r = 4 r_s)", fraction_to_text(well_depth()), (long long)"1/4");
    ok = expect_equal((long long)"time-dilation factor A = 1 - x = 3/4", fraction_to_text(time_dilation_factor()), (long long)"3/4");
    ok = expect_bool((long long)"clocks run SLOW (factor below the One)", time_runs_slow());
    ok = expect_bool((long long)"the horizon reaches the FORBIDDEN zero (time stops)", horizon_reaches_forbidden_zero());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long well_depth() {
    ...
    ret_val = fraction_from_ratio(1, sample_radius_in_horizon_radii());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long time_dilation_factor() {
    ...
    depth = fraction_from_ratio(1, sample_radius_in_horizon_radii());
    ret_val = fraction_subtract(one, depth);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long time_runs_slow() {
    ...
    one = fraction_from_whole_number(1);
    order = fraction_compare(factor, one);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long horizon_factor() {
    ...
    ep_gc_push_root(&one);
    ep_gc_push_root(&full);

    ep_gc_maybe_collect();

    one = fraction_from_whole_number(1);
    full = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, full);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long horizon_reaches_forbidden_zero() {
    ...
    zero = fraction_subtract(one_a, one_b);
    ret_val = fraction_compare(factor, zero) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(factor);
    factor = 0;
