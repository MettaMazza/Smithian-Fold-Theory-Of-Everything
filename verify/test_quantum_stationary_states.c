...
long long level_spacing(long long);
long long ground_state(long long);
long long ground_folds_to_spacing(long long);
long long two_grounds_make_one_step(long long);
long long energy_level(long long, long long);
long long gap_is_one_step(long long, long long);
long long phase_rotate_step(long long, long long);
long long phase_returns_after_full_cycle(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== stationary states (the fixed spectrum) ===");
    ok = expect_equal((long long)"the grid step at depth 3 = 1/8", fraction_to_text(level_spacing(3)), (long long)"1/8");
    ok = expect_equal((long long)"the ground state = half a step = 1/16", fraction_to_text(ground_state(3)), (long long)"1/16");
    ok = expect_bool((long long)"the ground folds to the spacing: fold(E0) = 1/8", ground_folds_to_spacing(3));
    ok = expect_bool((long long)"two ground shares make one step", two_grounds_make_one_step(3));
    ok = expect_equal((long long)"the n=2 level = 1/16 + 2/8 = 5/16", fraction_to_text(energy_level(2, 3)), (long long)"5/16");
    ok = expect_bool((long long)"every gap is exactly one grid step (uniform spectrum)", gap_is_one_step(2, 3));
    ok = expect_bool((long long)"the phase returns exactly after a full cycle (magnitude held)", phase_returns_after_full_cycle(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long level_spacing(long long k) {
    long long volume = 0;
    long long ret_val = 0;
...
long long ground_state(long long k) {
    long long volume = 0;
    long long ret_val = 0;
    ...
    volume = whole_power(binary_count(), (k + 1));
    ret_val = fraction_from_ratio(1, volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long ground_folds_to_spacing(long long k) {
    long long folded = 0;
    long long ground = 0;
    long long spacing = 0;
    long long ret_val = 0;
    ...
    ground = ground_state(k);
    folded = fold_fraction(ground);
    spacing = level_spacing(k);
    ret_val = fraction_compare(folded, spacing) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long two_grounds_make_one_step(long long k) {
    long long ground = 0;
    long long ground_again = 0;
    long long spacing = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&ground);
    ep_gc_push_root(&ground_again);
    ep_gc_push_root(&spacing);
    ...
    ground = ground_state(k);
    ground_again = ground_state(k);
    total = fraction_add(ground, ground_again);
    spacing = level_spacing(k);
    ret_val = fraction_compare(total, spacing) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(spacing);
...
long long energy_level(long long n, long long k) {
    long long climb = 0;
    long long ground = 0;
    long long n_fraction = 0;
    long long spacing = 0;
    long long ret_val = 0;

    ep_gc_push_root(&climb);
    ep_gc_push_root(&ground);
    ep_gc_push_root(&n_fraction);
    ep_gc_push_root(&spacing);
    ep_gc_push_root(&n);
    ep_gc_push_root(&k);
    ...
    ground = ground_state(k);
    spacing = level_spacing(k);
    n_fraction = fraction_from_whole_number(n);
    climb = fraction_multiply(n_fraction, spacing);
...
long long gap_is_one_step(long long n, long long k) {
    long long above = 0;
    long long gap = 0;
    long long here = 0;
    long long spacing = 0;
    ...
    here = energy_level(n, k);
    above = energy_level((n + 1), k);
    gap = fraction_subtract(above, here);
    spacing = level_spacing(k);
    ret_val = fraction_compare(gap, spacing) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
...
long long phase_rotate_step(long long phase, long long amount) {
    long long advanced = 0;
    long long ret_val = 0;

    ep_gc_push_root(&advanced);
    ep_gc_push_root(&phase);
    ep_gc_push_root(&amount);

    ep_gc_maybe_collect();

    advanced = fraction_add(phase, amount);
    ret_val = cast_out_whole_ones(advanced);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long phase_returns_after_full_cycle(long long k) {
    long long one = 0;
    long long phase = 0;
    long long spacing = 0;
    long long tick = 0;
    ...
    spacing = level_spacing(k);
    phase = fraction_from_whole_number(1);
    volume = whole_power(binary_count(), k);
    tick = 0;
    while (tick < volume) {
    phase = phase_rotate_step(phase, spacing);
    tick = (tick + 1);
    }
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(phase, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
