...
long long _main();
long long charged_channel(long long);
long long neutral_channel(long long);
long long preserved_combination(long long);
long long preserved_is_massless(long long);
long long channel_mass(long long);
long long broken_channels_are_massive();
long long reach_of_mass(long long, long long);
long long massive_reaches_are_finite();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the massless/massive split (photon vs W) ===");
    ok = expect_equal((long long)"EW preserved combination: 1/2 + 1/2 = the One", fraction_to_text(preserved_combination(2)), (long long)"1");
    ok = expect_equal((long long)"strong preserved combination: 2/3 + 1/3 = the One", fraction_to_text(preserved_combination(3)), (long long)"1");
    ok = expect_bool((long long)"the EW combination is MASSLESS (no shortfall exists)", preserved_is_massless(2));
    ok = expect_bool((long long)"the strong combination is MASSLESS (the gluon)", preserved_is_massless(3));
    ok = expect_bool((long long)"every broken channel is MASSIVE (positive shortfall)", broken_channels_are_massive());
    ok = expect_equal((long long)"reach of mass 1/2: 1 step (run, not asserted)", int_to_string(reach_of_mass(1, 2)), (long long)"1");
    ok = expect_equal((long long)"reach of mass 1/3: 2 steps", int_to_string(reach_of_mass(1, 3)), (long long)"2");
    ok = expect_bool((long long)"all massive reaches finite and tiny (why the weak is short-range)", massive_reaches_are_finite());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long charged_channel(long long m) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(2, 3);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long neutral_channel(long long m) {
    long long ret_val = 0;
    ...
    ret_val = fraction_from_ratio(1, 3);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}

long long preserved_combination(long long m) {
    long long c = 0;
    long long n = 0;
    long long ret_val = 0;
...
long long preserved_is_massless(long long m) {
    long long combo = 0;
    long long one = 0;
    long long shortfall = 0;
    long long ret_val = 0;

    ep_gc_push_root(&combo);
    ep_gc_push_root(&one);
    ...
    shortfall = fraction_subtract(one, combo);
    ret_val = ({ long long _fap = shortfall; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long channel_mass(long long part) {
    long long one = 0;
    long long ret_val = 0;
    ...
    one = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, part);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long broken_channels_are_massive() {
    long long ew_charged = 0;
    long long first_ok = 0;
    long long second_ok = 0;
    long long strong_charged = 0;
    long long strong_neutral = 0;
    long long third_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&ew_charged);
    ep_gc_push_root(&strong_charged);
    ep_gc_push_root(&strong_neutral);

    ep_gc_maybe_collect();

    ew_charged = channel_mass(charged_channel(2));
    strong_charged = channel_mass(charged_channel(3));
    strong_neutral = channel_mass(neutral_channel(3));
    first_ok = ({ long long _fap = ew_charged; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    second_ok = ({ long long _fap = strong_charged; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    third_ok = ({ long long _fap = strong_neutral; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) > 0;
    if (first_ok) {
    if (second_ok) {
    ret_val = third_ok;
    goto L_cleanup;
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(ew_charged);
    ew_charged = 0;
    free_struct_Fraction(strong_charged);
    strong_charged = 0;
    free_struct_Fraction(strong_neutral);
    strong_neutral = 0;
    return ret_val;
}

long long reach_of_mass(long long m_num, long long m_den) {
    long long comparison = 0;
    long long intensity = 0;
    long long mass = 0;
    long long mass_again = 0;
    long long reach = 0;
    long long stepping = 0;
    long long ret_val = 0;

    ep_gc_push_root(&intensity);
    ep_gc_push_root(&mass);
    ep_gc_push_root(&mass_again);
    ep_gc_push_root(&m_num);
    ep_gc_push_root(&m_den);

    ep_gc_maybe_collect();

    mass = fraction_from_ratio(m_num, m_den);
    intensity = fraction_from_whole_number(1);
    reach = 0;
    stepping = 1LL;
    while (stepping) {
    comparison = fraction_compare(intensity, mass);
    if (comparison > 0) {
    mass_again = fraction_from_ratio(m_num, m_den);
    intensity = fraction_subtract(intensity, mass_again);
    reach = (reach + 1);
    } else {
    stepping = 0LL;
    }
...
long long massive_reaches_are_finite() {
    long long first_ok = 0;
    long long half_reach = 0;
    long long second_ok = 0;
    long long third_ok = 0;
    long long third_reach = 0;
    long long two_thirds_reach = 0;
    long long ret_val = 0;

    half_reach = reach_of_mass(1, 2);
    third_reach = reach_of_mass(1, 3);
    two_thirds_reach = reach_of_mass(2, 3);
    first_ok = half_reach == 1;
    second_ok = third_reach == 2;
    third_ok = two_thirds_reach == 1;
    if (first_ok) {
    if (second_ok) {
    ret_val = third_ok;
    goto L_cleanup;
