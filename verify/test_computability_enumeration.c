...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold enumeration law ===");
    ok = expect_true((long long)"rank 85 round-trips through its depth-7 word", fold_enumeration_round_trip(85, 7));
    ok = expect_true((long long)"all depth-7 ranks enumerate uniquely", complete_fold_enumeration_at_depth(7));
    ok = expect_equal((long long)"depth-7 enumeration contains b^7 words", int_to_string(fold_enumeration_count(7)), int_to_string(128));
    ok = expect_equal((long long)"enumeration through depth seven contains 255 configurations", int_to_string(fold_enumeration_through_depth(7)), int_to_string(255));
    ok = expect_true((long long)"outside ranks are rejected", outside_fold_rank_is_rejected(7));
    ok = expect_true((long long)"the complete enumeration census holds", complete_fold_enumeration_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
