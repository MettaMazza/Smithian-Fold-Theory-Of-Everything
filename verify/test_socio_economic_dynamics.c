...
long long bust_phase();
long long boom_phase();
long long bust_folds_to_boom();
long long boom_folds_to_bust();
long long equilibrium_balance();
long long phases_partition_the_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== socio-economic cycles (boom and bust) ===");
    ok = expect_equal((long long)"bust phase = 1/3, boom = 2/3", fraction_to_text(bust_phase()), (long long)"1/3");
    ok = expect_equal((long long)"boom phase = 2/3", fraction_to_text(boom_phase()), (long long)"2/3");
    ok = expect_bool((long long)"bust folds to boom", bust_folds_to_boom());
    ok = expect_bool((long long)"boom folds back to bust (closed cycle, never rests)", boom_folds_to_bust());
    ok = expect_equal((long long)"the notional equilibrium is the crossing balance 1/2", fraction_to_text(equilibrium_balance()), (long long)"1/2");
    ok = expect_equal((long long)"boom and bust partition the whole economy", fraction_to_text(phases_partition_the_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long bust_folds_to_boom() {
    ...
    ep_gc_push_root(&boom);
    ep_gc_push_root(&bust);
    ep_gc_push_root(&folded);

    ep_gc_maybe_collect();

    bust = fraction_from_ratio(1, colour_count());
    folded = fold_fraction(bust);
    boom = fraction_from_ratio(binary_count(), colour_count());
    ret_val = fraction_compare(folded, boom) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(boom);
    boom = 0;
...
long long boom_folds_to_bust() {
    ...
    boom = fraction_from_ratio(binary_count(), colour_count());
    folded = fold_fraction(boom);
    bust = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(folded, bust) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(bust);
    bust = 0;
    free_struct_Fraction(folded);
    folded = 0;
    return ret_val;
...
long long equilibrium_balance() {
    ...
    whole = fraction_add(bust_phase(), boom_phase());
...
long long phases_partition_the_whole() {
    ...
    ret_val = fraction_add(bust_phase(), boom_phase());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
