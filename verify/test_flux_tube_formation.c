...
long long carrier_self_charge();
long long accumulated_source(long long);
long long strong_confines();
long long photon_accumulated_source(long long);
long long electromagnetism_does_not_confine();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the flux tube (confinement) ===");
    ok = expect_equal((long long)"bare source charge = 1/2", fraction_to_text(source_charge()), (long long)"1/2");
    ok = expect_equal((long long)"gluon self-charge = the One (carries colour)", fraction_to_text(carrier_self_charge()), (long long)"1");
    ok = expect_equal((long long)"accumulated source at length 1 = 3/2", fraction_to_text(accumulated_source(1)), (long long)"3/2");
    ok = expect_equal((long long)"accumulated source at length 2 = 5/2 (grows)", fraction_to_text(accumulated_source(2)), (long long)"5/2");
    ok = expect_bool((long long)"the strong force CONFINES (charge grows with length)", strong_confines());
    ok = expect_bool((long long)"electromagnetism does NOT confine (charge constant)", electromagnetism_does_not_confine());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long accumulated_source(long long length) {
    long long fed = 0;
    long long base = 0;
    long long ret_val = 0;
    ...
    base = fraction_from_ratio(1, binary_count());
    fed = fraction_from_whole_number(length);
    ret_val = fraction_add(base, fed);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long strong_confines() {
    ...
    near = accumulated_source(1);
    far = accumulated_source(2);
    order = fraction_compare(far, near);
    if (order > 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
...
long long photon_accumulated_source(long long length) {
    long long ret_val = 0;
...
long long electromagnetism_does_not_confine() {
    ...
    near = fraction_from_ratio(1, binary_count());
    far = fraction_from_ratio(1, binary_count());
    ret_val = fraction_compare(near, far) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(near);
    near = 0;
    free_struct_Fraction(far);
    far = 0;
    return ret_val;
}
