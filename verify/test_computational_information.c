...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold information law ===");
    ok = expect_equal((long long)"depth-7 information quantity", int_to_string(information_quantity(7)), int_to_string(7));
    ok = expect_equal((long long)"seven binary distinctions carry 128 states", int_to_string(information_multiplicity(7)), int_to_string(128));
    ok = expect_equal((long long)"three observations retain four distinctions", int_to_string(retained_information(7, 3)), int_to_string(4));
    ok = expect_equal((long long)"three observations close three distinctions", int_to_string(closed_information(3)), int_to_string(3));
    ok = expect_equal((long long)"retained plus closed reconstructs the initial quantity", int_to_string(information_accounting(7, 3)), int_to_string(7));
    ok = expect_true((long long)"all depth-7 branch shares partition the One", information_branches_partition_one(7));
    ok = expect_true((long long)"the general branch share meets the established measurement weight", measurement_branch_bridge_holds());
    ok = expect_true((long long)"uncertainty support equals information multiplicity", uncertainty_matches_information(7));
    ok = expect_true((long long)"dyadic distinctions close under observation", dyadic_distinction_closes(7));
    ok = expect_true((long long)"periodic distinctions return and are retained", periodic_distinction_is_retained());
    ok = expect_true((long long)"perturbed accounting is rejected", perturbed_information_accounting_is_rejected(7, 3));
    ok = expect_true((long long)"an incomplete cycle is not called an identity", incomplete_cycle_identity_is_rejected());
    ok = expect_true((long long)"complete information census holds", complete_information_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
