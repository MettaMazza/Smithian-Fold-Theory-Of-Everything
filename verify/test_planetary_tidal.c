...
long long lock_state();
long long lock_is_self_antipodal();
long long rhythms_merge_to_one();
long long shares_sum_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== tidal locking (one face forever) ===");
    ok = expect_equal((long long)"the lock state = 1/2 (equal rhythm shares)", fraction_to_text(lock_state()), (long long)"1/2");
    ok = expect_bool((long long)"the lock is self-antipodal (nothing left to drain)", lock_is_self_antipodal());
    ok = expect_equal((long long)"at the lock the rhythms merge into ONE period", fraction_to_text(rhythms_merge_to_one()), (long long)"1");
    ok = expect_equal((long long)"the equal shares sum to the One", fraction_to_text(shares_sum_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long rhythms_merge_to_one() {
    ...
    ret_val = fold_fraction(lock_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long shares_sum_to_one() {
    ...
    lock = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(lock, lock);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
