...
long long branching_depth();
long long scaling_exponent();
long long is_not_two_thirds();
long long exponent_folds_to_balance();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== network scaling (the 3/4 metabolic law) ===");
    ok = expect_equal((long long)"branching depth m = b^2 = 4", int_to_string(branching_depth()), (long long)"4");
    ok = expect_equal((long long)"scaling exponent (m-1)/m = 3/4 (Kleiber)", fraction_to_text(scaling_exponent()), (long long)"3/4");
    ok = expect_bool((long long)"it is NOT the naive surface-to-volume 2/3", is_not_two_thirds());
    ok = expect_equal((long long)"the exponent folds to the balance (supply=demand)", fraction_to_text(exponent_folds_to_balance()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long scaling_exponent() {
    ...
    m = (smallest_fold_period_above(1) * smallest_fold_period_above(1));
...
long long is_not_two_thirds() {
    ...
    ep_gc_push_root(&naive);

    ep_gc_maybe_collect();

    exponent = scaling_exponent();
    naive = fraction_from_ratio(binary_count(), colour_count());
    same = fraction_compare(exponent, naive) == 0;
    if (same) {
    ret_val = 0;
    goto L_cleanup;
    }
...
long long exponent_folds_to_balance() {
    ...
    ret_val = fold_fraction(scaling_exponent());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
