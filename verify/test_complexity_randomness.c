...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold randomness law ===");
    ok = expect_equal((long long)"three unresolved distinctions have eight histories", int_to_string(unresolved_fold_history_count(3)), int_to_string(8));
    ok = expect_equal((long long)"each history has exact share 1/8", fraction_to_text(unresolved_fold_history_share(3)), (long long)"1/8");
    ok = expect_true((long long)"all unresolved history shares partition the One", unresolved_fold_histories_partition_one(3));
    ok = expect_equal((long long)"randomness quantity equals closed distinction count", int_to_string(fold_randomness_quantity(3)), int_to_string(3));
    ok = expect_true((long long)"held labels resolve the source deterministically", held_labels_resolve_fold_randomness(85, 7, 3));
    ok = expect_true((long long)"the complete randomness census holds", complete_fold_randomness_census());
    ok = expect_true((long long)"a nonbinary history count is rejected", nonbinary_random_history_is_rejected(3));
    ok = expect_true((long long)"a perturbed history share is rejected", perturbed_random_share_is_rejected(3));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
