...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== enforcement (forced values pass) ===");
    a = forced_to_be((long long)"two forced as two", 2, 2);
    ok = expect_equal((long long)"a value equal to its forced derivation passes through", a, 2);
    b = forced_to_be((long long)"five forced as five", 5, 5);
    ok = expect_equal((long long)"another forced value passes through", b, 5);
    u = forced_unique((long long)"uniquely forced", 1, 0);
    ok = expect_equal((long long)"a uniquely forced value passes through", u, 1);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
