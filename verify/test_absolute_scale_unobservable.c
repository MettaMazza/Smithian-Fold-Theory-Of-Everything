...
long long probe_state();
long long rescaled_by_seven();
long long rescaled_by_three();
long long rescaling_is_invisible();
long long fold_images_identical();
long long common_image();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the unobservable absolute scale (only ratios are physical) ===");
    ok = expect_equal((long long)"the probe state = 2/5", fraction_to_text(probe_state()), (long long)"2/5");
    ok = expect_bool((long long)"rescaling by 7 (14/35) and by 3 (6/15) is INVISIBLE", rescaling_is_invisible());
    ok = expect_bool((long long)"the fold agrees: identical images from all three forms", fold_images_identical());
    ok = expect_equal((long long)"the common image = 4/5 (no trace of the scale factor)", fraction_to_text(common_image()), (long long)"4/5");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long probe_state() {
    ...
    ret_val = fraction_from_ratio(2, 5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long rescaled_by_seven() {
    ...
    ret_val = fraction_from_ratio(14, 35);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long rescaled_by_three() {
    ...
    ret_val = fraction_from_ratio(6, 15);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long rescaling_is_invisible() {
    ...
    long long seven_same = 0;
    long long three_same = 0;
    long long ret_val = 0;

    ep_gc_push_root(&by_seven);
    ep_gc_push_root(&by_three);
    ep_gc_push_root(&original);

    ep_gc_maybe_collect();

    original = fraction_from_ratio(2, 5);
    by_seven = fraction_from_ratio(14, 35);
    by_three = fraction_from_ratio(6, 15);
    seven_same = fraction_compare(original, by_seven) == 0;
    three_same = fraction_compare(original, by_three) == 0;
    if (seven_same) {
    ret_val = three_same;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(by_seven);
    by_seven = 0;
    free_struct_Fraction(by_three);
    by_three = 0;
    free_struct_Fraction(original);
    original = 0;
    return ret_val;
}

long long fold_images_identical() {
    long long from_original = 0;
    long long from_seven = 0;
    long long from_three = 0;
    long long seven_same = 0;
    long long three_same = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_original);
    ep_gc_push_root(&from_seven);
    ep_gc_push_root(&from_three);

    ep_gc_maybe_collect();

    from_original = fold_fraction(probe_state());
    from_seven = fold_fraction(rescaled_by_seven());
    from_three = fold_fraction(rescaled_by_three());
    seven_same = fraction_compare(from_original, from_seven) == 0;
    three_same = fraction_compare(from_original, from_three) == 0;
    if (seven_same) {
    ret_val = three_same;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(from_original);
    from_original = 0;
    free_struct_Fraction(from_seven);
    from_seven = 0;
    free_struct_Fraction(from_three);
    from_three = 0;
    return ret_val;
}
...
long long common_image() {
    ...
    ret_val = fold_fraction(probe_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
