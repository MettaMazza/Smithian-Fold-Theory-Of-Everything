...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the gyromagnetic ratio (g-factor) ===");
    ok = expect_equal((long long)"critical coupling g_em = 1/2", fraction_to_text(critical_coupling()), (long long)"1/2");
    ok = expect_equal((long long)"g-factor = 1/(1/2) = 2", fraction_to_text(bare_g_factor()), (long long)"2");
    ok = expect_equal((long long)"g-factor = binary count", int_to_string(bare_g_factor_is_binary()), int_to_string(2));
    printf("%s\n", (char*)(long long)"--- comparison ---");
    ok = expect_equal((long long)"forced g = 2.000 (measured ~2.00232, anomaly is QED)", fraction_to_decimal(bare_g_factor(), 3), (long long)"2.000");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
