...
long long _main();
long long tipping_threshold();
long long two_basins_one_lock();
long long unit_power_law_holds_over(long long);
long long release_is_atomic();
long long ringdown_amplitude(long long);
long long ringdown_decays_over(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== earth and high-energy astro: four coats, one fold ===");
    ok = expect_equal((long long)"TIPPING: the lock threshold is the self-antipodal half-One", fraction_to_text(tipping_threshold()), (long long)"1/2");
    ok = expect_bool((long long)"the two basins 1/4 and 3/4 fold to the SAME lock (bistability)", two_basins_one_lock());
    ok = expect_bool((long long)"QUAKES: the unit power law exact over ten octaves (Gutenberg-Richter b=1)", unit_power_law_holds_over(10));
    ok = expect_bool((long long)"BURSTS: the release is atomic -- half-One to the One in ONE move", release_is_atomic());
    ok = expect_equal((long long)"RINGDOWN: the amplitude at cycle 4", fraction_to_text(ringdown_amplitude(4)), (long long)"1/16");
    ok = expect_bool((long long)"the ringdown halves each cycle and never reaches zero (10 cycles)", ringdown_decays_over(10));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long two_basins_one_lock() {
    long long doubled_high = 0;
    long long doubled_high_raw = 0;
    long long doubled_low = 0;
    long long high_locks = 0;
    long long low_locks = 0;
    long long lower_basin = 0;
    long long threshold = 0;
    long long threshold_twin = 0;
    long long upper_basin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&doubled_high);
    ep_gc_push_root(&doubled_high_raw);
    ep_gc_push_root(&doubled_low);
    ep_gc_push_root(&lower_basin);
    ep_gc_push_root(&threshold);
    ep_gc_push_root(&threshold_twin);
    ep_gc_push_root(&upper_basin);

    ep_gc_maybe_collect();

    lower_basin = fraction_from_ratio(1, 4);
    doubled_low = fraction_add(lower_basin, lower_basin);
    upper_basin = fraction_from_ratio(3, 4);
    doubled_high_raw = fraction_add(upper_basin, upper_basin);
    doubled_high = cast_out_whole_ones(doubled_high_raw);
    threshold = tipping_threshold();
    low_locks = fraction_compare(doubled_low, threshold) == 0;
    threshold_twin = tipping_threshold();
    high_locks = fraction_compare(doubled_high, threshold_twin) == 0;
    if (low_locks) {
    ret_val = high_locks;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(doubled_high);
    doubled_high = 0;
    free_struct_Fraction(doubled_low);
    doubled_low = 0;
    free_struct_Fraction(threshold);
    threshold = 0;
    free_struct_Fraction(threshold_twin);
    threshold_twin = 0;
    return ret_val;
}

long long unit_power_law_holds_over(long long octaves) {
    ...
    k = 1;
    while (k < (octaves + 1)) {
    size = whole_power(2, k);
    count = fraction_from_ratio(1, size);
    size_fraction = fraction_from_whole_number(size);
    product = fraction_multiply(count, size_fraction);
    one_twin = fraction_from_whole_number(1);
    exact = fraction_compare(product, one_twin) == 0;
    if (exact) {
    all_hold = all_hold;
    } else {
    all_hold = 0LL;
...
long long release_is_atomic() {
    ...
    long long stored = 0;
    long long stored_twin = 0;
    long long ret_val = 0;

    ep_gc_push_root(&one);
    ep_gc_push_root(&released);
    ep_gc_push_root(&stored);
    ep_gc_push_root(&stored_twin);

    ep_gc_maybe_collect();

    stored = fraction_from_ratio(1, 2);
    stored_twin = fraction_from_ratio(1, 2);
    released = fraction_add(stored, stored_twin);
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(released, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(one);
    one = 0;
    free_struct_Fraction(released);
    released = 0;
    return ret_val;
}

long long ringdown_amplitude(long long cycle) {
    long long tower = 0;
    long long ret_val = 0;

    ep_gc_push_root(&tower);
    ep_gc_push_root(&cycle);

    ep_gc_maybe_collect();

    tower = whole_power(2, cycle);
...
long long ringdown_decays_over(long long cycles) {
    long long alive = 0;
    long long all_hold = 0;
    long long current = 0;
    long long exact_half = 0;
    long long half = 0;
    long long halved = 0;
    long long k = 0;
    long long next_amp = 0;
    ...
    k = 1;
    while (k < cycles) {
    current = ringdown_amplitude(k);
    halved = fraction_multiply(current, half);
    next_amp = ringdown_amplitude((k + 1));
    exact_half = fraction_compare(next_amp, halved) == 0;
    if (exact_half) {
    all_hold = all_hold;
    } else {
    all_hold = 0LL;
    ...
    half = fraction_from_ratio(1, 2);
    survivor = ringdown_amplitude((k + 1));
