...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the two harmonic families ===");
    ok = expect_equal((long long)"harmonic family count = generator count (two routes agree)", int_to_string(harmonic_family_count()), (long long)"2");
    ok = expect_equal((long long)"colour family cos^2 = 1/b^2 (the third turn, squared)", fraction_to_text(colour_cos_squared()), (long long)"1/4");
    ok = expect_bool((long long)"the third-turn whole identity: 4 * cos^2 = 1", third_turn_identity_holds());
    ok = expect_bool((long long)"colour family meets the forced electroweak mixing: cos^2 + cos^2(theta_W) = 1", colour_family_meets_electroweak());
    ok = expect_bool((long long)"families distinct: the two generator periods differ", families_distinct());
    printf("%s\n", (char*)(long long)"--- the Measured wall: the atlas checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    measured family count (two: dyadic + smooth; slant excludes a third) meets the forced count");
    printf("%s\n", (char*)(long long)"  note  the selection law (which tensor writes in which family) is NOT forced here");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
