...
long long higgs_vacuum_value();
long long vacuum_doubles_to_one();
long long vacuum_is_nonzero();
long long vacuum_is_self_antipodal();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the Higgs vacuum ===");
    ok = expect_equal((long long)"vacuum value = displaced half-One = 1/2", fraction_to_text(higgs_vacuum_value()), (long long)"1/2");
    ok = expect_equal((long long)"the forcing equation: 1/2 + 1/2 = the One", fraction_to_text(vacuum_doubles_to_one()), (long long)"1");
    ok = expect_true((long long)"the vacuum is strictly nonzero (not the empty value)", vacuum_is_nonzero());
    ok = expect_equal((long long)"the vacuum is its own antipode: One - 1/2 = 1/2", fraction_to_text(vacuum_is_self_antipodal()), (long long)"1/2");
    ok = expect_equal((long long)"the vacuum folds up to the One: fold(1/2) = 1", fraction_to_text(vacuum_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long higgs_vacuum_value() {
    ...
    ret_val = fraction_divide(one, binary);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long vacuum_doubles_to_one() {
    ...
    first = higgs_vacuum_value();
    second = higgs_vacuum_value();
