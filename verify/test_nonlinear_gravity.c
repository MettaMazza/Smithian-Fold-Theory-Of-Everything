...
long long _main();
long long matter_source();
long long coupling();
long long linear_field();
long long field_energy();
long long corrected_field();
long long self_sourcing_correction();
long long correction_matches_structural();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== nonlinear gravity (the field sources itself) ===");
    ok = expect_equal((long long)"matter source M = 1/3", fraction_to_text(matter_source()), (long long)"1/3");
    ok = expect_equal((long long)"linear field f1 = g*M = 1/6", fraction_to_text(linear_field()), (long long)"1/6");
    ok = expect_equal((long long)"field energy = f1^2 = 1/36 (this gravitates)", fraction_to_text(field_energy()), (long long)"1/36");
    ok = expect_equal((long long)"corrected field f2 = g*(M+energy) = 13/72", fraction_to_text(corrected_field()), (long long)"13/72");
    ok = expect_equal((long long)"self-sourcing correction = f2 - f1 = 1/72", fraction_to_text(self_sourcing_correction()), (long long)"1/72");
    ok = expect_bool((long long)"it matches the structural 1/8 * 1/9 EXACTLY", correction_matches_structural());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long linear_field() {
    ...
    m = fraction_from_ratio(1, colour_count());
    ret_val = fraction_multiply(g, m);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long field_energy() {
    ...
    f1 = linear_field();
    ret_val = fraction_multiply(f1, f1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long corrected_field() {
    ...
    m = fraction_from_ratio(1, colour_count());
    energy = field_energy();
    source_total = fraction_add(m, energy);
    g = fraction_from_ratio(1, binary_count());
...
long long self_sourcing_correction() {
    ...
    f2 = corrected_field();
    f1 = linear_field();
    ret_val = fraction_subtract(f2, f1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long correction_matches_structural() {
    long long colour_scale = 0;
    long long correction = 0;
    long long depth_scale = 0;
    long long structural = 0;
    long long ret_val = 0;

    ep_gc_push_root(&colour_scale);
    ep_gc_push_root(&correction);
    ep_gc_push_root(&depth_scale);
    ep_gc_push_root(&structural);

    ep_gc_maybe_collect();

    correction = self_sourcing_correction();
    depth_scale = fraction_from_ratio(1, whole_power(binary_count(), colour_count()));
    colour_scale = fraction_from_ratio(1, whole_power(colour_count(), binary_count()));
    structural = fraction_multiply(depth_scale, colour_scale);
    ret_val = fraction_compare(correction, structural) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(correction);
    correction = 0;
