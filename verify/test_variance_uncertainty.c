...
long long _main();
long long spacing_at(long long);
long long weighted_variance(long long, long long);
long long minimal_variance_product(long long);
long long variance_floor(long long);
long long minimal_state_saturates();
long long wider_state_exceeds_floor();
long long floor_is_spacing_squared();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the variance uncertainty (Heisenberg's product, exact) ===");
    ok = expect_equal((long long)"the minimal variance product at depth 2 = 1/16", fraction_to_text(minimal_variance_product(2)), (long long)"1/16");
    ok = expect_equal((long long)"the structural floor 1/2^(2k) = 1/16", fraction_to_text(variance_floor(2)), (long long)"1/16");
    ok = expect_bool((long long)"EQUALITY: the minimal state saturates the bound exactly", minimal_state_saturates());
    ok = expect_bool((long long)"no state beats Heisenberg: a wider state exceeds the floor", wider_state_exceeds_floor());
    ok = expect_bool((long long)"the floor is the spacing squared (one factor per conjugate)", floor_is_spacing_squared());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long spacing_at(long long k) {
    long long volume = 0;
    long long ret_val = 0;
...
long long weighted_variance(long long s, long long k) {
    long long dx = 0;
    long long spread = 0;
    long long width = 0;
    ...
    dx = spacing_at(k);
    spread = fraction_from_whole_number(s);
    width = fraction_multiply(spread, dx);
    width_twin = fraction_make(fraction_numerator(width), fraction_denominator(width));
    ret_val = fraction_multiply(width, width_twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(6);
    return ret_val;
...
long long minimal_variance_product(long long k) {
    long long var_p = 0;
    long long var_x = 0;
    long long ret_val = 0;

    ep_gc_push_root(&var_p);
    ep_gc_push_root(&var_x);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    var_x = weighted_variance(binary_count(), k);
    var_p = weighted_variance(binary_count(), k);
    ret_val = fraction_multiply(var_x, var_p);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long variance_floor(long long k) {
    long long volume = 0;
    long long ret_val = 0;
    ...
    volume = whole_power(binary_count(), (2 * k));
    ret_val = fraction_from_ratio(1, volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long minimal_state_saturates() {
    ...
    product = minimal_variance_product(2);
    floor = variance_floor(2);
    ret_val = fraction_compare(product, floor) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(floor);
    floor = 0;
...
long long wider_state_exceeds_floor() {
    ...
    var_x = weighted_variance(3, 2);
    var_p = weighted_variance(binary_count(), 2);
    product = fraction_multiply(var_x, var_p);
    floor = variance_floor(2);
    comparison = fraction_compare(product, floor);
...
long long floor_is_spacing_squared() {
    ...
    ep_gc_push_root(&dx);
    ep_gc_push_root(&dx_twin);
    ep_gc_push_root(&floor);
    ep_gc_push_root(&squared);

    ep_gc_maybe_collect();

    floor = variance_floor(2);
    dx = spacing_at(2);
    dx_twin = fraction_make(fraction_numerator(dx), fraction_denominator(dx));
    squared = fraction_multiply(dx, dx_twin);
    ret_val = fraction_compare(floor, squared) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(floor);
    floor = 0;
    free_struct_Fraction(squared);
    squared = 0;
    return ret_val;
}
