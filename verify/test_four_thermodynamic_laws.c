...
long long zeroth_law_in_equilibrium();
long long first_law_total_is_the_one();
long long first_law_one_is_invariant();
long long second_law_information_lost();
long long second_law_entropy_bits_per_step();
long long third_law_ground_above_zero();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the four laws of thermodynamics ===");
    printf("%s\n", (char*)(long long)"--- zeroth: equilibrium is transitive (a shared temperature) ---");
    ok = expect_true((long long)"1/4 and 3/4 fold to a common image (in equilibrium)", zeroth_law_in_equilibrium());
    printf("%s\n", (char*)(long long)"--- first: energy (the One) is conserved ---");
    ok = expect_equal((long long)"a partition sums back to the One (1/4 + 3/4 = 1)", fraction_to_text(first_law_total_is_the_one()), (long long)"1");
    ok = expect_equal((long long)"the fold holds the One fixed: fold(1) = 1", fraction_to_text(first_law_one_is_invariant()), (long long)"1");
    printf("%s\n", (char*)(long long)"--- second: entropy increases ---");
    ok = expect_true((long long)"distinct states share an image (information is lost)", second_law_information_lost());
    ok = expect_equal((long long)"entropy lost per step (bits)", int_to_string(second_law_entropy_bits_per_step()), int_to_string(1));
    printf("%s\n", (char*)(long long)"--- third: absolute zero is unreachable ---");
    ok = expect_true((long long)"the ground state 1/2 is strictly above zero", third_law_ground_above_zero());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long zeroth_law_in_equilibrium() {
    ...
    first_image = fold_fraction(fraction_from_ratio(1, 4));
    second_image = fold_fraction(fraction_from_ratio(3, 4));
...
long long first_law_total_is_the_one() {
    ...
    lower = fraction_from_ratio(1, 4);
    upper = fraction_from_ratio(3, 4);
...
long long second_law_information_lost() {
    ...
    from_lower = fold_fraction(fraction_from_ratio(1, 4));
    from_upper = fold_fraction(fraction_from_ratio(3, 4));
...
long long second_law_entropy_bits_per_step() {
    ...
    bits = 0;
    value = 1;
    while (value < smallest_fold_period_above(1)) {
    value = (value + value);
    bits = (bits + 1);
    }
    ret_val = bits;
...
long long third_law_ground_above_zero() {
    ...
    ground = fraction_from_ratio(1, binary_count());
