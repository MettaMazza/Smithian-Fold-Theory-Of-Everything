...
long long colour_cycle_length();
long long cycle_closes_to_one();
long long cycle_length_is_colour_count();
long long binding_lock_completes();
long long bare_share();
long long binding_dominates();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== nucleon binding dominance (mass is the held cycle) ===");
    ok = expect_equal((long long)"the colour cycle length = 3 (period of 1/7)", int_to_string(colour_cycle_length()), (long long)"3");
    ok = expect_bool((long long)"the cycle length equals the colour count (three quarks)", cycle_length_is_colour_count());
    ok = expect_equal((long long)"the three orbit shares close to the One (1/7 + 2/7 + 4/7)", fraction_to_text(cycle_closes_to_one()), (long long)"1");
    ok = expect_bool((long long)"the binding lock completes: fold(1/2) = 1 (the bound whole)", binding_lock_completes());
    ok = expect_equal((long long)"the bare (constituent) share = one step at depth 7 = 1/128", fraction_to_text(bare_share()), (long long)"1/128");
    ok = expect_bool((long long)"binding dominates: 1/128 < 1/100 (under one percent bare)", binding_dominates());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long colour_cycle_length() {
    ...
    ret_val = fold_period_of_unit_fraction(7);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long cycle_closes_to_one() {
    ...
    first = fraction_from_ratio(1, 7);
    second = fold_fraction(first);
    third = fold_fraction(second);
...
long long cycle_length_is_colour_count() {
    ...
    length = fold_period_of_unit_fraction(7);
    colours = smallest_fold_period_above(binary_count());
    ret_val = length == colours;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long bare_share() {
    ...
    volume = whole_power(binary_count(), 7);
...
long long binding_dominates() {
    ...
    long long one_percent = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bare);
    ep_gc_push_root(&one_percent);

    ep_gc_maybe_collect();

    bare = bare_share();
    one_percent = fraction_from_ratio(1, 100);
    comparison = fraction_compare(bare, one_percent);
