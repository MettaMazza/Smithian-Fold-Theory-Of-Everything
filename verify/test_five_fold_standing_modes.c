...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== five-fold standing modes (a second route to three generations) ===");
    ok = expect_equal((long long)"the down-depth fold is m = binary + colour = 5", int_to_string(down_depth_fold()), (long long)"5");
    ok = expect_equal((long long)"the five-fold has exactly 3 interior standing modes", int_to_string(five_fold_standing_mode_count()), (long long)"3");
    ok = expect_equal((long long)"that count = the colour/generation count = 3", int_to_string(colour_count()), (long long)"3");
    ok = expect_equal((long long)"the two-fold has none", int_to_string(two_fold_standing_mode_count()), (long long)"0");
    ok = expect_bool((long long)"the half-One is the central standing mode", half_is_a_standing_mode());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
