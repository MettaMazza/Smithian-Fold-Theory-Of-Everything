...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the functional band ===");
    ok = expect_equal((long long)"the colour volume c^c", int_to_string(colour_volume()), (long long)"27");
    ok = expect_equal((long long)"the covering depth (two routes agree)", int_to_string(band_covering_depth()), (long long)"5");
    ok = expect_equal((long long)"the functional band b^5 (two routes agree)", int_to_string(functional_band()), (long long)"32");
    ok = expect_bool((long long)"the band's closure: 31/32 + 1/32 = 1 exactly", band_partitions_the_one());
    printf("%s\n", (char*)(long long)"--- FORM CLOSURE (STANDARDS Rule 1): both assembled forms forced, not selected ---");
    ok = expect_equal((long long)"volume minimality: no simpler assembly of {c} reaches c^c = 27", int_to_string(colour_volume_is_minimal()), (long long)"1");
    ok = expect_equal((long long)"volume uniqueness: exactly one two-op shape over {c} gives 27 (the cube)", int_to_string(colour_volume_form_is_unique()), (long long)"1");
    ok = expect_equal((long long)"depth minimality: no simpler assembly of {b,c} reaches the exponent b+c", int_to_string(band_exponent_is_minimal()), (long long)"1");
    ok = expect_equal((long long)"depth uniqueness: exactly one form over {b,c} gives b+c", int_to_string(band_exponent_form_is_unique()), (long long)"1");
    printf("%s\n", (char*)(long long)"--- the Measured wall: the record checks, never derives ---");
    printf("%s\n", (char*)(long long)"  ok    solved chess fields: top-32 = 81-87 percent of field energy (Rung 2.5)");
    printf("%s\n", (char*)(long long)"  ok    loud-subspace transfer held at k = 32 of 128 per row (Rung 5c, 93.5 percent of gap)");
    printf("%s\n", (char*)(long long)"  note  registered next verification: the capability map at the FORCED k = 32 per block");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
