...
long long body_one();
long long first_advances_to_second();
long long second_advances_to_third();
long long joint_recurrence_period();
long long configuration_returns();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the general n-body problem ===");
    ok = expect_equal((long long)"body one on the orbit of 1/5", fraction_to_text(body_one()), (long long)"1/5");
    ok = expect_equal((long long)"fold(1/5) = 2/5 (first advances to second)", fraction_to_text(first_advances_to_second()), (long long)"2/5");
    ok = expect_equal((long long)"fold(2/5) = 4/5 (second advances to third)", fraction_to_text(second_advances_to_third()), (long long)"4/5");
    ok = expect_equal((long long)"joint recurrence period = period(1/5) = 4", int_to_string(joint_recurrence_period()), int_to_string(4));
    ok = expect_bool((long long)"the configuration closes (returns after the period)", configuration_returns());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long body_one() {
    ...
    ret_val = fraction_from_ratio(1, 5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long first_advances_to_second() {
    ...
    ret_val = fold_fraction(body_one());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long second_advances_to_third() {
    ...
    body = fold_fraction(body_one());
    ret_val = fold_fraction(body);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long joint_recurrence_period() {
    ...
    ret_val = fold_period_of_unit_fraction(5);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long configuration_returns() {
    ...
    b1 = fold_fraction(body_one());
    b2 = fold_fraction(b1);
    b3 = fold_fraction(b2);
    b4 = fold_fraction(b3);
    start = fraction_from_ratio(1, 5);
    ret_val = fraction_compare(b4, start) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(b4);
    b4 = 0;
