...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== entropy and the second law (the fold is 2-to-1) ===");
    ok = expect_equal((long long)"low preimage of the balance = 1/4", fraction_to_text(low_preimage()), (long long)"1/4");
    ok = expect_equal((long long)"high preimage of the balance = 3/4", fraction_to_text(high_preimage()), (long long)"3/4");
    ok = expect_equal((long long)"both fold to the same image 1/2 (low)", fraction_to_text(low_preimage_image()), (long long)"1/2");
    ok = expect_equal((long long)"both fold to the same image 1/2 (high)", fraction_to_text(high_preimage_image()), (long long)"1/2");
    ok = expect_equal((long long)"preimages per image = binary count = 2 (one bit lost)", int_to_string(preimages_per_image()), (long long)"2");
    ok = expect_bool((long long)"the two preimages are genuinely distinct", preimages_are_distinct());
    ok = expect_bool((long long)"their images coincide -> the fold cannot be inverted", images_coincide());
    ok = expect_equal((long long)"the balance folds to the One (equilibrium)", fraction_to_text(balance_folds_to_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
