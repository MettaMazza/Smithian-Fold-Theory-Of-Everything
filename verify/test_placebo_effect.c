...
long long expectation_bias();
long long raw_observation();
long long expectation_folds_to_lock();
long long observation_folds_to_lock();
long long both_reach_the_same_lock();
long long lock_folds_to_experience();
long long expectation_plus_observation();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the placebo effect (expectation feeds the balance) ===");
    ok = expect_equal((long long)"expectation bias = 3/4", fraction_to_text(expectation_bias()), (long long)"3/4");
    ok = expect_equal((long long)"raw observation = 1/4", fraction_to_text(raw_observation()), (long long)"1/4");
    ok = expect_bool((long long)"both fold to the SAME lock 1/2 (belief feeds the balance)", both_reach_the_same_lock());
    ok = expect_equal((long long)"the lock folds to the whole experience", fraction_to_text(lock_folds_to_experience()), (long long)"1");
    ok = expect_equal((long long)"expectation + observation partition the One", fraction_to_text(expectation_plus_observation()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long expectation_bias() {
    ...
    ret_val = fraction_from_ratio(colour_count(), (binary_count() * binary_count()));
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long expectation_folds_to_lock() {
    ...
    ret_val = fold_fraction(expectation_bias());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long observation_folds_to_lock() {
    ...
    ret_val = fold_fraction(raw_observation());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long both_reach_the_same_lock() {
    long long from_expectation = 0;
    long long from_observation = 0;
    long long ret_val = 0;

    ep_gc_push_root(&from_expectation);
    ep_gc_push_root(&from_observation);

    ep_gc_maybe_collect();

    from_expectation = fold_fraction(expectation_bias());
    from_observation = fold_fraction(raw_observation());
    ret_val = fraction_compare(from_expectation, from_observation) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(from_expectation);
    from_expectation = 0;
    free_struct_Fraction(from_observation);
    from_observation = 0;
    return ret_val;
}
...
long long lock_folds_to_experience() {
    ...
    ret_val = fold_fraction(expectation_folds_to_lock());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long expectation_plus_observation() {
    long long bias = 0;
    long long observation = 0;
    long long ret_val = 0;

    ep_gc_push_root(&bias);
    ep_gc_push_root(&observation);

    ep_gc_maybe_collect();

    bias = fraction_from_ratio(colour_count(), (binary_count() * binary_count()));
    observation = fraction_from_ratio(1, (binary_count() * binary_count()));
    ret_val = fraction_add(bias, observation);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
