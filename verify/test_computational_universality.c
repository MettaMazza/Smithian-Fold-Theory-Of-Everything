...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold computational universality law ===");
    ok = expect_true((long long)"one executor covers an arbitrary admitted process prefix", universal_fold_covers_process(85, 7, 4));
    ok = expect_true((long long)"one executor covers every admitted process", complete_universal_fold_census());
    ok = expect_equal((long long)"depth-7 has 128 times 8 process descriptions", int_to_string(universal_fold_description_count(7)), int_to_string(1024));
    ok = expect_true((long long)"the same executor covers the recurrent machine", universal_fold_covers_recurrent_machine());
    ok = expect_true((long long)"universal execution composes at every split", universal_fold_execution_composes(85, 7));
    ok = expect_true((long long)"a non-Fold shifted transition is rejected", nonfold_transition_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
