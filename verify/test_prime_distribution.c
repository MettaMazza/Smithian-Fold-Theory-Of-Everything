...
long long _main();
long long orbit_period(long long);
long long period_divides_p_minus_one(long long);
long long fermat_holds_through_thirteen();
long long smallest_periods_are_the_generators();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the prime distribution (the fold's spectrum is number theory) ===");
    ok = expect_equal((long long)"period(1/3) = ord_3(2) = 2", int_to_string(orbit_period(3)), (long long)"2");
    ok = expect_equal((long long)"period(1/5) = 4", int_to_string(orbit_period(5)), (long long)"4");
    ok = expect_equal((long long)"period(1/7) = 3", int_to_string(orbit_period(7)), (long long)"3");
    ok = expect_equal((long long)"period(1/9) = 6", int_to_string(orbit_period(9)), (long long)"6");
    ok = expect_equal((long long)"period(1/15) = 4", int_to_string(orbit_period(15)), (long long)"4");
    ok = expect_bool((long long)"Fermat: the period divides p-1 through p = 13", fermat_holds_through_thirteen());
    ok = expect_bool((long long)"the two smallest periods ARE the generators 2 and 3", smallest_periods_are_the_generators());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long orbit_period(long long n) {
    long long ret_val = 0;
    ...
    ret_val = fold_period_of_unit_fraction(n);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long period_divides_p_minus_one(long long p) {
    long long period = 0;
    ...
    period = fold_period_of_unit_fraction(p);
    remainder = ((p - 1) % period);
    ret_val = remainder == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
...
long long fermat_holds_through_thirteen() {
    long long ok_eleven = 0;
    long long ok_five = 0;
    long long ok_seven = 0;
    long long ok_thirteen = 0;
    long long ok_three = 0;
    long long ret_val = 0;

    ok_three = period_divides_p_minus_one(3);
    ok_five = period_divides_p_minus_one(5);
    ok_seven = period_divides_p_minus_one(7);
    ok_eleven = period_divides_p_minus_one(11);
    ok_thirteen = period_divides_p_minus_one(13);
    if (ok_three) {
    if (ok_five) {
    if (ok_seven) {
    if (ok_eleven) {
...
long long smallest_periods_are_the_generators() {
    long long match_binary = 0;
    long long match_colour = 0;
    long long period_seven = 0;
    long long period_three = 0;
    long long ret_val = 0;

    period_three = fold_period_of_unit_fraction(3);
    period_seven = fold_period_of_unit_fraction(7);
    match_binary = period_three == smallest_fold_period_above(1);
    match_colour = period_seven == smallest_fold_period_above(binary_count());
    if (match_binary) {
    ret_val = match_colour;
    goto L_cleanup;
    }
    ret_val = 0;
    goto L_cleanup;
