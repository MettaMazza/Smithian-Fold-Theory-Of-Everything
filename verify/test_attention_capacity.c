...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== attention capacity (one focus) ===");
    ok = expect_equal((long long)"the focus lock = 1/2", fraction_to_text(focus_lock()), (long long)"1/2");
    ok = expect_bool((long long)"a single focus fully holds the lock (self-antipodal)", single_focus_is_complete());
    ok = expect_equal((long long)"one focus folds to unison (one bound experience)", fraction_to_text(focus_folds_to_unison()), (long long)"1");
    ok = expect_equal((long long)"splitting attention halves the lock -> 1/4", fraction_to_text(split_focus()), (long long)"1/4");
    ok = expect_bool((long long)"a split focus is NOT self-antipodal (binds nothing fully)", split_focus_incomplete());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
