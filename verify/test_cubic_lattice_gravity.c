...
long long potential_of(long long);
long long axis_curvature_at(long long);
long long cubic_laplacian_at(long long);
long long laplacian_is_exact_six();
long long dimension_times_expansion();
long long generators_product();
long long three_routes_agree();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== cubic-lattice gravity (Laplacian = 6 = d x m) ===");
    ok = expect_equal((long long)"one axis curvature of x^2 at spacing 1/8 = 2 EXACTLY", fraction_to_text(axis_curvature_at(3)), (long long)"2");
    ok = expect_equal((long long)"the 3D Laplacian: three axes = 6", fraction_to_text(cubic_laplacian_at(3)), (long long)"6");
    ok = expect_bool((long long)"exactly 6 at every spacing (depths 3 and 5)", laplacian_is_exact_six());
    ok = expect_equal((long long)"Route B1: d x m = period(1/7) x measured expansion = 6", fraction_to_text(dimension_times_expansion()), (long long)"6");
    ok = expect_equal((long long)"Route B2: binary x colour = 6 (the joint period)", int_to_string(generators_product()), (long long)"6");
    ok = expect_bool((long long)"the three routes agree: walked = d*m = 2*3", three_routes_agree());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long potential_of(long long x) {
    long long twin = 0;
    long long ret_val = 0;
    ...
    twin = fraction_make(fraction_numerator(x), fraction_denominator(x));
    ret_val = fraction_multiply(x, twin);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long axis_curvature_at(long long k) {
    long long f_centre = 0;
    long long f_centre_again = 0;
    long long f_left = 0;
    long long f_right = 0;
    long long left = 0;
    long long outer = 0;
    ...
    long long s_twin = 0;
    long long second_difference = 0;
    long long two_centre = 0;
    long long volume = 0;
    long long x = 0;
    long long ret_val = 0;

    ep_gc_push_root(&f_centre);
    ep_gc_push_root(&f_centre_again);
    ep_gc_push_root(&f_left);
    ep_gc_push_root(&f_right);
    ep_gc_push_root(&left);
    ep_gc_push_root(&outer);
    ep_gc_push_root(&right);
    ep_gc_push_root(&s);
    ep_gc_push_root(&s_squared);
    ep_gc_push_root(&s_twin);
    ep_gc_push_root(&second_difference);
    ep_gc_push_root(&two_centre);
    ...
    x = fraction_from_ratio(1, 2);
    volume = whole_power(binary_count(), k);
    s = fraction_from_ratio(1, volume);
    right = fraction_add(x, s);
    left = fraction_subtract(x, s);
    f_right = potential_of(right);
    f_left = potential_of(left);
    f_centre = potential_of(x);
    f_centre_again = potential_of(fraction_from_ratio(1, 2));
    two_centre = fraction_add(f_centre, f_centre_again);
    outer = fraction_add(f_right, f_left);
    second_difference = fraction_subtract(outer, two_centre);
    s_twin = fraction_from_ratio(1, volume);
    s_squared = fraction_multiply(s, s_twin);
    ret_val = fraction_divide(second_difference, s_squared);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(15);
    return ret_val;
...
long long cubic_laplacian_at(long long k) {
    long long first_axis = 0;
    long long pair = 0;
    long long second_axis = 0;
    long long third_axis = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_axis);
    ep_gc_push_root(&pair);
    ep_gc_push_root(&second_axis);
    ep_gc_push_root(&third_axis);
    ep_gc_push_root(&k);

    ep_gc_maybe_collect();

    first_axis = axis_curvature_at(k);
    second_axis = axis_curvature_at(k);
    third_axis = axis_curvature_at(k);
    pair = fraction_add(first_axis, second_axis);
    ret_val = fraction_add(pair, third_axis);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long laplacian_is_exact_six() {
    ...
    long long five_ok = 0;
    long long six = 0;
    long long three_ok = 0;
    long long ret_val = 0;

    ep_gc_push_root(&at_five);
    ep_gc_push_root(&at_three);
    ep_gc_push_root(&six);

    ep_gc_maybe_collect();

    six = fraction_from_whole_number(6);
    at_three = cubic_laplacian_at(3);
    at_five = cubic_laplacian_at(5);
    three_ok = fraction_compare(at_three, six) == 0;
    five_ok = fraction_compare(at_five, six) == 0;
    if (three_ok) {
    ret_val = five_ok;
    goto L_cleanup;
    }
...
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(at_five);
    at_five = 0;
    free_struct_Fraction(at_three);
    at_three = 0;
    free_struct_Fraction(six);
    six = 0;
    return ret_val;
}
...
long long dimension_times_expansion() {
    long long d = 0;
    long long d_fraction = 0;
    long long fold_gap = 0;
    long long fold_p = 0;
    long long fold_q = 0;
    long long gap = 0;
    long long m = 0;
    ...
    ep_gc_push_root(&d);
    ep_gc_push_root(&d_fraction);
    ep_gc_push_root(&fold_gap);
    ep_gc_push_root(&fold_p);
    ep_gc_push_root(&fold_q);
    ep_gc_push_root(&gap);
    ep_gc_push_root(&m);
    ep_gc_push_root(&p);
    ep_gc_push_root(&q);
    ...
    d = fold_period_of_unit_fraction(7);
    p = fraction_from_ratio(1, 5);
    q = fraction_from_ratio(1, 7);
    gap = fraction_subtract(p, q);
    fold_p = fold_fraction(p);
    fold_q = fold_fraction(q);
    fold_gap = fraction_subtract(fold_p, fold_q);
    m = fraction_divide(fold_gap, gap);
    d_fraction = fraction_from_whole_number(d);
    ret_val = fraction_multiply(d_fraction, m);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    return ret_val;
...
long long three_routes_agree() {
    long long first_match = 0;
    long long laplacian = 0;
    long long product = 0;
    long long second_match = 0;
    long long structural = 0;
    long long ret_val = 0;

    ep_gc_push_root(&laplacian);
    ep_gc_push_root(&product);
    ep_gc_push_root(&structural);

    ep_gc_maybe_collect();

    laplacian = cubic_laplacian_at(3);
    structural = dimension_times_expansion();
    product = fraction_from_whole_number(generators_product());
    first_match = fraction_compare(laplacian, structural) == 0;
    second_match = fraction_compare(structural, product) == 0;
    if (first_match) {
    ret_val = second_match;
    goto L_cleanup;
    }
    ret_val = 0LL;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(laplacian);
    laplacian = 0;
    free_struct_Fraction(product);
    product = 0;
    free_struct_Fraction(structural);
    structural = 0;
    return ret_val;
}
