...
long long _main();
long long source_strength();
long long field_at(long long);
long long flux_between(long long, long long);
long long flux_first_pair();
long long flux_second_pair();
long long flux_is_conserved();
long long flux_equals_source();
...
long long _main() {
    ...
    ep_gc_push_root(&r_half);
    ep_gc_push_root(&r_one);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the Schwarzschild solution (conserved flux) ===");
    ok = expect_equal((long long)"source rs = 1/4", fraction_to_text(source_strength()), (long long)"1/4");
    r_half = fraction_from_ratio(1, 2);
    ok = expect_equal((long long)"field at r=1/2: A = 1/2", fraction_to_text(field_at(r_half)), (long long)"1/2");
    r_one = fraction_from_whole_number(1);
    ok = expect_equal((long long)"field at r=1: A = 3/4", fraction_to_text(field_at(r_one)), (long long)"3/4");
    ok = expect_equal((long long)"flux over pair (1/2, 1) = 1/4", fraction_to_text(flux_first_pair()), (long long)"1/4");
    ok = expect_equal((long long)"flux over pair (1/2, 3/4) = 1/4", fraction_to_text(flux_second_pair()), (long long)"1/4");
    ok = expect_bool((long long)"flux is CONSERVED (same across pairs)", flux_is_conserved());
    ok = expect_bool((long long)"the flux constant equals the source mass", flux_equals_source());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(r_half);
    r_half = 0;
    free_struct_Fraction(r_one);
    r_one = 0;
    return ret_val;
}
...
long long source_strength() {
    ...
    four = (smallest_fold_period_above(1) * smallest_fold_period_above(1));
    ret_val = fraction_from_ratio(1, four);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long field_at(long long radius) {
    long long claim = 0;
    long long one = 0;
    long long rs = 0;
    long long ret_val = 0;

    ep_gc_push_root(&claim);
    ep_gc_push_root(&one);
    ep_gc_push_root(&rs);
    ...
    rs = source_strength();
    claim = fraction_divide(rs, radius);
    one = fraction_from_whole_number(1);
    ret_val = fraction_subtract(one, claim);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long flux_between(long long r1, long long r2) {
    long long field_difference = 0;
    long long field_one = 0;
    long long field_two = 0;
    long long inverse_difference = 0;
    long long inverse_one = 0;
    long long inverse_two = 0;
    long long one = 0;
    long long ret_val = 0;

    ep_gc_push_root(&field_difference);
    ep_gc_push_root(&field_one);
    ep_gc_push_root(&field_two);
    ep_gc_push_root(&inverse_difference);
    ep_gc_push_root(&inverse_one);
    ep_gc_push_root(&inverse_two);
    ep_gc_push_root(&one);
    ep_gc_push_root(&r1);
    ep_gc_push_root(&r2);
    ...
    field_two = field_at(r2);
    field_one = field_at(r1);
    field_difference = fraction_subtract(field_two, field_one);
    one = fraction_from_whole_number(1);
    inverse_one = fraction_divide(one, r1);
    inverse_two = fraction_divide(one, r2);
    inverse_difference = fraction_subtract(inverse_one, inverse_two);
    ret_val = fraction_divide(field_difference, inverse_difference);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(9);
    return ret_val;
...
long long flux_first_pair() {
    ...
    r1 = fraction_from_ratio(1, 2);
    r2 = fraction_from_whole_number(1);
    ret_val = flux_between(r1, r2);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long flux_is_conserved() {
    ...
    first = flux_first_pair();
    second = flux_second_pair();
...
long long flux_equals_source() {
    ...
    flux = flux_first_pair();
    rs = source_strength();
    ret_val = fraction_compare(flux, rs) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(flux);
    flux = 0;
    free_struct_Fraction(rs);
    rs = 0;
    return ret_val;
}
