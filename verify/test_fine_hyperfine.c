...
long long _main();
long long fine_structure_coupling_powers();
long long fine_to_gross_ratio();
long long fine_is_below_gross();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== fine and hyperfine structure ===");
    ok = expect_equal((long long)"fine-structure coupling powers = binary = 2 (alpha^2)", int_to_string(fine_structure_coupling_powers()), int_to_string(2));
    ok = expect_equal((long long)"fine/gross ratio = 250^2 / 34259^2 = 62500/1173679081", fraction_to_text(fine_to_gross_ratio()), (long long)"62500/1173679081");
    ok = expect_bool((long long)"fine structure is below the gross ladder (< the One)", fine_is_below_gross());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long fine_to_gross_ratio() {
    ...
    long long one_over_alpha = 0;
    long long powers = 0;
    long long ret_val = 0;

    ep_gc_push_root(&den_int);
    ep_gc_push_root(&den_pow);
    ep_gc_push_root(&denominator);
    ep_gc_push_root(&num_int);
    ep_gc_push_root(&num_pow);
    ep_gc_push_root(&numerator);
    ep_gc_push_root(&one_over_alpha);
    ep_gc_push_root(&powers);

    ep_gc_maybe_collect();

    one_over_alpha = inverse_fine_structure_first_order();
    powers = smallest_fold_period_above(1);
    numerator = exact_integer_from_sign_and_digits(({ long long _fap = one_over_alpha; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }), ({ long long _fap = one_over_alpha; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_digits; }));
    denominator = exact_integer_from_sign_and_digits(1, ({ long long _fap = one_over_alpha; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'denominator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->denominator_digits; }));
    num_int = text_to_number(exact_integer_to_text(numerator));
    den_int = text_to_number(exact_integer_to_text(denominator));
    num_pow = whole_power(num_int, powers);
    den_pow = whole_power(den_int, powers);
    ret_val = fraction_from_ratio(den_pow, num_pow);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(8);
    free_struct_ExactInteger(denominator);
    denominator = 0;
    free_struct_ExactInteger(numerator);
    numerator = 0;
    return ret_val;
}
...
long long fine_is_below_gross() {
    ...
    ratio = fine_to_gross_ratio();
    one = fraction_from_whole_number(1);
    order = fraction_compare(ratio, one);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
