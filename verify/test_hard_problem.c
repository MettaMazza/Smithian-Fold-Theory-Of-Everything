...
long long unity_state();
long long experience_is_one_whole();
long long carrier_low();
long long carrier_high();
long long carrier_cannot_reach_one_within(long long);
long long carrier_partitions_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the hard problem (unity + interiority) ===");
    ok = expect_equal((long long)"UNITY: bound processing at the balance = 1/2", fraction_to_text(unity_state()), (long long)"1/2");
    ok = expect_equal((long long)"experience is one whole (folds to the One)", fraction_to_text(experience_is_one_whole()), (long long)"1");
    ok = expect_equal((long long)"INTERIORITY: carrier states 1/3 and 2/3", fraction_to_text(carrier_low()), (long long)"1/3");
    ok = expect_bool((long long)"the carrier cannot reach the One from within (no self-view)", carrier_cannot_reach_one_within(12));
    ok = expect_equal((long long)"the carrier states partition the whole it holds", fraction_to_text(carrier_partitions_one()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long experience_is_one_whole() {
    ...
    ret_val = fold_fraction(unity_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long carrier_cannot_reach_one_within(long long horizon) {
    long long current = 0;
    long long one = 0;
    long long reached = 0;
    long long step = 0;
    ...
    current = fraction_from_ratio(1, colour_count());
    step = 0;
    reached = 0;
    while (step < horizon) {
