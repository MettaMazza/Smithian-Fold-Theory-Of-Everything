...
long long _main();
long long observation_depth();
long long branch_weight();
long long branch_count();
long long branches_partition_one();
long long weight_is_indivisible();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the measurement problem (one branch, indivisible) ===");
    ok = expect_equal((long long)"observation depth = colour = 3", int_to_string(observation_depth()), (long long)"3");
    ok = expect_equal((long long)"branch weight = 1/2^3 = 1/8 (indivisible)", fraction_to_text(branch_weight()), (long long)"1/8");
    ok = expect_equal((long long)"branch count = 2^3 = 8", int_to_string(branch_count()), (long long)"8");
    ok = expect_equal((long long)"the 8 branches partition the One (Born certainty)", fraction_to_text(branches_partition_one()), (long long)"1");
    ok = expect_bool((long long)"the branch weight is indivisible (denominator = 2^k)", weight_is_indivisible());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long branch_weight() {
    ...
    volume = whole_power(binary_count(), observation_depth());
...
long long branch_count() {
    ...
    ret_val = whole_power(binary_count(), observation_depth());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long branches_partition_one() {
    ...
    count = whole_power(binary_count(), observation_depth());
    total = fraction_from_whole_number(0);
    i = 0;
    while (i < count) {
    total = fraction_add(total, weight);
    i = (i + 1);
    }
    ret_val = total;
    goto L_cleanup;
...
long long weight_is_indivisible() {
    ...
    weight = branch_weight();
    denominator = exact_integer_from_sign_and_digits(1, ({ long long _fap = weight; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'denominator_digits' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->denominator_digits; }));
    expected = exact_integer_from_number(branch_count());
    comparison = exact_integer_compare(denominator, expected);
    ret_val = comparison == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    free_struct_Fraction(weight);
    weight = 0;
    return ret_val;
}
