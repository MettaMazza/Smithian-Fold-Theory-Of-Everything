...
long long _main();
long long covalent_bond();
long long ionic_bond();
long long equal_atoms_share_evenly();
long long covalent_below_ionic();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== electronegativity ===");
    ok = expect_equal((long long)"covalent bond = 1/2 (equal sharing)", fraction_to_text(covalent_bond()), (long long)"1/2");
    ok = expect_equal((long long)"ionic bond = the One (full transfer)", fraction_to_text(ionic_bond()), (long long)"1");
    ok = expect_bool((long long)"equal atoms share evenly (at the balance 1/2)", equal_atoms_share_evenly());
    ok = expect_bool((long long)"bond runs from covalent (shared) to ionic (transferred)", covalent_below_ionic());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long equal_atoms_share_evenly() {
    ...
    bond = fraction_from_ratio(1, binary_count());
    half = fraction_from_ratio(1, binary_count());
    ret_val = fraction_compare(bond, half) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long covalent_below_ionic() {
    long long covalent = 0;
    long long order = 0;
    long long ionic = 0;
    long long ret_val = 0;

    ep_gc_push_root(&covalent);
    ep_gc_push_root(&ionic);

    ep_gc_maybe_collect();

    covalent = fraction_from_ratio(1, binary_count());
    ionic = fraction_from_whole_number(1);
    order = fraction_compare(covalent, ionic);
    if (order < 0) {
    ret_val = 1LL;
    goto L_cleanup;
    }
