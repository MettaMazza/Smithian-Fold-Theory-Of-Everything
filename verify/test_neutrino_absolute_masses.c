...
long long _main();
long long tower_ratio();
long long m25_scale();
long long forced_splitting_ratio();
long long atmospheric_from_anchor(long long, long long);
long long mass_root(long long, long long, long long, long long, long long);
long long middle_mass(long long, long long);
long long heavy_mass(long long, long long);
long long mass_sum(long long, long long);
...
long long _main() {
    ...
    long long atmospheric_fresh = 0;
    long long below = 0;
    long long fiftieth = 0;
    long long gap = 0;
    long long lower = 0;
    long long measured_atm = 0;
    long long measured_twin = 0;
    long long near_cmp = 0;
    long long ok = 0;
    long long planck = 0;
    long long total = 0;
    long long under = 0;
    ...
    ep_gc_push_root(&atmospheric_fresh);
    ep_gc_push_root(&below);
    ep_gc_push_root(&fiftieth);
    ep_gc_push_root(&gap);
    ep_gc_push_root(&lower);
    ep_gc_push_root(&measured_atm);
    ep_gc_push_root(&measured_twin);
    ep_gc_push_root(&near_cmp);
    ep_gc_push_root(&planck);
    ep_gc_push_root(&total);
    ep_gc_push_root(&under);
    ep_gc_push_root(&upper);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== absolute neutrino masses (the cosmological sum, predicted) ===");
    ok = expect_equal((long long)"the tower ratio at the counted down-depth = 1/32", fraction_to_text(tower_ratio()), (long long)"1/32");
    ok = expect_equal((long long)"the M25 scale = (5^2 - 1)/5^2 = 24/25", fraction_to_text(m25_scale()), (long long)"24/25");
    ok = expect_equal((long long)"the FORCED splitting ratio = 3/100 (the ratio 33)", fraction_to_text(forced_splitting_ratio()), (long long)"3/100");
    atmospheric = atmospheric_from_anchor(742, 10000000);
    printf("%s\n", (char*)concat((long long)"  info  forced dm31^2 from the solar anchor = ", fraction_to_decimal(atmospheric, 7)));
    measured_atm = fraction_from_ratio(251, 100000);
    measured_twin = fraction_from_ratio(251, 100000);
    atmospheric_fresh = atmospheric_from_anchor(742, 10000000);
    gap = fraction_subtract(measured_twin, atmospheric_fresh);
    fiftieth = fraction_from_ratio(1, 50);
    allowed = fraction_multiply(fiftieth, measured_atm);
    near_cmp = fraction_compare(gap, allowed);
    ok = expect_bool((long long)"forced atmospheric lands within 2% of measured 2.51e-3", near_cmp < 0);
    total = mass_sum(742, 10000000);
    printf("%s\n", (char*)concat((long long)"  info  PREDICTION: Sigma m_nu = ", fraction_to_decimal(total, 5)));
    lower = fraction_from_ratio(55, 1000);
    upper = fraction_from_ratio(62, 1000);
    above = fraction_compare(total, lower);
    below = fraction_compare(total, upper);
    ok = expect_bool((long long)"the sum sits in [0.055, 0.062] eV (normal ordering)", above > 0);
    ok = expect_bool((long long)"(and below the upper bracket)", below < 0);
    planck = fraction_from_ratio(12, 100);
    under = fraction_compare(total, planck);
    ok = expect_bool((long long)"under the Planck bound 0.12 eV; testable at ~0.02 eV surveys", under < 0);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(15);
    free_struct_Fraction(atmospheric);
    atmospheric = 0;
    return ret_val;
}
...
long long tower_ratio() {
    ...
    depth = minimal_binary_cover(whole_power(colour_count(), 3));
    volume = whole_power(binary_count(), depth);
    ret_val = fraction_from_ratio(1, volume);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long m25_scale() {
    ...
    ep_gc_push_root(&square);

    ep_gc_maybe_collect();

    prime = (smallest_fold_period_above(1) + smallest_fold_period_above(binary_count()));
    square = (prime * prime);
    ret_val = fraction_from_ratio((square - 1), square);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long forced_splitting_ratio() {
    ...
    tower = tower_ratio();
    scale = m25_scale();
    ret_val = fraction_multiply(tower, scale);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long atmospheric_from_anchor(long long a_num, long long a_den) {
    long long anchor = 0;
    long long ratio = 0;
    long long ret_val = 0;
    ...
    anchor = fraction_from_ratio(a_num, a_den);
    ratio = forced_splitting_ratio();
    ret_val = fraction_divide(anchor, ratio);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
...
long long mass_root(long long t, long long lo_num, long long lo_den, long long hi_num, long long hi_den) {
    ...
    comparison = fraction_compare(mid_squared, t);
    if (comparison < 0) {
    lo = mid;
    } else {
    hi = mid;
...
long long middle_mass(long long a_num, long long a_den) {
    long long anchor = 0;
    long long ret_val = 0;
    ...
    anchor = fraction_from_ratio(a_num, a_den);
    ret_val = mass_root(anchor, 1, 1000, 2, 100);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long heavy_mass(long long a_num, long long a_den) {
    long long atmospheric = 0;
    long long ret_val = 0;
    ...
    atmospheric = atmospheric_from_anchor(a_num, a_den);
    ret_val = mass_root(atmospheric, 1, 100, 1, 10);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long mass_sum(long long a_num, long long a_den) {
    long long m2 = 0;
    long long m3 = 0;
    long long ret_val = 0;
    ...
    m2 = middle_mass(a_num, a_den);
    m3 = heavy_mass(a_num, a_den);
    ret_val = fraction_add(m2, m3);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    return ret_val;
