...
long long _main() {
    ...
    ep_gc_push_root(&iterative);
    ep_gc_push_root(&recursive);
    ep_gc_push_root(&state);
    ep_gc_push_root(&word);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== Fold recursion and self-application law ===");
    state = fraction_from_ratio(85, computation_state_count(7));
    recursive = recursive_fold_evaluate(state, 3);
    iterative = computation_steps(state, 3);
    ok = expect_true((long long)"recursive and iterative Fold evaluation agree", fraction_is_equal(recursive, iterative));
    ok = expect_true((long long)"all recursive representations agree for one prefix", recursive_fold_routes_agree(85, 7, 3));
    word = encode_computation_state(85, 7);
    ok = expect_equal((long long)"recursive word rewriting reaches the empty base", int_to_string(length_list(recursive_fold_word(word))), int_to_string(0));
    ok = expect_true((long long)"the complete recursive Fold census holds", complete_recursive_fold_census());
    ok = expect_true((long long)"the Fold rule closes its established self-state", recursive_self_application_closes());
    ok = expect_true((long long)"one further self-application remains at the fixed base", recursive_self_application_reaches_fixed_base());
    ok = expect_true((long long)"truncated self-application is rejected", truncated_self_application_is_rejected());
    ok = expect_true((long long)"identity in place of recursive Fold is rejected", identity_recursion_is_rejected());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(iterative);
    iterative = 0;
    free_struct_Fraction(recursive);
    recursive = 0;
    free_list(word);
    word = 0;
    return ret_val;
