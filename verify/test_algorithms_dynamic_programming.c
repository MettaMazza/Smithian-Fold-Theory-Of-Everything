...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold dynamic programming ===");
    ok = expect_equal((long long)"shared depth-seven closure work is 254", int_to_string(fold_dynamic_closure_work(7)), int_to_string(254));
    ok = expect_equal((long long)"the complete closure table has 255 nodes", int_to_string(fold_dynamic_closure_table_nodes(7)), int_to_string(255));
    ok = expect_equal((long long)"dynamic work equals the transition circuit", int_to_string(fold_dynamic_work_is_forced(7)), int_to_string(254));
    ok = expect_true((long long)"the shared table solves every terminal", fold_dynamic_table_is_complete(7));
    ok = expect_true((long long)"shared suffixes save exact work", fold_dynamic_reuse_saves_work(7));
    ok = expect_true((long long)"the complete dynamic census holds", complete_fold_dynamic_programming_census());
    ok = expect_true((long long)"an omitted dynamic layer is rejected", omitted_dynamic_layer_is_rejected(7));
    ok = expect_true((long long)"a false no-sharing claim is rejected", false_no_sharing_claim_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
