...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold process and machine law ===");
    ok = expect_equal((long long)"the deepest first process reaches terminal in seven steps", int_to_string(process_length_to_terminal(1, 7)), int_to_string(7));
    ok = expect_equal((long long)"the depth-7 machine has one transition per state", int_to_string(machine_transition_count(7)), int_to_string(128));
    ok = expect_equal((long long)"the depth-7 machine has exactly one terminal", int_to_string(machine_terminal_count(7)), int_to_string(1));
    ok = expect_true((long long)"every depth-7 state reaches the terminal", every_machine_state_terminates(7));
    ok = expect_true((long long)"every depth-7 transition is single-valued", every_transition_is_single_valued(7));
    ok = expect_true((long long)"every depth-7 process composes and decomposes", every_process_composes_and_decomposes(7));
    ok = expect_equal((long long)"the recurrent machine period is b", int_to_string(recurrent_machine_period()), int_to_string(2));
    ok = expect_true((long long)"the recurrent machine is forced", recurrent_machine_is_forced());
    ok = expect_true((long long)"the recurrent machine has no terminal", recurrent_machine_has_no_terminal());
    ok = expect_true((long long)"a false period-one cycle is rejected", false_period_one_is_rejected());
    ok = expect_true((long long)"an extra terminal is rejected", extra_terminal_is_rejected(7));
    ok = expect_true((long long)"complete process and machine census holds", complete_process_machine_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
