...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold computational-model equivalence law ===");
    ok = expect_true((long long)"all five models agree at zero steps", all_fold_models_agree(85, 7, 0));
    ok = expect_true((long long)"all five models agree after one step", all_fold_models_agree(85, 7, 1));
    ok = expect_true((long long)"all five models agree at an intermediate prefix", all_fold_models_agree(85, 7, 4));
    ok = expect_true((long long)"all five models agree at terminal closure", all_fold_models_agree(85, 7, 7));
    ok = expect_true((long long)"the complete model-equivalence census holds", complete_fold_model_equivalence_census());
    ok = expect_true((long long)"a shifted process step is rejected", shifted_model_step_is_rejected(7));
    ok = expect_true((long long)"a changed source coordinate is rejected", changed_model_source_is_rejected(7));
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
