...
long long _main();
long long doubling_fold(long long);
long long tripling_fold(long long);
long long generation_site(long long);
long long fold_home(long long);
long long all_generations_reach_home();
long long all_triple_to_same_waypoint();
long long structural_depth();
long long depth_matches_factor_count();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the generation depth (all three fold home in two steps) ===");
    ok = expect_equal((long long)"generation site 1 = 1/6", fraction_to_text(generation_site(1)), (long long)"1/6");
    ok = expect_equal((long long)"site 1 folds home: triple then double = 1", fraction_to_text(fold_home(1)), (long long)"1");
    ok = expect_bool((long long)"ALL three generations reach the One in the same two steps", all_generations_reach_home());
    ok = expect_bool((long long)"all three triple to the SAME waypoint 1/2 (one shared gate)", all_triple_to_same_waypoint());
    ok = expect_equal((long long)"the structural depth: exponent sum of 6 = 2^1*3^1 -> 2", int_to_string(structural_depth()), (long long)"2");
    ok = expect_bool((long long)"the walked depth equals the factor count", depth_matches_factor_count());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long doubling_fold(long long size) {
    long long doubled = 0;
    long long twin = 0;
    long long ret_val = 0;
    ...
    doubled = fraction_add(size, twin);
...
long long generation_site(long long i) {
    long long ret_val = 0;
...
long long fold_home(long long i) {
    long long after_tripling = 0;
    long long site = 0;
    long long ret_val = 0;

    ep_gc_push_root(&after_tripling);
    ep_gc_push_root(&site);
    ep_gc_push_root(&i);

    ep_gc_maybe_collect();

    site = fraction_from_ratio(((2 * i) - 1), 6);
    after_tripling = tripling_fold(site);
    ret_val = doubling_fold(after_tripling);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long all_generations_reach_home() {
    ...
    while (i < 4) {
    landed = fold_home(i);
    lands = fraction_compare(landed, one) == 0;
    if (lands) {
    all_land = all_land;
    } else {
    all_land = 0LL;
...
long long structural_depth() {
    long long exponent_sum = 0;
    long long remainder = 0;
    long long size = 0;
    long long ret_val = 0;

    size = (smallest_fold_period_above(1) * smallest_fold_period_above(binary_count()));
    exponent_sum = 0;
    remainder = size;
    while ((remainder % smallest_fold_period_above(1)) == 0) {
    remainder = (remainder / smallest_fold_period_above(1));
    exponent_sum = (exponent_sum + 1);
    }
    while ((remainder % smallest_fold_period_above(binary_count())) == 0) {
    remainder = (remainder / smallest_fold_period_above(binary_count()));
    exponent_sum = (exponent_sum + 1);
    }
    ret_val = exponent_sum;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
