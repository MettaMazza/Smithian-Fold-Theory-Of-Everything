...
long long edge_channel();
long long edge_exit_is_whole_fold();
long long exit_lands_on_bulk();
long long edge_plus_bulk();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== topological matter (edge protection by discreteness) ===");
    ok = expect_equal((long long)"protected edge channel = the balance 1/2", fraction_to_text(edge_channel()), (long long)"1/2");
    ok = expect_equal((long long)"the only exit is a whole fold (-> the One)", fraction_to_text(edge_exit_is_whole_fold()), (long long)"1");
    ok = expect_bool((long long)"the exit lands exactly on the bulk (a complete jump)", exit_lands_on_bulk());
    ok = expect_equal((long long)"edge + bulk shares sum to the One", fraction_to_text(edge_plus_bulk()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long edge_exit_is_whole_fold() {
    ...
    ret_val = fold_fraction(edge_channel());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long exit_lands_on_bulk() {
    ...
    ep_gc_push_root(&landing);
    ep_gc_push_root(&one);

    ep_gc_maybe_collect();

    landing = fold_fraction(edge_channel());
    one = fraction_from_whole_number(1);
    ret_val = fraction_compare(landing, one) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(landing);
    landing = 0;
...
long long edge_plus_bulk() {
    ...
    edge = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(edge, edge);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
