...
long long _main();
long long beta_exponent();
long long nu_exponent();
long long gamma_exponent();
long long delta_exponent();
long long widom_closes();
long long rushbrooke_alpha_is_floor();
long long fisher_closes();
...
long long critical_threshold();
long long order_parameter_exponent();
long long exponent_is_rational();
long long threshold_folds_to_one();
long long phases_sum_to_one();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== critical exponents made exact ===");
    ok = expect_equal((long long)"beta (order parameter): the half-One, from the proven anchor", fraction_to_text(beta_exponent()), (long long)"1/2");
    ok = expect_equal((long long)"nu (correlation length): the half-One", fraction_to_text(nu_exponent()), (long long)"1/2");
    ok = expect_equal((long long)"gamma (susceptibility): the One", fraction_to_text(gamma_exponent()), (long long)"1");
    ok = expect_equal((long long)"delta (critical isotherm): the counted colour", int_to_string(delta_exponent()), (long long)"3");
    ok = expect_bool((long long)"WIDOM closes exactly: gamma = beta(delta-1)", widom_closes());
    ok = expect_bool((long long)"RUSHBROOKE: alpha lands exactly on the floor (zero residual)", rushbrooke_alpha_is_floor());
    ok = expect_bool((long long)"FISHER: gamma = 2 nu exactly (eta at the floor)", fisher_closes());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long widom_closes() {
    ...
    ep_gc_push_root(&beta);
    ep_gc_push_root(&delta);
    ep_gc_push_root(&gamma);
    ep_gc_push_root(&steps);
    ep_gc_push_root(&widom);

    ep_gc_maybe_collect();

    beta = fraction_from_ratio(1, binary_count());
    delta = smallest_fold_period_above(binary_count());
    steps = fraction_from_whole_number((delta - 1));
    widom = fraction_multiply(beta, steps);
    gamma = fraction_from_whole_number(1);
    ret_val = fraction_compare(widom, gamma) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    free_struct_Fraction(gamma);
    gamma = 0;
    free_struct_Fraction(widom);
    widom = 0;
    return ret_val;
}

long long rushbrooke_alpha_is_floor() {
    long long after_beta = 0;
    long long alpha = 0;
    long long beta = 0;
    long long beta_twin = 0;
    long long gamma = 0;
    long long two = 0;
    long long two_beta = 0;
    long long ret_val = 0;

    ep_gc_push_root(&after_beta);
    ep_gc_push_root(&alpha);
    ep_gc_push_root(&beta);
    ep_gc_push_root(&beta_twin);
    ep_gc_push_root(&gamma);
    ep_gc_push_root(&two);
    ep_gc_push_root(&two_beta);

    ep_gc_maybe_collect();

    two = fraction_from_whole_number(2);
    beta = fraction_from_ratio(1, binary_count());
    beta_twin = fraction_from_ratio(1, binary_count());
    two_beta = fraction_add(beta, beta_twin);
    after_beta = fraction_subtract(two, two_beta);
    gamma = fraction_from_whole_number(1);
    alpha = fraction_subtract(after_beta, gamma);
    ret_val = ({ long long _fap = alpha; if (_fap == 0) { fprintf(stderr, "Error: Null pointer when accessing field 'numerator_sign' on 'Fraction'\n"); exit(1); } ((EpStruct_Fraction*)(_fap))->numerator_sign; }) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(7);
    free_struct_Fraction(alpha);
    alpha = 0;
    return ret_val;
}
...
long long fisher_closes() {
    ...
    ep_gc_push_root(&gamma);
    ep_gc_push_root(&nu);
    ep_gc_push_root(&nu_twin);
    ep_gc_push_root(&two_nu);

    ep_gc_maybe_collect();

    nu = fraction_from_ratio(1, 2);
    nu_twin = fraction_from_ratio(1, 2);
    two_nu = fraction_add(nu, nu_twin);
    gamma = fraction_from_whole_number(1);
    ret_val = fraction_compare(two_nu, gamma) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
    free_struct_Fraction(gamma);
    gamma = 0;
    free_struct_Fraction(two_nu);
    two_nu = 0;
    return ret_val;
}
...
long long critical_threshold() {
    ...
    m = smallest_fold_period_above(1);
    ret_val = fraction_from_ratio((m - 1), m);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long exponent_is_rational() {
    ...
    denominator = smallest_fold_period_above(1);
    ret_val = denominator != 0;
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long threshold_folds_to_one() {
    ...
    ret_val = fold_fraction(critical_threshold());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
