...
long long _main();
long long cp_phase_position();
long long fold_fraction(long long);
long long cp_phase_folds_to_one();
long long cp_phase_antipode();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the CP-violating phase ===");
    ok = expect_equal((long long)"phase position = 1/2 (maximal)", fraction_to_text(cp_phase_position()), (long long)"1/2");
    ok = expect_equal((long long)"fold(1/2) = the One", fraction_to_text(cp_phase_folds_to_one()), (long long)"1");
    ok = expect_equal((long long)"One - 1/2 = 1/2 (self-antipodal, maximal separation)", fraction_to_text(cp_phase_antipode()), (long long)"1/2");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long cp_phase_folds_to_one() {
    ...
    phase = fraction_from_ratio(1, binary_count());
    ret_val = fold_fraction(phase);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long cp_phase_antipode() {
    ...
    phase = fraction_from_ratio(1, binary_count());
    ret_val = fraction_subtract(one, phase);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
