...
long long _main();
long long half_one_self_antipodal();
long long pair_casts_to_one(long long, long long);
long long whole_prime(long long);
long long smallest_prime_pair(long long);
long long prime_pair_count(long long);
long long goldbach_holds_to(long long);
long long goldbach_census_size(long long);
long long twin_pair_count_to(long long);
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== prime pairs as fold antipodes: Goldbach and the twins ===");
    ok = expect_bool((long long)"THE FRAME: the half-One is self-antipodal (1 - 1/2 = 1/2)", half_one_self_antipodal());
    ok = expect_bool((long long)"the pair (3, 7) about 10's half-One casts to the One", pair_casts_to_one(10, 3));
    ok = expect_equal((long long)"the census size: evens in [4, 10000]", int_to_string(goldbach_census_size(10000)), (long long)"4999");
    ok = expect_bool((long long)"GOLDBACH: every even in [4, 10000] carries a prime antipodal pair", goldbach_holds_to(10000));
    ok = expect_equal((long long)"the fewest pairs never dip to zero: E=12 carries exactly 1 (5+7)", int_to_string(prime_pair_count(12)), (long long)"1");
    ok = expect_equal((long long)"E=100 carries 6 prime pairs", int_to_string(prime_pair_count(100)), (long long)"6");
    ok = expect_equal((long long)"THE TWINS: pairs (p, p+2) below 10^4 (published: 205)", int_to_string(twin_pair_count_to(10000)), (long long)"205");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long half_one_self_antipodal() {
    ...
    antipode = fraction_subtract(one, half);
    half_twin = fraction_from_ratio(1, 2);
    ret_val = fraction_compare(antipode, half_twin) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(4);
...
long long pair_casts_to_one(long long e, long long k) {
    long long far_side = 0;
    long long near_side = 0;
    long long one = 0;
    long long total = 0;
    long long ret_val = 0;

    ep_gc_push_root(&far_side);
    ep_gc_push_root(&near_side);
    ep_gc_push_root(&one);
    ...
    near_side = fraction_from_ratio(k, e);
    far_side = fraction_from_ratio((e - k), e);
    total = fraction_add(near_side, far_side);
    one = fraction_from_whole_number(1);
...
long long whole_prime(long long n) {
    long long divisor = 0;
...
long long smallest_prime_pair(long long e) {
    long long casts = 0;
    long long p = 0;
    long long p_prime = 0;
    long long partner_prime = 0;
    long long ret_val = 0;
    ...
    p = 2;
    while (p < ((e / 2) + 1)) {
    p_prime = whole_prime(p);
    if (p_prime) {
    partner_prime = whole_prime((e - p));
    if (partner_prime) {
    casts = pair_casts_to_one(e, p);
    if (casts) {
    ret_val = p;
    goto L_cleanup;
    }
...
long long prime_pair_count(long long e) {
    long long casts = 0;
    long long count = 0;
    long long p = 0;
    long long p_prime = 0;
...
long long goldbach_holds_to(long long bound) {
...
long long goldbach_census_size(long long bound) {
    long long ret_val = 0;
...
long long twin_pair_count_to(long long bound) {
    long long count = 0;
    long long p = 0;
    long long p_prime = 0;
    long long partner_prime = 0;
