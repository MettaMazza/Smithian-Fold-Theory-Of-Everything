...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== Fold recognition law ===");
    word = encode_computation_state(85, 7);
    ok = expect_true((long long)"a generated word is recognized by exact closure", fold_recognizes_word(word));
    ok = expect_true((long long)"the complete depth-7 language is recognized", fold_recognizes_complete_language(7));
    ok = expect_equal((long long)"recognition counts all b^7 generated words", int_to_string(fold_recognized_word_count(7)), int_to_string(128));
    ok = expect_true((long long)"an outside-symbol word is rejected", fold_rejects_outside_word(7));
    ok = expect_true((long long)"a word asserted at the wrong depth is rejected", fold_rejects_wrong_depth(7));
    ok = expect_true((long long)"the complete recognition census holds", complete_fold_recognition_census());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
