...
long long _main();
long long mersenne_rung(long long);
long long solar_rung();
long long atmospheric_rung();
long long splitting_ratio_by_division();
long long splitting_ratio_by_identity();
long long routes_agree();
long long factorisation_holds();
...
long long _main() {
    ...
    long long ok = 0;
    long long twentieth = 0;
    long long ret_val = 0;

    ep_gc_push_root(&allowed);
    ep_gc_push_root(&comparison);
    ep_gc_push_root(&forced);
    ep_gc_push_root(&gap);
    ep_gc_push_root(&measured);
    ep_gc_push_root(&twentieth);

    ep_gc_maybe_collect();

    printf("%s\n", (char*)(long long)"=== the neutrino splitting ratio (exactly 33) ===");
    ok = expect_equal((long long)"the solar rung: 2^5 - 1 = 31 states", int_to_string(solar_rung()), (long long)"31");
    ok = expect_equal((long long)"the atmospheric rung: 2^10 - 1 = 1023 states", int_to_string(atmospheric_rung()), (long long)"1023");
    ok = expect_equal((long long)"ROUTE A: the ratio by division = 33", fraction_to_text(splitting_ratio_by_division()), (long long)"33");
    ok = expect_bool((long long)"ROUTE B: the geometric identity gives the same 33 = 2^5 + 1", routes_agree());
    ok = expect_bool((long long)"the factorisation holds: 1023 = 31 * 33", factorisation_holds());
    measured = fraction_from_ratio(100, 3);
    forced = splitting_ratio_by_division();
    gap = fraction_subtract(measured, forced);
    twentieth = fraction_from_ratio(1, 20);
    allowed = fraction_multiply(twentieth, measured);
    comparison = fraction_compare(gap, allowed);
    ok = expect_bool((long long)"33 sits within 5% of measured 33.3 (1.0% below)", comparison < 0);
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long mersenne_rung(long long d) {
    long long ret_val = 0;
    ...
    ret_val = (whole_power(binary_count(), d) - 1);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long solar_rung() {
    ...
    ret_val = (whole_power(binary_count(), 5) - 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long atmospheric_rung() {
    ...
    ret_val = (whole_power(binary_count(), 10) - 1);
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long splitting_ratio_by_division() {
    ...
    atmospheric = (whole_power(binary_count(), 10) - 1);
    solar = (whole_power(binary_count(), 5) - 1);
    ret_val = fraction_from_ratio(atmospheric, solar);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long splitting_ratio_by_identity() {
    ...
    x = whole_power(binary_count(), 5);
    ret_val = fraction_from_whole_number((x + 1));
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
...
long long routes_agree() {
    ...
    ep_gc_push_root(&identity);

    ep_gc_maybe_collect();

    division = splitting_ratio_by_division();
    identity = splitting_ratio_by_identity();
    ret_val = fraction_compare(division, identity) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(division);
    division = 0;
    free_struct_Fraction(identity);
    identity = 0;
    return ret_val;
}
