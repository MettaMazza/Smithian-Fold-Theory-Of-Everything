...
long long superfluid_state();
long long condensate_moves_as_one();
long long dissipation_gap();
long long gap_is_strictly_positive();
long long shares_complete_the_whole();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== superfluidity (flow without loss) ===");
    ok = expect_equal((long long)"the flowing condensate = the balance 1/2", fraction_to_text(superfluid_state()), (long long)"1/2");
    ok = expect_equal((long long)"it moves as ONE (folds to unison)", fraction_to_text(condensate_moves_as_one()), (long long)"1");
    ok = expect_equal((long long)"the first available loss is a whole level: gap = 1/4", fraction_to_text(dissipation_gap()), (long long)"1/4");
    ok = expect_bool((long long)"the gap is strictly positive -> viscosity is ABSENT", gap_is_strictly_positive());
    ok = expect_equal((long long)"two half-shares complete the whole", fraction_to_text(shares_complete_the_whole()), (long long)"1");
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long condensate_moves_as_one() {
    ...
    ret_val = fold_fraction(superfluid_state());
    goto L_cleanup;
L_cleanup:
    return ret_val;
}
...
long long dissipation_gap() {
    long long level_down = 0;
    long long state = 0;
    long long ret_val = 0;

    ep_gc_push_root(&level_down);
    ep_gc_push_root(&state);

    ep_gc_maybe_collect();

    state = fraction_from_ratio(1, binary_count());
    level_down = fraction_from_ratio(1, (binary_count() * binary_count()));
    ret_val = fraction_subtract(state, level_down);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    return ret_val;
...
long long shares_complete_the_whole() {
    ...
    state = fraction_from_ratio(1, binary_count());
    ret_val = fraction_add(state, state);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(1);
    return ret_val;
