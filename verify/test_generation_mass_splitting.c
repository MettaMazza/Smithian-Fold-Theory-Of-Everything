...
long long generation_gap(long long);
long long gap_is_one_over_colour();
long long spacing_is_uniform();
...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== generation mass-splitting ===");
    ok = expect_equal((long long)"generation count = colour = 3", int_to_string(generation_count()), int_to_string(3));
    ok = expect_equal((long long)"generation 0 = 1/6", fraction_to_text(generation_position(0)), (long long)"1/6");
    ok = expect_equal((long long)"generation 1 = 1/2", fraction_to_text(generation_position(1)), (long long)"1/2");
    ok = expect_equal((long long)"generation 2 = 5/6", fraction_to_text(generation_position(2)), (long long)"5/6");
    ok = expect_equal((long long)"gap between generations = 1/3", fraction_to_text(generation_gap(0)), (long long)"1/3");
    ok = expect_bool((long long)"the gap is 1/colour = 1/3", gap_is_one_over_colour());
    ok = expect_bool((long long)"the spacing is uniform (gap 0->1 = gap 1->2)", spacing_is_uniform());
    printf("%s\n", (char*)(long long)"=== done ===");
    ret_val = 0;
    goto L_cleanup;
L_cleanup:
...
long long generation_position(long long k) {
    ...
    numerator = fraction_add(balance, k_fraction);
    colour_fraction = fraction_from_whole_number(colour_count());
    ret_val = fraction_divide(numerator, colour_fraction);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(5);
    return ret_val;
...
long long generation_gap(long long k) {
    long long higher = 0;
    long long lower = 0;
    long long ret_val = 0;
    ...
    lower = generation_position(k);
    higher = generation_position((k + 1));
    ret_val = fraction_subtract(higher, lower);
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(3);
    return ret_val;
...
long long gap_is_one_over_colour() {
    ...
    gap = generation_gap(0);
    expected = fraction_from_ratio(1, colour_count());
    ret_val = fraction_compare(gap, expected) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
...
long long spacing_is_uniform() {
    ...
    long long second_gap = 0;
    long long ret_val = 0;

    ep_gc_push_root(&first_gap);
    ep_gc_push_root(&second_gap);

    ep_gc_maybe_collect();

    first_gap = generation_gap(0);
    second_gap = generation_gap(1);
    ret_val = fraction_compare(first_gap, second_gap) == 0;
    goto L_cleanup;
L_cleanup:
    ep_gc_pop_roots(2);
    free_struct_Fraction(first_gap);
    first_gap = 0;
    free_struct_Fraction(second_gap);
    second_gap = 0;
    return ret_val;
}
