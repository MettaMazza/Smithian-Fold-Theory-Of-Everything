...
long long _main() {
    ...
    printf("%s\n", (char*)(long long)"=== the fold is forced (machine-checked) ===");
    printf("%s\n", (char*)(long long)"--- Lemma 1: cast_out collapses every integer to the One ---");
    ok = expect_equal((long long)"cast_out(2) = 1", fraction_to_text(cast_out_of_whole_number(2)), (long long)"1");
    ok = expect_equal((long long)"cast_out(3) = 1", fraction_to_text(cast_out_of_whole_number(3)), (long long)"1");
    ok = expect_equal((long long)"cast_out(7) = 1", fraction_to_text(cast_out_of_whole_number(7)), (long long)"1");
    printf("%s\n", (char*)(long long)"--- Lemma 2: raw doubling is not a self-map (closure forces size 2) ---");
    ok = expect_true((long long)"x + x exceeds the One at x = 3/4", raw_doubling_exceeds_one());
    printf("%s\n", (char*)(long long)"--- Lemma 3: run each candidate; only the fold generates ---");
    ok = expect_true((long long)"identity (tag 1) does NOT generate (static)", is_false(candidate_generates(1)));
    ok = expect_true((long long)"square (tag 2) does NOT generate (contracts)", is_false(candidate_generates(2)));
    ok = expect_true((long long)"constant (tag 3) does NOT generate (collapses)", is_false(candidate_generates(3)));
    ok = expect_true((long long)"fold (tag 4) GENERATES", candidate_generates(4));
    ok = expect_equal((long long)"fold's return period from 1/7 = 3 (a real cycle)", int_to_string(candidate_return_period(4)), int_to_string(3));
    ok = expect_equal((long long)"identity's return period = 1 (static)", int_to_string(candidate_return_period(1)), int_to_string(1));
    printf("%s\n", (char*)(long long)"--- Main: exactly one generator, and it is the fold ---");
...
long long is_false(long long value) {
    long long ret_val = 0;
