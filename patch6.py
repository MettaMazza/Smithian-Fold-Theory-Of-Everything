...
def optimize_empirical_tm(sequence, exp_pdb_path):
    ...
    for gen in range(generations):
        ...
        for i, ind in enumerate(population):
            # Only do full local search on elite to save time, or 25% random
            if gen == 0 or random.random() < 0.25 or i < 3:
                sc, tm, drmsd, opt_ind, atoms = local_search(ind)
            else:
                tm, drmsd, atoms = eval_candidate_sequence_multi(sequence, ind, Q, sft_candidates)
                sc = objective(tm, drmsd)
                opt_ind = ind
            evaluated.append((sc, tm, drmsd, opt_ind, atoms))
